...
function createPromptRouteTestEnv(prefix: string): Record<string, string> {
  ...
  env.GOUSB_AI_MODEL = "MiniMax-M2.7";
  env.GOUSB_AI_API_BASE = "https://api.minimaxi.com/v1";
  env.GOUSB_AI_API_KEY = "minimax-key";
  env.GOUSB_AI_HEADERS_JSON = JSON.stringify({
    "x-go-usb-ai-narp-api-mode": "chat_completions",
...
describe("buildHermesAcpBridgeLaunchEnv", () => {
  it("injects a synthetic route for hermes acp probes", () => {
    ...
    expect(env.GOUSB_AI_MODEL).toBe("go-usb-ai-hermes-acp-probe");
    expect(env.GOUSB_AI_API_BASE).toBe("http://127.0.0.1:1/v1");
    expect(env.GOUSB_AI_API_KEY).toBe("go-usb-ai-hermes-acp-probe-key");
    expect(env.GOUSB_AI_HERMES_ACP_ROUTE_BRIDGE).toBe("1");
  ...
  it("prefers the source bridge directory when the workspace package is available", () => {
    ...
    expect(env.GOUSB_AI_HERMES_ACP_ROUTE_BRIDGE_DIR).toBe(expectedSourceBridgeDir);
    expect(env.PYTHONPATH?.split(":")[0]).toBe(expectedSourceBridgeDir);
  });
});

describe("Hermes ACP bridge sitecustomize session agents", () => {
  it("creates Hermes session agents from the GoUsbAi RuntimeRoute with a live tool surface", () => {
    const fakeHermesRoot = createTempDir("go-usb-ai-hermes-acp-fake-");
    seedFakeHermesPythonPackage(fakeHermesRoot);
    const env = buildHermesAcpBridgeLaunchEnv({
      ...createHermesAcpConfig(),
      baseEnv: {
    ...
    env.GOUSB_AI_MODEL = "qwen3.5-plus";
    env.GOUSB_AI_API_BASE = "http://127.0.0.1:18999/v1";
    env.GOUSB_AI_API_KEY = "bridge-key";
    env.GOUSB_AI_HEADERS_JSON = JSON.stringify({
      "x-go-usb-ai-narp-api-mode": "chat_completions",
    ...
    expect(output.provider_detected).toBe("go-usb-ai");
    expect(output.has_provider).toBe(true);
    expect(output).toEqual({
      provider_detected: "go-usb-ai",
      has_provider: true,
      model: "qwen3.5-plus",
      provider: "custom",
...
describe("Hermes ACP bridge sitecustomize reasoning mapping", () => {
  it("remaps ACP thinking callbacks onto reasoning during agent runs", () => {
    const fakeHermesRoot = createTempDir("go-usb-ai-hermes-acp-reasoning-");
    seedFakeHermesPythonPackage(fakeHermesRoot);
    const env = buildHermesAcpBridgeLaunchEnv({
      ...createHermesAcpConfig(),
      baseEnv: {
...
describe("Hermes ACP bridge sitecustomize prompt-routed execution", () => {
  it("rebuilds the Hermes execution agent from flattened ACP meta without dropping tools", () => {
    const env = createPromptRouteTestEnv("go-usb-ai-hermes-acp-prompt-route-");
  ...
  it("rebuilds the Hermes execution agent on the second prompt in the same session", () => {
    const env = createPromptRouteTestEnv("go-usb-ai-hermes-acp-second-prompt-");
    const pythonResult = runFakeHermesPythonScript(env, [
      "import asyncio",
      "import json",
      "from acp_adapter.server import HermesACPAgent",
