"""GoUsbAi bridge for Hermes ACP RuntimeRoute passthrough.
...
GoUsbAi RuntimeRoute instead of resolving its own provider config first.
"""
...
LOGGER = logging.getLogger("go-usb-ai.hermes_acp_route_bridge")
ROUTE_ENABLE_ENV = "GOUSB_AI_HERMES_ACP_ROUTE_BRIDGE"
CREATE_EXECUTION_AGENT = None

_ROUTE_HELPER_SPEC = importlib.util.spec_from_file_location(
    "go-usb-ai_hermes_acp_runtime_route",
    Path(__file__).with_name("go-usb-ai-hermes-acp-runtime-route.py"),
)
if _ROUTE_HELPER_SPEC is None or _ROUTE_HELPER_SPEC.loader is None:
    raise ImportError("Failed to load go-usb-ai Hermes ACP runtime route helper.")
...
read_go-usb-ai_prompt_route = _ROUTE_HELPER_MODULE.read_go-usb-ai_prompt_route
read_go-usb-ai_route = _ROUTE_HELPER_MODULE.read_go-usb-ai_route
...
def _merge_openai_headers(agent: Any, headers: Dict[str, str]) -> None:
    ...
    if callable(replace_client):
        replace_client(reason="go-usb-ai_runtime_route_bridge")
...
def _patch_session_manager() -> None:
    ...
    def create_execution_agent(
        *,
        ...
        if route is None:
            raise RuntimeError(
                "Missing GoUsbAi providerRoute for Hermes ACP request-scoped execution."
            )
...
def _patch_prompt_execution() -> None:
    ...
    if not callable(original_prompt):
        return
    if getattr(original_prompt, "__go-usb-ai_request_scoped_agent_bridge__", False):
        return

    async def bridged_prompt(self, prompt, session_id: str, **kwargs):
        if not callable(CREATE_EXECUTION_AGENT):
            raise RuntimeError("GoUsbAi Hermes ACP request-scoped agent bridge is not ready.")
        state = self.session_manager.get_session(session_id)
        if state is None:
            return await original_prompt(self, prompt, session_id, **kwargs)

        prompt_route = read_go-usb-ai_prompt_route(kwargs)
        if prompt_route is not None:
            ...
            try:
                ...
                raise RuntimeError(
                    "GoUsbAi Hermes ACP failed to create the prompt-routed execution agent."
                ) from exc

        return await original_prompt(self, prompt, session_id, **kwargs)

    bridged_prompt.__go-usb-ai_request_scoped_agent_bridge__ = True
...
def _patch_acp_reasoning_mapping() -> None:
    ...
    if not callable(original_run_conversation):
        return
    if getattr(original_run_conversation, "__go-usb-ai_reasoning_bridge__", False):
        return
    ...
    bridged_run_conversation.__go-usb-ai_reasoning_bridge__ = True
