...
API_MODE_HEADER = "x-go-usb-ai-narp-api-mode"
NARP_PROMPT_META_KEY = "go-usb-ai_narp"
...
def _read_headers_env() -> Dict[str, str]:
    raw = read_text_env("GOUSB_AI_HEADERS_JSON")
    if not raw:
        return {}
    try:
        parsed = json.loads(raw)
    except Exception:
        LOGGER.debug("Failed to parse GOUSB_AI_HEADERS_JSON", exc_info=True)
        return {}
...
def read_go-usb-ai_route(
    ...
    model = explicit_model or read_text_env("GOUSB_AI_MODEL")
    api_base = read_text_env("GOUSB_AI_API_BASE")
    api_key = read_text_env("GOUSB_AI_API_KEY")
...
def read_go-usb-ai_prompt_route(kwargs: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    go-usb-ai_meta = kwargs.get(NARP_PROMPT_META_KEY)
    if not isinstance(go-usb-ai_meta, dict):
        meta = kwargs.get("_meta")
        if not isinstance(meta, dict):
            return None
        go-usb-ai_meta = meta.get(NARP_PROMPT_META_KEY)
        if not isinstance(go-usb-ai_meta, dict):
            return None
    return _normalize_route_payload(go-usb-ai_meta.get("providerRoute"))
