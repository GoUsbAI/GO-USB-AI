...
if (!existsSync(targetDir)) {
  fail(`missing dist bridge directory: ${targetDir}. Run 'pnpm --filter @go-usb-ai/go-usb-ai-hermes-acp-bridge build'.`);
}
...
if (JSON.stringify(sourceFiles) !== JSON.stringify(targetFiles)) {
  fail(
    `source/dist bridge file sets differ. Run 'pnpm --filter @go-usb-ai/go-usb-ai-hermes-acp-bridge build' to resync.`,
  );
}
...
for (const relativePath of sourceFiles) {
  ...
  if (sourceContent !== targetContent) {
    fail(
      `source/dist bridge content differs for '${relativePath}'. Run 'pnpm --filter @go-usb-ai/go-usb-ai-hermes-acp-bridge build' to resync.`,
    );
  }
}
