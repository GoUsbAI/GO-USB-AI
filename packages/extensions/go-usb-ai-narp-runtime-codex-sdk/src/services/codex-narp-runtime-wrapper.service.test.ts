...
import type { NcpAgentRunInput, NcpAgentRuntime } from "@go-usb-ai/ncp";
...
describe("CodexNarpRuntimeWrapper", () => {
  ...
  it("uses the Codex Responses bridge for MiniMax chat-compatible routes", async () => {
    const wrapper = new CodexNarpRuntimeWrapper(
      ...
      async (bridgeConfig) => {
        expect(bridgeConfig).toMatchObject({
          ...
          modelPrefixes: ["minimax", "go-usb-ai-codex-bridge-minimax"],
        });
    ...
    expect(config).toMatchObject({
      ...
      model: "MiniMax-M2.7",
      cliConfig: {
        model_provider: "go-usb-ai-codex-bridge-minimax",
        model_providers: {
          "go-usb-ai-codex-bridge-minimax": {
      ...
      threadOptions: {
        model: "go-usb-ai-codex-bridge-minimax/MiniMax-M2.7",
      },
    });
  });
  ...
  it("uses the Codex Responses bridge for any chat-completions provider route", async () => {
    const wrapper = new CodexNarpRuntimeWrapper(
      ...
      async (bridgeConfig) => {
        expect(bridgeConfig).toMatchObject({
          ...
          modelPrefixes: ["deepseek", "go-usb-ai-codex-bridge-deepseek"],
        });
    ...
    expect(config).toMatchObject({
      ...
      model: "deepseek-v4-flash",
      cliConfig: {
        model_provider: "go-usb-ai-codex-bridge-deepseek",
        model_providers: {
          "go-usb-ai-codex-bridge-deepseek": {
      ...
      threadOptions: {
        model: "go-usb-ai-codex-bridge-deepseek/deepseek-v4-flash",
      },
    });
  });
});
