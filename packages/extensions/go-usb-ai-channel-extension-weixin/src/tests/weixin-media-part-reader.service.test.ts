...
const originalEndpoint = process.env.GOUSB_AI_EXTENSION_ENDPOINT;

afterEach(() => {
  vi.restoreAllMocks();
  if (originalEndpoint === undefined) {
    delete process.env.GOUSB_AI_EXTENSION_ENDPOINT;
  } else {
    process.env.GOUSB_AI_EXTENSION_ENDPOINT = originalEndpoint;
  }
});
...
describe("WeixinMediaPartReader", () => {
  it("falls back to the asset content url when assetUri cannot be resolved in the extension process", async () => {
    process.env.GOUSB_AI_EXTENSION_ENDPOINT = "http://127.0.0.1:18892";
