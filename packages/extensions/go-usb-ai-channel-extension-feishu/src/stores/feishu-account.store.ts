...
function resolveGoUsbAiHome(): string {
  const override = process.env[GOUSB_AI_HOME_ENV_KEY]?.trim();
  return resolve(override || join(homedir(), GOUSB_AI_DEFAULT_HOME_DIR));
}

function resolveAccountsDir(): string {
  return join(resolveGoUsbAiHome(), "channels", "feishu", "accounts");
}
