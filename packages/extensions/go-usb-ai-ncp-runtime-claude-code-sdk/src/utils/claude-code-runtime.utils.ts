...
import type { NcpAgentRunInput } from "@go-usb-ai/ncp";
...
function resolveGoUsbAiDataDir(env: Record<string, string | undefined>): string {
  const override = readEnvString(env[GOUSB_AI_HOME_ENV_KEY]);
  if (override) {
    return ensureDir(resolve(override));
  }
  return ensureDir(resolve(homedir(), GOUSB_AI_DEFAULT_HOME_DIR));
}
...
function resolveClaudeConfigDir(env: Record<string, string | undefined>): string {
  ...
  return ensureDir(resolve(resolveGoUsbAiDataDir(env), "runtime", "claude-code"));
}
...
export async function resolveClaudeGatewayAccess(params: {
  ...
  const fallbackCredential =
    ...
    "go-usb-ai-local-claude-gateway";
