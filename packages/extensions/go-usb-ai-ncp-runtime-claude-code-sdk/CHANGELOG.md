...
- Expose Claude reasoning, tool call, and tool result events through the shared NCP event stream so GoUsbAi chat renders Claude sessions like other runtimes.
...
- Add the first Claude-backed NCP runtime and official plugin package so GoUsbAi can expose Claude sessions through the pluggable runtime registry and marketplace flow.
