import type { NcpAgentRuntime } from "@go-usb-ai/ncp";
import {
  NarpStdioRuntimeWrapper,
  type NarpStdioRuntimeWrapperContext,
} from "@go-usb-ai/go-usb-ai-narp-stdio-runtime-wrapper";
import {
  ClaudeCodeSdkNcpAgentRuntime,
  type ClaudeCodeSdkNcpAgentRuntimeConfig,
} from "@go-usb-ai/go-usb-ai-ncp-runtime-claude-code-sdk";

const NARP_API_MODE_HEADER = "x-go-usb-ai-narp-api-mode";
...
export class ClaudeCodeNarpRuntimeWrapper {
  ...
  start = (): void => {
    new NarpStdioRuntimeWrapper({
      agentName: "GoUsbAi Claude Code NARP",
  ...
  buildRuntimeConfig = (
    ...
    const model =
      ...
      readString(modelId) ??
      readString(process.env.GOUSB_AI_MODEL) ??
