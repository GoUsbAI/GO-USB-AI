...
describe("createPluginRuntime agent helpers", () => {
  it("exposes agent runtime helpers for plugin-side provider and prompt resolution", () => {
    const runtime = createPluginRuntime({
      workspace: "/tmp/go-usb-ai-plugin-runtime",
      config: {
        agents: {
          defaults: {
            workspace: "/tmp/go-usb-ai-plugin-runtime-default",
    ...
    expect(runtime.agent.resolveWorkspacePath()).toBe("/tmp/go-usb-ai-plugin-runtime");
    expect(
    ...
    expect(prompt).toContain("GoUsbAi host workspace directory: /tmp/go-usb-ai-plugin-runtime");
  });
});
...
describe("createPluginRuntime workspace context", () => {
  it("loads both project and host workspace context without losing host workspace skills", () => {
    const hostWorkspace = createTempWorkspace("go-usb-ai-plugin-runtime-host-");
    const projectWorkspace = createTempWorkspace("go-usb-ai-plugin-runtime-project-");
    ...
    expect(prompt).toContain(`GoUsbAi host workspace directory: ${hostWorkspace}`);
