...
function notAvailable(functionName: string): never {
  throw new Error(
    `@mariozechner/pi-coding-agent shim: ${functionName} is not available inside the GoUsbAi plugin loader runtime`
  );
}
...
export const CURRENT_SESSION_VERSION = "go-usb-ai-plugin-loader-shim";
