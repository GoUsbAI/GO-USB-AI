...
const createTempPluginDir = (pluginId: string): string => {
  const dir = mkdtempSync(join(tmpdir(), "go-usb-ai-plugin-uninstall-"));
  tempDirs.push(dir);
...
describe("uninstallPlugin", () => {
  it("removes managed plugin copies from both global and workspace extension directories", async () => {
    const homeDir = mkdtempSync(join(tmpdir(), "go-usb-ai-plugin-home-"));
    const workspaceDir = mkdtempSync(join(tmpdir(), "go-usb-ai-plugin-workspace-"));
    ...
    const workspaceExtensionsDir = join(workspaceDir, ".go-usb-ai", "extensions");
