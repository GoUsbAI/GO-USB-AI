import type { OpenAITool, NcpProviderRuntimeRoute } from "@go-usb-ai/ncp";

export const NARP_STDIO_PROMPT_META_KEY = "go-usb-ai_narp";
const DEFAULT_STARTUP_TIMEOUT_MS = 10_000;
const DEFAULT_PROBE_TIMEOUT_MS = 3_000;
const DEFAULT_REQUEST_TIMEOUT_MS = 120_000;
const DEFAULT_RUNTIME_ROUTE_BRIDGE_FIELDS = {
  model: "GOUSB_AI_MODEL",
  apiBase: "GOUSB_AI_API_BASE",
  apiKey: "GOUSB_AI_API_KEY",
  headers: "GOUSB_AI_HEADERS_JSON",
} as const;
...
export class StdioRuntimeConfigResolver {
  ...
  resolve = (): StdioRuntimeResolvedConfig => {
    ...
    const command =
      ...
      readString(process.env.GOUSB_AI_NARP_STDIO_COMMAND);
    if (!command) {
    ...
    return {
      ...
      args:
        ...
        readStringArray(parseJsonArray(process.env.GOUSB_AI_NARP_STDIO_ARGS)) ??
        [],
      ...(readString(this.source.cwd) ?? readString(process.env.GOUSB_AI_NARP_STDIO_CWD)
        ? {
            cwd:
              readString(this.source.cwd) ??
              readString(process.env.GOUSB_AI_NARP_STDIO_CWD),
          }
        : {}),
      ...(readStringRecord(this.source.env) ?? readStringRecord(parseJsonObject(process.env.GOUSB_AI_NARP_STDIO_ENV))
        ? {
            env:
              readStringRecord(this.source.env) ??
              readStringRecord(parseJsonObject(process.env.GOUSB_AI_NARP_STDIO_ENV)),
          }
        : {}),
      startupTimeoutMs:
        readPositiveInteger(this.source.startupTimeoutMs) ??
        readPositiveInteger(process.env.GOUSB_AI_NARP_STDIO_STARTUP_TIMEOUT_MS) ??
