...
import { RestartCommands } from "@go-usb-ai-cli/cli/commands/restart/index.js";
...
vi.mock("@go-usb-ai-cli/cli/shared/stores/local-ui-runtime.store.js", () => ({
  localUiRuntimeStore: mocks.localUiRuntimeStore
}));

vi.mock("@go-usb-ai-cli/cli/shared/stores/managed-service-state.store.js", () => ({
  managedServiceStateStore: mocks.managedServiceStateStore
}));

vi.mock("@go-usb-ai-cli/cli/shared/utils/cli.utils.js", () => ({
...
vi.mock("@go-usb-ai-cli/cli/shared/utils/runtime-helpers.js", () => ({
  resolveManagedServiceUiOverrides: (...args: unknown[]) => mocks.resolveManagedServiceUiOverrides(...args)
}));

vi.mock("@go-usb-ai-cli/cli/shared/services/runtime/runtime-command.service.js", () => ({
...
describe("RestartCommands", () => {
  ...
  it("keeps refusing an unrelated healthy listener when no tracked foreground runtime matches the target port", async () => {
    ...
    expect(errorSpy).toHaveBeenCalledWith(
      "Error: Cannot restart go-usb-ai because the target UI/API port is already served by a healthy unmanaged instance."
    );
    errorSpy.mockRestore();
  });

  it("adopts a healthy go-usb-ai listener on the target port when local runtime state is stale", async () => {
    const processKillSpy = vi.spyOn(process, "kill").mockImplementation(() => true as never);
    const runtimeCommandService = {
      stopService: vi.fn()
    };
    ...
    mocks.cliUtils.findListeningProcessByPort.mockReturnValue({
      pid: 46598,
      command: "/Users/peiwang/.nvm/versions/node/v22.16.0/bin/node /Users/peiwang/.nvm/versions/node/v22.16.0/lib/node_modules/go-usb-ai/dist/cli/app/index.js serve"
    });
    mocks.cliUtils.isProcessRunning.mockImplementation((pid: number) => pid === 46598);
    mocks.describeUnmanagedHealthyTargetMessage.mockResolvedValue("healthy unmanaged listener");
