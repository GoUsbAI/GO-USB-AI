...
import { APP_NAME, APP_TAGLINE } from "@go-usb-ai/core";
import { registerRemoteCommands } from "@go-usb-ai/remote";
import { GoUsbAiDistributionService, GoUsbAiServiceRuntime } from "@go-usb-ai/service";
...
const runtime = new GoUsbAiServiceRuntime({
  logo: LOGO
});
const withRepeatableTag = (value: string, previous: string[] = []) => [...previous, value];
const registerRemoteCommandGroup = (target: Command, gousb: typeof runtime): void => {
  registerRemoteCommands(target, gousb.commands.remote);
};
...
program
  .command("login")
  .description("Sign in to GoUsbAi Platform and save the platform token locally (browser flow by default)")
...
const account = program.command("account").description("Inspect and manage your GoUsbAi account");
...
account
  ...
  .description("Set your GoUsbAi username for personal marketplace publishing")
  .option("--api-base <url>", "Platform API base (supports /v1 suffix)")
  .option("--json", "Output JSON", false)
