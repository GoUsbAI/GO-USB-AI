...
import type { GoUsbAiServiceRuntime } from "@go-usb-ai/service";

type RegisterHostServiceControlsParams = {
  program: Command;
  gousb: GoUsbAiServiceRuntime;
};
...
export const registerHostServiceControls = ({
  ...
  const serviceCommands = gousb.commands.service;
  const service = program.command("service").description("Manage host service integrations");

  service
    .command("install-systemd")
    .description("Install a managed Linux systemd service for GoUsbAi")
  ...
  service
    ...
    .description("Remove a managed Linux systemd service for GoUsbAi")
  ...
  service
    ...
    .description("Install a managed macOS LaunchAgent for GoUsbAi")
    .option("--dry-run", "Show what would be installed without making changes", false)
    .option("--json", "Output JSON", false)
  ...
  service
    ...
    .description("Remove a managed macOS LaunchAgent for GoUsbAi")
    .option("--dry-run", "Show what would be removed without making changes", false)
    .option("--json", "Output JSON", false)
  ...
  service
    ...
    .description("Install a managed Windows Scheduled Task for GoUsbAi")
    .option("--dry-run", "Show what would be installed without making changes", false)
    .option("--json", "Output JSON", false)
  ...
  service
    ...
    .description("Remove a managed Windows Scheduled Task for GoUsbAi")
    .option("--dry-run", "Show what would be removed without making changes", false)
    .option("--json", "Output JSON", false)
