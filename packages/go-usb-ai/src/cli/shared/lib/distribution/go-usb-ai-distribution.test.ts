...
import { createGoUsbAiDistribution } from "./go-usb-ai-distribution.utils.js";

describe("createGoUsbAiDistribution", () => {
  it("derives package-owned distribution metadata from the package entrypoint", () => {
    const packageRoot = mkdtempSync(resolve(tmpdir(), "go-usb-ai-distribution-"));

    try {
      writeFileSync(resolve(packageRoot, "package.json"), JSON.stringify({ version: "0.19.4" }));

      const distribution = createGoUsbAiDistribution(
