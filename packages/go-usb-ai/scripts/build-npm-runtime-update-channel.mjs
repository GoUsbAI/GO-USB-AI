...
const DEFAULT_BASE_URL = "https://Peiiii.github.io/go-usb-ai/npm-runtime-updates";
...
function resolvePrivateKey(args) {
  const inlineKey =
    ...
    process.env.GOUSB_AI_UPDATE_BUNDLE_PRIVATE_KEY?.trim() ||
    process.env.GOUSB_AI_DESKTOP_BUNDLE_PRIVATE_KEY?.trim();
  if (inlineKey) {
  ...
  const privateKeyPath =
    ...
    process.env.GOUSB_AI_UPDATE_BUNDLE_PRIVATE_KEY_FILE?.trim() ||
    process.env.GOUSB_AI_DESKTOP_BUNDLE_PRIVATE_KEY_FILE?.trim();
  if (privateKeyPath) {
    return createPrivateKey(readFileSync(resolve(privateKeyPath), "utf8"));
  }

  throw new Error(
    "Missing runtime update signing key. Provide --private-key, --private-key-file, GOUSB_AI_UPDATE_BUNDLE_PRIVATE_KEY, or GOUSB_AI_UPDATE_BUNDLE_PRIVATE_KEY_FILE."
  );
}
...
class NpmRuntimeUpdateChannelBuilder {
  ...
  ensureFreshRuntimeArtifacts = () => {
    [["--filter", "go-usb-ai...", "build"], ["install", "--frozen-lockfile", "--offline"]].forEach((args) => runCommand("pnpm", args));
  };
  ...
  createWorkspaceTempRoot = () => {
    ...
    return mkdtempSync(join(tempParent, "go-usb-ai-npm-runtime-bundle-"));
  };
  ...
  prepareBundleWorkspace = async (workspace) => {
    ...
    runCommand(
      "pnpm",
      ["--config.node-linker=hoisted", "--filter", "go-usb-ai", "--prod", "deploy", workspace.runtimeDeployPath],
      { cwd: workspaceRoot }
    );
  ...
  writeBundleArchive = async (bundleRoot) => {
    ...
    const archivePath = resolve(channelDir, `go-usb-ai-runtime-${this.platform}-${this.arch}-${this.version}.zip`);
