...
const TRUNCATION_MARKER = "[GoUsbAi tool result truncated]";
...
export class ToolResultContentManager {
  ...
  private readonly redactObject = (
    ...
    if (entries.length > this.maxObjectKeys) {
      output["_goUsbAiOmittedKeys"] = entries.length - this.maxObjectKeys;
    }
    return output;
  };
  ...
  private readonly attachNotice = (
    ...
    if (value && typeof value === "object" && !Array.isArray(value)) {
      return {
        ...(value as Record<string, unknown>),
        "_goUsbAiToolResultNotice": notice,
      };
    }
    return {
      type: "goUsbAi.tool_result_sanitized",
      notice,
      result: value,
    };
  };
  ...
  private readonly buildEnvelope = (params: {
    ...
    return {
      type: "go-usb-ai.tool_result_truncated",
