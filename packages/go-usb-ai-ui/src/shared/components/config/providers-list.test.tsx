...
const mocks = vi.hoisted(() => ({
  ...
  configQuery: {
    data: {
      providers: {
        ...
        "go-usb-ai": { enabled: true, apiKeySet: true },
      },
    },
    isLoading: false,
  },
  metaQuery: {
    data: {
      providers: [
        { name: 'go-usb-ai', displayName: 'GO-USB-AI Builtin', defaultApiBase: 'https://ai-gateway-api.go-usb-ai.io/v1' },
...
describe('ProvidersList', () => {
  it('keeps the go-usb-ai builtin provider at the end of the list', () => {
    ...
    const providerButtons = Array.from(sidebarSection.querySelectorAll('button[type="button"]')).filter((button) =>
      ['OpenAI', 'Anthropic', 'GO-USB-AI Builtin'].some((label) => button.textContent?.includes(label)),
    );

    expect(providerButtons.map((button) => button.textContent)).toEqual([
      expect.stringContaining('OpenAI'),
      expect.stringContaining('Anthropic'),
      expect.stringContaining('GO-USB-AI Builtin'),
    ]);
  });
});
