import {
  GoUsbAiClient,
  type GoUsbAiRealtimeEvent,
  type GoUsbAiQueryParams,
  type GoUsbAiTransport,
  type GoUsbAiTransportRequestInput,
  type GoUsbAiTransportUploadInput
} from '@go-usb-ai/client-sdk';
import { API_BASE } from '@/shared/lib/api/api-base';
import { appClient } from '@/shared/lib/transport';
import type { ApiResponse } from '@/shared/lib/api/types';

const goUsbAiUiTransport: GoUsbAiTransport = {
  request: async <T>({ body, method, path, query, signal, timeoutMs }: GoUsbAiTransportRequestInput): Promise<T> => {
  ...
  upload: async <T>({ formData, headers, path, signal }: GoUsbAiTransportUploadInput): Promise<T> => {
  ...
  subscribe: (handler: (event: GoUsbAiRealtimeEvent) => void) => appClient.subscribe(handler)
};

export const goUsbAiClient = new GoUsbAiClient({
  baseUrl: API_BASE,
  transport: goUsbAiUiTransport
});
...
function serializeQuery(query?: GoUsbAiQueryParams): URLSearchParams | undefined {
  if (!query) {
    return undefined;
  }
