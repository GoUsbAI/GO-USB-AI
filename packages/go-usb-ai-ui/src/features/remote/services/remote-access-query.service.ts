...
const DEFAULT_GOUSB_AI_WEB_BASE = 'https://platform.go-usb-ai.io';
const PREVIEW_GOUSB_AI_WEB_BASE = 'https://go-usb-ai-platform-console.pages.dev';
...
function resolveWebBaseFromRawUrl(rawUrl: string | null | undefined): string | undefined {
  ...
  if (parsedUrl.hostname === 'platform.go-usb-ai.io' || parsedUrl.hostname === 'go-usb-ai-platform-console.pages.dev') {
    return parsedUrl.origin;
  }

  if (parsedUrl.hostname === 'ai-gateway-api.go-usb-ai.io') {
    return DEFAULT_GOUSB_AI_WEB_BASE;
  }

  if (parsedUrl.hostname.includes('go-usb-ai-provider-gateway-api') && parsedUrl.hostname.endsWith('.workers.dev')) {
    return PREVIEW_GOUSB_AI_WEB_BASE;
  }
