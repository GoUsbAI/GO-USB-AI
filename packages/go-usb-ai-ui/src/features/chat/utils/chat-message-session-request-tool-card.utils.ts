...
import type { ChatToolPartViewModel } from "@go-usb-ai/agent-chat-ui";
...
function readSessionRequestResult(value: unknown): SessionRequestResult | null {
  if (!isRecord(value) || value.kind !== "go-usb-ai.session_request") {
    return null;
  }
...
function readSessionSpawnResult(value: unknown): SessionSpawnResult | null {
  if (!isRecord(value) || value.kind !== "go-usb-ai.session") {
    return null;
  }
