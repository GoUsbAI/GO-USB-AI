...
const PWA_DEV_RESET_SESSION_KEY = 'go-usb-ai-pwa-dev-sw-reset';
...
export class PwaRuntimeManager {
  ...
  private cleanupDevelopmentRegistrations = async () => {
    ...
    const goUsbAiRegistrations = registrations.filter((registration) =>
    ...
    if (goUsbAiRegistrations.length === 0) {
      this.clearDevResetMarker();
      return;
    }

    await Promise.all(goUsbAiRegistrations.map(async (registration) => await registration.unregister()));
    await this.clearGoUsbAiCaches();
  ...
  private clearGoUsbAiCaches = async () => {
    if (typeof window === 'undefined' || !('caches' in window) || typeof caches.keys !== 'function') {
      return;
    }

    const keys = await caches.keys();
    await Promise.all(keys.filter((key) => key.startsWith('go-usb-ai-ui-')).map(async (key) => await caches.delete(key)));
  };
