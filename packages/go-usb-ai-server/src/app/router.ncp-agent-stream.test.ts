...
import { NcpEventType } from "@go-usb-ai/ncp";
import { EventBus, eventKeys } from "@go-usb-ai/shared";
...
function createTempConfigPath(): string {
  return join(createTempDir("go-usb-ai-ui-ncp-stream-config-"), "config.json");
}

function useIsolatedHome(): void {
  process.env.GOUSB_AI_HOME = createTempDir("go-usb-ai-ui-ncp-stream-home-");
}
