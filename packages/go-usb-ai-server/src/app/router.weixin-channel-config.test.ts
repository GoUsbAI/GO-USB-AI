...
import { ConfigSchema, loadConfig, saveConfig } from "@go-usb-ai/core";
...
function createTempConfigPath(): string {
  const dir = mkdtempSync(join(tmpdir(), "go-usb-ai-ui-weixin-config-test-"));
  tempDirs.push(dir);
...
function createWeixinPluginBinding(): PluginChannelBinding {
  return {
    pluginId: "go-usb-ai-channel-weixin",
    channelId: "weixin",
    channel: {
      id: "weixin",
      meta: {
...
describe("weixin plugin channel config route", () => {
  it("projects weixin into UI config meta, schema, and update flow", async () => {
    ...
    expect(saved.plugins.entries?.["go-usb-ai-channel-weixin"]).toEqual({
      enabled: true
    });
