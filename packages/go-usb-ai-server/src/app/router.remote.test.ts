...
function createTempConfigPath(): string {
  const dir = mkdtempSync(join(tmpdir(), "go-usb-ai-ui-remote-router-test-"));
  tempDirs.push(dir);
...
function createRemoteHost(): UiRemoteAccessHost {
  return {
    ...
    startBrowserAuth: vi.fn(async () => ({
      ...
      verificationUri: "https://ai-gateway-api.go-usb-ai.io/platform/auth/browser?sessionId=session-123",
