...
function createTempConfigPath(): string {
  const dir = mkdtempSync(join(tmpdir(), "go-usb-ai-ui-provider-enabled-test-"));
  tempDirs.push(dir);
...
describe("provider enabled state route", () => {
  it("exposes built-in go-usb-ai provider as disabled by default for empty config", async () => {
    ...
    expect(configPayload.data.providers.go-usb-ai.enabled).toBe(false);
    expect(configPayload.data.providers.go-usb-ai.apiKeySet).toBe(true);
  });
