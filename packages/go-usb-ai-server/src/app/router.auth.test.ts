...
import { UiAuthService } from "@go-usb-ai-server/features/auth/index.js";
...
function createTempConfigPath(): string {
  const dir = createTempDir("go-usb-ai-ui-auth-config-");
  return join(dir, "config.json");
}

function useIsolatedHome(): string {
  const homeDir = createTempDir("go-usb-ai-ui-auth-home-");
  process.env.GOUSB_AI_HOME = homeDir;
  return homeDir;
}
