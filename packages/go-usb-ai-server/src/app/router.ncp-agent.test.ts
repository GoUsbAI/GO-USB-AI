...
import { ConfigSchema, getSkillsPath, getWorkspacePath, saveConfig } from "@go-usb-ai/core";
import {
  NcpEventType,
...
function createTempConfigPath(): string {
  const dir = createTempDir("go-usb-ai-ui-ncp-config-");
  return join(dir, "config.json");
}

function useIsolatedHome(): void {
  process.env.GOUSB_AI_HOME = createTempDir("go-usb-ai-ui-ncp-home-");
}
...
class StubNcpAgent implements NcpSessionApi {
  private readonly attachmentRootDir = createTempDir("go-usb-ai-ui-ncp-attachments-");
...
it("proxies ncp send, patch, and abort flows", async () => {
  const { app, agent } = createTestApp();
  const validProjectRoot = realpathSync(createTempDir("go-usb-ai-ui-ncp-project-root-"));
...
it("exposes session-scoped skills for persisted and draft sessions", async () => {
  useIsolatedHome();
  const configPath = createTempConfigPath();
  const hostWorkspace = createTempDir("go-usb-ai-ui-host-workspace-");
  const projectRoot = realpathSync(createTempDir("go-usb-ai-ui-session-project-"));
...
it("exposes skills installed in the GOUSB_AI_HOME default workspace", async () => {
  useIsolatedHome();
  const configPath = createTempConfigPath();
...
it("creates a lightweight session when patching a draft session", async () => {
  const { app } = createTestApp();
  const validProjectRoot = realpathSync(createTempDir("go-usb-ai-ui-draft-project-root-"));
