...
import { ConfigSchema, DEFAULT_WORKSPACE_PATH, loadConfig, saveConfig } from "@go-usb-ai/core";
...
function createTempConfigPath(): string {
  const dir = mkdtempSync(join(tmpdir(), "go-usb-ai-ui-model-config-test-"));
  tempDirs.push(dir);
  ...
  it("persists workspace updates and normalizes blank values", async () => {
    ...
    const updateResponse = await app.request("http://localhost/api/config/model", {
      ...
      body: JSON.stringify({
        ...
        workspace: "  ~/projects/go-usb-ai-workspace  "
      })
    });
    ...
    expect(updatePayload.data.workspace).toBe("~/projects/go-usb-ai-workspace");
    expect(loadConfig(configPath).agents.defaults.workspace).toBe("~/projects/go-usb-ai-workspace");
