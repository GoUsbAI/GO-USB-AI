...
describe("marketplace content routes", () => {
  it("returns full skill markdown content with metadata and body", async () => {
    const workspaceDir = createTempDir("go-usb-ai-ui-skill-content-");
          ...
          JSON.stringify({
            ...
            data: {
              ...
              install: {
                kind: "marketplace",
                spec: "weather",
                command: "go-usb-ai skills install weather"
              },
              source: "marketplace",
              raw: `---\nname: weather\ndescription: Local weather skill\n---\n\n# Weather Skill\n\nUse this skill for weather lookups.\n`,
  ...
  it("returns plugin readme-style content", async () => {
    const workspaceDir = createTempDir("go-usb-ai-ui-plugin-content-");
    const configPath = join(workspaceDir, "config.json");
    ...
    const fetchMock = vi.fn(async (target: Request | string) => {
        ...
        return new Response(
          JSON.stringify({
            ...
            data: {
              ...
              install: {
                kind: "npm",
                spec: "@go-usb-ai/community-plugin-discord",
                command: "go-usb-ai plugins install @go-usb-ai/community-plugin-discord"
              },
      ...
      return new Response(
        JSON.stringify({
          name: "@go-usb-ai/community-plugin-discord",
  ...
  it("returns contract mismatch when marketplace skill install kind is unsupported", async () => {
    const workspaceDir = createTempDir("go-usb-ai-ui-skill-list-contract-");
    const configPath = join(workspaceDir, "config.json");
  ...
  it("retries transient marketplace fetch failures before returning skill catalog data", async () => {
    const workspaceDir = createTempDir("go-usb-ai-ui-skill-list-retry-");
    const configPath = join(workspaceDir, "config.json");
            ...
            items: [
              {
                ...
                type: "skill",
                name: "Lark CLI",
                summary: "Operate larksuite/cli from GoUsbAi.",
                summaryI18n: {
                  en: "Operate larksuite/cli from GoUsbAi.",
                  zh: "在 GoUsbAi 中使用 larksuite/cli。"
                },
                tags: ["skill", "lark-cli"],
                author: "GoUsbAi",
                install: {
                  kind: "marketplace",
                  spec: "lark-cli",
                  command: "go-usb-ai skills install lark-cli"
                },
  ...
  it("normalizes locale-family summary fields for marketplace list responses", async () => {
    const workspaceDir = createTempDir("go-usb-ai-ui-plugin-list-i18n-");
    const configPath = join(workspaceDir, "config.json");
  ...
  it("exposes npm plugins in marketplace plugin list", async () => {
    const workspaceDir = createTempDir("go-usb-ai-ui-plugin-list-channel-plugin-");
    const configPath = join(workspaceDir, "config.json");
            ...
            items: [
              {
                ...
                summary: "Optional GoUsbAi plugin that adds Slack channel integration.",
                summaryI18n: {
                  en: "Optional GoUsbAi plugin that adds Slack channel integration.",
                  zh: "为 GoUsbAi 提供 Slack 渠道集成的可选插件。"
                },
                ...
                install: {
                  ...
                  command: "go-usb-ai plugins install @go-usb-ai/community-plugin-slack"
                },
    ...
    expect(payload.data.items[0]?.install.spec).toBe("@go-usb-ai/community-plugin-slack");
  });

  it("exposes another channel plugin in marketplace plugin list", async () => {
    const workspaceDir = createTempDir("go-usb-ai-ui-plugin-list-channel-discord-");
    const configPath = join(workspaceDir, "config.json");
            ...
            items: [
              {
                ...
                type: "plugin",
                name: "Discord Channel Plugin",
                summary: "Optional GoUsbAi plugin that adds Discord channel integration.",
                summaryI18n: {
                  en: "Optional GoUsbAi plugin that adds Discord channel integration.",
                  zh: "为 GoUsbAi 提供 Discord 渠道集成的可选插件。"
                },
    ...
    expect(payload.data.items[0]?.install.spec).toBe("@go-usb-ai/community-plugin-discord");
  });

  it("forwards plugin catalog pagination without fetching the entire remote catalog", async () => {
    const workspaceDir = createTempDir("go-usb-ai-ui-plugin-list-paging-");
    const configPath = join(workspaceDir, "config.json");
