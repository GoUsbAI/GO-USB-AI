...
function createTempConfigPath(): string {
  const dir = createTempDir("go-usb-ai-ui-ncp-runtime-config-");
  return join(dir, "config.json");
}

function useIsolatedHome(): void {
  process.env.GOUSB_AI_HOME = createTempDir("go-usb-ai-ui-ncp-runtime-home-");
}
...
it("routes ncp send through AgentRunRequestManager and stores the assistant reply", async () => {
  ...
  const sessionsDir = createTempDir("go-usb-ai-ui-ncp-runtime-sessions-");
