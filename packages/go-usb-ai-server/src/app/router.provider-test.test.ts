...
function createTempConfigPath(): string {
  const dir = mkdtempSync(join(tmpdir(), "go-usb-ai-ui-provider-test-"));
  tempDirs.push(dir);
...
describe("provider connection test route", () => {
  ...
  it("imports qwen-portal access token from qwen cli credentials", async () => {
    const configPath = createTempConfigPath();
    saveConfig(ConfigSchema.parse({}), configPath);
    const fakeHome = mkdtempSync(join(tmpdir(), "go-usb-ai-qwen-home-"));
  ...
  it("returns 400 when qwen cli credentials file is missing", async () => {
    const configPath = createTempConfigPath();
    saveConfig(ConfigSchema.parse({}), configPath);
    const fakeHome = mkdtempSync(join(tmpdir(), "go-usb-ai-qwen-home-missing-"));
    tempDirs.push(fakeHome);
  ...
  it("returns 400 when imported qwen cli credential is expired", async () => {
    const configPath = createTempConfigPath();
    saveConfig(ConfigSchema.parse({}), configPath);
    const fakeHome = mkdtempSync(join(tmpdir(), "go-usb-ai-qwen-home-expired-"));
    tempDirs.push(fakeHome);
    const qwenDir = join(fakeHome, ".qwen");
    mkdirSync(qwenDir, { recursive: true });
    writeFileSync(
