...
function createTempConfigPath(): string {
  const dir = createTempDir("go-usb-ai-ui-server-path-config-");
...
describe("ServerPathRoutesController", () => {
  it("browses server directories and filters out files by default", async () => {
    ...
    const root = realpathSync(createTempDir("go-usb-ai-ui-server-path-root-"));
  ...
  it("reads a text file preview relative to a base path", async () => {
    const app = createTestApp();
    const root = realpathSync(createTempDir("go-usb-ai-ui-server-path-read-root-"));
  ...
  it("returns binary metadata for non-text files instead of forcing a text preview", async () => {
    const app = createTestApp();
    const root = realpathSync(createTempDir("go-usb-ai-ui-server-path-read-binary-"));
