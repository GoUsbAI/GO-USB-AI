...
import { discoverPluginStatusReport } from "@go-usb-ai/openclaw-compat";
...
import type { SkillsLoaderConstructor, SkillsLoaderInstance, UiRouterOptions } from "@go-usb-ai-server/app/types/router-options.types.js";
...
const getWorkspacePathFromConfig = GoUsbAiCore.getWorkspacePathFromConfig;
...
function createSkillsLoader(workspace: string): SkillsLoaderInstance | null {
  const ctor = (GoUsbAiCore as { SkillsLoader?: SkillsLoaderConstructor }).SkillsLoader;
  if (!ctor) {
    return null;
  }
