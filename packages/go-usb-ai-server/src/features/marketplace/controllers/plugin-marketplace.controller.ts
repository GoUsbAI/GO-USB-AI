...
import { err, isRecord, ok, readJson } from "@go-usb-ai-server/shared/utils/http-response.utils.js";
import type { UiRouterOptions } from "@go-usb-ai-server/app/types/router-options.types.js";
import { emitConfigUpdated } from "@go-usb-ai-server/shared/utils/app-events.utils.js";
import {
  fetchMarketplaceData,
  normalizeMarketplaceItemForUi,
  sanitizeMarketplaceItemView,
  sanitizeMarketplaceListItems
} from "@go-usb-ai-server/features/marketplace/utils/marketplace-catalog.utils.js";
import {
  collectPluginMarketplaceInstalledView,
  isSupportedMarketplacePluginItem,
  resolvePluginManageTargetId
} from "@go-usb-ai-server/features/marketplace/utils/marketplace-installed.utils.js";
...
export class PluginMarketplaceController {
  ...
  readonly getItem = async (c: Context) => {
    ...
    if (!isSupportedMarketplacePluginItem(sanitized)) {
      return c.json(err("NOT_FOUND", "marketplace item not supported by go-usb-ai"), 404);
    }
