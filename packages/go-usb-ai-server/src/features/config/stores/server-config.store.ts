...
import type { LlmProviderManager } from "@go-usb-ai/kernel";
import { createDefaultProviderConfigFromSpec } from "@go-usb-ai-server/features/config/utils/default-provider-config.utils.js";
import {
...
} from "@go-usb-ai-server/features/config/utils/plugin-channel-config-projection.utils.js";
import { findServerBuiltinProviderByName, listServerBuiltinProviders } from "@go-usb-ai-server/features/config/providers/server-builtin-provider.provider.js";
import { buildSearchView, SEARCH_PROVIDER_META } from "@go-usb-ai-server/features/config/utils/search-config.utils.js";
import type {
  ConfigMetaView,
  RuntimeConfigUpdate,
  ...
  SecretsConfigUpdate,
  SecretsView
} from "@go-usb-ai-server/shared/types/server-api.types.js";
export { updateSearch } from "@go-usb-ai-server/features/config/utils/search-config.utils.js";
...
async function runFeishuVerifyAction(params: {
  ...
  if (!result.ok) {
    return {
      ok: false,
      status: "failed",
      message: `${messageOrDefault(action, "failure", "Verification failed")}: ${result.detail}`,
      data: {
        error: result.detail,
        appId
      },
...
export function buildConfigView(config: Config, options?: PluginConfigProjectionOptions): ConfigView {
  ...
  return {
    ...
    mcp: sanitizePublicConfigValue(config.mcp, "mcp", uiHints),
    secrets: {
