...
import { eventKeys, GoUsbAiClient, GoUsbAiClientError } from "./index.js";

describe("@go-usb-ai/client-sdk", () => {
  it("lists sessions from the existing ncp api", async () => {
    ...
    const client = new GoUsbAiClient({
  ...
  it("throws a typed client error for api failures", async () => {
    ...
    await expect(client.sessions.list()).rejects.toBeInstanceOf(GoUsbAiClientError);
  });
});
