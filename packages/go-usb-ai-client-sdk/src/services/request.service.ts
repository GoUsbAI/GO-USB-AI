import type { ApiError, ApiResponse } from "@go-usb-ai/server";
import type {
  GoUsbAiClientOptions,
  GoUsbAiRequestOptions,
  GoUsbAiUploadOptions
} from "../types/go-usb-ai-request.types.js";
import type { GoUsbAiQueryParams } from "../types/go-usb-ai-transport.types.js";
import { appendQueryToPath, resolveApiUrl } from "../utils/url.utils.js";

export class GoUsbAiClientError extends Error {
...
export class RequestService {
  ...
  readonly request = async <T>(path: string, options: GoUsbAiRequestOptions = {}): Promise<T> => {
    ...
    try {
      ...
      if (!this.isApiResponse<T>(payload)) {
        throw new GoUsbAiClientError({
          message: "Unexpected GoUsbAi API response shape.",
          status: response.status
        });
      }
      ...
      if (error instanceof GoUsbAiClientError) {
        throw error;
      }
      if (error instanceof Error && error.name === "AbortError") {
        throw new GoUsbAiClientError({
          message: `GoUsbAi API request timed out after ${effectiveTimeoutMs}ms.`
        });
      }
      throw new GoUsbAiClientError({
        message: error instanceof Error ? error.message : "GoUsbAi API request failed."
      });
    } finally {
  ...
  readonly get = async <T>(
    path: string,
    options: Omit<GoUsbAiRequestOptions, "method" | "body"> = {}
  ): Promise<T> => {
  ...
  readonly upload = async <T>(
    ...
    try {
      ...
      if (!this.isApiResponse<T>(payload) || !payload.ok) {
        throw new GoUsbAiClientError({
          message: "Unexpected GoUsbAi upload response shape.",
          status: response.status
        });
      }
      return payload.data;
      ...
      if (error instanceof Error && error.name === "AbortError") {
        throw new GoUsbAiClientError({
          message: `GoUsbAi API upload timed out after ${effectiveTimeoutMs}ms.`
        });
      }
      throw new GoUsbAiClientError({
        message: error instanceof Error ? error.message : "GoUsbAi API upload failed."
      });
    } finally {
      signal?.removeEventListener("abort", abortOnSignal);
      clearTimeout(timeoutId);
  ...
  private readonly extractApiError = (value: unknown): ApiError => {
    ...
    if (value !== null && typeof value === "object" && "message" in value) {
      ...
      return {
        ...
        message: typeof errorCandidate.message === "string" ? errorCandidate.message : "GoUsbAi API request failed.",
        details:
    ...
    return {
      code: "HTTP_ERROR",
      message: "GoUsbAi API request failed."
    };
  };
  ...
  private readonly serializeQuery = (query?: GoUsbAiQueryParams): URLSearchParams | undefined => {
    if (!query) {
      return undefined;
    }
