...
import { DefaultNcpStreamEncoder } from "@go-usb-ai/ncp-agent-runtime";
import {
  NcpEventType,
...
class HermesHttpAdapterRouteService {
  ...
  private readProviderRoute = (
    ...
    if (!providerRoute.apiBase) {
      throw new Error(
        `[hermes-http-adapter] missing provider apiBase for model "${providerRoute.model}". Configure the selected GoUsbAi provider before using Hermes.`,
      );
    }
