...
import { NcpHttpAgentClientEndpoint } from "@go-usb-ai/ncp-http-agent-client";
...
describe("HermesHttpAdapterServer", () => {
  ...
  it("forwards GoUsbAi provider routing and skips the legacy Hermes /model preflight", async () => {
    const hermes = await startMockHermesServer();
    const adapter = await startAdapterServer(hermes.baseUrl);
    const endpoint = new NcpHttpAgentClientEndpoint({
      baseUrl: adapter.baseUrl,
    ...
    expect(hermes.requests.chat[0]?.body.goUsbAi_provider_route).toEqual({
      model: "MiniMax-M2.7",
      provider: "openai",
      api_key: "minimax-key",
