  ...
  resolve = (): HermesHttpAdapterResolvedConfig => {
    const basePath = normalizeBasePath(
      this.source.basePath ??
        process.env.GOUSB_AI_HERMES_ADAPTER_BASE_PATH ??
        DEFAULT_BASE_PATH,
    );
    ...
    const resolved: HermesHttpAdapterConfig = {
      host:
        readString(this.source.host ?? process.env.GOUSB_AI_HERMES_ADAPTER_HOST) ??
        DEFAULT_HOST,
      port:
        readPositiveInteger(this.source.port) ??
        readPositiveInteger(process.env.GOUSB_AI_HERMES_ADAPTER_PORT) ??
        DEFAULT_PORT,
      basePath,
      hermesBaseUrl,
      ...
      streamWaitTimeoutMs:
        ...
        readPositiveInteger(process.env.GOUSB_AI_HERMES_ADAPTER_STREAM_WAIT_TIMEOUT_MS) ??
        DEFAULT_STREAM_WAIT_TIMEOUT_MS,
      healthcheckTimeoutMs:
        readPositiveInteger(this.source.healthcheckTimeoutMs) ??
        readPositiveInteger(process.env.GOUSB_AI_HERMES_ADAPTER_HEALTHCHECK_TIMEOUT_MS) ??
