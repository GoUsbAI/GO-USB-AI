...
describe("WorkspaceRepositoryIdentityResolver", () => {
  it("selects origin and normalizes GitHub SSH remotes into web URLs", () => {
    const repoRoot = createTempDir("go-usb-ai-repo-identity-");
    ...
    writeGitConfig(
      ...
      [
        '[remote "origin"]',
        "  url = ssh://git@ssh.github.com:443/Peiiii/go-usb-ai.git",
        '[remote "origin-https"]',
        "  url = https://github.com/Peiiii/go-usb-ai.git",
      ].join("\n"),
    );
    ...
    expect(identity.canonicalRemoteUrl).toBe("ssh://git@ssh.github.com:443/Peiiii/go-usb-ai.git");
    expect(identity.canonicalWebUrl).toBe("https://github.com/Peiiii/go-usb-ai");
  });

  it("reads linked gitdir config files used by git worktrees", () => {
    const repoRoot = createTempDir("go-usb-ai-repo-worktree-");
    const gitDir = createTempDir("go-usb-ai-repo-gitdir-");
    mkdirSync(join(repoRoot, ".git"), { recursive: true });
  ...
  it("returns an empty identity when the workspace is not inside a git repository", () => {
    const workspace = createTempDir("go-usb-ai-repo-none-");
    const resolver = new WorkspaceRepositoryIdentityResolver();
    const identity = resolver.resolve(workspace);
