...
describe("provider apiBase routing", () => {
    ...
    configureProviderCatalog([
      {
        ...
        providers: [
          {
            name: "go-usb-ai",
            keywords: ["go-usb-ai", "dashscope/", "qwen3.5", "qwen"],
            envKey: "GOUSB_AI_API_KEY",
            defaultApiBase: "https://ai-gateway-api.go-usb-ai.io/v1",
            isGateway: true,
            isLocal: false
          },
          {
  ...
  it("skips disabled providers during routing", () => {
    ...
    expect(getProviderName(config, "deepseek-chat")).toBe("go-usb-ai");
    expect(getApiBase(config, "deepseek-chat")).toBe("https://ai-gateway-api.go-usb-ai.io/v1");
  });
});
