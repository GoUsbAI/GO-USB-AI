...
function createTempConfigPath(): string {
  const dir = mkdtempSync(join(tmpdir(), "go-usb-ai-agent-profiles-test-"));
  tempDirs.push(dir);
...
describe("createAgentProfile", () => {
  ...
  it("creates default agent homes under GOUSB_AI_HOME when the workspace setting is default", () => {
    const configPath = createTempConfigPath();
    const home = mkdtempSync(join(tmpdir(), "go-usb-ai-agent-profiles-home-"));
    tempDirs.push(home);
