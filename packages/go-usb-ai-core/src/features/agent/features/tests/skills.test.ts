...
function createWorkspace(): string {
  const workspace = mkdtempSync(join(tmpdir(), "go-usb-ai-skills-test-"));
...
describe("SkillsLoader builtin skills", () => {
  it("loads builtin skills even when the workspace has no copied skill directories", () => {
    ...
    const skill = loader.listSkills(false).find((entry) => entry.name === "go-usb-ai-self-manage");
  ...
  it("loads the GoUsbAi autostart builtin skill", () => {
    const workspace = createWorkspace();
    const loader = new SkillsLoader(workspace);

    const skill = loader.listSkills(false).find((entry) => entry.name === "go-usb-ai-autostart");
    ...
    expect(loader.loadSkill("go-usb-ai-autostart")).toContain("go-usb-ai service autostart status");
  });
  ...
  it("does not let a workspace copy shadow a builtin skill with the same name", () => {
    const workspace = createWorkspace();
    const copiedDir = join(workspace, "skills", "go-usb-ai-self-manage");
    ...
    const loader = new SkillsLoader(workspace);
    const matches = loader.listSkills(false).filter((entry) => entry.name === "go-usb-ai-self-manage");
  ...
  it("keeps always-on builtin skills active", () => {
    ...
    expect(loader.getAlwaysSkills()).toContainEqual(
      expect.stringContaining("go-usb-ai-self-manage"),
    );
  });
});
