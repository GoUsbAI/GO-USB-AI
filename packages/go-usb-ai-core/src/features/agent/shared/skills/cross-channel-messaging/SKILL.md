...
description: Use when the user wants the AI to send, relay, or notify through another GoUsbAi session or chat channel, especially after work completes, including deciding between a normal reply and message, and resolving route/account details without guessing.
description_zh: 当用户希望 AI 通过另一个 GoUsbAi 会话或聊天渠道发送、转发、通知结果时使用，包含目标路由、账号和是否需要跨渠道发送的判断。
---

# Cross-Channel Messaging

This is a GoUsbAi built-in skill for the AI itself.

It does not introduce a new runtime abstraction. It teaches the AI how to use existing GoUsbAi messaging primitives in a predictable way.
...
4. Channel discovery command:
   If the exact channel id, default account, or bound self user is not already explicit, run `go-usb-ai channels list --json` and treat its output as authoritative channel state.
...
2. Channel ids and account facts returned by `go-usb-ai channels list --json`.
...
1. Run `go-usb-ai channels list --json`.
...
- run `go-usb-ai channels list --json` if `weixin`, the default account, or the bound user route is not already known,
...
Before asking the user for a Weixin `user_id`, check whether `go-usb-ai channels list --json` already exposes:
...
If `message` returns `unknown channel`, run `go-usb-ai channels list --json`, pick only an exact returned `id`, and retry only when the target route is otherwise unambiguous.
...
- Skipping `go-usb-ai channels list --json` when the exact channel id is unknown.
...
- and it keeps the solution lightweight by using existing GoUsbAi primitives only.
