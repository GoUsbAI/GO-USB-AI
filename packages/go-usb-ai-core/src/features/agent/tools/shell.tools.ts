  ...
  execute = async (params: Record<string, unknown>): Promise<ExecToolResult> => {
    ...
    try {
      ...
      if (this.context.sessionKey) {
        env.GOUSB_AI_RUNTIME_SESSION_KEY = this.context.sessionKey;
      }
      if (this.context.channel) {
        env.GOUSB_AI_RUNTIME_CHANNEL = this.context.channel;
      }
      if (this.context.chatId) {
        env.GOUSB_AI_RUNTIME_CHAT_ID = this.context.chatId;
      }
