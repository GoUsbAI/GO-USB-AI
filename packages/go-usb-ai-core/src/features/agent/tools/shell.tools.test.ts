...
import {
  ...
  GOUSB_AI_COMMAND_SURFACE_BIN_ENV,
...
describe("ExecTool", () => {
  ...
  it("removes development node conditions before launching external commands", async () => {
    ...
    try {
      const result = await tool.execute({ command: "go-usb-ai cron list" });
...
describe("createExternalCommandEnv", () => {
  ...
  it("augments PATH with the current node bin dir and ancestor node_modules bins", () => {
    const workspace = mkdtempSync(join(tmpdir(), "go-usb-ai-external-env-"));
  ...
  it("prepends the managed GoUsbAi command surface bin when present", () => {
    const workspace = mkdtempSync(join(tmpdir(), "go-usb-ai-command-surface-env-"));
    ...
    try {
      const env = createExternalCommandEnv({
        PATH: "/usr/bin:/bin",
        [GOUSB_AI_COMMAND_SURFACE_BIN_ENV]: commandSurfaceBin
      });
      const pathEntries = String(env.PATH ?? "").split(":");
  ...
  it("ignores missing managed command surface bin directories", () => {
    const env = createExternalCommandEnv({
      PATH: "/usr/bin:/bin",
      [GOUSB_AI_COMMAND_SURFACE_BIN_ENV]: "/tmp/go-usb-ai-missing-command-surface-bin"
    });

    expect(String(env.PATH ?? "").split(":")).not.toContain("/tmp/go-usb-ai-missing-command-surface-bin");
  });
});
