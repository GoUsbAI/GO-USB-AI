...
export function buildActiveSkillsSystemSection(skills: SkillsLoader, skillSelectors: string[]): string {
  ...
  return [
    ...
    "For GoUsbAi self-management intents, read the built-in GoUsbAi self-management guide before loading any unrelated generic skill.",
...
export function buildAvailableSkillsSystemSection(skills: SkillsLoader): string {
  ...
  return [
    ...
    "- If the user is asking to manage GoUsbAi itself, read the built-in GoUsbAi self-management guide first and do not open unrelated generic skills before that.",
