...
import { resolveGoUsbAiSelfManageGuidePaths } from "../features/self-manage/guide-path.js";
...
export class ContextBuilder {
  ...
  private getIdentity = (availableTools?: ToolCatalogEntry[]): string => {
    ...
    const selfManageGuide = resolveGoUsbAiSelfManageGuidePaths();
    const lines = [
      ...
      `- For ${APP_NAME} self-management operations (version/status/doctor/service/plugins/channels/config/agents/cron/remote/update), read \`${selfManageGuide.primaryPath ?? "the built-in GoUsbAi self-management guide"}\` first.`,
