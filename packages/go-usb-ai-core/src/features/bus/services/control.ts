...
export const GOUSB_AI_CONTROL_METADATA_KEY = "__go-usb-ai_control";
...
function readControl(metadata: Record<string, unknown> | undefined): Record<string, unknown> | null {
  ...
  const raw = metadata[GOUSB_AI_CONTROL_METADATA_KEY];
...
export function readGoUsbAiControl(metadata: Record<string, unknown> | undefined): GoUsbAiControl | null {
  return (
...
export function isGoUsbAiControlMessage(msg: Pick<OutboundMessage, "metadata">): boolean {
  return readGoUsbAiControl(msg.metadata) !== null;
}
