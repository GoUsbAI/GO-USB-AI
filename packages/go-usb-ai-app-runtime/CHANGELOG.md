# @go-usb-ai/app-runtime

## 0.7.12

### Patch Changes

- Auto-generated full public beta release batch.

  Packages:
  - @go-usb-ai/agent-chat
  - @go-usb-ai/agent-chat-ui
  - @go-usb-ai/app-runtime
  - @go-usb-ai/app-sdk
  - @go-usb-ai/channel-extension-feishu
  - @go-usb-ai/channel-extension-qq
  - @go-usb-ai/channel-extension-weixin
  - @go-usb-ai/channel-plugin-dingtalk
  - @go-usb-ai/channel-plugin-discord
  - @go-usb-ai/channel-plugin-email
  - @go-usb-ai/channel-plugin-slack
  - @go-usb-ai/channel-plugin-telegram
  - @go-usb-ai/channel-plugin-wecom
  - @go-usb-ai/channel-plugin-whatsapp
  - @go-usb-ai/channel-runtime
  - @go-usb-ai/client-sdk
  - @go-usb-ai/companion
  - @go-usb-ai/core
  - @go-usb-ai/extension-sdk
  - @go-usb-ai/feishu-core
  - @go-usb-ai/kernel
  - @go-usb-ai/mcp
  - @go-usb-ai/ncp
  - @go-usb-ai/ncp-agent-runtime
  - @go-usb-ai/ncp-http-agent-client
  - @go-usb-ai/ncp-http-agent-server
  - @go-usb-ai/ncp-mcp
  - @go-usb-ai/ncp-react
  - @go-usb-ai/ncp-react-ui
  - @go-usb-ai/ncp-toolkit
  - @go-usb-ai/go-usb-ai-hermes-acp-bridge
  - @go-usb-ai/go-usb-ai-narp-runtime-claude-code-sdk
  - @go-usb-ai/go-usb-ai-narp-runtime-codex-sdk
  - @go-usb-ai/go-usb-ai-narp-stdio-runtime-wrapper
  - @go-usb-ai/go-usb-ai-ncp-runtime-adapter-hermes-http
  - @go-usb-ai/go-usb-ai-ncp-runtime-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-http-client
  - @go-usb-ai/go-usb-ai-ncp-runtime-stdio-client
  - @go-usb-ai/openclaw-compat
  - @go-usb-ai/remote
  - @go-usb-ai/runtime
  - @go-usb-ai/server
  - @go-usb-ai/service
  - @go-usb-ai/shared
  - @go-usb-ai/ui
  - go-usb-ai

## 0.7.11

### Patch Changes

- Auto-generated full public beta release batch.

  Packages:
  - @go-usb-ai/agent-chat
  - @go-usb-ai/agent-chat-ui
  - @go-usb-ai/app-runtime
  - @go-usb-ai/app-sdk
  - @go-usb-ai/channel-extension-feishu
  - @go-usb-ai/channel-extension-qq
  - @go-usb-ai/channel-extension-weixin
  - @go-usb-ai/channel-plugin-dingtalk
  - @go-usb-ai/channel-plugin-discord
  - @go-usb-ai/channel-plugin-email
  - @go-usb-ai/channel-plugin-slack
  - @go-usb-ai/channel-plugin-telegram
  - @go-usb-ai/channel-plugin-wecom
  - @go-usb-ai/channel-plugin-whatsapp
  - @go-usb-ai/channel-runtime
  - @go-usb-ai/client-sdk
  - @go-usb-ai/companion
  - @go-usb-ai/core
  - @go-usb-ai/extension-sdk
  - @go-usb-ai/feishu-core
  - @go-usb-ai/kernel
  - @go-usb-ai/mcp
  - @go-usb-ai/ncp
  - @go-usb-ai/ncp-agent-runtime
  - @go-usb-ai/ncp-http-agent-client
  - @go-usb-ai/ncp-http-agent-server
  - @go-usb-ai/ncp-mcp
  - @go-usb-ai/ncp-react
  - @go-usb-ai/ncp-react-ui
  - @go-usb-ai/ncp-toolkit
  - @go-usb-ai/go-usb-ai-hermes-acp-bridge
  - @go-usb-ai/go-usb-ai-narp-runtime-claude-code-sdk
  - @go-usb-ai/go-usb-ai-narp-runtime-codex-sdk
  - @go-usb-ai/go-usb-ai-narp-stdio-runtime-wrapper
  - @go-usb-ai/go-usb-ai-ncp-runtime-adapter-hermes-http
  - @go-usb-ai/go-usb-ai-ncp-runtime-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-http-client
  - @go-usb-ai/go-usb-ai-ncp-runtime-stdio-client
  - @go-usb-ai/openclaw-compat
  - @go-usb-ai/remote
  - @go-usb-ai/runtime
  - @go-usb-ai/server
  - @go-usb-ai/service
  - @go-usb-ai/shared
  - @go-usb-ai/ui
  - go-usb-ai

## 0.7.10

### Patch Changes

- Auto-generated full public beta release batch.

  Packages:
  - @go-usb-ai/agent-chat
  - @go-usb-ai/agent-chat-ui
  - @go-usb-ai/app-runtime
  - @go-usb-ai/app-sdk
  - @go-usb-ai/channel-extension-feishu
  - @go-usb-ai/channel-extension-qq
  - @go-usb-ai/channel-extension-weixin
  - @go-usb-ai/channel-plugin-dingtalk
  - @go-usb-ai/channel-plugin-discord
  - @go-usb-ai/channel-plugin-email
  - @go-usb-ai/channel-plugin-slack
  - @go-usb-ai/channel-plugin-telegram
  - @go-usb-ai/channel-plugin-wecom
  - @go-usb-ai/channel-plugin-whatsapp
  - @go-usb-ai/channel-runtime
  - @go-usb-ai/client-sdk
  - @go-usb-ai/companion
  - @go-usb-ai/core
  - @go-usb-ai/extension-sdk
  - @go-usb-ai/feishu-core
  - @go-usb-ai/kernel
  - @go-usb-ai/mcp
  - @go-usb-ai/ncp
  - @go-usb-ai/ncp-agent-runtime
  - @go-usb-ai/ncp-http-agent-client
  - @go-usb-ai/ncp-http-agent-server
  - @go-usb-ai/ncp-mcp
  - @go-usb-ai/ncp-react
  - @go-usb-ai/ncp-react-ui
  - @go-usb-ai/ncp-toolkit
  - @go-usb-ai/go-usb-ai-hermes-acp-bridge
  - @go-usb-ai/go-usb-ai-narp-runtime-claude-code-sdk
  - @go-usb-ai/go-usb-ai-narp-runtime-codex-sdk
  - @go-usb-ai/go-usb-ai-narp-stdio-runtime-wrapper
  - @go-usb-ai/go-usb-ai-ncp-runtime-adapter-hermes-http
  - @go-usb-ai/go-usb-ai-ncp-runtime-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-http-client
  - @go-usb-ai/go-usb-ai-ncp-runtime-stdio-client
  - @go-usb-ai/openclaw-compat
  - @go-usb-ai/remote
  - @go-usb-ai/runtime
  - @go-usb-ai/server
  - @go-usb-ai/service
  - @go-usb-ai/shared
  - @go-usb-ai/ui
  - go-usb-ai

## 0.7.9

### Patch Changes

- Auto-generated full public beta release batch.

  Packages:
  - @go-usb-ai/agent-chat
  - @go-usb-ai/agent-chat-ui
  - @go-usb-ai/app-runtime
  - @go-usb-ai/app-sdk
  - @go-usb-ai/channel-extension-feishu
  - @go-usb-ai/channel-extension-qq
  - @go-usb-ai/channel-extension-weixin
  - @go-usb-ai/channel-plugin-dingtalk
  - @go-usb-ai/channel-plugin-discord
  - @go-usb-ai/channel-plugin-email
  - @go-usb-ai/channel-plugin-slack
  - @go-usb-ai/channel-plugin-telegram
  - @go-usb-ai/channel-plugin-wecom
  - @go-usb-ai/channel-plugin-whatsapp
  - @go-usb-ai/channel-runtime
  - @go-usb-ai/client-sdk
  - @go-usb-ai/companion
  - @go-usb-ai/core
  - @go-usb-ai/extension-sdk
  - @go-usb-ai/feishu-core
  - @go-usb-ai/kernel
  - @go-usb-ai/mcp
  - @go-usb-ai/ncp
  - @go-usb-ai/ncp-agent-runtime
  - @go-usb-ai/ncp-http-agent-client
  - @go-usb-ai/ncp-http-agent-server
  - @go-usb-ai/ncp-mcp
  - @go-usb-ai/ncp-react
  - @go-usb-ai/ncp-react-ui
  - @go-usb-ai/ncp-toolkit
  - @go-usb-ai/go-usb-ai-hermes-acp-bridge
  - @go-usb-ai/go-usb-ai-narp-runtime-claude-code-sdk
  - @go-usb-ai/go-usb-ai-narp-runtime-codex-sdk
  - @go-usb-ai/go-usb-ai-narp-stdio-runtime-wrapper
  - @go-usb-ai/go-usb-ai-ncp-runtime-adapter-hermes-http
  - @go-usb-ai/go-usb-ai-ncp-runtime-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-http-client
  - @go-usb-ai/go-usb-ai-ncp-runtime-stdio-client
  - @go-usb-ai/openclaw-compat
  - @go-usb-ai/remote
  - @go-usb-ai/runtime
  - @go-usb-ai/server
  - @go-usb-ai/service
  - @go-usb-ai/shared
  - @go-usb-ai/ui
  - go-usb-ai

## 0.7.8

### Patch Changes

- Auto-generated full public beta release batch.

  Packages:
  - @go-usb-ai/agent-chat
  - @go-usb-ai/agent-chat-ui
  - @go-usb-ai/app-runtime
  - @go-usb-ai/app-sdk
  - @go-usb-ai/channel-extension-feishu
  - @go-usb-ai/channel-extension-qq
  - @go-usb-ai/channel-extension-weixin
  - @go-usb-ai/channel-plugin-dingtalk
  - @go-usb-ai/channel-plugin-discord
  - @go-usb-ai/channel-plugin-email
  - @go-usb-ai/channel-plugin-slack
  - @go-usb-ai/channel-plugin-telegram
  - @go-usb-ai/channel-plugin-wecom
  - @go-usb-ai/channel-plugin-whatsapp
  - @go-usb-ai/channel-runtime
  - @go-usb-ai/client-sdk
  - @go-usb-ai/companion
  - @go-usb-ai/core
  - @go-usb-ai/extension-sdk
  - @go-usb-ai/feishu-core
  - @go-usb-ai/kernel
  - @go-usb-ai/mcp
  - @go-usb-ai/ncp
  - @go-usb-ai/ncp-agent-runtime
  - @go-usb-ai/ncp-http-agent-client
  - @go-usb-ai/ncp-http-agent-server
  - @go-usb-ai/ncp-mcp
  - @go-usb-ai/ncp-react
  - @go-usb-ai/ncp-react-ui
  - @go-usb-ai/ncp-toolkit
  - @go-usb-ai/go-usb-ai-hermes-acp-bridge
  - @go-usb-ai/go-usb-ai-narp-runtime-claude-code-sdk
  - @go-usb-ai/go-usb-ai-narp-runtime-codex-sdk
  - @go-usb-ai/go-usb-ai-narp-stdio-runtime-wrapper
  - @go-usb-ai/go-usb-ai-ncp-runtime-adapter-hermes-http
  - @go-usb-ai/go-usb-ai-ncp-runtime-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-http-client
  - @go-usb-ai/go-usb-ai-ncp-runtime-stdio-client
  - @go-usb-ai/openclaw-compat
  - @go-usb-ai/remote
  - @go-usb-ai/runtime
  - @go-usb-ai/server
  - @go-usb-ai/service
  - @go-usb-ai/shared
  - @go-usb-ai/ui
  - go-usb-ai

## 0.7.7

### Patch Changes

- Auto-generated full public beta release batch.

  Packages:
  - @go-usb-ai/agent-chat
  - @go-usb-ai/agent-chat-ui
  - @go-usb-ai/app-runtime
  - @go-usb-ai/app-sdk
  - @go-usb-ai/channel-extension-feishu
  - @go-usb-ai/channel-extension-weixin
  - @go-usb-ai/channel-plugin-dingtalk
  - @go-usb-ai/channel-plugin-discord
  - @go-usb-ai/channel-plugin-email
  - @go-usb-ai/channel-plugin-mochat
  - @go-usb-ai/channel-plugin-qq
  - @go-usb-ai/channel-plugin-slack
  - @go-usb-ai/channel-plugin-telegram
  - @go-usb-ai/channel-plugin-wecom
  - @go-usb-ai/channel-plugin-whatsapp
  - @go-usb-ai/channel-runtime
  - @go-usb-ai/client-sdk
  - @go-usb-ai/companion
  - @go-usb-ai/core
  - @go-usb-ai/extension-sdk
  - @go-usb-ai/feishu-core
  - @go-usb-ai/kernel
  - @go-usb-ai/mcp
  - @go-usb-ai/ncp
  - @go-usb-ai/ncp-agent-runtime
  - @go-usb-ai/ncp-http-agent-client
  - @go-usb-ai/ncp-http-agent-server
  - @go-usb-ai/ncp-mcp
  - @go-usb-ai/ncp-react
  - @go-usb-ai/ncp-react-ui
  - @go-usb-ai/ncp-toolkit
  - @go-usb-ai/go-usb-ai-hermes-acp-bridge
  - @go-usb-ai/go-usb-ai-narp-runtime-claude-code-sdk
  - @go-usb-ai/go-usb-ai-narp-runtime-codex-sdk
  - @go-usb-ai/go-usb-ai-narp-stdio-runtime-wrapper
  - @go-usb-ai/go-usb-ai-ncp-runtime-adapter-hermes-http
  - @go-usb-ai/go-usb-ai-ncp-runtime-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-http-client
  - @go-usb-ai/go-usb-ai-ncp-runtime-plugin-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-plugin-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-stdio-client
  - @go-usb-ai/openclaw-compat
  - @go-usb-ai/remote
  - @go-usb-ai/runtime
  - @go-usb-ai/server
  - @go-usb-ai/service
  - @go-usb-ai/shared
  - @go-usb-ai/ui
  - go-usb-ai

## 0.7.6

### Patch Changes

- Auto-generated full public beta release batch.

  Packages:
  - @go-usb-ai/agent-chat
  - @go-usb-ai/agent-chat-ui
  - @go-usb-ai/app-runtime
  - @go-usb-ai/app-sdk
  - @go-usb-ai/channel-extension-feishu
  - @go-usb-ai/channel-extension-weixin
  - @go-usb-ai/channel-plugin-dingtalk
  - @go-usb-ai/channel-plugin-discord
  - @go-usb-ai/channel-plugin-email
  - @go-usb-ai/channel-plugin-mochat
  - @go-usb-ai/channel-plugin-qq
  - @go-usb-ai/channel-plugin-slack
  - @go-usb-ai/channel-plugin-telegram
  - @go-usb-ai/channel-plugin-wecom
  - @go-usb-ai/channel-plugin-whatsapp
  - @go-usb-ai/channel-runtime
  - @go-usb-ai/client-sdk
  - @go-usb-ai/companion
  - @go-usb-ai/core
  - @go-usb-ai/extension-sdk
  - @go-usb-ai/feishu-core
  - @go-usb-ai/kernel
  - @go-usb-ai/mcp
  - @go-usb-ai/ncp
  - @go-usb-ai/ncp-agent-runtime
  - @go-usb-ai/ncp-http-agent-client
  - @go-usb-ai/ncp-http-agent-server
  - @go-usb-ai/ncp-mcp
  - @go-usb-ai/ncp-react
  - @go-usb-ai/ncp-react-ui
  - @go-usb-ai/ncp-toolkit
  - @go-usb-ai/go-usb-ai-hermes-acp-bridge
  - @go-usb-ai/go-usb-ai-narp-runtime-claude-code-sdk
  - @go-usb-ai/go-usb-ai-narp-runtime-codex-sdk
  - @go-usb-ai/go-usb-ai-narp-stdio-runtime-wrapper
  - @go-usb-ai/go-usb-ai-ncp-runtime-adapter-hermes-http
  - @go-usb-ai/go-usb-ai-ncp-runtime-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-http-client
  - @go-usb-ai/go-usb-ai-ncp-runtime-plugin-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-plugin-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-stdio-client
  - @go-usb-ai/openclaw-compat
  - @go-usb-ai/remote
  - @go-usb-ai/runtime
  - @go-usb-ai/server
  - @go-usb-ai/service
  - @go-usb-ai/shared
  - @go-usb-ai/ui
  - go-usb-ai

## 0.7.5

### Patch Changes

- Auto-generated full public stable release batch.

  Packages:
  - @go-usb-ai/agent-chat
  - @go-usb-ai/agent-chat-ui
  - @go-usb-ai/app-runtime
  - @go-usb-ai/app-sdk
  - @go-usb-ai/channel-extension-feishu
  - @go-usb-ai/channel-extension-weixin
  - @go-usb-ai/channel-plugin-dingtalk
  - @go-usb-ai/channel-plugin-discord
  - @go-usb-ai/channel-plugin-email
  - @go-usb-ai/channel-plugin-mochat
  - @go-usb-ai/channel-plugin-qq
  - @go-usb-ai/channel-plugin-slack
  - @go-usb-ai/channel-plugin-telegram
  - @go-usb-ai/channel-plugin-wecom
  - @go-usb-ai/channel-plugin-whatsapp
  - @go-usb-ai/channel-runtime
  - @go-usb-ai/client-sdk
  - @go-usb-ai/companion
  - @go-usb-ai/core
  - @go-usb-ai/extension-sdk
  - @go-usb-ai/feishu-core
  - @go-usb-ai/kernel
  - @go-usb-ai/mcp
  - @go-usb-ai/ncp
  - @go-usb-ai/ncp-agent-runtime
  - @go-usb-ai/ncp-http-agent-client
  - @go-usb-ai/ncp-http-agent-server
  - @go-usb-ai/ncp-mcp
  - @go-usb-ai/ncp-react
  - @go-usb-ai/ncp-react-ui
  - @go-usb-ai/ncp-toolkit
  - @go-usb-ai/go-usb-ai-hermes-acp-bridge
  - @go-usb-ai/go-usb-ai-narp-runtime-claude-code-sdk
  - @go-usb-ai/go-usb-ai-narp-runtime-codex-sdk
  - @go-usb-ai/go-usb-ai-narp-stdio-runtime-wrapper
  - @go-usb-ai/go-usb-ai-ncp-runtime-adapter-hermes-http
  - @go-usb-ai/go-usb-ai-ncp-runtime-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-http-client
  - @go-usb-ai/go-usb-ai-ncp-runtime-plugin-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-plugin-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-stdio-client
  - @go-usb-ai/openclaw-compat
  - @go-usb-ai/remote
  - @go-usb-ai/runtime
  - @go-usb-ai/server
  - @go-usb-ai/service
  - @go-usb-ai/shared
  - @go-usb-ai/ui
  - go-usb-ai

## 0.7.4

### Patch Changes

- Auto-generated full public beta release batch.

  Packages:
  - @go-usb-ai/agent-chat
  - @go-usb-ai/agent-chat-ui
  - @go-usb-ai/app-runtime
  - @go-usb-ai/app-sdk
  - @go-usb-ai/channel-extension-weixin
  - @go-usb-ai/channel-plugin-dingtalk
  - @go-usb-ai/channel-plugin-discord
  - @go-usb-ai/channel-plugin-email
  - @go-usb-ai/channel-plugin-feishu
  - @go-usb-ai/channel-plugin-mochat
  - @go-usb-ai/channel-plugin-qq
  - @go-usb-ai/channel-plugin-slack
  - @go-usb-ai/channel-plugin-telegram
  - @go-usb-ai/channel-plugin-wecom
  - @go-usb-ai/channel-plugin-whatsapp
  - @go-usb-ai/channel-runtime
  - @go-usb-ai/client-sdk
  - @go-usb-ai/companion
  - @go-usb-ai/core
  - @go-usb-ai/extension-sdk
  - @go-usb-ai/feishu-core
  - @go-usb-ai/kernel
  - @go-usb-ai/mcp
  - @go-usb-ai/ncp
  - @go-usb-ai/ncp-agent-runtime
  - @go-usb-ai/ncp-http-agent-client
  - @go-usb-ai/ncp-http-agent-server
  - @go-usb-ai/ncp-mcp
  - @go-usb-ai/ncp-react
  - @go-usb-ai/ncp-react-ui
  - @go-usb-ai/ncp-toolkit
  - @go-usb-ai/go-usb-ai-hermes-acp-bridge
  - @go-usb-ai/go-usb-ai-narp-runtime-claude-code-sdk
  - @go-usb-ai/go-usb-ai-narp-runtime-codex-sdk
  - @go-usb-ai/go-usb-ai-narp-stdio-runtime-wrapper
  - @go-usb-ai/go-usb-ai-ncp-runtime-adapter-hermes-http
  - @go-usb-ai/go-usb-ai-ncp-runtime-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-http-client
  - @go-usb-ai/go-usb-ai-ncp-runtime-plugin-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-plugin-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-stdio-client
  - @go-usb-ai/openclaw-compat
  - @go-usb-ai/remote
  - @go-usb-ai/runtime
  - @go-usb-ai/server
  - @go-usb-ai/service
  - @go-usb-ai/shared
  - @go-usb-ai/ui
  - go-usb-ai

## 0.7.3

### Patch Changes

- Auto-generated full public beta release batch.

  Packages:
  - @go-usb-ai/agent-chat
  - @go-usb-ai/agent-chat-ui
  - @go-usb-ai/app-runtime
  - @go-usb-ai/app-sdk
  - @go-usb-ai/channel-extension-weixin
  - @go-usb-ai/channel-plugin-dingtalk
  - @go-usb-ai/channel-plugin-discord
  - @go-usb-ai/channel-plugin-email
  - @go-usb-ai/channel-plugin-feishu
  - @go-usb-ai/channel-plugin-mochat
  - @go-usb-ai/channel-plugin-qq
  - @go-usb-ai/channel-plugin-slack
  - @go-usb-ai/channel-plugin-telegram
  - @go-usb-ai/channel-plugin-wecom
  - @go-usb-ai/channel-plugin-whatsapp
  - @go-usb-ai/channel-runtime
  - @go-usb-ai/client-sdk
  - @go-usb-ai/companion
  - @go-usb-ai/core
  - @go-usb-ai/extension-sdk
  - @go-usb-ai/feishu-core
  - @go-usb-ai/kernel
  - @go-usb-ai/mcp
  - @go-usb-ai/ncp
  - @go-usb-ai/ncp-agent-runtime
  - @go-usb-ai/ncp-http-agent-client
  - @go-usb-ai/ncp-http-agent-server
  - @go-usb-ai/ncp-mcp
  - @go-usb-ai/ncp-react
  - @go-usb-ai/ncp-react-ui
  - @go-usb-ai/ncp-toolkit
  - @go-usb-ai/go-usb-ai-hermes-acp-bridge
  - @go-usb-ai/go-usb-ai-narp-runtime-claude-code-sdk
  - @go-usb-ai/go-usb-ai-narp-runtime-codex-sdk
  - @go-usb-ai/go-usb-ai-narp-stdio-runtime-wrapper
  - @go-usb-ai/go-usb-ai-ncp-runtime-adapter-hermes-http
  - @go-usb-ai/go-usb-ai-ncp-runtime-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-http-client
  - @go-usb-ai/go-usb-ai-ncp-runtime-plugin-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-plugin-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-stdio-client
  - @go-usb-ai/openclaw-compat
  - @go-usb-ai/remote
  - @go-usb-ai/runtime
  - @go-usb-ai/server
  - @go-usb-ai/service
  - @go-usb-ai/shared
  - @go-usb-ai/ui
  - go-usb-ai

## 0.7.2

### Patch Changes

- Stable minor release for the GoUsbAi npm package, with patch releases for the workspace dependency closure.

  Packages:
  - @go-usb-ai/agent-chat
  - @go-usb-ai/agent-chat-ui
  - @go-usb-ai/app-runtime
  - @go-usb-ai/app-sdk
  - @go-usb-ai/channel-extension-weixin
  - @go-usb-ai/channel-plugin-dingtalk
  - @go-usb-ai/channel-plugin-discord
  - @go-usb-ai/channel-plugin-email
  - @go-usb-ai/channel-plugin-feishu
  - @go-usb-ai/channel-plugin-mochat
  - @go-usb-ai/channel-plugin-qq
  - @go-usb-ai/channel-plugin-slack
  - @go-usb-ai/channel-plugin-telegram
  - @go-usb-ai/channel-plugin-wecom
  - @go-usb-ai/channel-plugin-whatsapp
  - @go-usb-ai/channel-runtime
  - @go-usb-ai/client-sdk
  - @go-usb-ai/companion
  - @go-usb-ai/core
  - @go-usb-ai/extension-sdk
  - @go-usb-ai/feishu-core
  - @go-usb-ai/kernel
  - @go-usb-ai/mcp
  - @go-usb-ai/ncp
  - @go-usb-ai/ncp-agent-runtime
  - @go-usb-ai/ncp-http-agent-client
  - @go-usb-ai/ncp-http-agent-server
  - @go-usb-ai/ncp-mcp
  - @go-usb-ai/ncp-react
  - @go-usb-ai/ncp-react-ui
  - @go-usb-ai/ncp-toolkit
  - @go-usb-ai/go-usb-ai-hermes-acp-bridge
  - @go-usb-ai/go-usb-ai-narp-runtime-claude-code-sdk
  - @go-usb-ai/go-usb-ai-narp-runtime-codex-sdk
  - @go-usb-ai/go-usb-ai-narp-stdio-runtime-wrapper
  - @go-usb-ai/go-usb-ai-ncp-runtime-adapter-hermes-http
  - @go-usb-ai/go-usb-ai-ncp-runtime-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-http-client
  - @go-usb-ai/go-usb-ai-ncp-runtime-plugin-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-plugin-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-stdio-client
  - @go-usb-ai/openclaw-compat
  - @go-usb-ai/remote
  - @go-usb-ai/runtime
  - @go-usb-ai/server
  - @go-usb-ai/service
  - @go-usb-ai/shared
  - @go-usb-ai/ui
  - go-usb-ai

## 0.7.1

### Patch Changes

- a11f4fd: Auto-generated patch release for packages with meaningful drift after their latest version commit.

  Packages:
  - @go-usb-ai/agent-chat
  - @go-usb-ai/app-runtime
  - @go-usb-ai/app-sdk
  - @go-usb-ai/channel-plugin-dingtalk
  - @go-usb-ai/channel-plugin-discord
  - @go-usb-ai/channel-plugin-email
  - @go-usb-ai/channel-plugin-mochat
  - @go-usb-ai/channel-plugin-qq
  - @go-usb-ai/channel-plugin-slack
  - @go-usb-ai/channel-plugin-telegram
  - @go-usb-ai/channel-plugin-wecom
  - @go-usb-ai/channel-plugin-whatsapp
  - @go-usb-ai/channel-runtime
  - @go-usb-ai/client-sdk
  - @go-usb-ai/companion
  - @go-usb-ai/kernel
  - @go-usb-ai/mcp
  - @go-usb-ai/ncp-agent-runtime
  - @go-usb-ai/ncp-http-agent-client
  - @go-usb-ai/ncp-http-agent-server
  - @go-usb-ai/ncp-mcp
  - @go-usb-ai/ncp-react
  - @go-usb-ai/go-usb-ai-hermes-acp-bridge
  - @go-usb-ai/go-usb-ai-ncp-runtime-adapter-hermes-http
  - @go-usb-ai/go-usb-ai-ncp-runtime-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-http-client
  - @go-usb-ai/go-usb-ai-ncp-runtime-plugin-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-plugin-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-stdio-client
  - @go-usb-ai/openclaw-compat
  - @go-usb-ai/remote

- 2418020: Auto-generated full public beta release batch.

  Packages:
  - @go-usb-ai/agent-chat
  - @go-usb-ai/agent-chat-ui
  - @go-usb-ai/app-runtime
  - @go-usb-ai/app-sdk
  - @go-usb-ai/channel-plugin-dingtalk
  - @go-usb-ai/channel-plugin-discord
  - @go-usb-ai/channel-plugin-email
  - @go-usb-ai/channel-plugin-feishu
  - @go-usb-ai/channel-plugin-mochat
  - @go-usb-ai/channel-plugin-qq
  - @go-usb-ai/channel-plugin-slack
  - @go-usb-ai/channel-plugin-telegram
  - @go-usb-ai/channel-plugin-wecom
  - @go-usb-ai/channel-plugin-weixin
  - @go-usb-ai/channel-plugin-whatsapp
  - @go-usb-ai/channel-runtime
  - @go-usb-ai/client-sdk
  - @go-usb-ai/companion
  - @go-usb-ai/core
  - @go-usb-ai/feishu-core
  - @go-usb-ai/kernel
  - @go-usb-ai/mcp
  - @go-usb-ai/ncp
  - @go-usb-ai/ncp-agent-runtime
  - @go-usb-ai/ncp-http-agent-client
  - @go-usb-ai/ncp-http-agent-server
  - @go-usb-ai/ncp-mcp
  - @go-usb-ai/ncp-react
  - @go-usb-ai/ncp-react-ui
  - @go-usb-ai/ncp-toolkit
  - @go-usb-ai/go-usb-ai-hermes-acp-bridge
  - @go-usb-ai/go-usb-ai-ncp-runtime-adapter-hermes-http
  - @go-usb-ai/go-usb-ai-ncp-runtime-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-http-client
  - @go-usb-ai/go-usb-ai-ncp-runtime-plugin-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-plugin-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-stdio-client
  - @go-usb-ai/openclaw-compat
  - @go-usb-ai/remote
  - @go-usb-ai/runtime
  - @go-usb-ai/server
  - @go-usb-ai/ui
  - go-usb-ai

- a5da9d6: Auto-generated full public beta release batch.

  Packages:
  - @go-usb-ai/agent-chat
  - @go-usb-ai/agent-chat-ui
  - @go-usb-ai/app-runtime
  - @go-usb-ai/app-sdk
  - @go-usb-ai/channel-plugin-dingtalk
  - @go-usb-ai/channel-plugin-discord
  - @go-usb-ai/channel-plugin-email
  - @go-usb-ai/channel-plugin-feishu
  - @go-usb-ai/channel-plugin-mochat
  - @go-usb-ai/channel-plugin-qq
  - @go-usb-ai/channel-plugin-slack
  - @go-usb-ai/channel-plugin-telegram
  - @go-usb-ai/channel-plugin-wecom
  - @go-usb-ai/channel-plugin-weixin
  - @go-usb-ai/channel-plugin-whatsapp
  - @go-usb-ai/channel-runtime
  - @go-usb-ai/client-sdk
  - @go-usb-ai/companion
  - @go-usb-ai/core
  - @go-usb-ai/feishu-core
  - @go-usb-ai/kernel
  - @go-usb-ai/mcp
  - @go-usb-ai/ncp
  - @go-usb-ai/ncp-agent-runtime
  - @go-usb-ai/ncp-http-agent-client
  - @go-usb-ai/ncp-http-agent-server
  - @go-usb-ai/ncp-mcp
  - @go-usb-ai/ncp-react
  - @go-usb-ai/ncp-react-ui
  - @go-usb-ai/ncp-toolkit
  - @go-usb-ai/go-usb-ai-hermes-acp-bridge
  - @go-usb-ai/go-usb-ai-ncp-runtime-adapter-hermes-http
  - @go-usb-ai/go-usb-ai-ncp-runtime-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-http-client
  - @go-usb-ai/go-usb-ai-ncp-runtime-plugin-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-plugin-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-stdio-client
  - @go-usb-ai/openclaw-compat
  - @go-usb-ai/remote
  - @go-usb-ai/runtime
  - @go-usb-ai/server
  - @go-usb-ai/ui
  - go-usb-ai

- 1600643: Auto-generated full public beta release batch.

  Packages:
  - @go-usb-ai/agent-chat
  - @go-usb-ai/agent-chat-ui
  - @go-usb-ai/app-runtime
  - @go-usb-ai/app-sdk
  - @go-usb-ai/channel-plugin-dingtalk
  - @go-usb-ai/channel-plugin-discord
  - @go-usb-ai/channel-plugin-email
  - @go-usb-ai/channel-plugin-feishu
  - @go-usb-ai/channel-plugin-mochat
  - @go-usb-ai/channel-plugin-qq
  - @go-usb-ai/channel-plugin-slack
  - @go-usb-ai/channel-plugin-telegram
  - @go-usb-ai/channel-plugin-wecom
  - @go-usb-ai/channel-plugin-weixin
  - @go-usb-ai/channel-plugin-whatsapp
  - @go-usb-ai/channel-runtime
  - @go-usb-ai/client-sdk
  - @go-usb-ai/companion
  - @go-usb-ai/core
  - @go-usb-ai/feishu-core
  - @go-usb-ai/kernel
  - @go-usb-ai/mcp
  - @go-usb-ai/ncp
  - @go-usb-ai/ncp-agent-runtime
  - @go-usb-ai/ncp-http-agent-client
  - @go-usb-ai/ncp-http-agent-server
  - @go-usb-ai/ncp-mcp
  - @go-usb-ai/ncp-react
  - @go-usb-ai/ncp-react-ui
  - @go-usb-ai/ncp-toolkit
  - @go-usb-ai/go-usb-ai-hermes-acp-bridge
  - @go-usb-ai/go-usb-ai-ncp-runtime-adapter-hermes-http
  - @go-usb-ai/go-usb-ai-ncp-runtime-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-http-client
  - @go-usb-ai/go-usb-ai-ncp-runtime-plugin-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-plugin-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-stdio-client
  - @go-usb-ai/openclaw-compat
  - @go-usb-ai/remote
  - @go-usb-ai/runtime
  - @go-usb-ai/server
  - @go-usb-ai/ui
  - go-usb-ai

- 223037c: Auto-generated full public beta release batch.

  Packages:
  - @go-usb-ai/agent-chat
  - @go-usb-ai/agent-chat-ui
  - @go-usb-ai/app-runtime
  - @go-usb-ai/app-sdk
  - @go-usb-ai/channel-plugin-dingtalk
  - @go-usb-ai/channel-plugin-discord
  - @go-usb-ai/channel-plugin-email
  - @go-usb-ai/channel-plugin-feishu
  - @go-usb-ai/channel-plugin-mochat
  - @go-usb-ai/channel-plugin-qq
  - @go-usb-ai/channel-plugin-slack
  - @go-usb-ai/channel-plugin-telegram
  - @go-usb-ai/channel-plugin-wecom
  - @go-usb-ai/channel-plugin-weixin
  - @go-usb-ai/channel-plugin-whatsapp
  - @go-usb-ai/channel-runtime
  - @go-usb-ai/client-sdk
  - @go-usb-ai/companion
  - @go-usb-ai/core
  - @go-usb-ai/feishu-core
  - @go-usb-ai/kernel
  - @go-usb-ai/mcp
  - @go-usb-ai/ncp
  - @go-usb-ai/ncp-agent-runtime
  - @go-usb-ai/ncp-http-agent-client
  - @go-usb-ai/ncp-http-agent-server
  - @go-usb-ai/ncp-mcp
  - @go-usb-ai/ncp-react
  - @go-usb-ai/ncp-react-ui
  - @go-usb-ai/ncp-toolkit
  - @go-usb-ai/go-usb-ai-hermes-acp-bridge
  - @go-usb-ai/go-usb-ai-ncp-runtime-adapter-hermes-http
  - @go-usb-ai/go-usb-ai-ncp-runtime-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-http-client
  - @go-usb-ai/go-usb-ai-ncp-runtime-plugin-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-plugin-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-stdio-client
  - @go-usb-ai/openclaw-compat
  - @go-usb-ai/remote
  - @go-usb-ai/runtime
  - @go-usb-ai/server
  - @go-usb-ai/ui
  - go-usb-ai

## 0.7.1-beta.4

### Patch Changes

- Auto-generated full public beta release batch.

  Packages:
  - @go-usb-ai/agent-chat
  - @go-usb-ai/agent-chat-ui
  - @go-usb-ai/app-runtime
  - @go-usb-ai/app-sdk
  - @go-usb-ai/channel-plugin-dingtalk
  - @go-usb-ai/channel-plugin-discord
  - @go-usb-ai/channel-plugin-email
  - @go-usb-ai/channel-plugin-feishu
  - @go-usb-ai/channel-plugin-mochat
  - @go-usb-ai/channel-plugin-qq
  - @go-usb-ai/channel-plugin-slack
  - @go-usb-ai/channel-plugin-telegram
  - @go-usb-ai/channel-plugin-wecom
  - @go-usb-ai/channel-plugin-weixin
  - @go-usb-ai/channel-plugin-whatsapp
  - @go-usb-ai/channel-runtime
  - @go-usb-ai/client-sdk
  - @go-usb-ai/companion
  - @go-usb-ai/core
  - @go-usb-ai/feishu-core
  - @go-usb-ai/kernel
  - @go-usb-ai/mcp
  - @go-usb-ai/ncp
  - @go-usb-ai/ncp-agent-runtime
  - @go-usb-ai/ncp-http-agent-client
  - @go-usb-ai/ncp-http-agent-server
  - @go-usb-ai/ncp-mcp
  - @go-usb-ai/ncp-react
  - @go-usb-ai/ncp-react-ui
  - @go-usb-ai/ncp-toolkit
  - @go-usb-ai/go-usb-ai-hermes-acp-bridge
  - @go-usb-ai/go-usb-ai-ncp-runtime-adapter-hermes-http
  - @go-usb-ai/go-usb-ai-ncp-runtime-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-http-client
  - @go-usb-ai/go-usb-ai-ncp-runtime-plugin-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-plugin-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-stdio-client
  - @go-usb-ai/openclaw-compat
  - @go-usb-ai/remote
  - @go-usb-ai/runtime
  - @go-usb-ai/server
  - @go-usb-ai/ui
  - go-usb-ai

## 0.7.1-beta.3

### Patch Changes

- Auto-generated full public beta release batch.

  Packages:
  - @go-usb-ai/agent-chat
  - @go-usb-ai/agent-chat-ui
  - @go-usb-ai/app-runtime
  - @go-usb-ai/app-sdk
  - @go-usb-ai/channel-plugin-dingtalk
  - @go-usb-ai/channel-plugin-discord
  - @go-usb-ai/channel-plugin-email
  - @go-usb-ai/channel-plugin-feishu
  - @go-usb-ai/channel-plugin-mochat
  - @go-usb-ai/channel-plugin-qq
  - @go-usb-ai/channel-plugin-slack
  - @go-usb-ai/channel-plugin-telegram
  - @go-usb-ai/channel-plugin-wecom
  - @go-usb-ai/channel-plugin-weixin
  - @go-usb-ai/channel-plugin-whatsapp
  - @go-usb-ai/channel-runtime
  - @go-usb-ai/client-sdk
  - @go-usb-ai/companion
  - @go-usb-ai/core
  - @go-usb-ai/feishu-core
  - @go-usb-ai/kernel
  - @go-usb-ai/mcp
  - @go-usb-ai/ncp
  - @go-usb-ai/ncp-agent-runtime
  - @go-usb-ai/ncp-http-agent-client
  - @go-usb-ai/ncp-http-agent-server
  - @go-usb-ai/ncp-mcp
  - @go-usb-ai/ncp-react
  - @go-usb-ai/ncp-react-ui
  - @go-usb-ai/ncp-toolkit
  - @go-usb-ai/go-usb-ai-hermes-acp-bridge
  - @go-usb-ai/go-usb-ai-ncp-runtime-adapter-hermes-http
  - @go-usb-ai/go-usb-ai-ncp-runtime-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-http-client
  - @go-usb-ai/go-usb-ai-ncp-runtime-plugin-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-plugin-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-stdio-client
  - @go-usb-ai/openclaw-compat
  - @go-usb-ai/remote
  - @go-usb-ai/runtime
  - @go-usb-ai/server
  - @go-usb-ai/ui
  - go-usb-ai

## 0.7.1-beta.2

### Patch Changes

- Auto-generated full public beta release batch.

  Packages:
  - @go-usb-ai/agent-chat
  - @go-usb-ai/agent-chat-ui
  - @go-usb-ai/app-runtime
  - @go-usb-ai/app-sdk
  - @go-usb-ai/channel-plugin-dingtalk
  - @go-usb-ai/channel-plugin-discord
  - @go-usb-ai/channel-plugin-email
  - @go-usb-ai/channel-plugin-feishu
  - @go-usb-ai/channel-plugin-mochat
  - @go-usb-ai/channel-plugin-qq
  - @go-usb-ai/channel-plugin-slack
  - @go-usb-ai/channel-plugin-telegram
  - @go-usb-ai/channel-plugin-wecom
  - @go-usb-ai/channel-plugin-weixin
  - @go-usb-ai/channel-plugin-whatsapp
  - @go-usb-ai/channel-runtime
  - @go-usb-ai/client-sdk
  - @go-usb-ai/companion
  - @go-usb-ai/core
  - @go-usb-ai/feishu-core
  - @go-usb-ai/kernel
  - @go-usb-ai/mcp
  - @go-usb-ai/ncp
  - @go-usb-ai/ncp-agent-runtime
  - @go-usb-ai/ncp-http-agent-client
  - @go-usb-ai/ncp-http-agent-server
  - @go-usb-ai/ncp-mcp
  - @go-usb-ai/ncp-react
  - @go-usb-ai/ncp-react-ui
  - @go-usb-ai/ncp-toolkit
  - @go-usb-ai/go-usb-ai-hermes-acp-bridge
  - @go-usb-ai/go-usb-ai-ncp-runtime-adapter-hermes-http
  - @go-usb-ai/go-usb-ai-ncp-runtime-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-http-client
  - @go-usb-ai/go-usb-ai-ncp-runtime-plugin-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-plugin-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-stdio-client
  - @go-usb-ai/openclaw-compat
  - @go-usb-ai/remote
  - @go-usb-ai/runtime
  - @go-usb-ai/server
  - @go-usb-ai/ui
  - go-usb-ai

## 0.7.1-beta.1

### Patch Changes

- Auto-generated full public beta release batch.

  Packages:
  - @go-usb-ai/agent-chat
  - @go-usb-ai/agent-chat-ui
  - @go-usb-ai/app-runtime
  - @go-usb-ai/app-sdk
  - @go-usb-ai/channel-plugin-dingtalk
  - @go-usb-ai/channel-plugin-discord
  - @go-usb-ai/channel-plugin-email
  - @go-usb-ai/channel-plugin-feishu
  - @go-usb-ai/channel-plugin-mochat
  - @go-usb-ai/channel-plugin-qq
  - @go-usb-ai/channel-plugin-slack
  - @go-usb-ai/channel-plugin-telegram
  - @go-usb-ai/channel-plugin-wecom
  - @go-usb-ai/channel-plugin-weixin
  - @go-usb-ai/channel-plugin-whatsapp
  - @go-usb-ai/channel-runtime
  - @go-usb-ai/client-sdk
  - @go-usb-ai/companion
  - @go-usb-ai/core
  - @go-usb-ai/feishu-core
  - @go-usb-ai/kernel
  - @go-usb-ai/mcp
  - @go-usb-ai/ncp
  - @go-usb-ai/ncp-agent-runtime
  - @go-usb-ai/ncp-http-agent-client
  - @go-usb-ai/ncp-http-agent-server
  - @go-usb-ai/ncp-mcp
  - @go-usb-ai/ncp-react
  - @go-usb-ai/ncp-react-ui
  - @go-usb-ai/ncp-toolkit
  - @go-usb-ai/go-usb-ai-hermes-acp-bridge
  - @go-usb-ai/go-usb-ai-ncp-runtime-adapter-hermes-http
  - @go-usb-ai/go-usb-ai-ncp-runtime-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-http-client
  - @go-usb-ai/go-usb-ai-ncp-runtime-plugin-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-plugin-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-stdio-client
  - @go-usb-ai/openclaw-compat
  - @go-usb-ai/remote
  - @go-usb-ai/runtime
  - @go-usb-ai/server
  - @go-usb-ai/ui
  - go-usb-ai

## 0.7.1-beta.0

### Patch Changes

- Auto-generated patch release for packages with meaningful drift after their latest version commit.

  Packages:
  - @go-usb-ai/agent-chat
  - @go-usb-ai/app-runtime
  - @go-usb-ai/app-sdk
  - @go-usb-ai/channel-plugin-dingtalk
  - @go-usb-ai/channel-plugin-discord
  - @go-usb-ai/channel-plugin-email
  - @go-usb-ai/channel-plugin-mochat
  - @go-usb-ai/channel-plugin-qq
  - @go-usb-ai/channel-plugin-slack
  - @go-usb-ai/channel-plugin-telegram
  - @go-usb-ai/channel-plugin-wecom
  - @go-usb-ai/channel-plugin-whatsapp
  - @go-usb-ai/channel-runtime
  - @go-usb-ai/client-sdk
  - @go-usb-ai/companion
  - @go-usb-ai/kernel
  - @go-usb-ai/mcp
  - @go-usb-ai/ncp-agent-runtime
  - @go-usb-ai/ncp-http-agent-client
  - @go-usb-ai/ncp-http-agent-server
  - @go-usb-ai/ncp-mcp
  - @go-usb-ai/ncp-react
  - @go-usb-ai/go-usb-ai-hermes-acp-bridge
  - @go-usb-ai/go-usb-ai-ncp-runtime-adapter-hermes-http
  - @go-usb-ai/go-usb-ai-ncp-runtime-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-http-client
  - @go-usb-ai/go-usb-ai-ncp-runtime-plugin-claude-code-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-plugin-codex-sdk
  - @go-usb-ai/go-usb-ai-ncp-runtime-stdio-client
  - @go-usb-ai/openclaw-compat
  - @go-usb-ai/remote

## 0.6.0

### Minor Changes

- Add a `ts-http-lite` scaffold template for users who want a smaller official TypeScript WASI HTTP backend path, while keeping the default `ts-http` template for the Hono-based experience.

## 0.5.0

### Minor Changes

- Add TypeScript/Hono WASI HTTP app scaffolding, `napp doctor`, and `napp build --install` so ordinary users can create, build, preview, package, install, and run sandboxed GoUsbAi Apps through the app runtime skill.

## 0.4.0

### Minor Changes

- Add `napp publish`, official apps registry defaults, scaffolded marketplace metadata, and standalone GoUsbAi Apps distribution support.

## 0.3.0

### Minor Changes

- 5319511: Add configurable registry installs, app updates, explicit permission management, and npm-style registry configuration commands to `napp`.

## 0.2.0

### Minor Changes

- 8d70b86: Add `napp create` to scaffold a minimal runnable GoUsbAi micro app starter.
- 7f14a24: Prepare `@go-usb-ai/app-runtime` for public release with marketplace-facing CLI onboarding and packaging metadata.
