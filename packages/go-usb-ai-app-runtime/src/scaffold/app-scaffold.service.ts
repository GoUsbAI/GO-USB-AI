...
export class AppScaffoldService {
  ...
  private buildManifest = (appId: string, appName: string) => {
    return {
      ...
      name: appName,
      version: "0.1.0",
      description: "A minimal GoUsbAi micro app scaffold created by napp.",
...
`;
  ...
  private buildMarketplaceMetadata = (appName: string) => {
    return {
      ...
      description: `A minimal GoUsbAi app scaffold for ${appName}, ready for local run and marketplace publish.`,
      descriptionI18n: {
        en: `A minimal GoUsbAi app scaffold for ${appName}, ready for local run and marketplace publish.`,
        zh: `一个面向 ${appName} 的最小 GoUsbAi 应用骨架，可直接本地运行并发布到 marketplace。`,
      },
      author: "GoUsbAi",
