...
export class AppHomeService {
  constructor(
    private readonly appHomeDirectory: string = process.env.GOUSB_AI_APP_HOME
      ? path.resolve(process.env.GOUSB_AI_APP_HOME)
      : path.join(process.env.HOME ?? process.cwd(), ".go-usb-ai", "apps"),
  ) {}
