...
export class AppPublishService {
  ...
  private resolvePublishActor = async (params: {
    ...
    const envAdminToken = process.env.GOUSB_AI_MARKETPLACE_ADMIN_TOKEN?.trim();
    if (explicitToken) {
    ...
    throw new Error("发布需要 GoUsbAi 平台登录态。请先运行 go-usb-ai login，或传入 --token。");
  };
  ...
  private fetchCurrentPlatformUser = async (params: {
    ...
    if (!response.ok) {
      ...
      throw new Error(`读取 GoUsbAi 登录态失败：${message}`);
    }
  ...
  private resolvePlatformApiBase = (configuredApiBase?: string): string => {
    const source = configuredApiBase?.trim() || process.env.GOUSB_AI_PLATFORM_API_BASE?.trim() || DEFAULT_PLATFORM_API_BASE;
  ...
  private buildUserPublisher = (
    ...
    const isOfficialScope = appId.startsWith("go-usb-ai.");
    if (isOfficialScope && user.role === "admin") {
      return this.buildOfficialPublisher();
    }
    if (!user.username) {
      throw new Error("当前 GoUsbAi 账号还没有 username，无法发布个人 scope app。请先在平台账号页设置用户名。");
    }
    return {
      id: user.username,
      name: user.username,
      url: `https://platform.go-usb-ai.io/account`,
    };
  };
