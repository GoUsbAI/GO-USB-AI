...
describe("AppPublishService", () => {
  ...
  it("packs an app and publishes marketplace payload", async () => {
    ...
    const publish = vi.fn().mockResolvedValue({
      ...
      item: {
        ...
        name: "App",
        latestVersion: "0.1.0",
        webUrl: "https://apps.go-usb-ai.io/apps/app",
        install: {
          kind: "registry",
          spec: "go-usb-ai.app",
          command: "napp install go-usb-ai.app",
    ...
    const service = new AppPublishService(
      ...
      } as never,
      {
        readCurrentAuthState: () => ({
          token: "user-token",
          apiBaseUrl: "https://ai-gateway-api.go-usb-ai.io/v1",
        }),
    ...
    expect(result.item.appId).toBe("go-usb-ai.app");
    expect(result.item.webUrl).toBe("https://apps.go-usb-ai.io/apps/app");
  ...
  it("publishes ts-http manifests with wasi-http-component main", async () => {
    ...
    const publish = vi.fn().mockResolvedValue({
      ...
      item: {
        slug: "todo-app",
        appId: "go-usb-ai.todo-app",
        name: "Todo App",
        latestVersion: "0.1.0",
        install: {
          kind: "registry",
          spec: "go-usb-ai.todo-app",
          command: "napp install go-usb-ai.todo-app",
          registry: "https://apps-registry.go-usb-ai.io/api/v1/apps/registry/",
        },
      },
      distribution: {
