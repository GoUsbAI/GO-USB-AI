...
const installResult = await runCli(["install", packResult.bundle.bundlePath, "--json"], {
  GOUSB_AI_APP_HOME: appHomeDirectory,
});
...
const registryServer = createServer((request, response) => {
  ...
  if (requestUrl.pathname === `/${encodeURIComponent("go-usb-ai.hello-notes")}`) {
    response.setHeader("content-type", "application/json");
    response.end(
      JSON.stringify({
        name: "go-usb-ai.hello-notes",
        ...
        versions: {
          "0.1.0": {
            ...
            publisher: {
              id: "go-usb-ai",
              name: "GoUsbAi Official",
              url: "https://go-usb-ai.com",
            },
            dist: {
      ...
      JSON.stringify({
        ...
        data: {
          ...
          item: {
            ...
            webUrl: `https://apps.go-usb-ai.io/apps/${payload.slug}`,
            install: {
              kind: "registry",
              spec: payload.appId,
              command: `napp install ${payload.appId}`,
              registry: "https://apps-registry.go-usb-ai.io/api/v1/apps/registry/",
            },
          },
...
try {
  const publishResult = await runCli(
    ...
    {
      GOUSB_AI_APP_HOME: publishWorkspace,
    },
  );
  if (publishResult.publish.item.appId !== "go-usb-ai.hello-notes") {
  ...
  const remoteInstallResult = await runCli(["install", "go-usb-ai.hello-notes", "--json"], {
    GOUSB_AI_APP_HOME: appHomeDirectory,
  });
  ...
  const remotePermissionsBeforeGrant = await runCli(
    ["permissions", "go-usb-ai.hello-notes", "--json"],
    {
      GOUSB_AI_APP_HOME: appHomeDirectory,
    },
  );
  ...
  await runCli(
    ["grant", "go-usb-ai.hello-notes", "--document", `notes=${notesDirectory}`, "--json"],
    {
      GOUSB_AI_APP_HOME: appHomeDirectory,
    },
  );
  ...
  latestVersion = "0.2.0";
  const updateResult = await runCli(["update", "go-usb-ai.hello-notes", "--json"], {
    GOUSB_AI_APP_HOME: appHomeDirectory,
  });
  ...
  const updatedInfoResult = await runCli(["info", "go-usb-ai.hello-notes", "--json"], {
    GOUSB_AI_APP_HOME: appHomeDirectory,
  });
  ...
  await runCli(["revoke", "go-usb-ai.hello-notes", "--document", "notes", "--json"], {
    GOUSB_AI_APP_HOME: appHomeDirectory,
  });
  ...
  await runCli(["uninstall", "go-usb-ai.hello-notes", "--json"], {
    GOUSB_AI_APP_HOME: appHomeDirectory,
  });
} finally {
