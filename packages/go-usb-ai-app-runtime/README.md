...
`@go-usb-ai/app-runtime` 是一个独立的微应用 runtime/CLI 包。
...
当前版本先聚焦“独立可运行的微应用宿主 + 可分发、可安装、可更新、可授权的 CLI/runtime 闭环”。普通用户入口推荐通过 GoUsbAi 的 `go-usb-ai-app-runtime` skill 编排这些命令。
...
- 安装目录：`~/.go-usb-ai/apps/packages/<app-id>/<version>/`
- 用户数据目录：`~/.go-usb-ai/apps/data/<app-id>/`
- 本地 registry：`~/.go-usb-ai/apps/registry.json`
- 本地 config：`~/.go-usb-ai/apps/config.json`
- 默认 registry：`https://apps-registry.go-usb-ai.io/api/v1/apps/registry/`
...
napp list
napp info go-usb-ai.my-first-napp
napp run go-usb-ai.my-first-napp
```
...
napp update go-usb-ai.hello-notes
```

卸载与权限回收：

```bash
napp revoke go-usb-ai.my-first-napp --document notes
napp uninstall go-usb-ai.my-first-napp
napp uninstall go-usb-ai.my-first-napp --purge-data
```
...
napp list
napp info go-usb-ai.my-first-napp
napp permissions go-usb-ai.my-first-napp
napp run go-usb-ai.my-first-napp
```
...
- Web：`https://apps.go-usb-ai.io`
- Registry/API：`https://apps-registry.go-usb-ai.io`
