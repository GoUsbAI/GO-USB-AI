import { ensureUiBridgeSecret } from "@go-usb-ai/server";
...
export class RemoteRelayBridge {
  ...
  async ensureLocalUiHealthy(): Promise<void> {
    ...
    if (!response.ok) {
      throw new Error(`Local UI is not healthy at ${this.localOrigin}. Start GoUsbAi first.`);
    }
  }
  ...
  async requestBridgeCookie(): Promise<string | null> {
    const response = await fetch(`${this.localOrigin}/api/auth/bridge`, {
      method: "POST",
      headers: {
        "x-go-usb-ai-ui-bridge-secret": ensureUiBridgeSecret()
      }
    });
