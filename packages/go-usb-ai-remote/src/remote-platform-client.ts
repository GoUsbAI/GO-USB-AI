...
export class RemotePlatformClient {
  ...
  private resolvePlatformAccess(opts: RemoteConnectCommandOptions): {
    ...
    const token = typeof goUsbAiProvider?.apiKey === "string" ? goUsbAiProvider.apiKey.trim() : "";
    const tokenState = readPlatformSessionTokenState(token);
    if (tokenState.reason === "missing") {
      throw new Error('GoUsbAi platform token is missing. Run "go-usb-ai login" first.');
    }
    if (tokenState.reason === "expired") {
      throw new Error('GoUsbAi platform token expired. Run "go-usb-ai login" or browser sign-in again.');
    }
    if (tokenState.reason === "malformed") {
      throw new Error('GoUsbAi platform token is invalid. Run "go-usb-ai login" again.');
    }
    const configuredApiBase =
      normalizeOptionalString(config.remote.platformApiBase)
      ?? (typeof goUsbAiProvider?.apiBase === "string" ? goUsbAiProvider.apiBase.trim() : "");
    const rawApiBase = normalizeOptionalString(opts.apiBase) ?? configuredApiBase;
    if (!rawApiBase) {
      throw new Error("Platform API base is missing. Pass --api-base, run go-usb-ai login, or set remote.platformApiBase.");
    }
