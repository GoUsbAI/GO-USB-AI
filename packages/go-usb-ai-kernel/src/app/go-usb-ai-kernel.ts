...
import { LocalAssetStore } from "@go-usb-ai/ncp-agent-runtime";
import {
  ChannelManager,
  ensureDir,
  expandHome,
...
import { EventBus, Ingress } from "@go-usb-ai/shared";
import { resolve } from "node:path";

export type GoUsbAiKernelOptions = {
  homeDir?: string;
  configPath?: string;
};
...
function resolveKernelSessionsDir(options: GoUsbAiKernelOptions): string {
...
function resolveKernelAutomationStorePath(options: GoUsbAiKernelOptions): string {
  const homeDir = options.homeDir?.trim();
  if (homeDir) {
...
type GoUsbAiKernelRuntimeControl<
  TGatewayInput,
  TUiInput,
  TStartInput,
> = {
...
class GoUsbAiKernelControlManager<
  TGatewayInput,
  TUiInput,
  TStartInput,
> {
  private runtimeControl: GoUsbAiKernelRuntimeControl<
    TGatewayInput,
    TUiInput,
    TStartInput
  > | null = null;
  ...
  installRuntimeControl = (
    runtimeControl: GoUsbAiKernelRuntimeControl<
      TGatewayInput,
      TUiInput,
      TStartInput
    >,
...
export class GoUsbAiKernel {
  ...
  readonly control: GoUsbAiKernelControlManager<
    unknown,
    unknown,
    unknown
  >;
  ...
  constructor(options: GoUsbAiKernelOptions = {}) {
    ...
    this.control = new GoUsbAiKernelControlManager<
      unknown,
      unknown,
      unknown
    >();
