import type { Session, SessionEvent, SessionMessage } from "@go-usb-ai/core";
import type { NcpMessage } from "@go-usb-ai/ncp";

export const GOUSB_AI_TIMELINE_KIND_METADATA_KEY = "go-usb-ai_timeline_kind";
...
type ContextCompactionTimelineMetadata = {
  [GOUSB_AI_TIMELINE_KIND_METADATA_KEY]: typeof CONTEXT_COMPACTION_TIMELINE_KIND;
...
function readTimelineMetadata(message: SessionMessage | null | undefined): ContextCompactionTimelineMetadata | null {
  ...
  return {
    [GOUSB_AI_TIMELINE_KIND_METADATA_KEY]: CONTEXT_COMPACTION_TIMELINE_KIND,
    checkpoint,
  };
}
...
export function isContextCompactionTimelineMessage(message: { metadata?: Record<string, unknown> | undefined } | null | undefined): boolean {
  return message?.metadata?.[GOUSB_AI_TIMELINE_KIND_METADATA_KEY] === CONTEXT_COMPACTION_TIMELINE_KIND;
}
