import { getDataPath } from "@go-usb-ai/core";
...
const EXTENSION_MANIFEST_FILE = "go-usb-ai.extension.json";
...
function resolveDevFirstPartyExtensionDir(
  explicitDir = process.env.GOUSB_AI_DEV_FIRST_PARTY_EXTENSION_DIR,
...
export function resolveBuiltinExtensionManifestRoots(): string[] {
  if (process.env.GOUSB_AI_DISABLE_BUILTIN_EXTENSIONS === "1") {
    return [];
  }
  const roots: string[] = [];
...
export function resolveExtensionManifestRoots(params: {
  ...
  return uniquePaths([
    ...
    join(params.workspace, ".go-usb-ai", "extensions"),
