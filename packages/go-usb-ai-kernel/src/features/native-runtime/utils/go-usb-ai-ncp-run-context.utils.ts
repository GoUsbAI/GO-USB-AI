...
} from "@kernel/features/native-runtime/utils/go-usb-ai-ncp-session-preferences.utils.js";
import {
  resolveAgentHandoffDepth,
  type ToolRunContext,
} from "@kernel/managers/tool.manager.js";

export type GoUsbAiNcpResolvedAgentProfile = {
  agentId: string;
...
export type GoUsbAiNcpRunContextParams = {
...
export type GoUsbAiNcpResolvedRunContext = {
  channel: string;
  chatId: string;
  config: Config;
  effectiveModel: string;
  effectiveWorkspace: string;
  profile: GoUsbAiNcpResolvedAgentProfile;
  requestMetadata: Record<string, unknown>;
...
function resolveAgentProfile(params: {
  ...
  requestMetadata: Record<string, unknown>;
  storedAgentId?: string;
}): GoUsbAiNcpResolvedAgentProfile {
...
export function buildSessionOrchestrationSection(): string {
  return [
    ...
    "- Before passing a non-default `runtime` to `sessions_spawn` or agent creation/update flows, inspect the installed runtime kinds with `go-usb-ai agents runtimes --json`.",
...
export function resolveGoUsbAiNcpRunContext(
  params: GoUsbAiNcpRunContextParams,
): GoUsbAiNcpResolvedRunContext {
  const {
    configManager,
