...
} from "@go-usb-ai/ncp-agent-runtime";
import type {
  NcpAgentRunInput,
  NcpContextBuilder,
...
import { buildCurrentTurnState } from "@kernel/features/native-runtime/utils/go-usb-ai-ncp-current-turn.utils.js";
...
import {
  ...
  type GoUsbAiNcpResolvedRunContext,
} from "@kernel/features/native-runtime/utils/go-usb-ai-ncp-run-context.utils.js";

type GoUsbAiNcpContextBuilderOptions = {
  agentId?: string;
...
type PreparedRunContext = GoUsbAiNcpResolvedRunContext & {
...
export class GoUsbAiNcpContextBuilder implements NcpContextBuilder {
  private readonly inputBudgetPruner = new InputBudgetPruner();

  constructor(
    private readonly options: GoUsbAiNcpContextBuilderOptions,
  ) {}
  ...
  private prepareRunContext = (input: NcpAgentRunInput): PreparedRunContext => {
    ...
    const resolved = resolveGoUsbAiNcpRunContext({
