import type { SessionMessage } from "@go-usb-ai/core";
import { type NcpMessage, type NcpMessagePart, sanitizeAssistantReplyTags } from "@go-usb-ai/ncp";
import {
  ensureIsoTimestamp,
  normalizeString,
...
function contentToParts(content: unknown): NcpMessagePart[] {
  ...
  if (Array.isArray(content)) {
    return content.length > 0
      ? [
          {
            type: "extension",
            extensionType: "go-usb-ai.legacy.content-array",
  ...
  if (content && typeof content === "object") {
    return [
      {
        type: "extension",
        extensionType: "go-usb-ai.legacy.content-object",
        data: structuredClone(content)
      }
    ];
  }
