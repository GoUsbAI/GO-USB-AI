import type { UsageCommandOptions } from "@go-usb-ai-service/shared/types/cli.types.js";
import {
  LlmUsageManager,
  type LlmUsageStats,
  ...
  private readonly showSnapshot = (opts: UsageCommandOptions): void => {
    ...
    if (!snapshot) {
      console.log([
        ...
        "Run `go-usb-ai agent -m \"ping\"` or use the local UI once, then retry `go-usb-ai usage`.",
      ].join("\n"));
      process.exitCode = 0;
      return;
    }
  ...
  private readonly showHistory = (opts: UsageCommandOptions): void => {
    ...
    if (records.length === 0) {
      console.log([
        ...
        "Run `go-usb-ai agent -m \"ping\"` or use the local UI once, then retry `go-usb-ai usage --history`.",
      ].join("\n"));
      process.exitCode = 0;
      return;
    }
  ...
  private readonly showStats = (opts: UsageCommandOptions): void => {
    ...
    if (stats.totalRecords === 0) {
      console.log([
        "No LLM usage history recorded yet.",
        `History path: ${this.usageManager.historyPath}`,
        "Run `go-usb-ai agent -m \"ping\"` or use the local UI once, then retry `go-usb-ai usage --stats`.",
      ].join("\n"));
      process.exitCode = 0;
      return;
    }
