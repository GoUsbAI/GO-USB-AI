...
function createTempSkillDir(skillMarkdown: string, metadataJson?: string): string {
  const root = mkdtempSync(join(tmpdir(), "go-usb-ai-marketplace-publish-"));
...
`, JSON.stringify({
    ...
    await publishMarketplaceSkill({
      skillDir,
      slug: "humanizer",
      packageName: "@go-usb-ai/humanizer",
      apiBaseUrl: "https://marketplace-api.go-usb-ai.io",
      token: "admin-token"
    });

    expect(capturedBody?.packageName).toBe("@go-usb-ai/humanizer");
