import type { PlatformMeResult } from "@go-usb-ai-service/commands/platform-auth/index.js";
...
function assertOfficialScopeAllowed(adminTokenPresent: boolean, currentUser: PlatformMeResult | null): void {
  if (!canPublishOfficialScope(adminTokenPresent, currentUser)) {
    throw new Error("Publishing to @go-usb-ai/* requires admin permission.");
  }
}
...
function requirePlatformUsername(currentUser: PlatformMeResult | null): string {
  ...
  if (!username) {
    throw new Error(
      ...
      + "Open GoUsbAi Web account settings: https://platform.go-usb-ai.io/account "
      + "or run `go-usb-ai account set-username <username>`."
    );
  }
  return username;
}
