﻿﻿﻿﻿﻿﻿﻿﻿﻿import { getConfigPath, loadConfig, saveConfig } from "@go-usb-ai/core";
...
function resolveProviderConfig(opts: LoginCommandOptions): {
  ...
  providers: Record<string, GoUsbAiProviderConfig>;
  goUsbAiProvider: GoUsbAiProviderConfig;
  platformBase: string;
  v1Base: string;
  inputApiBase: string;
} {
  ...
  const providers = config.providers as Record<string, GoUsbAiProviderConfig>;
  const goUsbAiProvider = providers["go-usb-ai"] ?? {
    displayName: "",
    apiKey: "",
    apiBase: null,
    extraHeaders: null,
  ...
  const configuredApiBase =
    typeof goUsbAiProvider.apiBase === "string" && goUsbAiProvider.apiBase.trim().length > 0
      ? goUsbAiProvider.apiBase.trim()
      : "https://ai-gateway-api.go-usb-ai.io/v1";
  ...
  const { platformBase, v1Base, inputApiBase } = resolvePlatformApiBase({
    ...
    fallbackApiBase: "https://ai-gateway-api.go-usb-ai.io/v1"
  });
  return {
    configPath,
    config,
...
function persistPlatformToken(params: {
  ...
  const { configPath, config, providers, goUsbAiProvider, v1Base, token } = params;
  goUsbAiProvider.apiBase = v1Base;
  goUsbAiProvider.apiKey = token;
  providers["go-usb-ai"] = goUsbAiProvider;
...
export class PlatformAuthCommands {
  ...
  private printLoginSuccess = (result: PlatformLoginResult): void => {
    console.log(`✓ Logged in to GoUsbAi platform (${result.platformBase})`);
    console.log(`✓ Account: ${result.email} (${result.role})`);
    console.log('✓ Token saved into providers["go-usb-ai"].apiKey');
  };
  ...
  private printBrowserLoginIntro = (params: {
    ...
    console.log("GoUsbAi browser sign-in");
  ...
  private waitForBrowserLoginResult = async (params: {
    ...
    while (true) {
      ...
      if (result.status === "expired") {
        throw new Error(`${result.message} Run \`go-usb-ai login\` again to generate a new sign-in link.`);
      }
      return {
        token: result.token,
        role: result.role,
  ...
  private readStoredToken = (params: { apiBase?: string } = {}): {
    ...
    const token = resolved.goUsbAiProvider.apiKey?.trim() ?? "";
    if (!token) {
      throw new Error("Not logged in. Run `go-usb-ai login` first.");
    }
    return {
      ...resolved,
      token
  ...
  loginResult = async (opts: LoginCommandOptions = {}): Promise<PlatformLoginResult> => {
    const { configPath, config, providers, goUsbAiProvider, platformBase, v1Base, inputApiBase } = resolveProviderConfig(opts);
  ...
  pollBrowserAuth = async (params: {
    ...
    const { configPath, config, providers, goUsbAiProvider, platformBase, v1Base, inputApiBase } = resolveProviderConfig({
      apiBase: params.apiBase
    });
  ...
  logout = (): { cleared: boolean } => {
    const { configPath, config, providers, goUsbAiProvider } = resolveProviderConfig({});
    const cleared = Boolean(goUsbAiProvider.apiKey?.trim());
    goUsbAiProvider.apiKey = "";
    providers["go-usb-ai"] = goUsbAiProvider;
    saveConfig(config, configPath);
    return { cleared };
  };
