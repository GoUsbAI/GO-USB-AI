...
const DEFAULT_SYSTEMD_SERVICE_NAME = "go-usb-ai.service";
...
export class LinuxSystemdAutostartService {
  ...
  private buildPlan = (scope: HostAutostartScope) => {
    ...
    return {
      ...
      unitContent: [
        "[Unit]",
        "Description=GoUsbAi managed local service",
        ...
        `Environment=GOUSB_AI_HOME=${this.escapeSystemdValue(homeDir)}`,
        `ExecStart=${command}`,
        "Restart=on-failure",
        "RestartSec=2",
        "",
