...
describe("WindowsTaskAutostartService", () => {
  it("writes a launcher script and registers a scheduled task", async () => {
    ...
    const service = new WindowsTaskAutostartService({
      ...
      runtimeService: new HostAutostartRuntimeService({
        ...
        argvEntry: "C:\\go-usb-ai\\dist\\cli\\index.js",
        importMetaUrl: "file:///C:/go-usb-ai/dist/cli/commands/service/index.js",
        getDataDir: () => "C:\\Users\\alice\\AppData\\Roaming\\go-usb-ai",
      }),
    });
    ...
    expect(result.resourcePath).toBe("C:\\Users\\alice\\AppData\\Roaming\\go-usb-ai\\autostart\\go-usb-ai-host-autostart.cmd");
    const launcher = String(writeFileSync.mock.calls[0]?.[1]);
    expect(launcher).toContain("set \"GOUSB_AI_HOME=C:\\Users\\alice\\AppData\\Roaming\\go-usb-ai\"");
    expect(launcher).toContain("\"C:\\Program Files\\nodejs\\node.exe\" \"C:\\go-usb-ai\\dist\\cli\\index.js\" \"serve\"");
    ...
    expect(powershellScript).toContain("GoUsbAi Host Autostart");
  });
  ...
  it("reads scheduled task status through PowerShell JSON", async () => {
    ...
    expect(status.resourceName).toBe("GoUsbAi Host Autostart");
  });
