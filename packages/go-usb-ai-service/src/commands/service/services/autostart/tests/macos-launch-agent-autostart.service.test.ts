  ...
  it("writes a launch agent plist and bootstraps the current GUI session", async () => {
    ...
    const service = new MacosLaunchAgentAutostartService({
      ...
      runtimeService: new HostAutostartRuntimeService({
        ...
        getDataDir: () => "/Users/alice/.go-usb-ai",
      }),
    });
    ...
    expect(result.resourcePath).toBe("/Users/alice/Library/LaunchAgents/io.go-usb-ai.host-agent.plist");
    expect(writeFileSync).toHaveBeenCalledTimes(1);
    const plist = String(writeFileSync.mock.calls[0]?.[1]);
    expect(plist).toContain("<key>Label</key><string>io.go-usb-ai.host-agent</string>");
    expect(plist).toContain("<key>RunAtLoad</key><true/>");
    expect(plist).toContain("<key>KeepAlive</key><false/>");
    expect(plist).toContain("<key>GOUSB_AI_HOME</key>");
    expect(plist).toContain("<string>/Users/alice/.go-usb-ai</string>");
    expect(runCommand).toHaveBeenNthCalledWith(1, "launchctl", ["print", "gui/501"]);
    expect(runCommand).toHaveBeenNthCalledWith(2, "launchctl", ["bootout", "gui/501/io.go-usb-ai.host-agent"]);
    expect(runCommand).toHaveBeenNthCalledWith(3, "launchctl", [
      "bootstrap",
      "gui/501",
      "/Users/alice/Library/LaunchAgents/io.go-usb-ai.host-agent.plist",
    ]);
  });

  it("reads enabled and active state from launchctl status", async () => {
    const resourcePath = "/Users/alice/Library/LaunchAgents/io.go-usb-ai.host-agent.plist";
    const runCommand = vi.fn()
      .mockResolvedValueOnce({
        code: 0,
        stdout: `\n\tdisabled services = {\n\t\t"io.go-usb-ai.host-agent" => enabled\n\t}\n`,
        stderr: "",
      })
      .mockResolvedValueOnce({ code: 0, stdout: "", stderr: "" })
