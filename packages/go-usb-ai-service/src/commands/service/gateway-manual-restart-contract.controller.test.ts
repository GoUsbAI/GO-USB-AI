...
import { ConfigSchema, GatewayTool, type Config } from "@go-usb-ai/core";
import { ConfigManager, LlmProviderManager } from "@go-usb-ai/kernel";
...
vi.mock("@go-usb-ai-service/launcher/npm-runtime-update-command.service.js", () => ({
...
describe("gateway manual restart contract", () => {
  let configDir = "";
  let configPath = "";
  let originalGoUsbAiHome: string | undefined;
  ...
  beforeEach(() => {
    originalGoUsbAiHome = process.env.GOUSB_AI_HOME;
    configDir = mkdtempSync(join(tmpdir(), "go-usb-ai-gateway-manual-restart-"));
    configPath = join(configDir, "config.json");
    process.env.GOUSB_AI_HOME = configDir;
  ...
  afterEach(() => {
    pendingRestartStore.clear();
    if (originalGoUsbAiHome === undefined) {
