...
import { readPlatformSessionTokenState } from "@go-usb-ai/remote";
import type {
  RemoteAccessView,
  RemoteAccountView,
...
export class RemoteAccessHost implements UiRemoteAccessHost {
  ...
  getStatus = (): RemoteAccessView => {
    ...
    const account = this.readAccountView({
      token: normalizeOptionalString(config.providers.go-usb-ai?.apiKey),
      apiBase: normalizeOptionalString(config.providers.go-usb-ai?.apiBase),
