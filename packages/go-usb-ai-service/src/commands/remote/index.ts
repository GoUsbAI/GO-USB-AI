import { getConfigPath, loadConfig, saveConfig, type Config } from "@go-usb-ai/core";
import {
...
import { localUiDiscoveryService } from "@go-usb-ai-service/shared/services/ui/local-ui-discovery.service.js";
import { createGoUsbAiRemoteConnector, resolveGoUsbAiRemoteStatusSnapshot } from "./utils/remote-runtime-support.utils.js";

export {
  buildGoUsbAiConfiguredRemoteState,
  createGoUsbAiRemoteConnector,
  createGoUsbAiRemoteStatusStore,
  hasRunningGoUsbAiManagedService,
  resolveGoUsbAiRemoteStatusSnapshot
...
function describePlatformTokenCheck(token: string | undefined): RemoteCommandDoctorCheck {
  ...
  if (tokenState.reason === "expired") {
    return {
      name: "platform-token",
      ok: false,
      detail: 'platform session token expired; run remote browser login or "go-usb-ai login" again'
    };
  }
  if (tokenState.reason === "malformed") {
    return {
      name: "platform-token",
      ok: false,
      detail: 'platform session token invalid; run remote browser login or "go-usb-ai login" again'
    };
  }
  return {
    name: "platform-token",
    ok: false,
    detail: 'run remote browser login or "go-usb-ai login" first'
  };
}
...
export class RemoteCommands {
  ...
  async connect(opts: RemoteConnectCommandOptions = {}): Promise<void> {
    const connector = createGoUsbAiRemoteConnector();
    await connector.run({
      ...opts,
      mode: "foreground"
    });
  ...
  getStatusView(): RemoteCommandStatusView {
    const config = loadConfig(getConfigPath());
    const snapshot = resolveGoUsbAiRemoteStatusSnapshot(config);
    ...
    return {
      ...
      platformBase:
        ...
        normalizeOptionalString(config.providers.go-usb-ai?.apiBase) ??
        null
    };
  }
  ...
  async status(opts: RemoteStatusCommandOptions = {}): Promise<void> {
    ...
    console.log("GoUsbAi Remote Status");
  ...
  async getDoctorView(): Promise<RemoteCommandDoctorView> {
    ...
    const token = normalizeOptionalString(config.providers.go-usb-ai?.apiKey);
  ...
  async doctor(opts: RemoteDoctorCommandOptions = {}): Promise<void> {
    ...
    console.log("GoUsbAi Remote Doctor");
