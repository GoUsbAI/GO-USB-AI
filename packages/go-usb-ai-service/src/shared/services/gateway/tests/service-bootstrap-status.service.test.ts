...
import { createUiRouter } from "@go-usb-ai/server";
...
describe("ServiceBootstrapStatusStore", () => {
  ...
  it("syncs remote runtime state into the bootstrap view", () => {
    ...
    store.syncRemoteRuntimeState({
      ...
      mode: "service",
      state: "error",
      lastError: "Remote access is already owned by running GoUsbAi service PID 1000.",
      updatedAt: "2026-03-24T00:00:01.000Z"
    });
    expect(store.getStatus().remote).toEqual({
      state: "conflict",
      message: "Remote access is already owned by running GoUsbAi service PID 1000."
    });
  });
  ...
  it("returns the synced remote runtime state through the bootstrap API route", async () => {
    ...
    const app = createUiRouter({
      ...
      configPath: "/tmp/go-usb-ai-bootstrap-status-route-test.json",
