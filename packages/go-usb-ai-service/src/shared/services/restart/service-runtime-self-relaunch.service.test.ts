...
import { GoUsbAiServiceRuntime } from "../../../service-runtime.service.js";
...
describe("GoUsbAiServiceRuntime self relaunch", () => {
  let tempHome = "";
  let originalHome: string | undefined;
  let originalArgv: string[];

  beforeEach(() => {
    tempHome = mkdtempSync(join(tmpdir(), "go-usb-ai-service-runtime-test-"));
    originalHome = process.env.GOUSB_AI_HOME;
  ...
  it("relaunches through the current CLI app entry instead of the service package index", () => {
    ...
    const runtime = new GoUsbAiServiceRuntime({ logo: "test" }) as unknown as RelaunchRuntime;
    ...
    expect(helperScript).not.toContain("@go-usb-ai/service/dist/index.js");
  });

  it("preserves tsx launch arguments for source CLI entries", () => {
    process.argv[1] = "/repo/packages/go-usb-ai/src/cli/app/index.ts";
    const runtime = new GoUsbAiServiceRuntime({ logo: "test" }) as unknown as RelaunchRuntime;
    ...
    expect(helperScript).toContain("/repo/packages/go-usb-ai/src/cli/app/index.ts");
    expect(helperScript).toContain('"start","--ui-port","19199"');
  });
});
