...
import { managedServiceStateStore, type ManagedServiceState } from "@go-usb-ai-service/shared/stores/managed-service-state.store.js";
import {
  resolveManagedServiceReadySnapshot,
  resolveManagedServiceUiBinding,
  resolveSessionRouteCandidate,
} from "@go-usb-ai-service/shared/services/runtime/utils/managed-service-routing.utils.js";
import {
  isProcessRunning,
  openBrowser,
  ...
  resolveUiConfig,
  waitForExit
} from "@go-usb-ai-service/shared/utils/cli.utils.js";
import { probeHealthEndpoint } from "@go-usb-ai-service/shared/utils/service-port-probe.utils.js";
import { ManagedServiceSupervisor } from "@go-usb-ai-service/shared/services/runtime/managed-service-supervisor.service.js";

const { APP_NAME, loadConfig } = GoUsbAiCore;
...
export class ManagedServiceCommandService {
  private readonly loggingRuntime = GoUsbAiCore.getLoggingRuntime();
  ...
  startService = async (options: StartServiceOptions): Promise<void> => {
    ...
    if (!staticDir) {
      return void (
        process.exitCode = 1,
        console.error(`Error: ${APP_NAME} UI frontend bundle not found. Reinstall or rebuild ${APP_NAME}. For dev-only overrides, set GOUSB_AI_UI_STATIC_DIR to a built frontend directory.`)
      );
    }
  ...
  stopService = async (): Promise<void> => {
    ...
    if (!stopped) {
      this.serviceLogger.warn("runtime.process.stop_requested", {
        runtimeKind: "managed-service",
        pid: state.pid,
        reason: "go-usb-ai-stop-command-force",
        signal: "SIGKILL",
        uiUrl: state.uiUrl,
        apiUrl: state.apiUrl
      });
  ...
  private resolveStartupTimeoutMs = (overrideTimeoutMs: number | undefined): number => {
    ...
    const envRaw = process.env.GOUSB_AI_START_TIMEOUT_MS?.trim();
