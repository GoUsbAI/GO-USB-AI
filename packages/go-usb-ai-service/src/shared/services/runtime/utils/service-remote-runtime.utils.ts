...
import { RemoteServiceModule, type RemoteRuntimeState } from "@go-usb-ai/remote";
import {
  closeSync,
  existsSync,
  mkdirSync,
...
import { getDataDir } from "@go-usb-ai/core";
import { resolveUiApiBase, isProcessRunning } from "@go-usb-ai-service/shared/utils/cli.utils.js";
import {
  managedServiceStateStore,
  type ManagedServiceLease,
  type ManagedServiceState
} from "@go-usb-ai-service/shared/stores/managed-service-state.store.js";
import {
  buildGoUsbAiConfiguredRemoteState,
  createGoUsbAiRemoteConnector,
  createGoUsbAiRemoteStatusStore
} from "@go-usb-ai-service/commands/remote/utils/remote-runtime-support.utils.js";
...
function buildManagedServiceOwnershipError(params: {
  ...
  return (
    `Remote access is already owned by running GoUsbAi service PID ${params.pid}`
    + `${params.ownerOrigin ? ` (${params.ownerOrigin})` : ""}. `
    + "Stop that service or disable remote there before starting another process with the same GOUSB_AI_HOME."
  );
}
...
function buildLocalProcessOwnershipError(record: RemoteRuntimeOwnerRecord): string {
  ...
  return (
    `Remote access is already owned by local GoUsbAi process PID ${record.pid}${originText}. `
    + "Stop that process or use a different GOUSB_AI_HOME before starting another remote-enabled process."
  );
}
...
export function createManagedRemoteModule(params: {
  ...
  return new RemoteServiceModule({
    ...
    localOrigin: params.localOrigin,
    statusStore: createGoUsbAiRemoteStatusStore("service", params.onRemoteStateChange),
    createConnector: (logger) => createGoUsbAiRemoteConnector({ logger }),
...
export function createManagedRemoteModuleForUi(params: {
  ...
  const explicitLocalOrigin = params.localOriginOverride?.trim() ?? process.env.GOUSB_AI_REMOTE_LOCAL_ORIGIN?.trim();
...
export function writeInitialManagedServiceState(params: {
  ...
  managedServiceStateStore.write({
    ...
    ...(config.remote.enabled ? { remote: buildGoUsbAiConfiguredRemoteState(config) } : {})
  });
}
