...
describe("claimManagedRemoteRuntimeOwnership", () => {
  ...
  it("fails explicitly when another managed service already owns remote access", () => {
    const claim = claimManagedRemoteRuntimeOwnership({
      ...
      readServiceStateFn: () =>
        ({
          ...
          uiHost: "0.0.0.0",
          uiPort: 9808,
          logPath: "/tmp/go-usb-ai-service.log",
          remote: {
            enabled: true,
            mode: "service",
            state: "connected",
    ...
    expect(claim.ok ? "" : claim.error).toContain("running GoUsbAi service PID 1000");
  ...
  it("reclaims a stale local ownership lock and releases it after use", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "go-usb-ai-remote-owner-"));
