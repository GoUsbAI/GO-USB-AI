...
import type { ManagedServiceState, ManagedServiceStateStore } from "@go-usb-ai-service/shared/stores/managed-service-state.store.js";
...
vi.mock("@go-usb-ai-service/shared/services/runtime/utils/service-remote-runtime.utils.js", () => ({
...
describe("spawnManagedService", () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "go-usb-ai-service-startup-"));
    originalArgv = [...process.argv];
    process.env.GOUSB_AI_RUNTIME_BUNDLE_CHILD = "1";
    process.env.GOUSB_AI_DISABLE_RUNTIME_BUNDLE_LAUNCHER = "1";
    vi.clearAllMocks();
  });

  afterEach(() => {
    process.argv = originalArgv;
    delete process.env.GOUSB_AI_RUNTIME_BUNDLE_CHILD;
    delete process.env.GOUSB_AI_DISABLE_RUNTIME_BUNDLE_LAUNCHER;
  ...
  it("spawns the managed service through the resolved CLI entry", () => {
    ...
    expect((options.env as NodeJS.ProcessEnv).GOUSB_AI_RUNTIME_BUNDLE_CHILD).toBeUndefined();
    expect((options.env as NodeJS.ProcessEnv).GOUSB_AI_DISABLE_RUNTIME_BUNDLE_LAUNCHER).toBeUndefined();
