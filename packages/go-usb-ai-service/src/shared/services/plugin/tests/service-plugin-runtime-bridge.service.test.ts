...
import type * as GoUsbAiKernelModule from "@go-usb-ai/kernel";
...
vi.mock("@go-usb-ai/openclaw-compat", () => ({
  setPluginRuntimeBridge: setPluginRuntimeBridgeMock,
}));

vi.mock("@go-usb-ai/kernel", async (importOriginal) => {
  const actual = await importOriginal<typeof GoUsbAiKernelModule>();
  return {
    ...actual,
...
function createGateway(): GoUsbAiGatewayRuntime {
  return {
    kernel: {
    ...
    sessionManager: {},
  } as unknown as GoUsbAiGatewayRuntime;
}
