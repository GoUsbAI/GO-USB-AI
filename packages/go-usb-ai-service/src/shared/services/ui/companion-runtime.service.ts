...
import { APP_NAME } from "@go-usb-ai/core";
import { localUiDiscoveryService, type LocalUiDiscoveryService } from "@go-usb-ai-service/shared/services/ui/local-ui-discovery.service.js";
...
} from "@go-usb-ai-service/shared/stores/companion-runtime.store.js";
...
export class CompanionRuntimeService {
  ...
  readonly ensureStarted = async (options: { baseUrl: string }): Promise<CompanionRuntimeState> => {
    ...
    const child = spawn(launchSpec.command, [...launchSpec.args, "--base-url", baseUrl], {
      ...
      env: {
        ...process.env,
        GOUSB_AI_COMPANION_RUNTIME_STATE_PATH: this.runtimeStore.path
      }
    });
    child.unref();
  ...
  private readonly resolveLaunchSpec = (): { command: string; args: string[] } => {
    ...
    try {
      packageJsonPath = require.resolve("@go-usb-ai/companion/package.json");
    } catch {
      throw new CompanionUnavailableError(
        "@go-usb-ai/companion is not installed. Install it separately to use the companion shell."
      );
    }
    ...
    if (!existsSync(mainPath)) {
      throw new CompanionUnavailableError(
        `Companion app build is missing at ${mainPath}. Install or build @go-usb-ai/companion separately to use the companion shell.`
      );
    }
