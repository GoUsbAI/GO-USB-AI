import { mkdtempSync, mkdirSync, rmSync, writeFileSync } from "node:fs";
import { tmpdir } from "node:os";
import { join } from "node:path";
import { afterEach, describe, expect, it, vi } from "vitest";
import { resolveUiStaticDir } from "./cli.utils.js";

const cleanupDirs: string[] = [];

afterEach(() => {
  vi.unstubAllEnvs();
  while (cleanupDirs.length > 0) {
    const dir = cleanupDirs.pop();
    if (!dir) {
      continue;
    }
    rmSync(dir, { recursive: true, force: true });
  }
});

function createTempDir(prefix: string): string {
  const dir = mkdtempSync(join(tmpdir(), prefix));
  cleanupDirs.push(dir);
  return dir;
}

describe("resolveUiStaticDir", () => {
  it("returns null when GOUSB_AI_DISABLE_STATIC_UI is enabled", () => {
    vi.stubEnv("GOUSB_AI_DISABLE_STATIC_UI", "1");
    vi.stubEnv("GOUSB_AI_UI_STATIC_DIR", "");

    expect(resolveUiStaticDir()).toBeNull();
  });

  it("uses the explicit GOUSB_AI_UI_STATIC_DIR when it points to a built frontend", () => {
    const dir = createTempDir("go-usb-ai-ui-static-env-");
    writeFileSync(join(dir, "index.html"), "<html></html>");
    vi.stubEnv("GOUSB_AI_UI_STATIC_DIR", dir);

    expect(resolveUiStaticDir()).toBe(dir);
  });

  it("fails instead of silently falling back when GOUSB_AI_UI_STATIC_DIR is invalid", () => {
    const dir = createTempDir("go-usb-ai-ui-static-invalid-");
    vi.stubEnv("GOUSB_AI_UI_STATIC_DIR", dir);

    expect(resolveUiStaticDir()).toBeNull();
  });

  it("returns null when no packaged ui-dist is provided", () => {
    vi.stubEnv("GOUSB_AI_UI_STATIC_DIR", "");

    expect(resolveUiStaticDir()).toBeNull();
  });

  it("does not borrow the repo frontend dist from the current working directory", () => {
    const cwdRoot = createTempDir("go-usb-ai-ui-static-cwd-");
    const repoFrontendDist = join(cwdRoot, "packages", "go-usb-ai-ui", "dist");
    mkdirSync(repoFrontendDist, { recursive: true });
    writeFileSync(join(repoFrontendDist, "index.html"), "<html>repo dist</html>");
    vi.stubEnv("GOUSB_AI_UI_STATIC_DIR", "");
    const originalCwd = process.cwd();
    process.chdir(cwdRoot);

    try {
      const resolved = resolveUiStaticDir();
      expect(resolved).not.toBe(repoFrontendDist);
    } finally {
      process.chdir(originalCwd);
    }
  });

  it("resolves the packaged ui-dist provided by the distribution owner", () => {
    const packageRoot = createTempDir("go-usb-ai-packaged-root-");
    const bundledUiDir = join(packageRoot, "ui-dist");
    const bundledAssetsDir = join(bundledUiDir, "assets");
    mkdirSync(bundledAssetsDir, { recursive: true });
    writeFileSync(join(bundledUiDir, "index.html"), "<html>bundled dist</html>");
    writeFileSync(join(bundledAssetsDir, "index.js"), "console.log('ok');");
    vi.stubEnv("GOUSB_AI_UI_STATIC_DIR", "");

    const resolved = resolveUiStaticDir(bundledUiDir);
    expect(resolved).toBe(bundledUiDir);
  });
});
