...
import type { UpdateCommandOptions } from "@go-usb-ai-service/shared/types/cli.types.js";
import type { UpdateProgress, UpdateSnapshot } from "@go-usb-ai/kernel";
...
export class NpmRuntimeUpdateCommandService {
  ...
  runManaged = async (opts: UpdateCommandOptions): Promise<UpdateSnapshot> => {
    if (this.env.GOUSB_AI_DESKTOP_COMMAND_SURFACE === "1") {
  ...
  private printSnapshot = (snapshot: UpdateSnapshot): void => {
    ...
    if (snapshot.status === "up-to-date") {
      console.log(`GoUsbAi runtime is already up to date (${snapshot.currentVersion ?? snapshot.hostVersion ?? "unknown"}).`);
      return;
    }
    if (snapshot.status === "downloaded") {
      console.log(`Runtime update downloaded: ${snapshot.downloadedVersion ?? snapshot.availableVersion}`);
      console.log("Run `go-usb-ai update --apply` to switch to the downloaded runtime.");
      return;
    }
    if (snapshot.status === "restart-required") {
      console.log(`Runtime update applied: ${snapshot.currentVersion}`);
      console.log("Restart the running GoUsbAi service or start a new CLI process to use it.");
      return;
    }
  ...
  private createDesktopCommandSurfaceBlockedSnapshot = (opts: UpdateCommandOptions): UpdateSnapshot => {
    ...
    return {
      ...
      recoveryCommand: "Use the desktop update settings or restart GoUsbAi Desktop to check for desktop updates.",
      errorMessage: "The desktop-installed go-usb-ai command uses the desktop update channel instead of the npm runtime updater.",
      preferences: {
