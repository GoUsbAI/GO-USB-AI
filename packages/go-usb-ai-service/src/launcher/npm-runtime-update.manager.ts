import type { UpdateManifest, UpdateProgress, UpdateSnapshot } from "@go-usb-ai/kernel";
import { getPackageVersion } from "@go-usb-ai-service/shared/utils/cli.utils.js";
  ...
  checkForUpdate = async (): Promise<UpdateSnapshot> => {
    ...
    if (!this.options.updateService.hasSignatureVerifier()) {
      return this.toSnapshotFromState(this.options.stateStore.read(), {
        ...
        recoveryCommand: "Set GOUSB_AI_UPDATE_BUNDLE_PUBLIC_KEY or GOUSB_AI_UPDATE_BUNDLE_PUBLIC_KEY_PATH",
    ...
    if (!manifestUrl) {
      return this.toSnapshotFromState(this.options.stateStore.read(), {
        ...
        recoveryCommand: "Set GOUSB_AI_UPDATE_MANIFEST_URL or GOUSB_AI_UPDATE_MANIFEST_BASE_URL",
  ...
  private toSnapshotAfterCheck = (
    ...
    if (availableUpdate.kind === "host-update-required") {
      ...
      return this.toSnapshotFromState(state, {
        ...
        errorMessage: `GoUsbAi npm launcher ${this.launcherVersion} is too old for runtime bundle ${availableUpdate.manifest.latestVersion}.`
      });
    }
