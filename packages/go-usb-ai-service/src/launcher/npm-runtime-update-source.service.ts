...
const DEFAULT_NPM_RUNTIME_UPDATE_BASE_URL = "https://Peiiii.github.io/go-usb-ai/npm-runtime-updates";
...
export class NpmRuntimeUpdateSourceService {
  ...
  resolveChannel = (explicitChannel?: unknown, launcherVersion?: string | null): NpmRuntimeReleaseChannel => {
    if (explicitChannel !== undefined || this.env.GOUSB_AI_UPDATE_CHANNEL !== undefined) {
      return normalizeChannel(explicitChannel ?? this.env.GOUSB_AI_UPDATE_CHANNEL);
    }
  ...
  resolveManifestUrl = (channel: NpmRuntimeReleaseChannel, explicitManifestUrl?: unknown): string | null => {
    const manifestUrl = normalizeOptionalString(explicitManifestUrl) ?? normalizeOptionalString(this.env.GOUSB_AI_UPDATE_MANIFEST_URL);
    if (manifestUrl) {
      return manifestUrl;
    }
    const baseUrl = normalizeOptionalString(this.env.GOUSB_AI_UPDATE_MANIFEST_BASE_URL) ?? DEFAULT_NPM_RUNTIME_UPDATE_BASE_URL;
  ...
  resolveBundlePublicKey = (): string | null => {
    const explicitPublicKey = normalizeOptionalString(this.env.GOUSB_AI_UPDATE_BUNDLE_PUBLIC_KEY);
    if (explicitPublicKey) {
      return explicitPublicKey;
    }
    const publicKeyPath = normalizeOptionalString(this.env.GOUSB_AI_UPDATE_BUNDLE_PUBLIC_KEY_PATH);
