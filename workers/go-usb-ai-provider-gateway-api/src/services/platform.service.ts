...
export async function ensurePlatformBootstrap(env: Env): Promise<void> {
  const existing = bootstrapReady.get(env.GOUSB_AI_PLATFORM_DB);
  if (existing) {
    await existing;
    return;
  }
  ...
  const task = (async () => {
    ...
    await env.GOUSB_AI_PLATFORM_DB.batch([
      env.GOUSB_AI_PLATFORM_DB.prepare(
  ...
  bootstrapReady.set(env.GOUSB_AI_PLATFORM_DB, task);
  await task;
}
...
> {
  ...
  const user = await getUserById(c.env.GOUSB_AI_PLATFORM_DB, payload.sub);
  if (!user) {
    return {
      ok: false,
