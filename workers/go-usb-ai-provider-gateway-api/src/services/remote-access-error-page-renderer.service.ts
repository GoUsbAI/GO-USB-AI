...
function resolveDescription(message: string, status: number): string {
  if (message === "Remote access session expired.") {
    return "This remote access session has expired. Open the device again from GoUsbAi to create a fresh browser session.";
  }
  if (message === "Remote access session revoked.") {
    return "This remote access session was revoked. Reopen the device from GoUsbAi if you still need access.";
  }
  ...
  if (status === 404) {
    return "This remote access session no longer exists. Open the device again from GoUsbAi to create a fresh browser session.";
  }
  return "Remote access is temporarily unavailable. Return to GoUsbAi and start a new remote session.";
}
...
export function renderRemoteAccessErrorPage(params: {
  ...
  const action = homeLink
    ? `<a class="primary" href="${escapeHtml(homeLink)}">Open GoUsbAi Web</a>`
    : "";
...
<html lang="en">
  ...
  <body>
    <main class="card">
      <div class="eyebrow">GoUsbAi Remote Access</div>
