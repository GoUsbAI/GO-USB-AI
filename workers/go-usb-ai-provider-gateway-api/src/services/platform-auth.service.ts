...
async function ensureUserLoginUnlocked(params: {
  ...
  const security = await ensureUserSecurityRow(env.GOUSB_AI_PLATFORM_DB, user.id, nowIso);
  const lockedUntil = parseIsoDate(security.login_locked_until);
  if (lockedUntil && lockedUntil.getTime() > now.getTime()) {
    await appendLoginAttempt(env.GOUSB_AI_PLATFORM_DB, {
      email,
      ip: clientIp,
      success: false,
      reason: "locked",
  ...
  if (lockedUntil) {
    await resetUserLoginSecurity(env.GOUSB_AI_PLATFORM_DB, user.id, nowIso);
  }
}
...
async function handleInvalidCredentials(params: {
  ...
  if (lockState.lockedUntil) {
    await appendAuditLog(env.GOUSB_AI_PLATFORM_DB, {
      actorUserId: user.id,
...
export async function registerPlatformUser(params: {
  ...
  const existing = await getUserByEmail(env.GOUSB_AI_PLATFORM_DB, email);
  if (existing) {
  ...
  const inserted = await env.GOUSB_AI_PLATFORM_DB.prepare(
    `INSERT INTO users (
  ...
  const user = await getUserById(env.GOUSB_AI_PLATFORM_DB, userId);
  if (!user) {
    throw new PlatformAuthServiceError(500, "REGISTER_FAILED", "User created but cannot be loaded.");
  }
  await ensureUserSecurityRow(env.GOUSB_AI_PLATFORM_DB, user.id, now);
  return user;
}
...
export async function updatePlatformUserPassword(params: {
  ...
  const digest = await hashPassword(password);
  const updated = await env.GOUSB_AI_PLATFORM_DB.prepare(
    `UPDATE users
        SET password_hash = ?,
            password_salt = ?,
            updated_at = ?
  ...
  await resetUserLoginSecurity(env.GOUSB_AI_PLATFORM_DB, existing.id, now);
  const user = await getUserById(env.GOUSB_AI_PLATFORM_DB, existing.id);
  if (!user) {
...
export async function updatePlatformUserProfile(params: {
  ...
  const existing = await getUserById(env.GOUSB_AI_PLATFORM_DB, userId);
  if (!existing) {
  ...
  const owner = await getUserByUsername(env.GOUSB_AI_PLATFORM_DB, username);
...
export async function authenticatePlatformUser(params: {
  ...
  const user = await getUserByEmail(env.GOUSB_AI_PLATFORM_DB, email);
