...
async function deliverResendEmail(params: {
  ...
  const subject = purpose === "register"
    ? "Verify your GoUsbAi email"
    : "Reset your GoUsbAi password";
  const intro = purpose === "register"
    ? "Use this code to verify your email and finish creating your GoUsbAi Account."
    : "Use this code to reset the password for your GoUsbAi Account.";
  const html = `
    <div style="font-family: SF Pro Text, Segoe UI, sans-serif; color: #111827; line-height: 1.6;">
      <p style="margin: 0 0 12px; font-size: 12px; letter-spacing: 0.24em; text-transform: uppercase; color: #2563eb;">GoUsbAi Account</p>
...
export async function sendPlatformEmailAuthCode(
  ...
  const existingUser = await getUserByEmail(env.GOUSB_AI_PLATFORM_DB, email);
  ...
  const db = env.GOUSB_AI_PLATFORM_DB;
  const now = new Date();
...
export async function verifyPlatformEmailAuthCode(
  ...
  await consumeEmailCode(env.GOUSB_AI_PLATFORM_DB, pending.id, new Date().toISOString());
  return { email };
}
