...
export const REMOTE_SESSION_COOKIE = "go-usb-ai_remote_session";
...
function readWebBaseUrl(c: Context<{ Bindings: Env }>): string | null {
  const configured = optionalTrimmedString(c.env.GOUSB_AI_WEB_BASE_URL ?? "");
  if (configured) {
...
export async function resolveRemoteAccessSession(c: Context<{ Bindings: Env }>): Promise<RemoteAccessSessionRow | null> {
  ...
  const cookieSession = token
    ? await getRemoteAccessSessionByToken(c.env.GOUSB_AI_PLATFORM_DB, token)
    : null;
...
> {
  ...
  if (session.source_grant_id) {
    const grant = await getRemoteShareGrantById(c.env.GOUSB_AI_PLATFORM_DB, session.source_grant_id);
