...
export async function openRemoteBrowserRelaySocket(params: {
  ...
  const stub = env.GOUSB_AI_REMOTE_RELAY.get(env.GOUSB_AI_REMOTE_RELAY.idFromName(instanceId));
  const headers = new Headers(rawRequest.headers);
  headers.set("x-go-usb-ai-remote-role", "browser");
  headers.set("x-go-usb-ai-remote-device-id", instanceId);
  headers.set("x-go-usb-ai-remote-client-id", clientId);
  headers.set("x-go-usb-ai-remote-user-id", session.user_id);
  headers.set("x-go-usb-ai-remote-session-id", session.id);
  headers.set("x-go-usb-ai-remote-quota-ticket", quota.data.ticket);
