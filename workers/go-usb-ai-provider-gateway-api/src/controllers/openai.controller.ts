  ...
  if (!runtimeModelSpec && !fallbackModelSpec) {
    return {
      ...
      response: openaiError(
        c,
        400,
        `Model '${requestedModel}' is not available in GoUsbAi catalog.`,
        "model_not_found",
      ),
    };
  }
...
export async function healthHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  return c.json({
    ...
    data: {
      status: "ok",
      service: "go-usb-ai-provider-gateway-api",
      authRequired: true,
...
export async function usageHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  const snapshot = await readBillingSnapshot(c.env.GOUSB_AI_PLATFORM_DB, auth.user.id);
  if (!snapshot) {
    return openaiError(c, 404, "User not found.", "user_not_found");
  }

  return c.json({
    object: "go-usb-ai.usage",
    data: {
...
export async function chatCompletionsHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  const precheckSnapshot = await readBillingSnapshot(c.env.GOUSB_AI_PLATFORM_DB, auth.user.id);
  if (!precheckSnapshot) {
    return openaiError(c, 404, "User not found.", "user_not_found");
  }
...
async function resolveRuntimeModelSpecs(env: Env): Promise<RuntimeModelSpec[]> {
  const dynamic = await listRuntimeModelSpecs(env.GOUSB_AI_PLATFORM_DB);
  if (dynamic.length > 0) {
    return dynamic;
  }
