...
export class GoUsbAiRemoteRelayDurableObject {
  ...
  fetch = async (request: Request): Promise<Response> => {
    ...
    if (request.headers.get("upgrade")?.toLowerCase() === "websocket") {
      const role = request.headers.get("x-go-usb-ai-remote-role")?.trim();
      if (role === "browser") {
  ...
  private handleConnectorUpgrade = async (request: Request): Promise<Response> => {
    const deviceId = request.headers.get("x-go-usb-ai-remote-device-id")?.trim();
    if (!deviceId) {
  ...
  private setDeviceStatus = async (
    ...
    await touchRemoteInstance(this.env.GOUSB_AI_PLATFORM_DB, deviceId, {
      status,
      lastSeenAt: at
    });
  }
