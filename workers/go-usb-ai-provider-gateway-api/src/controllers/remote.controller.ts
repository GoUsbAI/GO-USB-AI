...
function requireRemoteShareUrl(c: Context<{ Bindings: Env }>, grantToken: string): string | Response {
  ...
  if (!shareUrl) {
    return apiError(c, 503, "REMOTE_SHARE_URL_UNAVAILABLE", "GoUsbAi Web base URL is not configured.");
  }
  return shareUrl;
}
...
async function maybeRenderRemoteAccessErrorPage(c: Context<{ Bindings: Env }>, response: Response): Promise<Response> {
  ...
  const webBaseUrl = optionalTrimmedString(c.env.GOUSB_AI_WEB_BASE_URL ?? "");
...
export async function listRemoteShareGrantsHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  const instance = await getRemoteInstanceById(c.env.GOUSB_AI_PLATFORM_DB, instanceId);
  if (!instance || instance.user_id !== auth.user.id) {
    return apiError(c, 404, "INSTANCE_NOT_FOUND", "Remote instance not found.");
  }

  const rows = await listRemoteShareGrantsByInstanceId(c.env.GOUSB_AI_PLATFORM_DB, instanceId);
...
export async function createRemoteShareGrantHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  await createRemoteShareGrant(c.env.GOUSB_AI_PLATFORM_DB, {
    id: grantId,
    token,
...
export async function revokeRemoteShareGrantHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  const grant = await getRemoteShareGrantById(c.env.GOUSB_AI_PLATFORM_DB, grantId);
  if (!grant) {
  ...
  await revokeRemoteShareGrant(c.env.GOUSB_AI_PLATFORM_DB, grant.id, revokedAt);
  await closeRemoteAccessSessionsByGrantId(c.env.GOUSB_AI_PLATFORM_DB, grant.id, revokedAt);
...
export async function openRemoteShareSessionHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  const grant = await getRemoteShareGrantByToken(c.env.GOUSB_AI_PLATFORM_DB, grantToken);
  ...
  const instance = await getRemoteInstanceById(c.env.GOUSB_AI_PLATFORM_DB, grant.instance_id);
...
export async function openRemoteSessionRedirectHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  const session = await getRemoteAccessSessionByToken(c.env.GOUSB_AI_PLATFORM_DB, token);
...
export async function remoteConnectorWebSocketHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  const stub = c.env.GOUSB_AI_REMOTE_RELAY.get(c.env.GOUSB_AI_REMOTE_RELAY.idFromName(instance.id));
  const headers = new Headers(c.req.raw.headers);
  headers.set("x-go-usb-ai-remote-role", "connector");
  headers.set("x-go-usb-ai-remote-device-id", instance.id);
  headers.set("x-go-usb-ai-remote-user-id", auth.user.id);
...
async function resolveValidatedRemoteProxyContext(c: Context<{ Bindings: Env }>) {
  ...
  const instance = await getRemoteInstanceById(c.env.GOUSB_AI_PLATFORM_DB, session.instance_id);
  if (!instance) {
  ...
  if (!Number.isFinite(lastUsedMs) || now - lastUsedMs >= REMOTE_SESSION_TOUCH_THROTTLE_MS) {
    await touchRemoteAccessSession(c.env.GOUSB_AI_PLATFORM_DB, session.id, new Date(now).toISOString());
  }
...
export async function remoteProxyHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  const response = await stub.fetch("https://remote-relay.internal/proxy", {
    ...
    headers: {
      ...
      "x-go-usb-ai-remote-device-id": instance.id
    },
    body: JSON.stringify({
      method: c.req.method,
      path,
