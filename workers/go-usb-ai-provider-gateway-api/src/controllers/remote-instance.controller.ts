...
export async function registerRemoteInstanceHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  const existing = await getRemoteInstanceByInstallId(c.env.GOUSB_AI_PLATFORM_DB, instanceInstallId);
  ...
  await upsertRemoteInstance(c.env.GOUSB_AI_PLATFORM_DB, {
    id: instanceId,
    userId: auth.user.id,
    instanceInstallId,
    displayName,
...
export async function listRemoteInstancesHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  const rows = await listRemoteInstancesByUserId(c.env.GOUSB_AI_PLATFORM_DB, auth.user.id, {
...
export async function openRemoteInstanceHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  await createRemoteAccessSession(c.env.GOUSB_AI_PLATFORM_DB, {
    id: sessionId,
    token,
    userId: auth.user.id,
...
export async function archiveRemoteInstanceHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  await archiveRemoteInstance(c.env.GOUSB_AI_PLATFORM_DB, before.id, archivedAt);
  const after = await getRemoteInstanceById(c.env.GOUSB_AI_PLATFORM_DB, before.id);
  if (!after) {
...
export async function unarchiveRemoteInstanceHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  await unarchiveRemoteInstance(c.env.GOUSB_AI_PLATFORM_DB, before.id, updatedAt);
  const after = await getRemoteInstanceById(c.env.GOUSB_AI_PLATFORM_DB, before.id);
  if (!after) {
...
export async function deleteRemoteInstanceHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  await deleteRemoteAccessSessionsByInstanceId(c.env.GOUSB_AI_PLATFORM_DB, instance.id);
  await deleteRemoteShareGrantsByInstanceId(c.env.GOUSB_AI_PLATFORM_DB, instance.id);
  await deleteRemoteInstanceById(c.env.GOUSB_AI_PLATFORM_DB, instance.id);
