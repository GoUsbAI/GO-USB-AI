...
async function loadPlatformAuthSession(c: Context<{ Bindings: Env }>, sessionId: string) {
  const session = await getPlatformAuthSessionById(c.env.GOUSB_AI_PLATFORM_DB, sessionId);
  if (!session) {
    return null;
  }
  ...
  if (session.status !== "expired") {
    await updatePlatformAuthSessionStatus(c.env.GOUSB_AI_PLATFORM_DB, {
      id: session.id,
      status: "expired",
...
function getBrowserAuthHeading(pageState: "pending" | "authorized" | "expired" | "missing"): {
  ...
  if (pageState === "authorized") {
    return {
      ...
      subtitle: "You can return to GoUsbAi. This browser page can be closed now."
    };
  }
  if (pageState === "expired") {
    return {
      title: "Authorization Expired",
      subtitle: "This authorization session has expired. Return to GoUsbAi and start again."
    };
  }
  if (pageState === "missing") {
    return {
      title: "Authorization Not Found",
      subtitle: "This authorization session is invalid. Return to GoUsbAi and start again."
    };
  }
  return {
    title: "Connect GoUsbAi Device",
    subtitle: "Sign in on this page to authorize the local GoUsbAi device."
  };
}
...
export async function startBrowserAuthHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  await createPlatformAuthSession(c.env.GOUSB_AI_PLATFORM_DB, {
    id: sessionId,
    expiresAt
  });
  return c.json({
...
export async function pollBrowserAuthHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  const user = await getUserById(c.env.GOUSB_AI_PLATFORM_DB, session.user_id);
  if (!user) {
...
export async function authorizeBrowserAuthHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  try {
    ...
    return renderBrowserAuthPage({
      ...
      email,
      pageState: "authorized",
      successMessage: "Authorization completed. You can close this page now. GoUsbAi should finish this device login automatically."
    });
  } catch (error) {
