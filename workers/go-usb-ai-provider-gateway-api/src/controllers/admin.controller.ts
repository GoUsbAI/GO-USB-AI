...
export async function adminOverviewHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  const globalFreeLimitUsd = await readPlatformNumberSetting(c.env.GOUSB_AI_PLATFORM_DB, "global_free_limit_usd", getGlobalFreeLimit(c.env));
  const globalFreeUsedUsd = await readPlatformNumberSetting(c.env.GOUSB_AI_PLATFORM_DB, "global_free_used_usd", 0);
  const userCount = await countUsers(c.env.GOUSB_AI_PLATFORM_DB);
  const pendingRechargeIntents = await countRechargeIntentsByStatus(c.env.GOUSB_AI_PLATFORM_DB, "pending");
...
export async function adminProvidersHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  const rows = await listProviderAccounts(c.env.GOUSB_AI_PLATFORM_DB);
  return c.json({
    ok: true,
    data: {
...
export async function createAdminProviderHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  await createProviderAccount(c.env.GOUSB_AI_PLATFORM_DB, {
    id: providerId,
  ...
  const created = await getProviderAccountById(c.env.GOUSB_AI_PLATFORM_DB, providerId);
  if (!created) {
    return apiError(c, 500, "PROVIDER_CREATE_FAILED", "Provider created but cannot be loaded.");
  }

  await appendAuditLog(c.env.GOUSB_AI_PLATFORM_DB, {
...
export async function patchAdminProviderHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  const before = await getProviderAccountById(c.env.GOUSB_AI_PLATFORM_DB, providerId);
  if (!before) {
  ...
  const changed = await patchProviderAccount(c.env.GOUSB_AI_PLATFORM_DB, providerId, {
  ...
  const after = await getProviderAccountById(c.env.GOUSB_AI_PLATFORM_DB, providerId);
  if (!after) {
...
export async function adminModelsHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  const [models, providers] = await Promise.all([
    listModelCatalog(c.env.GOUSB_AI_PLATFORM_DB),
    listProviderAccounts(c.env.GOUSB_AI_PLATFORM_DB)
  ]);
...
export async function putAdminModelHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  const provider = await getProviderAccountById(c.env.GOUSB_AI_PLATFORM_DB, input.providerAccountId);
  if (!provider) {
    return apiError(c, 404, "PROVIDER_NOT_FOUND", "providerAccountId does not exist.");
  }

  await upsertModelCatalog(c.env.GOUSB_AI_PLATFORM_DB, {
    publicModelId,
  ...
  const current = await listModelCatalog(c.env.GOUSB_AI_PLATFORM_DB);
...
export async function adminProfitOverviewHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  const overview = await readProfitOverview(c.env.GOUSB_AI_PLATFORM_DB, since);
...
export async function adminUsersHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  const [result, totalRow] = await Promise.all([
    c.env.GOUSB_AI_PLATFORM_DB.prepare(sql)
      .bind(...binds, limit + 1)
      .all<UserRow>(),
    c.env.GOUSB_AI_PLATFORM_DB.prepare(`SELECT COUNT(*) AS total FROM users ${filterWhereClause}`)
      .bind(...filterBinds)
...
export async function patchAdminUserHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  const userBefore = await getUserById(c.env.GOUSB_AI_PLATFORM_DB, userId);
  if (!userBefore) {
  ...
  const userAfter = await getUserById(c.env.GOUSB_AI_PLATFORM_DB, userId);
  if (!userAfter) {
...
export async function adminRechargeIntentsHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  const rows = await c.env.GOUSB_AI_PLATFORM_DB.prepare(sql).bind(...binds, limit + 1).all<RechargeIntentRow>();
...
export async function patchAdminSettingsHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  const prevLimit = await readPlatformNumberSetting(c.env.GOUSB_AI_PLATFORM_DB, "global_free_limit_usd", getGlobalFreeLimit(c.env));
  await writePlatformNumberSetting(c.env.GOUSB_AI_PLATFORM_DB, "global_free_limit_usd", nextLimit);
  ...
  const currentUsed = await readPlatformNumberSetting(c.env.GOUSB_AI_PLATFORM_DB, "global_free_used_usd", 0);
  return c.json({
    ok: true,
    data: {
