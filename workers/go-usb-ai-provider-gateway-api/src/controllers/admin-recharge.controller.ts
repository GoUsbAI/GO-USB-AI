...
export async function confirmRechargeIntentHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  const intent = await getRechargeIntentById(c.env.GOUSB_AI_PLATFORM_DB, intentId);
  if (!intent) {
  ...
  const markConfirmed = await c.env.GOUSB_AI_PLATFORM_DB.prepare(
  ...
  const creditUser = await c.env.GOUSB_AI_PLATFORM_DB.prepare(
    `UPDATE users
  ...
  if (!creditUser.success || (creditUser.meta.changes ?? 0) !== 1) {
    await c.env.GOUSB_AI_PLATFORM_DB.prepare(
      `UPDATE recharge_intents
          SET status = 'pending',
              confirmed_at = NULL,
  ...
  try {
    await appendLedger(c.env.GOUSB_AI_PLATFORM_DB, {
...
export async function rejectRechargeIntentHandler(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  const now = new Date().toISOString();
  const markRejected = await c.env.GOUSB_AI_PLATFORM_DB.prepare(
    `UPDATE recharge_intents
        SET status = 'rejected',
            rejected_at = ?,
            rejected_by_user_id = ?,
