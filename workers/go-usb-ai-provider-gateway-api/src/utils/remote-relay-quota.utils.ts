...
export function readRemoteBrowserAttachment(request: Request): ClientAttachment | null {
  const userId = request.headers.get("x-go-usb-ai-remote-user-id")?.trim();
  const sessionId = request.headers.get("x-go-usb-ai-remote-session-id")?.trim();
  const quotaTicket = request.headers.get("x-go-usb-ai-remote-quota-ticket")?.trim();
  const instanceId = request.headers.get("x-go-usb-ai-remote-device-id")?.trim();
  if (!userId || !sessionId || !quotaTicket || !instanceId) {
    return null;
  }
  return {
    type: "client",
    clientId: request.headers.get("x-go-usb-ai-remote-client-id")?.trim() || crypto.randomUUID(),
    userId,
    sessionId,
    instanceId,
    quotaTicket,
