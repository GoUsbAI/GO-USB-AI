...
class GoUsbAiFnosPackageBuilder {
  repoRoot = resolveRepoPath(import.meta.url);
  templateRoot = resolve(this.repoRoot, "apps/fnos-go-usb-ai/pack-template");
  outputRoot = resolve(this.repoRoot, "dist/fnos-go-usb-ai");
  iconSourcePath = resolve(this.repoRoot, "apps/desktop/build/icons/icon.png");
  licenseSourcePath = resolve(this.repoRoot, "LICENSE");
  go-usb-aiPackagePath = resolve(this.repoRoot, "packages/go-usb-ai/package.json");

  build = (rawArgs) => {
    const options = this.parseOptions(rawArgs);
    const version = this.readGoUsbAiVersion();
    const archOutputRoot = resolve(options.outputRoot, options.arch);
    const packageDir = resolve(archOutputRoot, "fnnas.go-usb-ai");
    ...
    this.deployGoUsbAiServer(serverDir);
  ...
  readGoUsbAiVersion = () => {
    const raw = readFileSync(this.go-usb-aiPackagePath, "utf8");
    const parsed = JSON.parse(raw);
    if (!parsed?.version) {
      throw new Error(`Missing version in ${this.go-usb-aiPackagePath}`);
    }
    return parsed.version;
  };

  ensureWorkspaceArtifacts = () => {
    this.run("pnpm", ["-r", "--filter", "go-usb-ai...", "build"]);
  };
  ...
  copyTemplate = ({ packageDir, version, arch, servicePort }) => {
    ...
    const replacements = [
      ["__GOUSB_AI_VERSION__", version],
      ["__FNOS_ARCH__", arch],
      ["__FNOS_PLATFORM__", this.resolvePlatform(arch)],
      ["__GOUSB_AI_SERVICE_PORT__", servicePort]
    ];
    const filesToRender = [
  ...
  buildFpk = (packageDir) => {
    const packageName = "fnnas.go-usb-ai.fpk";
...
new GoUsbAiFnosPackageBuilder().build(process.argv.slice(2));
