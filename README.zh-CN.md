...
<img src="images/marketing/go-usb-ai-omni-assistant-cn.jpg" alt="GoUsbAi — 让你的电脑成为强大的 AI 助手" width="720" />
...
[文档](https://docs.go-usb-ai.io/zh/) · [规划](docs/ROADMAP.md) · [Discord](https://discord.gg/j4Skbgye) · [微信群](images/contact/go-usb-ai-contact-wechat-group.png) · [Issues](https://github.com/Peiiii/go-usb-ai/issues) · [路线图](https://docs.go-usb-ai.io/zh/guide/roadmap)
...
GoUsbAi 在你的机器上运行，把 Agent、技能、CLI 工具、定时自动化与消息应用集成收敛到一个可控制的 AI 助手工作台。

- **一条命令启动** — `go-usb-ai start`，浏览器内配置一切
...
- **微信群** — 扫码加群：
  ...
  <img src="images/contact/go-usb-ai-contact-wechat-group.png" width="180" alt="微信群二维码" />
...
如果部署在云服务器上，GoUsbAi 对外提供的是 `55667` 端口的纯 HTTP 服务。你可以先用 `http://<服务器IP>:55667` 直连验证；如果要走 `80/443` 或 `https://`，请用 Nginx/Caddy 做反向代理和 TLS 终止，不要让 GoUsbAi 直接承担 HTTPS。
...
这会启动 GoUsbAi 的开发态前后端，并在终端打印本地访问地址。
开发态默认直接使用 `~/.go-usb-ai`，也就是和你平时本地使用同一套数据目录；如果需要切换到其他目录，可设置 `GOUSB_AI_HOME=/path/to/home`。

`pnpm dev start` 会主动禁用 runtime update host，避免开发态在发布 manifest 尚不存在时误显示“更新异常”。

> 完整配置指南：[docs.go-usb-ai.io](https://docs.go-usb-ai.io/zh/guide/configuration)
>
> 新手分步教程（含常见问题）：[快速开始文档](https://docs.go-usb-ai.io/zh/guide/getting-started)
...
![Agent 对话](images/screenshots/go-usb-ai-chat-page-cn.png)

**AI 提供商** — 在 UI 中配置与切换提供商：

![AI 提供商](images/screenshots/go-usb-ai-providers-page-cn.png)

**消息渠道** — 启用 Discord、Telegram、飞书、QQ 等：

![消息渠道](images/screenshots/go-usb-ai-channels-page-cn.png)

## 文档

访问 **[docs.go-usb-ai.io](https://docs.go-usb-ai.io/zh/)** 查看完整文档，包括：

- [模型选择](https://docs.go-usb-ai.io/zh/guide/model-selection)
- [命令参考](https://docs.go-usb-ai.io/zh/guide/commands)
- [愿景与路线图](https://docs.go-usb-ai.io/zh/guide/vision)
- [飞书接入教程](https://docs.go-usb-ai.io/zh/guide/tutorials/feishu)
...
GoUsbAi 的诞生离不开以下优秀项目的启发：

- [OpenClaw](https://github.com/openclaw/openclaw) — 全栈 AI 助手平台，启发了 GoUsbAi 的早期探索。
