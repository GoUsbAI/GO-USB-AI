# 2026-02-19 GoUsbAi 渠道插件化对齐 OpenClaw 方案（能力不扩展）

## 背景

当前 GoUsbAi 已进入“渠道由插件注册表驱动”的阶段，但内置渠道仍主要通过 core 内建映射注入，和 OpenClaw 的“每个渠道作为独立插件定义并走 register(api) 注册”仍有差距。
...
1. 在 `@go-usb-ai/openclaw-compat` 内新增 bundled 渠道插件目录：
   - 每个渠道一个插件定义，暴露 `id/name/description/configSchema/register(api)`。
   - `register(api)` 内调用 `api.registerChannel({ plugin: { id, go-usb-ai: { isEnabled, createChannel } } })`。
...
验收标准：
- 插件冲突/保留名校验行为与外部插件一致；
- `GOUSB_AI_ENABLE_OPENCLAW_PLUGINS=0` 时，bundled 渠道插件仍可注册并运行。
...
缓解：
- 先保留现有 `go-usb-ai.createChannel` 运行时钩子，不改渠道行为；
