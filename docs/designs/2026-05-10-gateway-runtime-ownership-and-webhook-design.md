...
本方案用于收敛 GoUsbAi gateway 启动链路的职责分配。
...
`GoUsbAiServiceRuntime` 是长期存在的 CLI/service facade，不应该持有 gateway 运行期状态。

`GoUsbAiGatewayRuntime` 应该表示一次具体 gateway 启动期间的 runtime owner。它不是散参数对象，不是 context bag，也不是一堆已经创建好的组件的容器。

UI server 不参与组装 gateway，也不维护单独的 gateway runtime / host / props 概念。生产链路里只有 `GoUsbAiGatewayRuntime` 这一个 gateway owner。UI server 只在启动瞬间接收 runtime 投影出来的 server 入参：
...
进入 server 的能力必须是确定的，不用 `?`，不用 `| undefined`，不靠调用顺序补字段。server 层如果需要 TypeScript 约束，只能约束 `startUiServer(...)` 的函数参数，不导出 `GoUsbAiGatewayRuntime`、`ServiceGateway`、`ServiceGatewayShell`、`UiServerGateway`、`StartUiServerGateway` 这类独立 gateway 模型。
...
- `packages/go-usb-ai-server/src/ui/server.types.ts` 已删除；
- `startUiServer(gateway)` 已改为接收单个 gateway 参数；
- server 包不再导出 `GoUsbAiGatewayRuntime` / `UiServerGateway` / `StartUiServerGateway` 这类 gateway 模型。
- service 侧已落地 `GoUsbAiGatewayRuntime` 作为 gateway 启动 owner；
...
- `startGatewayUiShell`
- `startUiShell`
- server 侧 `GoUsbAiGatewayRuntime` / `UiServerGateway` / `StartUiServerGateway` 之类换皮类型
- `createGatewayWebhook`
- `createServiceUiHosts`
- `createXxxHost` 薄工厂
- `new X(...).register(Y)` 反向装配链路

如果短期不能一次性删除全部 helper，也应该把它们收为 `GoUsbAiGatewayRuntime` 的内部实现细节，而不是主流程概念。

## Constructor 与 Start 边界

`GoUsbAiGatewayRuntime` 实例本身就是一次 gateway runtime。它不应该再创建或返回另一个 gateway 对象。
...
- 如果一个能力属于 gateway 生命周期，就直接成为 `GoUsbAiGatewayRuntime` 的字段。
...
```ts
class GoUsbAiGatewayRuntime {
  private readonly messageBus = new MessageBus();
  private readonly appEventBus = go-usb-ai.eventBus;
...
保留 shell-first 启动策略，但它应该是 `GoUsbAiGatewayRuntime` 的内部阶段，不应该变成外部 context / props / host 类型。

目标形态：

```txt
new GoUsbAiGatewayRuntime(...)
  -> constructor
     - 读取配置
...
`GoUsbAiGatewayRuntime` 应该自己创建并持有 gateway 启动期组件，但不应该把 constructor 变成所有字段的散装清单。
...
当前代码里的 bus 原本就是 `GoUsbAiCore.MessageBus`，应该按原名叫 `messageBus`，不要 invent 新概念，也不要叫模糊的 `getBus`。
...
当前 `publish` 不是一个独立机制。代码里已经存在 `@go-usb-ai/kernel` 的 `go-usb-ai.eventBus`：

```txt
service/router 调 publish(UiServerEvent)
  -> server publishUiServerEvent()
  -> kernel go-usb-ai.eventBus.emitEnvelope(...)
...
gateway.appEventBus
  ...
  - 当前实现复用 kernel go-usb-ai.eventBus
...
不再引入 `ServiceUiRealtimeEvents` / `UiEventPublisher` 这类新名字。正确做法是把已有 `go-usb-ai.eventBus` 显式作为 `gateway.appEventBus`，并删除 `publish` / `publishUiEvent` 裸函数传递链。
...
如果某个东西是有状态、有行为、有生命周期、或多个方法组成的能力，它应该是 class owner，并由 `GoUsbAiGatewayRuntime` 直接创建。

不要通过重命名掩盖结构问题：

```ts
GoUsbAiGatewayRuntime -> UiServerGateway -> StartUiServerGateway
GoUsbAiGatewayRuntime -> ServiceGateway -> ServiceGatewayShell
```
...
如果这段代码已经在 `GoUsbAiGatewayRuntime` 里，正确做法是让对应字段直接属于 `this`。
...
`UiRouterOptions` 可以继续服务 route/controller 单元测试和局部组合，但生产入口 `startUiServer(gateway)` 最终不应该直接继承 optional router contract。生产 gateway 对象必须来自 `GoUsbAiGatewayRuntime`，并且在进入 server 前已经完成确定性创建。
...
1. 定义 service 侧 `GoUsbAiGatewayRuntime` class 和最小外部依赖。
...
5. 把已有 kernel `go-usb-ai.eventBus` 显式作为 runtime 的 `appEventBus` 字段。
...
- server 层不导出 `GoUsbAiGatewayRuntime` / `UiServerGateway` / `StartUiServerGateway` 这类 gateway 换皮类型。
- `startUiServer(gateway)` 只接收单个 gateway 参数，不再接收 `{ host, port, staticDir, gateway }`。
- `packages/go-usb-ai-server/src/ui/server.types.ts` 不存在。

最终验收：

- `RuntimeCommandService.startGateway()` 不再手动创建 webhook、message bus、UI hosts 等 gateway 内部组件。
- service 侧存在真正的 `GoUsbAiGatewayRuntime` owner，由它创建并持有 gateway 启动期组件。
...
- gateway 显式持有 `appEventBus`，并复用已有 kernel `go-usb-ai.eventBus`。
- 不再有 `publish` / `publishUiEvent` 作为 gateway 内部裸函数传递链。
- `GoUsbAiGatewayRuntime` 不含 optional capability 字段。
...
- `GoUsbAiGatewayRuntime` 不再直接持有 `configPath`、`config`、`workspace`、`remoteModule`、`remoteAccess`、`marketplace`、`deferredChannelStarter` 这些散字段；它们分别归属到上面的 manager。
...
- `rg "StartUiServerGateway|UiServerGateway|server\\.types" packages/go-usb-ai-server packages/go-usb-ai-service`
- `rg "ServiceGateway|ServiceGatewayShell|createGateway\\(|createShellGateway\\(|initializeGateway|initGateway" packages/go-usb-ai-service`
- `rg "const gateway = \\{|Parameters<typeof startUiServer>|createGatewayWebhook|getBus|\\.register\\(webhook|WebhookDispatchService|WebhookController|ExtensionChannelWebhookService" packages/go-usb-ai-service packages/go-usb-ai-server`
- `rg "ServiceUiRealtimeEvents|publishUiEvent|publish: \\(event: UiServerEvent\\)" packages/go-usb-ai-server packages/go-usb-ai-service`
- `rg "startUiServer\\(" packages/go-usb-ai-server packages/go-usb-ai-service`
