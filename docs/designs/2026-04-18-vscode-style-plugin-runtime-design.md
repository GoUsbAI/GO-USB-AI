...
- GoUsbAi 的插件机制，到底应该更像“声明式清单”，还是更像“真正的运行时程序”
- 为什么当前 `channels.*` 变更会错误地牵连 `reload plugin registry`
- 如果以 VS Code 的 extension model 为母版，GoUsbAi 应该怎样设计一套更强、更灵活、边界更清晰的插件运行时
...
1. GoUsbAi 的插件不应该以“声明式注册表快照”为主，而应该以“运行时激活与动态注册”为主。
...
4. 但 GoUsbAi 不应机械照搬 VS Code 的所有表层接口；应尽量保持 VS Code 的机制与风格，只在 GoUsbAi 存在真实结构差异时做最小偏离。
...
这套机制必须服务 GoUsbAi 的长期方向：

- 插件应增强 GoUsbAi 作为统一入口的能力，而不是制造新的分裂入口
...
这几条里，GoUsbAi 最应该抄的是“机制”，不是“字面字段”。
...
GoUsbAi 现在把 3 过早提前到了“加载插件时”，所以才会被迫频繁 reload。
...
凡是 VS Code 已有成熟模型的地方，GoUsbAi 默认照抄：
...
### 原则 2：只有在 GoUsbAi 有真实结构差异时才扩展

GoUsbAi 与 VS Code 的关键差异不在“插件哲学”，而在运行对象上：

- GoUsbAi 有 `channel`
- GoUsbAi 有 `ncp agent runtime`
- GoUsbAi 有长生命周期网关、轮询器、webhook listener、reply dispatcher
...
```json
{
  "id": "go-usb-ai-channel-feishu",
  "version": "0.1.0",
  "main": "./dist/index.js",
  "displayName": "GoUsbAi Feishu",
  "description": "Feishu channel and tool integration for GoUsbAi",
  "contributes": {
    "configuration": {
      "title": "Feishu",
      "properties": {
        "go-usb-ai.channels.feishu": {}
      }
    }
  }
}
...
```ts
export function activate(context: GoUsbAiExtensionContext): Disposable | void;
...
## 三. 上下文层：`GoUsbAiExtensionContext`

默认形态尽量贴近 VS Code：

```ts
type GoUsbAiExtensionContext = {
...
## 四. 运行时宿主层：`GoUsbAiRuntimeHost`

这是 GoUsbAi 相比 VS Code 必须新增的部分，但它仍然遵守 VS Code 的 register/dispose 习惯。

```ts
type GoUsbAiRuntimeHost = {
...
```ts
export function activate(context: GoUsbAiExtensionContext): void {
  ...
  const reconcile = () => {
    const config = context.workspace.getConfiguration("go-usb-ai.channels.feishu");
  ...
  rootStore.add(
    context.workspace.onDidChangeConfiguration((event) => {
      if (event.affectsConfiguration("go-usb-ai.channels.feishu")) {
        reconcile();
      }
    }),
  );
...
GoUsbAi 的配置路径建议统一采用类似：

- `go-usb-ai.channels.feishu`
- `go-usb-ai.channels.weixin`
- `go-usb-ai.plugins.<pluginId>`
- `go-usb-ai.ncp.runtimes.<runtimeId>`
...
### 2. 更符合 GoUsbAi 的真实对象模型

GoUsbAi 不是编辑器，但它同样需要：
...
- `GoUsbAiExtensionContext` 精确定义
- `GoUsbAiRuntimeHost` 精确定义
