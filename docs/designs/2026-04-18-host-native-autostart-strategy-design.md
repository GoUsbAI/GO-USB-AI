# GoUsbAi Host-Native Autostart Strategy Design
...
GoUsbAi 想成为 AI 时代的个人操作层，就不能只在“用户手动点开它时”才勉强存在。对很多真实场景来说，用户真正关心的是：

- 机器重启后，GoUsbAi 能不能自己回来
- 我关掉窗口后，它是不是还在后台活着
...
2. CLI / npm 链路
   - 文档已经暴露 `go-usb-ai service install-systemd` / `uninstall-systemd`，说明 Linux 方向已经有“受管服务自启动”的产品语义。
...
本文的目标，就是把这些语义一次性拆开，并给出面向 GoUsbAi 的最小 PRD + 命令设计草案。
...
5. 可预期性不够
   - 用户可能误以为 `npm install -g go-usb-ai` 本身就会改系统启动项。
...
- 让 GoUsbAi 作为统一入口更可靠，而不是每次重启机器后都要重新手动拉起。
...
**GoUsbAi 可以编排和暴露“自启动能力”，但不应该掩盖宿主平台的真实机制。**
...
GoUsbAi 的自启动能力必须采用“宿主原生、自愿启用、显式权限、可卸载、可观测”的路线，而不是“安装即偷偷改系统启动项”的路线。
...
这三条优先于 macOS LaunchDaemon / Windows Service，因为它们更符合“个人操作层”的默认场景：用户登录后可用，而不是先把 GoUsbAi 做成企业后台守护进程。
...
- 不在没有硬件/网络前提的情况下对外承诺“GoUsbAi 能开机一台已关机机器”
...
go-usb-ai service install-systemd --system
go-usb-ai service uninstall-systemd --user
go-usb-ai service uninstall-systemd --system
...
```bash
go-usb-ai service install-launch-daemon
go-usb-ai service uninstall-launch-daemon
```
...
```powershell
go-usb-ai service install-windows-service
go-usb-ai service uninstall-windows-service
```
...
```bash
go-usb-ai service install-autostart
```
...
```bash
go-usb-ai service install-autostart --mode user
```
...
```text
Preparing to install GoUsbAi autostart:
- host owner: systemd user service
- unit: go-usb-ai.service
- command: /usr/bin/env go-usb-ai start --open false
- logs: journalctl --user -u go-usb-ai.service
...
```json
{
  ...
  "command": "go-usb-ai start --open false",
  "logHint": "journalctl --user -u go-usb-ai.service -f",
...
- owner 是否存在
- owner 是否 enabled
- 指向的 Node / go-usb-ai 命令是否仍可解析
- `GOUSB_AI_HOME` / 工作目录是否稳定
- 日志目标是否可访问
- 启动失败的最后错误提示
...
对 GoUsbAi 来说，最合适的路线不是“再发明一套自启动框架”，而是：
...
这是最符合 GoUsbAi 产品愿景、权限边界、平台现实和可维护性的方案。
