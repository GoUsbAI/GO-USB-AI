...
GoUsbAi 的长期目标是成为 AI 时代的个人操作层。会话系统是“自感知连续性”的核心基础设施：它保存用户意图、执行上下文、运行结果、工具调用和系统对当前状态的认识。
...
位置：`packages/go-usb-ai-core/src/features/session/managers/session.manager.ts`
...
位置：待新增，建议放在 `packages/go-usb-ai-kernel/src/managers/ncp-session.manager.ts`。
...
位置：`packages/go-usb-ai-kernel/src/managers/session-run.manager.ts`
...
位置：`packages/go-usb-ai-kernel/src/stores/ncp-agent-session-journal.store.ts`
...
位置：`packages/go-usb-ai-kernel/src/services/ncp-agent-session-store-adapter.service.ts`
...
位置：`packages/go-usb-ai-kernel/src/stores/ncp-agent-legacy-session.store.ts`
...
位置：`packages/go-usb-ai-kernel/src/managers/agent-run-request.manager.ts`
...
位置：`packages/go-usb-ai-core/src/features/session-request/managers/session-request.manager.ts`
...
位置：`packages/go-usb-ai-kernel/src/services/ncp-session-api.service.ts`
...
- `packages/go-usb-ai-kernel/src/contributions/session-context-window/index.ts`
...
- `rg "publisher|EventPublisher" packages/go-usb-ai-kernel/src/managers/session-run.manager.ts packages/go-usb-ai-kernel/src/utils/session-run.utils.ts` 无命中。
...
#### `NativeAgentRuntimeFactory` / `GoUsbAiNcpContextBuilder`
...
- 当上述迁移完成后，core `SessionManager` 可以从 GoUsbAi NCP runtime 中完全移除；若仍有 plugin compat 需要，可移到 compat 包或测试 fixture，而不是留在主 kernel 对象图里。
...
- `GoUsbAiKernel.sessionRequests`：构造时不再传 `sessions: this.sessions`，也不把 legacy `SessionManager` 作为 request manager 的事实源。
...
- request 状态事件 contract：`session.request.accepted/completed/failed` 必须作为 NCP journal 中的 session 历史事实写入，不能退回 UI-only tool result 或 legacy event store。阶段 2 可先使用 journal event string contract；若后续要提升为 `@go-usb-ai/ncp` 公共事件枚举，需要单独按 types 文件命名治理迁移。
...
- `rg "SessionManager" packages/go-usb-ai-kernel/src/tools packages/go-usb-ai-kernel/src/contributions/tool-contribution packages/go-usb-ai-kernel/src/contributions/learning-loop` 不再显示 NCP 工具主链路依赖。
- `rg "appendEvent\\(|save\\(" packages/go-usb-ai-core/src/features/session-request packages/go-usb-ai-kernel/src` 不再显示 request 状态写 legacy session 的生产调用。
...
这一组处理运行时主链路里更深的一层职责越界。`ContextBuilder` 不应该管理 session，它的合理职责只是“为本次模型调用组装上下文”。context compaction 则不是一个零散 helper，而是一个完整 feature：它应该有自己的 manager 内聚“预算评估、checkpoint 生命周期、摘要生成、timeline message、context window projection”这整件事。阶段 2 前的问题是 `NativeAgentRuntimeFactory` 把 `kernel.sessions` 传给 `GoUsbAiNcpContextBuilder` 和旧 `ContextCompactionPreflightService`，导致只读 builder 和 compaction feature 都拿到了 legacy `SessionManager`。

当前代码实情：

- `GoUsbAiNcpContextBuilder` 调用 `SessionManager.getOrCreate(input.sessionId)` 获取一个可变 legacy `Session` 对象。
...
- `GoUsbAiNcpContextBuilder` 从调用方传入的 immutable session snapshot 或 `NcpSessionManager.getSessionRecord()` 读取 metadata 与 messages，不再 `getOrCreate()` legacy session。
...
- `syncSessionThinkingPreference` 不能再原地修改 legacy session；改为返回 metadata patch，或由 `GoUsbAiNcpContextBuilder` 调 `NcpSessionManager.updateSessionMetadata()`。
...
- `GoUsbAiNcpContextBuilderOptions.sessionManager`。
...
- `rg "SessionManager|getOrCreate\\(|\\.save\\(" packages/go-usb-ai-kernel/src/features/native-runtime packages/go-usb-ai-kernel/src/features/context-compaction` 不再显示 runtime context / compaction 的 legacy session 读写。
...
- `go-usb-ai-ncp-dispatch.utils.ts`：direct prompt NCP dispatch 不应继续要求 legacy `SessionManager` 作为上下文依赖。
...
- `GoUsbAiKernel` 只把 `NcpSessionManager` 暴露给 `AgentRunRequestManager`、`SessionRunManager` 与 HTTP controller。
...
- `rg "ncpSessionApi|NcpSessionApiService" packages/go-usb-ai-kernel packages/go-usb-ai-server packages/go-usb-ai-service` 无生产引用。
- `rg "updateSession\\(" packages/go-usb-ai-kernel packages/go-usb-ai-server` 不再出现绕过 `NcpSessionManager` 的生产调用。
