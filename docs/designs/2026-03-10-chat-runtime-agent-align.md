...
`go-usb-ai-ui` 当前 chat 前端核心逻辑已经明显偏重，复杂度主要集中在以下几个方面：
...
- `@go-usb-ai/agent-chat` 当前更多被当作“消息事件容器”使用，而不是被真正当作通用 agent abstraction + runtime core 使用。
...
本次目标不是继续在 `go-usb-ai-ui` 上叠局部 manager，而是两层同时收敛：

1. 让 `@go-usb-ai/agent-chat` 更通用，能支持 nextbot 这种“后端流式 + 宿主控制 session/run/meta”的场景。
2. 让 `go-usb-ai-ui` 的 chat runtime 对齐 `agent-kit` 的思路，只保留宿主特有的 adapter 逻辑。

补充原则（强约束）：

- 优先把能力沉到 `@go-usb-ai/agent-chat` 的通用抽象（event/type/controller/agent interface），而不是堆在业务 hook。
- 当 nextbot 场景发现通用层不支持时，默认动作是“先完善 `agent-chat`”，再做业务侧接入。
- 业务侧 (`go-usb-ai-ui`) 仅保留不可通用的宿主职责（路由、查询同步、后端 schema 投影）。
...
- `go-usb-ai-ui` 的宿主 adapter。

## 实施方向

### A. 完善 `@go-usb-ai/agent-chat`
...
- `@go-usb-ai/agent-chat` 能够自然承接 nextbot 的后端 SSE agent；
- 后续其他场景也能复用，而不是只为 nextbot 私有定制。

### B. 在 `go-usb-ai-ui` 中收敛 runtime adapter
...
- `packages/go-usb-ai-agent-chat`
- `packages/go-usb-ai-ui/src/components/chat/useChatRuntimeController.ts`
- `packages/go-usb-ai-ui/src/components/chat/chat-stream/*`
...
3. `@go-usb-ai/agent-chat` 的抽象比当前更适合“后端流式 agent”场景。
4. `go-usb-ai-ui` 仍保持当前页面壳层行为，session list 与 UI 外围不被大范围牵动。
...
- `@go-usb-ai/agent-chat` 已补 run 生命周期通用能力：
