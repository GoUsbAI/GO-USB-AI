...
GoUsbAi 的自管理能力依赖一个稳定命令面：AI 在执行命令时按 `go-usb-ai status --json`、`go-usb-ai doctor --json`、`go-usb-ai config ...`、`go-usb-ai agents ...`、`go-usb-ai cron ...` 操作产品自身。

这个合同在 NPM 安装形态下天然成立，因为 `go-usb-ai` 来自 NPM package `bin`。但桌面安装形态下，用户只获得 Electron 桌面应用和桌面托管 runtime，并不天然获得 shell 里的 `go-usb-ai` 命令。

这会造成一个产品级断裂：桌面版明明承载了 GoUsbAi 的本地 runtime，AI 自己的 command tool 却可能找不到 `go-usb-ai`，从而无法无感使用自管理命令。
...
> 任意能承载 GoUsbAi AI command tool 的安装形态，都必须向该 command tool 暴露同名、同语义的 `go-usb-ai` 命令。
...
go-usb-ai doctor --json
go-usb-ai config get providers --json
go-usb-ai agents list --json
go-usb-ai cron list --json
```

第一阶段优先保障 GoUsbAi 自己启动的 AI command tool。人类在系统终端里能否直接 `go-usb-ai ...` 是第二阶段体验增强，不作为第一阶段阻塞条件。
...
- `开箱即用`：桌面用户不应该为了让 AI 管理 GoUsbAi 自己，再学习 Node / NPM / PATH 配置。

如果桌面版不能提供同一命令面，GoUsbAi 就会出现“UI 能跑，但 AI 自治链路不完整”的硬伤。

## 4. 当前证据

现有 NPM CLI 入口：

- `packages/go-usb-ai/package.json` 暴露 `bin.go-usb-ai = dist/cli/launcher/index.js`。
- `packages/go-usb-ai/src/cli/app/index.ts` 注册 `status`、`doctor`、`config`、`agents`、`cron` 等自管理命令。
...
- `apps/desktop/scripts/update/services/build-product-bundle.service.mjs` 已要求 runtime bundle 包含 `runtime/dist/cli/app/index.js` 与 `go-usb-ai-self-manage/SKILL.md`。

现有 AI command tool 入口：

- `packages/go-usb-ai-core/src/features/agent/tools/shell.tools.ts` 通过 `createExternalCommandEnv(process.env, {}, { cwd })` 构造命令环境。
- `createExternalCommandEnv` 已集中处理 PATH 增强，这适合作为统一注入点。

结论：现有系统已经具备大部分底层能力，缺的是桌面安装形态对 `go-usb-ai` 命令面的产品化暴露与验证合同。

## 5. 设计原则

命中的经典设计原则：

- `single-domain-owner`：同一安装形态下，`go-usb-ai` 命令面只能有一个事实 owner。
...
**安装形态负责提供 `go-usb-ai` 命令面，AI command tool 只消费统一命令名。**

## 6. 推荐方案

推荐采用“桌面托管命令面”的方案。

桌面 launcher 在启动时生成一个受 GoUsbAi 管理的命令目录，例如：
...
```text
<desktopDataDir>/command-surface/go-usb-ai-command-surface.json
```
...
`go-usb-ai` / `go-usb-ai.cmd` 本身只做一件事：调用当前桌面 app binary，让 Electron bundled Node 执行桌面命令桥：
...
桌面命令桥再动态解析当前 active bundle runtime，并把原始参数转交给真正的 GoUsbAi CLI app：

```text
AI command tool
  -> PATH 命中 command-surface/bin/go-usb-ai
  -> desktop app binary with ELECTRON_RUN_AS_NODE=1
  -> desktop command bridge
  -> current bundle runtime/dist/cli/app/index.js
  -> go-usb-ai status/config/agents/cron...
```
...
- 设置 `GOUSB_AI_HOME` / desktop data env。
...
```text
GOUSB_AI_COMMAND_SURFACE_BIN=<desktopDataDir>/command-surface/bin
```
...
createExternalCommandEnv(...)
  -> 如果 GOUSB_AI_COMMAND_SURFACE_BIN 存在且是目录
  -> 将它 prepend 到 PATH
```
...
最终会命中桌面托管的 shim。NPM 安装形态下没有 `GOUSB_AI_COMMAND_SURFACE_BIN` 时，仍然沿用系统 PATH 里的 NPM `go-usb-ai`。
...
- 设置 POSIX shim 可执行权限。
- 返回 runtime env patch：`GOUSB_AI_COMMAND_SURFACE_BIN=<binDir>`。
- 记录当前命令面状态用于日志和诊断。
...
```text
packages/go-usb-ai-core/src/shared/lib/core-utils/utils/child-process-env.utils.ts
```

增加一个通用 env key：

```text
GOUSB_AI_COMMAND_SURFACE_BIN
```
...
<desktopDataDir>/command-surface/
  go-usb-ai-command-surface.json
  bin/
    go-usb-ai
    go-usb-ai.cmd
```
...
1. `GOUSB_AI_COMMAND_SURFACE_BIN` 指向的托管命令面。
2. 当前 Node bin / local `node_modules/.bin`。
3. 用户原始 PATH。
4. 常见 POSIX bin 目录。

这保证桌面 runtime 内的 AI 优先使用当前桌面安装形态的 `go-usb-ai`，而不是误命中用户机器上旧的全局 NPM CLI。

人类系统终端第一阶段不修改 PATH，所以如果用户终端里已有 NPM `go-usb-ai`，仍按用户自己的 PATH 解析。后续可以在 UI 中提供“复制 PATH 配置”或“安装 shell command”动作，但不作为本阶段要求。

## 12. 更新语义

桌面安装形态下：

- `go-usb-ai --version` 返回当前 active bundle runtime 版本。
- `go-usb-ai status --json` 返回当前桌面 runtime 使用的同一 `GOUSB_AI_HOME` 状态。
- `go-usb-ai update` 不应走 NPM 更新语义；应返回桌面安装形态下的明确语义，或委托桌面 update channel。
...
关键原则：不要让桌面 shim 调到一份 NPM launcher，也不要让 `go-usb-ai update` 在桌面版里悄悄改 NPM global package。
...
manifest: <path>
recovery: open GoUsbAi Desktop once, or run desktop package repair/update
```
...
`go-usb-ai doctor --json` 后续也应能暴露 command surface 状态。
...
- `createExternalCommandEnv`：
  - `GOUSB_AI_COMMAND_SURFACE_BIN` 存在时 prepend 到 PATH。
  - 该目录不存在时不污染 PATH。
...
```bash
PATH="<command-surface-bin>:$PATH" go-usb-ai --version
PATH="<command-surface-bin>:$PATH" go-usb-ai status --json
PATH="<command-surface-bin>:$PATH" go-usb-ai doctor --json
```

对于 macOS / Linux，需要验证不依赖系统 Node：

```bash
env -u NODE -u npm_config_user_agent PATH="<command-surface-bin>:/usr/bin:/bin" go-usb-ai --version
```
...
```bash
command -v go-usb-ai || where go-usb-ai
go-usb-ai --version
go-usb-ai status --json
go-usb-ai doctor --json
```
...
- 命中的 `go-usb-ai` 位于 desktop command surface bin 目录。
- `status --json` 指向当前桌面 runtime home。
- 不要求系统全局安装 Node。
- 不要求系统全局安装 NPM `go-usb-ai`。
...
4. 将 `GOUSB_AI_COMMAND_SURFACE_BIN` 注入 desktop runtime env。
...
1. 扩展 `createExternalCommandEnv` 支持 `GOUSB_AI_COMMAND_SURFACE_BIN`。
2. 更新 `ExecTool` 相关测试，固定 `go-usb-ai ...` 命令优先命中托管 bin。
3. 确保 NPM 安装形态不受影响。
...
3. 更新 `docs/USAGE.md` 与 `packages/go-usb-ai/resources/USAGE.md`，说明桌面 AI command tool 内置 `go-usb-ai` 命令面；如触达自管理命令语义，运行 `packages/go-usb-ai/scripts/sync-usage-resource.mjs`。
4. 视变更范围更新 docs/logs 迭代记录。

## 16. 不推荐方案

### 16.1 要求桌面用户额外 `npm i -g go-usb-ai`
...
这会把安装形态细节泄漏到工具层，破坏 `go-usb-ai` 统一命令合同。后续 Docker、portable、系统包安装也会继续长出分支。
...
1. `go-usb-ai update` 在桌面形态下第一阶段是返回明确提示，还是直接接入桌面 update check？
...
- AI command tool 继续执行稳定的 `go-usb-ai ...`。
