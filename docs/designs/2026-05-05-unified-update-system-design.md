# GoUsbAi Unified Update System Design

## 1. 目标

本文定义 GoUsbAi 的统一更新设计，覆盖桌面版、npm 安装版，以及未来其它 host 形态。
...
- `packages/go-usb-ai-ui/src/features/system-status/components/desktop-update-config.tsx`
  - 已有更新设置 UI：检查、下载、应用、通道、自动检查、自动后台下载。
- `packages/go-usb-ai/src/cli/shared/services/update/self-update.service.ts`
  - 目前 npm CLI 更新仍是旧式全局包自更新：通过 `npm view` 检查，再执行 `npm i -g go-usb-ai`。
...
GoUsbAi 更新系统会长期承载桌面、npm、未来云端/移动/企业 host 等多种分发形态。
...
结合当前仓库，第一推荐 owner 是 `@go-usb-ai/kernel`。

理由：

- `@go-usb-ai/kernel` 已经定位为 GoUsbAi 产品 kernel skeleton，承载 agents、tasks、sessions、context、tools、skills、providers、automation、channels 等产品级类型与 manager。
- update contract 属于 GoUsbAi 产品操作层的基础生命周期能力，不是桌面专属，也不是 npm 专属。
- `@go-usb-ai/kernel` 当前比 `@go-usb-ai/core` 更适合作为纯产品域 contract owner；`@go-usb-ai/core` 已经包含配置、provider、channel、agent 执行等大量运行时业务。
- `@go-usb-ai/runtime` 更像 runtime assembly，不适合承载所有 host 都要遵守的协议源头。
...
如果实现阶段发现 `@go-usb-ai/kernel` 引入方向会导致循环依赖，或者桌面 launcher 不适合依赖整个 kernel 包，再考虑两种收敛方案：

```text
方案 A：调整 @go-usb-ai/kernel 导出边界，让纯 types/contract 可轻量引用。
方案 B：新增极小 @go-usb-ai/update-contract 包，只承载不可再归属的纯 contract。
```

新增包必须满足以下条件才允许：

- desktop launcher、npm launcher、UI、release scripts 都需要同一 contract。
- 放入 `@go-usb-ai/kernel` 会形成无法接受的依赖环或发布耦合。
...
- 未评估现有 owner 就新增 `packages/go-usb-ai-update-contracts`。
...
优先使用 GoUsbAi update manifest，而不是让每种安装形态自己猜版本。
...
```json
{
  ...
  "bundleUrl": "https://example.com/go-usb-ai-0.4.3.bundle.zip",
  "bundleSha256": "...",
  "bundleSignature": "...",
...
`npm view go-usb-ai version` 不应该作为产品内核更新的主检测路径。
...
- `@go-usb-ai/kernel`：产品内核 contract 与系统生命周期 owner 的长期归属。
- `@go-usb-ai/core`：foundation primitives，例如 config、session、provider、channel、agent、bus。
- `@go-usb-ai/runtime`：官方 runtime distribution / assembly，回答“官方默认装配哪些能力”。
- `go-usb-ai`：npm 用户入口包，长期应成为 npm launcher，而不是唯一可变产品本体。
- `apps/desktop`：Electron desktop host / launcher。
- `@go-usb-ai/ui`：展示层，消费 update snapshot，不定义更新协议。
...
npm 本身不提供“下载但不安装、点击后切版本”的能力。这个能力来自 GoUsbAi 自己的 npm launcher 和 runtime-bundles 目录。
...
其中 `runtime/` 是部署后的 `go-usb-ai` 运行体，`ui/` 是构建后的 UI 静态资源，`plugins/` 当前只是空占位目录。

重要边界：

- `@go-usb-ai/runtime` 是源码包。
- Product Runtime Bundle 是构建产物。
- `runtime-bundles/versions/<version>` 是用户机器上的已安装发布产物目录。
- 不应把 `runtime-bundles/versions` 做进 `@go-usb-ai/runtime` 源码包内部。
...
- 桌面 runtime home 通过 `GOUSB_AI_HOME` 指向用户数据目录，默认是 `~/.go-usb-ai`。
- 桌面 bundle 数据目录通过 Electron `userData` 管理，用于 `versions/`、`staging/`、`current.json`、`previous.json` 和 launcher state。
- 用户插件发现主要基于配置与 workspace，例如 `workspace/.go-usb-ai/extensions`，不属于 desktop bundle `versions/<version>/plugins`。

统一原则：

- bundle 可以包含随产品发布的内置资产或占位目录。
- bundle 不得包含或接管用户安装态。
- prune 只能清理 host 管理的 `versions/` 和 `staging/`，不得触碰 `GOUSB_AI_HOME`、workspace、用户插件目录或用户数据。
...
这里是 npm host 的建议位置，不是当前既成事实。实现前必须先验证它不会和现有 `~/.go-usb-ai/config.json`、`workspace/`、`sessions/`、`skills/`、用户插件目录发生冲突。
...
`npm install -g go-usb-ai@latest` 不属于 Product Runtime Bundle 更新主链路，也不需要被设计成一个独立 update adapter。
...
blocked
  ...
  recoveryCommand = "npm install -g go-usb-ai@latest"
```
...
- npm host 是全局 `go-usb-ai` launcher，当前尚未完成 runtime bundle 化，需要独立设计启动 shim。
- 桌面 host 的数据目录是 Electron `userData`；npm host 应在 `~/.go-usb-ai` 下选择不会污染现有用户数据的子目录。
...
- runtime bundle 存在 `~/.go-usb-ai/runtime-bundles/versions` 或等价隔离子目录。
...
packages/go-usb-ai/src/cli/launcher/
...
- 负责 `~/.go-usb-ai/runtime-bundles` 或等价目录布局。
...
- 把用户机器上的 `runtime-bundles/versions` 放进 `@go-usb-ai/runtime` 源码包。
...
```text
GOUSB_AI_DESKTOP_RUNTIME_SCRIPT=../../packages/go-usb-ai/dist/cli/app/index.js
```
...
npm launcher bundle resolution 只适用于真正的安装态 `go-usb-ai` 命令。

以下场景应视为开发态，默认直接运行源码或构建产物：

```bash
pnpm -C packages/go-usb-ai dev
pnpm -C packages/go-usb-ai start
tsx packages/go-usb-ai/src/cli/app/index.ts
```

只有从 npm global package 的 bin 入口启动时，才默认走：

```text
npm launcher -> ~/.go-usb-ai/launcher/runtime-bundles/current
```
...
```bash
GOUSB_AI_UPDATE_MANIFEST_URL=http://127.0.0.1:xxxx/manifest.json \
go-usb-ai update:dev-smoke
```
...
- smoke 必须使用隔离 `GOUSB_AI_HOME` 或等价临时目录，不能把测试数据写入真实用户 home。
...
5. 在 `@go-usb-ai/kernel` 增加 update contract/types，先不强行搬迁所有桌面实现。
...
2. `~/.go-usb-ai/launcher/runtime-bundles` 或等价隔离目录布局。
...
- 更新系统不覆盖 `~/.go-usb-ai/config.json`、workspace、sessions、skills、用户插件目录。
...
这条路线符合 GoUsbAi 的长期愿景：让系统具备自感知和自治能力，同时保持用户可控、可回滚、可治理。
