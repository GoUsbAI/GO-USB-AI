...
- [GoUsbAi UI API 设计（Phase 1）](./2026-02-12-ui-gateway-api.md)

## 1. 背景

这份文档沉淀本次关于 “GoUsbAi 是否应该内建一个同时支持 Web 与 Desktop 的浏览器面板，以及该能力应该如何分层” 的讨论结果。

讨论起点不是抽象地争论 `iframe`、`webview` 或 Electron 哪个名词更先进，而是几个很具体的产品判断：

- 当前 `GoUsbAi` 已经有一个右侧内嵌文档浏览器原型：
  - UI 入口与布局位于 `packages/go-usb-ai-ui/src/components/layout/AppLayout.tsx`
  - 状态与多标签逻辑位于 `packages/go-usb-ai-ui/src/components/doc-browser/DocBrowserContext.tsx`
  - 实际内容承载位于 `packages/go-usb-ai-ui/src/components/doc-browser/DocBrowser.tsx`
...
- 未来目标不止是文档浏览，而是：
  ...
  - 能逐步具备真正浏览器能力
  - 能被 AI / Agent 控制
  - 同时不能破坏 `GoUsbAi` 现有 “同一套 UI 同时支持 Web 与 Desktop” 的产品形态

这意味着我们需要回答的核心问题不是 “要不要再做一个 iframe”，而是：

**在保持统一产品壳层的前提下，GoUsbAi 应该如何引入一个跨 Web/Desktop 的浏览器面板子系统。**

## 2. 上位目标对齐

结合 [VISION.md](../VISION.md)，这项能力必须服务以下长期方向：

- 让 `GoUsbAi` 更像统一入口，而不是把用户频繁赶回传统浏览器再手工完成任务。
- 让 “意图 -> 浏览网页 -> 阅读/操作/协作” 这条链路尽量在 `GoUsbAi` 内闭环。
- 让桌面端与 Web 端共享同一套产品认知、交互习惯与操作入口，而不是长出两套完全不同的产品。
- 让未来 AI 对网页的读取、操作、自动化与结果回传，有清晰、可治理、可扩展的宿主边界。

这里追求的不是 “再塞一个功能点”，而是让 `GoUsbAi` 朝“默认入口 + 能力编排 + 自治操作层”再推进一层。
...
纯浏览器环境下，`GoUsbAi Web` 无法获得与 Electron 本地宿主等价的网页控制能力。
...
- Electron 主窗口加载本地 `GoUsbAi UI`
- `packages/go-usb-ai-ui` 继续作为统一 renderer 前端
...
当前桌面端已经通过 `preload` 在 `window.go-usb-aiDesktop` 上向 UI 暴露能力，例如：
...
- `packages/go-usb-ai-ui/src/desktop/managers/*`

因此浏览器面板不应重新发明另一套跨层模式，而应沿用同样的桥接思路，例如新增：

- `window.go-usb-aiBrowser`
- 或纳入 `window.go-usb-aiDesktop.browser`
...
- `packages/go-usb-ai-ui` 保持统一产品壳层
- 宿主能力由平台实现注入
...
禁止让 `packages/go-usb-ai-ui` 直接知道：
...
```text
┌────────────────────────────────────────────────────────────┐
│ Shared Product Shell (packages/go-usb-ai-ui)               │
│ - 右侧栏 UI                                                │
│ - tabs / 地址栏 / 工具栏 / AI 入口                         │
│ - 页面布局与面板状态                                        │
└────────────────────────────────────────────────────────────┘
...
- `packages/go-usb-ai-ui` 中增加 browser manager / store
...
这部分必须明确，否则团队很容易误以为 “既然 `GoUsbAi UI` 是网页，为什么还能把另一个网页塞进右侧栏，而且还不是普通 DOM 组件”。

正确理解方式是：

- `GoUsbAi UI` 本身是一张运行在 Electron 主窗口里的网页
- 这个网页继续负责画：
  - 整个应用壳层
  - 左侧导航
  - 主工作区
...
如果未来希望 `GoUsbAi Web` 也具备强浏览器能力，不能只靠浏览器页面自身，需要引入远程浏览器 runtime。

这条路线本质上是：

- 浏览器真实运行在远程容器 / 服务端宿主
- `GoUsbAi Web` 作为控制台连接远程浏览器会话
...
- `packages/go-usb-ai-ui/src/components/browser-panel/*`
- `packages/go-usb-ai-ui/src/browser/managers/browser-panel.manager.ts`
- `packages/go-usb-ai-ui/src/browser/stores/browser-panel.store.ts`
- `packages/go-usb-ai-ui/src/browser/browser-bridge.types.ts`
- `packages/go-usb-ai-ui/src/browser/browser-capabilities.ts`

### 13.2 Web Host

- `packages/go-usb-ai-ui/src/browser/bridges/web-iframe-browser.bridge.ts`
- `packages/go-usb-ai-ui/src/browser/bridges/web-remote-browser.bridge.ts`（预留）
...
- `packages/go-usb-ai-ui/src/components/doc-browser/*`
  - 短期作为旧实现保留
...
**GoUsbAi 应该把浏览器面板当成正式子系统来建设，而不是继续把 `DocBrowser` 作为局部 UI 特性不断补丁式长大。**
...
这样做的好处不是“工程上更优雅”而已，而是它真正符合 `GoUsbAi` 作为统一入口与能力编排层的长期方向：

用户未来不只是“在 GoUsbAi 里聊天”，而是越来越多地“在 GoUsbAi 里看网页、理解网页、操作网页、让 AI 与网页协作”。
