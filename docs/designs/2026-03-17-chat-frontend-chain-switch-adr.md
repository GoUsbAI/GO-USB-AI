...
`packages/go-usb-ai-ui/src/components/chat/ChatPage.tsx` 的聊天主视图装配层。

更具体地说：

1. 路由入口仍保持在 [`packages/go-usb-ai-ui/src/App.tsx`](../../packages/go-usb-ai-ui/src/App.tsx) 的 `/chat/:sessionId?`。
...
2. `@go-usb-ai/agent-chat-ui` 继续作为共享展示层基座，不参与链路判断。
