...
`GoUsbAi Apps` 是一套面向 GoUsbAi 生态的小应用形态：
...
这一轮不是在做“大而全的平台替代品”，而是在把 `GoUsbAi Apps` 收敛成一个可真正使用、可真正传播、可真正发布的产品闭环。

## 3. 本轮版本目标

本轮只交付一个结果：

**把现有 `@go-usb-ai/app-runtime` 的本地 MVP，推进为一个可发布、可发现、可安装、可更新的完整 GoUsbAi Apps 产品闭环。**

完成后要成立这条真实链路：

`开发 app -> napp publish -> 官方 registry 收录 -> apps.go-usb-ai.io 展示 -> 用户复制安装命令 -> napp install -> napp run`
...
- 人类入口：`https://apps.go-usb-ai.io`
- 机器入口：`https://apps-registry.go-usb-ai.io`
...
- web 首页：`https://apps.go-usb-ai.io/`
- app 详情页：`https://apps.go-usb-ai.io/apps/<slug>`
- 发布者页：`https://apps.go-usb-ai.io/publishers/<publisher-id>`
- 默认 registry base：`https://apps-registry.go-usb-ai.io/api/v1/apps/registry/`
- apps API base：`https://apps-registry.go-usb-ai.io/api/v1/apps`

### 4.4 实现约束

- 不对外继续主打 `marketplace-api.go-usb-ai.io`
- 底层实现可以继续复用现有 `workers/marketplace-api`
- 本轮允许通过新增域名路由或 CNAME，把现有 worker 作为 `apps-registry.go-usb-ai.io` 的承载端
...
- 官方 scope 固定为 `go-usb-ai.<app-name>`，只允许管理员发布
...
### 6.4 `@go-usb-ai/app-sdk`

新增轻量 SDK：

```text
packages/go-usb-ai-app-sdk/
```
...
- 默认发布到官方 apps API
- 默认从 `apps-registry.go-usb-ai.io` 安装与更新
- 默认优先复用当前 `go-usb-ai login` 登录态完成个人发布
...
```json
{
  ...
  "description": "A small GoUsbAi app that reads a granted notes directory and returns a compact summary through the local host bridge.",
  "descriptionI18n": {
    "en": "A small GoUsbAi app that reads a granted notes directory and returns a compact summary through the local host bridge.",
    "zh": "一个小型 GoUsbAi app，通过本地宿主桥接读取已授权 notes 目录并返回简洁摘要。"
  },
  "author": "GoUsbAi",
  "tags": ["notes", "documents", "local", "official"],
  "sourceRepo": "https://github.com/Peiiii/go-usb-ai",
  "homepage": "https://github.com/Peiiii/go-usb-ai",
  "featured": true,
  "publisher": {
    "id": "go-usb-ai",
    "name": "GoUsbAi",
    "url": "https://go-usb-ai.io"
  }
}
```
...
```text
packages/go-usb-ai-app-runtime/
packages/go-usb-ai-app-sdk/
apps/go-usb-ai-apps-web/
apps/examples/
...
1. 一个更新后的 `@go-usb-ai/app-runtime`
2. 一个新的 `@go-usb-ai/app-sdk`
3. 一个扩展了 apps 域的 `marketplace-api` worker
4. 一个独立的 `apps.go-usb-ai.io` web app store
5. 三个官方示例 apps
6. 一份本轮迭代记录
7. 一次真实的发布、部署与上线结果
