# GoUsbAi App Runtime 相对完善 MVP 一次性收尾方案

## 这份文档解决什么问题

这份文档只回答一个问题：

**在已经具备 `create / inspect / run / dev / pack / install / uninstall / list / info` 的前提下，`@go-usb-ai/app-runtime` 还要补哪些能力，才能从“本地闭环原型”推进到“相对完善、可分享、可安装、可更新、可管理”的 MVP。**

这里的目标不是做完整 app store，不是做最终安全体系，也不是把微应用直接接进 GoUsbAi 主产品，而是先把独立 runtime/CLI 这条线真正做成一个完整产品。

## 产品对齐

这次收尾仍然服务于 GoUsbAi 的长期愿景：
...
```json
{
  ...
  "versions": {
    "0.1.0": {
      ...
      "dist": {
        "bundle": "./go-usb-ai.hello-notes/-/go-usb-ai.hello-notes-0.1.0.napp",
        "sha256": "<sha256>"
      }
    }
  }
...
```json
{
  "schemaVersion": 1,
  "registry": {
    "url": "https://registry.go-usb-ai.com/"
  }
}
```
...
这轮完成后，`@go-usb-ai/app-runtime` 的定位将从：
