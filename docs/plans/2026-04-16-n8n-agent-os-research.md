# n8n 调研与 GoUsbAi Agent OS 基础设施构想

## 1. 背景

GoUsbAi 的长期目标不是做一个“功能很多的 AI 产品”，而是成为 AI 时代的个人操作层，也就是用户使用软件、互联网、系统、服务与云计算的默认入口。

在这个目标下，`workflow automation` 与 `agent runtime` 都重要，但它们不是同一件事：

- `n8n` 的中心思想更接近 `workflow-first`：人先搭流程，系统稳定执行。
- `GoUsbAi` 更应该走 `agent-first`：用户先表达目标，系统理解意图、挑选能力、组织步骤、执行、观察结果、沉淀经验。

因此，这份文档的目的不是“把 GoUsbAi 做成 AI 版 n8n”，而是系统研究 n8n 的工程底座，吸收其中对 Agent OS 有价值的部分，并明确哪些地方不该照搬。
...
2. 如果 GoUsbAi 真想成为 Agent OS，“AI 能不能回答得更聪明”并不是第一门槛；真正的门槛是是否具备一套可靠、可恢复、可审计、可扩展、可控风险的执行基础设施。
3. n8n 对 GoUsbAi 最大的启发，不是“多做节点”，而是：
   - 统一触发模型
   - 统一能力注册
   - 统一执行记录
   - 统一错误处理
   - 统一凭据治理
   - 统一扩展边界
4. GoUsbAi 不应该把 workflow builder 当作主入口；它应该把 workflow 当作 Agent 可调用、可沉淀、可复用的一类能力，而不是产品本体。

### 2.2 一句话定义

更适合 GoUsbAi 的方向是：

`GoUsbAi = Agent Runtime + Event Trigger Bus + Capability Registry + Durable Execution + Safe Runners + Memory + Human Approval + Observability + Marketplace`
...
这对 GoUsbAi 尤其重要，因为个人操作层的权限会更大，涉及本地文件、系统命令、浏览器、远程服务和私有数据。
...
对 GoUsbAi / Agent OS 来说，这几条路径应该从一开始就被当成正规产品能力，而不是遇到本地部署场景再临时补救
...
这也是这次调研里对 GoUsbAi 最值得直接继承的一组思想
...
- `GoUsbAi`: agent 可以调用 workflow

这两者看起来只差一句话，但产品中心完全不同。

## 5. GoUsbAi 不该照搬 n8n 的地方
...
n8n 的一个显性优势是节点多，但 GoUsbAi 的长期优势不该是“我们也有很多节点”。
...
GoUsbAi 不应该默认走这条路。它更应该把复杂度收敛在系统里，让用户尽可能表达目标，而不是自己手工搭太多“实现细节”。

### 5.4 不要忽略本地 OS 场景

n8n 更偏 SaaS / API / business workflow 编排。GoUsbAi 的潜在优势更大，因为它还可以覆盖：
...
这决定了 GoUsbAi 的能力底座不能只按“云服务节点”思路来建设。
...
如果 GoUsbAi 想成为 Agent OS，这层 bus 最终应该把这些来源都收敛成同一种事件契约，而不是让上层 agent 分别理解：
...
n8n 对 AI tool call 的 human-in-the-loop 很值得学。GoUsbAi 也应该把它做成底层能力，而不是某条流程里的技巧。
...
这一步的目标不是“功能很多”，而是让 GoUsbAi 先具备操作层的最小骨架。
...
GoUsbAi 的长期竞争力不应是“我们也能做自动化”，而应是：
...
n8n 对 GoUsbAi 最有价值的不是“节点生态”表层，而是它背后那套扎实的自动化工程底座。
...
GoUsbAi 应该在这些底座之上，按 `agent-first` 的方式重新组织产品，而不是简单复刻一个 workflow-first 平台。
...
- GoUsbAi 愿景文档：[VISION.md](../VISION.md)
