# GoUsbAi Account And Remote Access Product Design
...
- 用户路径仍然暴露了 `platform.go-usb-ai.io`、`platform api base`、`remote connect`、`restart service` 这类开发者概念。
...
4. `platform.go-usb-ai.io` 可以继续作为实现载体存在，但不应成为用户主心智；用户看到的应该是 “GoUsbAi Web / 网页版 GoUsbAi”。
...
- 中文：`在网页中查看设备`、`前往网页版`
- 英文：`Open in Web`、`Go to GoUsbAi Web`

不建议作为主按钮文案直接使用：

- `前往 platform.go-usb-ai.io`
- `打开平台控制台`
...
1. 用户在本机无需关心平台底层细节
2. 用户只知道“这台设备已可访问”
3. 用户在另一台设备打开 GoUsbAi Web，登录同一账号
4. 在“我的设备”里打开这台设备
...
- 将用户文案从 `platform.go-usb-ai.io` 收敛为 `GoUsbAi Web`
