...
`provider/base_url/api_mode` 重新建 agent。对 GoUsbAi 来说，这会把“当前
...
- Hermes session 常驻 agent 会保留 provider/base_url/header/client 等执行态，
  与 GoUsbAi 侧 prompt 级路由透传冲突。

## 边界约束

- 不直接修改 Hermes 上游源码。
- 只允许在 GoUsbAi 侧 bridge / stdio runtime / 文档层做适配。
...
- 每次 `prompt` 到来时：
  - 从 `go-usb-ai_narp.providerRoute` 解析当前请求的完整执行路由
...
- `sitecustomize.py`
  ...
  - 把初始化失败等请求级错误显式抛回 GoUsbAi
- `go-usb-ai-hermes-acp-runtime-route.py`
  - 解析 `go-usb-ai_narp.providerRoute`
  - 归一化 `model/provider/apiBase/apiKey/apiMode/headers`
  - 兼容 ACP router 把 `_meta.go-usb-ai_narp` 展平成 `kwargs.go-usb-ai_narp`
...
- Bridge 回归：
  ...
  - 真实 ACP router 展平 `_meta` 后，bridge 仍能正确读到 `go-usb-ai_narp.providerRoute`。
- Runtime 回归：
...
- 命令验证：
  ...
  - `pnpm --filter @go-usb-ai/go-usb-ai-ncp-runtime-stdio-client test`
  - `pnpm --filter @go-usb-ai/go-usb-ai-ncp-runtime-stdio-client lint`
  - `pnpm --filter @go-usb-ai/go-usb-ai-ncp-runtime-stdio-client tsc`
