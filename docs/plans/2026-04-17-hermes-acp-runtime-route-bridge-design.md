...
**Goal:** 把 `Hermes` 在 GoUsbAi 中的正式接入主链锁定为 `narp-stdio(acp)`，并通过一层极薄的 Python bridge 让 `Hermes ACP` 直接消费 GoUsbAi 已解析好的 `RuntimeRoute(model/apiBase/apiKey/headers)`。  
**Status:** 这份文档是当前批次唯一有效的 Hermes stdio 主方案设计。后续实现、skill、验证、验收一律以本文件为准。  
**Vision alignment:** 这条方案服务于 GoUsbAi “统一入口、统一模型选择、统一凭据 ownership、统一会话体验”的产品目标。用户看到的是正式的 `Hermes` session type，而不是插件、connector、API server 或额外私有配置系统。
...
3. `GoUsbAi` 才是 `model / apiBase / apiKey / headers` 的 owner
4. `Hermes ACP` 必须消费 GoUsbAi 的 `RuntimeRoute`，不能重新退回 Hermes 自己的 provider 解析主语
...
`GoUsbAi -> stdio connector -> Hermes API server`
...
`GoUsbAi runtime entry (type: "narp-stdio")`
-> `hermes acp`
-> `Hermes ACP session`
-> `Hermes AIAgent`
-> `上游模型 API`
...
- `RuntimeRoute` 来自 GoUsbAi，而不是 Hermes 自己的配置系统
...
GoUsbAi 中的 `Hermes` 是统一 runtime registry 里的一条正式 entry。
...
4. `RuntimeRoute` 必须由 GoUsbAi 通过内建环境变量桥接进去，不需要用户显式配置字段名
5. Hermes 专属 Python bridge 必须由独立 Hermes bridge 包承载，`@go-usb-ai/go-usb-ai-ncp-runtime-stdio-client` 只保留通用 stdio 运行时职责
...
- `model` 由 GoUsbAi 当前选中的模型决定
- `apiBase` 由 GoUsbAi 当前 provider/base 配置决定
- `apiKey` 由 GoUsbAi 当前 provider 凭据决定
- `headers` 必须支持完整透传
...
- GoUsbAi 负责 provider/base/key/model 的选择
...
1. 读取 GoUsbAi 注入的环境变量
2. 把这些值变成 `AIAgent(...)` 的显式 runtime 参数
3. 优先使用这些显式参数，而不是继续先走 Hermes 自己的 provider resolution
4. 在 Hermes ACP 对外事件语义和 GoUsbAi/NCP 合同不一致时，只在 Hermes 专属 bridge 内做最小必要协议映射，例如把误接到 ACP thought 事件里的瞬时 `thinking_callback` 重映射到真实 reasoning 通道，而不是修改 Hermes 源码或污染通用 stdio client
...
- 但当它被 GoUsbAi 以 `narp-stdio(acp)` 启动时，必须优先消费 GoUsbAi 的 `RuntimeRoute`
...
- 安装 `go-usb-ai-hermes-stdio-connector`
- 维护 connector 专属命令
...
- `@go-usb-ai/go-usb-ai-ncp-runtime-stdio-client` 自带 bridge 所需资源
- 用户安装 GoUsbAi 后，不需要额外安装 first-party connector 包
...
1. `Hermes` 作为 `narp-stdio` entry 能在 GoUsbAi 中显示为正式 session type
2. runtime entry 直接启动的是 `hermes acp`，而不是 connector
3. `model / apiBase / apiKey / headers` 都由 GoUsbAi 透传给 Hermes ACP
...
- `go-usb-ai-hermes-stdio-connector` 作为 Hermes 正式 runtime entry 的推荐 command
...
5. 真实验证 `hermes acp` 路径确实吃到 GoUsbAi 的 route
