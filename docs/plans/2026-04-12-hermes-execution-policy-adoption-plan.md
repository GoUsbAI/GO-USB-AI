...
**Goal:** 在 GoUsbAi 中以可插拔方式引入一套直接借鉴 Hermes 的执行纪律层，显著提升 GPT-5.4 / Codex / Gemini 等模型的主动性、任务一次性完成率和“先做后说”的稳定性。
...
**Tech Stack:** TypeScript、Vitest、GoUsbAi core/runtime/NCP packages、Markdown 设计文档。

---

## 长期目标对齐 / 可维护性推进

这次工作不是“把 prompt 再写长一点”，而是顺着 GoUsbAi 成为统一入口与操作层的方向，补齐一个长期缺失的系统能力：
...
GoUsbAi 最值得直接“抄” Hermes 的，不是更多工具，也不是更厚的 system prompt，而是这四个东西：
...
这是当前 GoUsbAi 与 Hermes 体感差距最大的地方，必须优先抄。
...
GoUsbAi 当前已经有 `memory_search` / `memory_get` 和 skills，但缺 `session recall` 这一层。第一阶段不需要立刻补完整搜索能力，但至少要先把分工提示抄过来。

#### D. Stable vs ephemeral layering

Hermes 会刻意把稳定规则层和瞬时层分开，避免每轮都把核心纪律拼碎。GoUsbAi 也应该这样做：
...
GoUsbAi 不应新增一套平行 agent mode。正确做法是：
...
建议在 [`packages/go-usb-ai-core/src/agent`](/Users/peiwang/Projects/nextbot/packages/go-usb-ai-core/src/agent) 下新增一组独立文件：
...
1. [`packages/go-usb-ai-core/src/agent/context.ts`](/Users/peiwang/Projects/nextbot/packages/go-usb-ai-core/src/agent/context.ts)
2. [`packages/go-usb-ai/src/cli/commands/ncp/go-usb-ai-ncp-context-builder.ts`](/Users/peiwang/Projects/nextbot/packages/go-usb-ai/src/cli/commands/ncp/go-usb-ai-ncp-context-builder.ts)
3. [`packages/go-usb-ai-core/src/runtime-context/runtime-user-prompt.ts`](/Users/peiwang/Projects/nextbot/packages/go-usb-ai-core/src/runtime-context/runtime-user-prompt.ts)
...
这样可以与现有 [`ContextBuilder`](/Users/peiwang/Projects/nextbot/packages/go-usb-ai-core/src/agent/context.ts#L85) 和 [`RuntimeUserPromptBuilder`](/Users/peiwang/Projects/nextbot/packages/go-usb-ai-core/src/runtime-context/runtime-user-prompt.ts#L99) 的分层方式对齐。
...
建议直接抄 Hermes 的最值钱部分，但收敛为 GoUsbAi 版本：
...
在 GoUsbAi 现有 memory/skills 基础上补清楚：
...
当前 [`ContextBuilder.getIdentity()`](/Users/peiwang/Projects/nextbot/packages/go-usb-ai-core/src/agent/context.ts#L197) 已经塞了较多工具风格、memory recall、subagent 使用建议。第一阶段不应继续堆在这里，而应：
...
当前 [`go-usb-ai-ncp-context-builder.ts`](/Users/peiwang/Projects/nextbot/packages/go-usb-ai/src/cli/commands/ncp/go-usb-ai-ncp-context-builder.ts) 主要负责 metadata、tool filtering、session orchestration。它应该做的是：
...
当前 [`RuntimeUserPromptBuilder`](/Users/peiwang/Projects/nextbot/packages/go-usb-ai-core/src/runtime-context/runtime-user-prompt.ts#L141) 更像 bootstrap-aware user overlay。这里应该接：

- policy resolver 返回的 ephemeral overlay sections
- 例如 continue / follow-up / internal task completion 的执行提醒

这样能与 Hermes 的 stable vs ephemeral 分层相似，但仍贴合 GoUsbAi 当前结构。
...
**Files:**
- Create: `packages/go-usb-ai-core/src/agent/execution-policy/execution-policy-types.ts`
- Create: `packages/go-usb-ai-core/src/agent/execution-policy/execution-policy-config.ts`
- Create: `packages/go-usb-ai-core/src/agent/execution-policy/execution-policy-pack.ts`
- Create: `packages/go-usb-ai-core/src/agent/execution-policy/execution-policy-sections.ts`
- Create: `packages/go-usb-ai-core/src/agent/execution-policy/model-execution-policy.ts`
- Create: `packages/go-usb-ai-core/src/agent/execution-policy/execution-policy-resolver.ts`
- Test: `packages/go-usb-ai-core/src/agent/execution-policy/execution-policy-resolver.test.ts`
...
Run: `pnpm --filter @go-usb-ai/core exec vitest run src/agent/execution-policy/execution-policy-resolver.test.ts`
...
```bash
git add packages/go-usb-ai-core/src/agent/execution-policy
...
- Test: `packages/go-usb-ai-core/src/agent/context.execution-policy.test.ts`
...
Run: `pnpm --filter @go-usb-ai/core exec vitest run src/agent/context.execution-policy.test.ts`
...
```bash
git add packages/go-usb-ai-core/src/agent/context.ts packages/go-usb-ai-core/src/agent/context.execution-policy.test.ts
...
**Files:**
- Modify: `packages/go-usb-ai/src/cli/commands/ncp/go-usb-ai-ncp-context-builder.ts`
- Test: `packages/go-usb-ai/src/cli/commands/ncp/context/go-usb-ai-ncp-context-builder.execution-policy.test.ts`
...
Run: `pnpm -C packages/go-usb-ai test -- --run src/cli/commands/ncp/context/go-usb-ai-ncp-context-builder.execution-policy.test.ts`
...
```bash
git add packages/go-usb-ai/src/cli/commands/ncp/go-usb-ai-ncp-context-builder.ts
...
**Files:**
- Modify: `packages/go-usb-ai-core/src/runtime-context/runtime-user-prompt.ts`
- Test: `packages/go-usb-ai-core/src/runtime-context/runtime-user-prompt.execution-policy.test.ts`
...
Run: `pnpm --filter @go-usb-ai/core exec vitest run src/runtime-context/runtime-user-prompt.execution-policy.test.ts`
...
```bash
git add packages/go-usb-ai-core/src/runtime-context/runtime-user-prompt.ts
...
Run: `pnpm --filter @go-usb-ai/core exec vitest run src/config/schema.execution-policy.test.ts`
...
```bash
git add packages/go-usb-ai-core/src/config docs/USAGE.md
...
```bash
pnpm --filter @go-usb-ai/core exec vitest run \
...
pnpm -C packages/go-usb-ai test -- --run \
  src/cli/commands/ncp/context/go-usb-ai-ncp-context-builder.execution-policy.test.ts
```
...
1. GPT/Codex 在 GoUsbAi 中获得 Hermes 风格的“先做后说”纪律。
