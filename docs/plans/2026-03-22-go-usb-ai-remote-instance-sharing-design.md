# GoUsbAi Remote Instance Sharing Design

日期：2026-03-22

## 1. 结论

本方案将 GoUsbAi remote access 的对外模型收敛为：

- 平台只做实例入口、分享授权与访问会话签发
- relay 只做 transport 与 session 路由
- 本地 GoUsbAi 继续作为唯一运行时与唯一产品面

本次明确做两件事：

1. 支持同一个浏览器同时访问多个远程 GoUsbAi 实例
...
- 用户实际访问的是一个运行中的 GoUsbAi 实例，而不是硬件设备本身
...
- 保持现有 GoUsbAi UI / API / WebSocket 作为唯一产品面
- 支持同一浏览器并行打开多个远程实例
- 支持通过分享 URL 让他人打开实例
...
```text
https://<instance-id>.remote.go-usb-ai.io/
```
...
```text
https://<access-session-id>.remote.go-usb-ai.io/
```
...
- 分享 URL：
  - `https://platform.go-usb-ai.io/share/<grant-token>`
- 访问 URL：
  - `https://<access-session-id>.remote.go-usb-ai.io/`
...
表示一个被平台识别、可被远程打开的 GoUsbAi 运行实例。
...
4. 平台返回 `openUrl`
5. 浏览器跳转到 `https://<access-session-id>.remote.go-usb-ai.io/`
...
- 实例列表：`https://platform.go-usb-ai.io/instances`
- 分享链接：`https://platform.go-usb-ai.io/share/<grant-token>`

### 9.2 远程访问入口

- 访问会话：`https://<access-session-id>.remote.go-usb-ai.io/`
...
```json
{
  ...
  "data": {
    "sessionId": "ras_xxx",
    "openUrl": "https://ras_xxx.remote.go-usb-ai.io/",
...
```json
{
  ...
  "data": {
    "grantId": "rsg_xxx",
    "shareUrl": "https://platform.go-usb-ai.io/share/rsgt_xxx",
