...
- 状态：讨论稿
- 日期：2026-05-15
- 范围：GoUsbAi 主产品内 Skill Marketplace 的前端展示与信息架构
...
当前 Skill Marketplace 更接近管理列表：搜索、排序、Marketplace / Installed tabs、列表卡片和安装按钮。这种形态稳定、清楚、适合已知目标用户，但对“我不知道有哪些能力、我想让 GoUsbAi 变会什么”的用户不够友好。

从 GoUsbAi 产品愿景看，Skill Marketplace 不只是一个下载页。它应该服务：

- 统一入口：用户通过 GoUsbAi 发现并接入新能力。
- 能力编排：skill 不是孤立包，而是用户目标到执行链路的一部分。
- 开箱即用：新用户进入后能快速理解 GoUsbAi 能扩展什么。
...
- “相关性”和“质量”是两个不同维度。GoUsbAi 第一阶段可以先做人工场景相关性，不做个性化。
- 编辑价值可以是手工策展，不必等推荐算法。
...
- 可信、安全、来源说明在扩展生态里很重要。
- 地区/设备/生态适配是货架维度之一，但 GoUsbAi 第一阶段可以先不做地域化。
...
- 新用户能在首屏看懂“这里是在扩展 GoUsbAi 能力”，而不是只看到包列表。
- 老用户仍可快速搜索、排序、安装、管理。
...
- 最符合 GoUsbAi 的意图到执行闭环。
- 不依赖推荐算法。
- 文案可以围绕“你想让 GoUsbAi 做什么”，而不是围绕包名。
- 与未来自感知推荐可以自然衔接。
...
- GoUsbAi 主产品内置 Skill Marketplace。
- 新用户和高频使用者共同使用的工作台入口。
...
场景
开发与调试 | 浏览器自动化 | 办公协作 | 写作内容 | 本地环境 | 社交平台 | GoUsbAi 官方
...
- `GoUsbAi 官方` 可以作为一个 scene，也可以后续升级为来源 badge；不要在没有明确契约时同时做两套语义。
...
3. `GoUsbAi 官方 / 用户发布 / 社区` 的 badge 是否需要后端补契约后再做？
...
这个方向对 GoUsbAi 更合适：用户不是来逛商品，而是来扩展自己的个人操作层。
