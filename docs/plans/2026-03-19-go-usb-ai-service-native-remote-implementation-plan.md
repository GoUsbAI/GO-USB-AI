# GoUsbAi Service-Native Remote Access Implementation Plan

## 目标

把 remote access 从前台 `go-usb-ai remote connect` 主路径，迁移到由 `go-usb-ai` 主服务统一托管的能力，并保持边界清晰、实现可插拔、后续扩展不膨胀。
...
- 保留 `go-usb-ai remote connect` 作为前台调试模式
- 更新使用文档与模板文档

### 验收标准

- 用户无需再把 `remote connect` 当作日常主入口
- `go-usb-ai --help` 与 `go-usb-ai remote --help` 能直接看到收敛后的命令面
...
- `go-usb-ai remote enable && go-usb-ai start` 后，无需单独保持另一个终端窗口
...
- `go-usb-ai status` 聚合输出 remote 启用状态、连接状态、设备名、平台、最近错误
- `go-usb-ai remote status` 读取同一状态源

### 验收标准

- `go-usb-ai status` 与 `go-usb-ai remote status` 对 remote 的判断一致
...
- 已完成 remote 状态并入 `go-usb-ai status`
- 已完成 token 日志脱敏
