...
**Architecture:** 继续以 `go-usb-ai-provider-gateway-api` 的 `user.username` 作为唯一身份源，不另造账号体系。产品面收敛为三层入口：`GoUsbAi Web` 作为用户主入口、桌面/本地 `Account Panel` 作为上下文快捷入口、CLI 作为脚本化和无 UI 场景的自助兜底。Marketplace 发布链路不再自行发明模糊提示，而是消费同一套“username readiness contract”。

**Tech Stack:** `apps/platform-console` React + TanStack Query、`packages/go-usb-ai-ui` React + Zustand、`packages/go-usb-ai` CLI、`workers/go-usb-ai-provider-gateway-api`、marketplace publish skill docs。
...
3. 平台网页 `platform.go-usb-ai.io` 作为用户账号站点，却没有同等可发现的 username 设置入口。
...
### 2.3 用户主入口应当是 GoUsbAi Web，不是裸域名和内部术语

此前账号设计已经明确：`platform.go-usb-ai.io` 可以是实现域名，但不应成为用户主心智。  
因此对用户和 AI 的统一文案都应收敛为：

- 中文：`GoUsbAi Web` / `网页版 GoUsbAi`
- 英文：`GoUsbAi Web`
...
- 把 `go-usb-ai.io` 改成 `platform.go-usb-ai.io`
- 在 skill 里写更准确的解释
...
- `GoUsbAi Web` 提供明确的 username/profile 入口
...
1. 用户执行发布。
2. 如果未登录：明确要求先登录 GoUsbAi Account。
3. 如果已登录但没有 username：
   - CLI 明确告诉用户“你还差 username”
   - 提供两个可执行完成路径：
     - `Open GoUsbAi Web account settings`
     - `Run go-usb-ai account set-username <name>`
4. 用户无论在 Web、本地 UI、还是 CLI，都能完成同一个字段设置。
5. 完成后，发布默认继续走 `@username/skill-name`。

AI 也应该稳定引用同一个答案：

`去 GoUsbAi Web 的账号设置页，或者直接用 go-usb-ai 的 account 命令设置 username。`
...
**Files:**
- Modify: `skills/publish-to-go-usb-ai-marketplace/SKILL.md`
- Modify: `packages/go-usb-ai/src/cli/skills/marketplace-identity.ts`
...
- Reference: `docs/plans/2026-03-21-go-usb-ai-account-and-remote-access-product-design.md`

**Steps:**
1. 把所有面对用户的“去平台设置 username”收敛成 `GoUsbAi Web` / `网页版 GoUsbAi`。
2. 在面向技术实现的地方保留真实域名 `https://platform.go-usb-ai.io`。
3. 统一文案语义：
   - `未登录`
   - `已登录但缺 username`
   - `username 已锁定`
...
**Verification:**
- `rg -n "go-usb-ai.io|platform account UI|remote account profile flow" skills/publish-to-go-usb-ai-marketplace/SKILL.md packages/go-usb-ai/src/cli/skills/marketplace-identity.ts apps/platform-console/src packages/go-usb-ai-ui/src`
- 期望：面向用户的错误指引不再引用 landing 域名，也不再使用模糊入口名。

### Task 2: 在 GoUsbAi Web 中补齐 username 设置入口
...
4. 如果 `username` 已存在：
   - 显示只读值
   - 明确“当前版本首次设置后不可改名”
5. URL 层如果需要单独入口，优先使用 `platform.go-usb-ai.io/account` 或 `platform.go-usb-ai.io/profile` 之一，但前端内部必须真实存在该语义页面，而不是任意 SPA 路由都返回 200 的假入口。
...
- Modify: `packages/go-usb-ai/src/cli/commands/platform-auth.ts`
- Modify: `packages/go-usb-ai/src/cli/commands/platform-auth.test.ts`
- Optional Create: `packages/go-usb-ai/src/cli/commands/account.controller.ts`

**Steps:**
1. 不再把 `me()` / `updateProfile()` 只留在内部调用。
2. 对外暴露明确命令，推荐命名：
   - `go-usb-ai account status`
   - `go-usb-ai account set-username <username>`
...
**Verification:**
- `pnpm -C packages/go-usb-ai test -- --run src/cli/commands/platform-auth.test.ts`
- 手动冒烟：
  - `pnpm -C packages/go-usb-ai exec tsx src/cli/index.ts account status`
  - `pnpm -C packages/go-usb-ai exec tsx src/cli/index.ts account set-username alice-dev`

### Task 4: 让 publish 链路返回真正可执行的下一步

**Files:**
- Modify: `packages/go-usb-ai/src/cli/skills/marketplace-identity.ts`
- Modify: `packages/go-usb-ai/src/cli/skills/marketplace.publish.test.ts`
- Optional Modify: `packages/go-usb-ai/src/cli/skills/marketplace.ts`
- Modify: `skills/publish-to-go-usb-ai-marketplace/SKILL.md`

**Steps:**
1. 将当前报错：
   - `Set your GoUsbAi username before publishing personal skills.`
   升级为包含下一步的错误。
2. 推荐输出结构：
   - 发生了什么
   - 为什么阻断
   - 两条完成路径
     - Web：`Open GoUsbAi Web account settings`
     - CLI：`go-usb-ai account set-username <name>`
3. skill 文档同步改为：
   - 优先引导用户去 GoUsbAi Web
...
- 断言 username 缺失错误中包含：
  - `GoUsbAi Web`
  - `go-usb-ai account set-username`

**Verification:**
- `pnpm -C packages/go-usb-ai test -- --run src/cli/skills/marketplace.publish.test.ts`

### Task 5: 对齐本地 Account Panel 与 GoUsbAi Web

**Files:**
- Modify: `packages/go-usb-ai-ui/src/account/components/account-panel.tsx`
- Modify: `packages/go-usb-ai-ui/src/account/managers/account.manager.ts`
- Modify: `packages/go-usb-ai-ui/src/components/remote/remote-access-page.tsx`
- Modify: `packages/go-usb-ai-ui/src/components/remote/remote-access-page.test.tsx`
- Modify: `packages/go-usb-ai-ui/src/lib/i18n.remote.ts`
- Optional Create: `packages/go-usb-ai-ui/src/account/components/account-panel.test.tsx`

**Steps:**
1. 保留本地 `Account Panel` 里的 username 设置能力。
2. 但把“查看我的设备”这类动作重新命名为更清晰的 `Open GoUsbAi Web` / `前往网页版`。
...
**Verification:**
- 文档链接手动检查
- `rg -n "Set your GoUsbAi username|GoUsbAi Web|account set-username" docs skills packages`
...
2. `platform.go-usb-ai.io` 登录后，能直接完成 username 设置。
3. 本地 `Account Panel` 仍然可以完成 username 设置。
4. CLI 能显式查询账号状态并设置 username。
5. `skills publish` 在 username 缺失时给出两条可执行路径。
6. publish-to-go-usb-ai-marketplace skill 不再把用户引到 `go-usb-ai.io` landing。
