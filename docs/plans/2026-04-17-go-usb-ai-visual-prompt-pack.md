...
- [2026-04-17-go-usb-ai-first-visual-briefs.md](./2026-04-17-go-usb-ai-first-visual-briefs.md)
- [2026-04-17-go-usb-ai-marketing-visual-asset-plan.md](./2026-04-17-go-usb-ai-marketing-visual-asset-plan.md)
...
- 第一轮也必须让人一眼认出这是 `GoUsbAi`，不能只做泛 AI 产品海报
- 默认必须上传真实 GoUsbAi 截图作为参考图，再让模型做海报化重组
- 如果模型不支持参考图，这套 prompt 只能拿来探索氛围，不能拿来出正式主视觉

## 这次失败的根因

上一版 prompt 的问题不是“不够华丽”，而是“不够像 GoUsbAi”。

根因有三个：

1. 没有把真实 GoUsbAi UI 作为硬锚点
2. 太强调“统一入口”和“高级感”，却没强调“必须像我们的真实产品”
3. 模型被允许自由发挥，于是自动收敛成最常见的泛 AI SaaS 概念图

这类图的问题在于：

- 即使写上 `GoUsbAi`，用户也不会相信这是 GoUsbAi
- 即使画得好看，也不能承担产品传播
- 即使有氛围，也没有产品身份

所以从这一版开始，策略改成：

**先让图明显像 GoUsbAi，再让它变得高级。**

## 必须使用的参考图

首批生成时，默认至少上传这三张：

- [images/screenshots/go-usb-ai-chat-page-en.png](/Users/peiwang/Projects/nextbot/images/screenshots/go-usb-ai-chat-page-en.png)
- [images/screenshots/go-usb-ai-providers-page-en.png](/Users/peiwang/Projects/nextbot/images/screenshots/go-usb-ai-providers-page-en.png)
- [images/screenshots/go-usb-ai-channels-page-en.png](/Users/peiwang/Projects/nextbot/images/screenshots/go-usb-ai-channels-page-en.png)

如需补充远程/浏览器感，可追加：

- [images/screenshots/go-usb-ai-micro-browser-dock-en.png](/Users/peiwang/Projects/nextbot/images/screenshots/go-usb-ai-micro-browser-dock-en.png)
...
- must visibly resemble the real GoUsbAi product UI
...
**GoUsbAi 是个人 AI 操作层，而不是普通聊天壳。**

### 英文主 Prompt

```text
Create a premium website hero image for GoUsbAi, using the provided real GoUsbAi screenshots as mandatory references.

The image must clearly look like GoUsbAi, not a generic AI dashboard. It should show GoUsbAi as a unified control console that brings together AI providers, communication channels, automation, remote access, and project context in one place.

Visual direction:
- dark premium product environment
- central elegant control-console composition built from recognizable GoUsbAi UI elements as the main focal point
...
- realistic interface panels based on the actual GoUsbAi product, not fantasy sci-fi
...
- viewers should still recognize real chat, providers, and channels sections from GoUsbAi
...
Input images: Image 1 = real GoUsbAi chat UI reference; Image 2 = real GoUsbAi providers UI reference; Image 3 = real GoUsbAi channels UI reference
Primary request: create a flagship marketing hero image for GoUsbAi, positioned as a personal AI operating layer rather than a simple chatbot. The image must communicate that one unified console controls AI providers, communication channels, automation, remote access, and real work context. It must visibly resemble the real GoUsbAi product and should feel like a premium composition built from the real UI, not a generic imagined dashboard.
Scene/backdrop: a dark premium environment with layered depth, subtle gradients, and calm atmospheric lighting
Subject: a central polished GoUsbAi console composition built from recognizable real interface sections, surrounded by connected modules representing providers, channels, automation, remote access, and project context
...
```text
请基于我提供的真实 GoUsbAi 截图，生成一张高端产品主视觉海报。它不是普通聊天工具，而是一个“个人 AI 操作层”产品。

画面核心是一个位于中心的 GoUsbAi 控制台，中枢感非常强。这个中枢必须明显保留真实 GoUsbAi 界面的结构特征，不能随便编一个通用控制台。周围通过有秩序的连接关系延展出五类能力模块：模型提供商、消息渠道、自动化、远程访问、项目上下文。
...
- 统一控制
- AI 进入真实工作流
- 这就是 GoUsbAi 本身，而不是别的泛 AI 产品

风格要求：
- 深色高端科技产品感
- 有未来感，但克制，不廉价
- 要像高级产品发布视觉，不要像游戏海报
- 必须使用真实 GoUsbAi 界面做锚点，不是纯抽象概念图
- 画面干净，层级清晰，中间主体非常明确
...
- `GoUsbAi Is Your Personal AI Operating Layer`
...
把 GoUsbAi 的价值讲清楚，让用户看完后能转述：

**用户先进入 GoUsbAi，再由 GoUsbAi 去连接模型、渠道、自动化和远程工作流。**

### 英文主 Prompt

```text
Create a premium product explanation graphic for GoUsbAi, using the provided real GoUsbAi screenshots as structural references.

The image should clearly look like GoUsbAi and explain how GoUsbAi acts as one central entry point that unifies AI providers, communication channels, automation, remote access, and project context.

Visual structure:
- one central GoUsbAi hub in the middle
...
- viewers should still recognize real GoUsbAi interface language
...
- clarity and control
- this is the real GoUsbAi product
...
Input images: Image 1 = real GoUsbAi chat UI reference; Image 2 = real GoUsbAi providers UI reference; Image 3 = real GoUsbAi channels UI reference
Primary request: create a refined product explanation graphic for GoUsbAi that shows how one unified console connects AI providers, communication channels, automation, remote access, and project context into a single user-facing entry point. The result must visibly resemble the real GoUsbAi product and preserve recognizable interface language from the provided screenshots.
Scene/backdrop: dark interface environment with subtle depth, clean grid hints, premium product mood
Subject: a central GoUsbAi hub surrounded by five structured capability modules, all visually connected in an elegant orchestration layout
...
```text
请基于我提供的真实 GoUsbAi 截图，生成一张高端产品解释图。它要讲清楚：GoUsbAi 是一个统一入口，用户先进入 GoUsbAi，再由 GoUsbAi 去连接模型、消息渠道、自动化、远程访问和项目工作上下文。

画面结构要求：
- 正中心是 GoUsbAi 中枢
- 周围有五个能力模块
- 每个模块都要像真实 GoUsbAi 产品卡片或产品面板，不是普通框图
- 中心和外围之间要有优雅、清晰、有秩序的连接关系
- 整体要像高端产品信息图，而不是技术架构图
- 画面必须保留可识别的真实 GoUsbAi 界面语言，不能凭空发明一个通用 AI 仪表盘
