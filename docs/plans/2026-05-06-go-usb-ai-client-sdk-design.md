# GoUsbAi Client SDK 方案设计

日期：2026-05-06

## 1. 文档目的

本文定义 `@go-usb-ai/client-sdk` 的正式长期设计，用于统一 GoUsbAi 所有上层前端访问后端能力的方式。
...
**GoUsbAi 的所有上层前端，应该通过什么样的一个统一 client contract 访问现有后端能力。**

## 2. 结论

`@go-usb-ai/client-sdk` 应成为 GoUsbAi 所有上层前端访问后端的唯一正式入口。
...
1. **补全 `@go-usb-ai/client-sdk`**
   - 补齐当前前端实际使用到的后端能力
   - 建立正式的 `transport + domain services + server-owned contracts` 结构

2. **前端接入 SDK**
   - `packages/go-usb-ai-ui` 必须开始真实接入 SDK
...
- server UI / runtime / config / remote / marketplace contract 归 `@go-usb-ai/server`
- runtime update contract 归 `@go-usb-ai/kernel/update-contract`
...
那么 `packages/go-usb-ai-ui` 一接入就会退化。
...
`packages/go-usb-ai-ui/src/shared/lib/api` 可以在迁移期继续存在，但其角色必须收缩为：

- 兼容旧调用点
- 内部转调 `@go-usb-ai/client-sdk`
...
```text
packages/go-usb-ai-client-sdk/src/
  index.ts
  go-usb-ai-client.manager.ts
  services/
    app.service.ts
    agents.service.ts
    auth.service.ts
    ...
    sessions.service.ts
  types/
    go-usb-ai-request.types.ts
    go-usb-ai-realtime.types.ts
    go-usb-ai-transport.types.ts
  utils/
...
- `go-usb-ai-client.manager.ts` 是 SDK 公共 client 聚合面，放在 `src/` 下作为独立入口文件，不归入 `services/`
...
```ts
const client = new GoUsbAiClient(options);
```

这个入口由根目录的 `index.ts` 作为 package public boundary 暴露，实际 SDK client 聚合实现位于 `go-usb-ai-client.manager.ts`。不再额外保留只包一层 `new GoUsbAiClient(options)` 的 factory。
...
```ts
type CreateGoUsbAiClientOptions = {
  baseUrl?: string;
...
当前 `packages/go-usb-ai-ui` 并不是一个单纯的浏览器端 `fetch` 调用者，它已经拥有自己的 runtime-aware transport：
...
```ts
type GoUsbAiTransport = {
  request<TResponse>(input: GoUsbAiRequestInput): Promise<TResponse>;
  openStream<TFinal = unknown>(input: GoUsbAiStreamInput): GoUsbAiStreamSession<TFinal>;
...
```ts
type GoUsbAiRequestInput = {
  method: "GET" | "POST" | "PUT" | "PATCH" | "DELETE";
  path: string;
  query?: Record<string, string | number | boolean | null | undefined>;
  headers?: Record<string, string>;
  body?: GoUsbAiRequestBody;
  timeoutMs?: number;
  signal?: AbortSignal;
};
```
...
```ts
type GoUsbAiStreamInput = {
  method: "GET" | "POST";
  path: string;
  headers?: Record<string, string>;
  body?: GoUsbAiRequestBody;
  signal?: AbortSignal;
  onEvent: (event: GoUsbAiStreamEvent) => void;
};
```
...
当前 `packages/go-usb-ai-ui/src/shared/lib/api` 里的能力，在迁移后应成为 SDK 的薄兼容适配层。
...
## 11. 与 `packages/go-usb-ai-ui` 的集成策略

### 11.1 UI 不直接依赖默认 transport

`packages/go-usb-ai-ui` 的正确接入方式不是：
...
- SDK 定义 `GoUsbAiTransport`
- UI 提供 `GoUsbAiTransport` 的 adapter 实现
...
```text
shared/lib/api -> @go-usb-ai/client-sdk -> UI transport adapter -> shared/lib/api
```
...
- 但它内部必须全部转为调用 `@go-usb-ai/client-sdk`
...
`apps/companion` 直接使用 `@go-usb-ai/client-sdk`，优先走默认 transport。
...
未来若 desktop shell 需要直接访问后端能力，也应复用 `@go-usb-ai/client-sdk`，而不是再长出一套 desktop 专属 API 层。
...
1. `@go-usb-ai/client-sdk` 覆盖当前前端正在使用的全部后端能力
2. `packages/go-usb-ai-ui/src/shared/lib/api` 内部不再维护独立请求 owner
...
9. `packages/go-usb-ai-ui` 已有真实模块通过 SDK 工作，而不是只有 SDK 包本身完成
...
1. **SDK 完整**
   - `@go-usb-ai/client-sdk` 覆盖当前前端真实使用面
...
- `@go-usb-ai/client-sdk` 的 `tsc`
- `@go-usb-ai/client-sdk` 的测试
- `packages/go-usb-ai-ui` 的 `tsc`
...
1. 正式确立 `@go-usb-ai/client-sdk` 作为唯一 client contract owner
