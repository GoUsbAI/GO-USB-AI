# NCP Phase 2.5：GoUsbAi Capability Assembly Plan
...
- 已有：`DefaultNcpAgentBackend` + `GoUsbAiAgentSessionStore` + `ProviderManagerNcpLLMApi`
- 缺失：GoUsbAi 原有 agent loop 中的工具体系、上下文体系、extension/gateway/session/memory/subagent 等能力装配

也就是说，当前链路更接近“通用 NCP agent backend + LLM provider”，而不是“装配了 GoUsbAi 真实能力的 NCP backend”。

## 目标

本阶段的目标不是继续扩展前端，而是让 NCP backend 真正承载 GoUsbAi agent 的核心能力，并达到以下结果：

1. NCP 新链路执行时复用 GoUsbAi 当前真实 agent 能力，而不是裸 LLM loop。
...
- `GatewayAgentRuntimePool`
  - 继续负责真正的 GoUsbAi agent 能力执行

## 实现拆分

### 1. Runtime Adapter

新增 `GoUsbAiUiNcpRuntime`：
...
`sessionKey` 直接使用 NCP `sessionId`，保持与 `GoUsbAiAgentSessionStore` 一致。
...
- `GoUsbAiUiNcpRuntime + DefaultNcpAgentBackend`

也就是说，UI NCP agent 不再自己实现一套空工具 registry，而是桥接到现有 GoUsbAi runtime pool。

### 4. 验证与切换

完成后必须验证：

1. NCP 路径可调用真实 GoUsbAi tool/session/memory 等能力
2. `stop` / abort 仍可用
...
- 将 NCP runtime 通用包改造成承载 GoUsbAi 业务逻辑

## 验收标准

1. NCP UI backend 执行的是真实 GoUsbAi runtime，而不是空 `DefaultNcpToolRegistry`
2. 通过 NCP 链路能触发 GoUsbAi tools / skills / session / memory / extension 能力
