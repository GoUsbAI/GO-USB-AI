...
- [NCP Phase 2.5：GoUsbAi Capability Assembly Plan](./2026-03-18-ncp-phase2-5-go-usb-ai-capability-assembly-plan.md)
  - 回答如何先通过 bridge runtime 快速把真实 GoUsbAi 能力挂进 NCP 通道，优先打通链路
- 本文
...
- GoUsbAi 的 context / tool / metadata 装配，已经通过消费方 NCP 组件接入
...
- `GoUsbAiUiNcpRuntime -> runtimePool.processDirect()` 不再是主执行路径
...
- `GoUsbAiAgentSessionStore` 可以继续存在，因为它是“存储一致性边界”
- `GoUsbAiUiNcpRuntime -> runtimePool.processDirect()` 不能长期存在，因为它意味着运行时核心还不属于 NCP

## 核心功能不变的口径

这次重构的重点不是追求每一个细枝末节都 100% 字节级一致，而是必须保证 GoUsbAi 的核心功能能力不变。

也就是说，用户角度真正重要的契约必须保持：

- 仍然是同一个 GoUsbAi agent，而不是退化成一个“只有聊天框的 NCP demo”
- 仍然具备 GoUsbAi 原本的产品能力，而不是只剩基础 text generation
- 仍然沿用 GoUsbAi 的核心上下文构造方式，而不是换成另一套完全不同的 prompt/runtime 语义

这里要明确，所谓“核心功能不变”，至少包括下面这些能力不能漏：

### 1. Context / Prompt Assembly 不变

必须保留 GoUsbAi 原本真正重要的上下文装配能力，包括：
...
也就是说，未来不是“随便做一个 NCP context builder”，而是要把 GoUsbAi 当前有效的 context/prompt 设计迁移为 NCP-native context builder。
...
- tools 的注册与调用
- skills 的请求、装配与执行
- GoUsbAi 原有 capability modules 的运行时接入
...
GoUsbAi 原本在 runtime 里承担的一些关键策略，不能在重构时意外丢失。
...
GoUsbAi 的很多内部能力并不一定都该硬编码在 runtime 里，其中一部分本来就应该通过 extension mechanism 进入。

所以终局不是“把所有东西写死进一个新的 runtime”，而是：

- 保留 GoUsbAi 可以注入内部组件的能力
...
- 在消费方实现 `GoUsbAiNcpContextBuilder`
- 在消费方实现 `GoUsbAiNcpToolRegistry`
- 复用现有 `llmApi` 接口接入 GoUsbAi 的模型与 provider 能力
- 继续通过 `GoUsbAiAgentSessionStore` 保持存储层一致
- 在 `createUiNcpAgent` 中直接装配这套 runtime，而不是继续桥接 `runtimePool.processDirect()`

这条路径的意义是：

- 先验证 `DefaultNcpAgentRuntime` 这套通用 engine 是否足以承接 GoUsbAi
- 先把 bridge runtime 从主链路里拿掉
- 先在真实实现里看清楚哪些地方只是 GoUsbAi 的消费方装配，哪些地方才是真正缺失的通用抽象
...
- 见 [tool.ts](/Users/peiwang/Projects/nextbot/packages/ncp-packages/go-usb-ai-ncp/src/agent-runtime/tool.ts#L7)

这意味着第一版实现时，如果发现大量 GoUsbAi 工具都在绕路获取 session / metadata / services / abort signal，那就说明这里后续值得沉淀更强的通用 tool contract。

#### 2. Context builder 是否开始膨胀成黑盒

当前 `NcpContextBuilder` 接口很薄，只是 `prepare(input, options)`：

- 见 [context-builder.ts](/Users/peiwang/Projects/nextbot/packages/ncp-packages/go-usb-ai-ncp/src/agent-runtime/context-builder.ts#L5)

这本身不阻止第一版实现，但如果 `GoUsbAiNcpContextBuilder` 很快膨胀成一个巨大的 prompt/memory/skill 黑盒，就说明后续值得把其中稳定的模式回沉为更通用的模块接口。

#### 3. Policy / lifecycle 是否只能靠隐式约定

当前 `DefaultNcpAgentRuntime` 已经有通用 loop，但缺少正式的 hooks / lifecycle 装配点：

- 见 [runtime.ts](/Users/peiwang/Projects/nextbot/packages/ncp-packages/go-usb-ai-ncp-agent-runtime/src/runtime.ts#L20)
...
- 见 [context-builder.ts](/Users/peiwang/Projects/nextbot/packages/ncp-packages/go-usb-ai-ncp-agent-runtime/src/context-builder.ts#L35)

第一版可以先这样实现，但如果后面发现 skill 在 GoUsbAi 里承载的是更强的能力装配语义，那就说明这里后续值得沉淀更明确的通用能力接口。
...
2. 第一阶段直接基于 `DefaultNcpAgentRuntime` 实现 GoUsbAi NCP-native runtime
...
接下来真正要做的，不是再包一层 bridge，也不是先发明大量新积木，而是先把 GoUsbAi 的产品能力装进现有 NCP runtime 积木，并在过程中识别真正值得沉淀的通用 building blocks。
...
- 如果存在 `GoUsbAiNcpCapabilityAssembler`，它也应属于 GoUsbAi 消费方装配层
- 它不是 NCP core 的官方组合层
- NCP 只需要提供让它可以被实现的通用接口和积木

### 2. GoUsbAi NCP tool registry
...
- 先在 GoUsbAi 消费方实现 policy 决策与装配
- 只在真实实现证明某些 lifecycle 需求稳定存在时，再把它们沉淀为 NCP 通用 hooks

### 5. GoUsbAi extension 接入
...
- 基于 `DefaultNcpAgentRuntime + GoUsbAiNcpContextBuilder` 生成上下文
- 基于 `GoUsbAiNcpToolRegistry` 执行工具调用
- 在消费方装配策略与能力
- 注入 GoUsbAi 所需产品能力
...
- `GoUsbAiUiNcpRuntime` 不再是主链路依赖
- 主执行路径进入 `DefaultNcpAgentRuntime + GoUsbAi` 消费方装配链路
...
- 产品需要的 GoUsbAi 内部能力都能通过 NCP-native 机制进入 runtime
...
### 2. 不做 GoUsbAi 私有黑盒 runtime

如果某项能力具有通用 runtime 价值，就应该作为 NCP 的扩展点或内部组件存在。

不能把它继续包在一个 GoUsbAi 私有黑盒里，再从外面用桥接方式消费。

### 3. 允许产品层装配，但不允许产品层重写通用 engine

GoUsbAi 可以在消费方完成自己的 capability composition，但 runtime 核心执行模型、extension mechanism、event semantics 必须属于 NCP。
...
4. 默认 runtime 可以扩展，也可以注入 GoUsbAi 的内部组件
5. GoUsbAi 当前关键产品能力在 NCP-native runtime 中没有被降级或遗漏
...
我们不是要“让 NCP 能调用 GoUsbAi runtime”，而是要“让 GoUsbAi 在选择 NCP 链路时，真正运行在 NCP runtime 之上”。

所以接下来的重点，不再是继续修 bridge，而是把 GoUsbAi 的核心运行时能力重组为 NCP-native building blocks，并让存储层成为唯一保留的一致性边界。
