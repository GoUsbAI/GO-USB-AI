# GoUsbAi UI Installable PWA Shell Design

**Goal:** 为 `packages/go-usb-ai-ui` 定义一套一次性可落地的 PWA 产品与实现方案，让同一套 GoUsbAi 主产品 UI 同时支持三种进入方式：浏览器直接访问、本机 `localhost` 访问后安装为 PWA、部署到服务器后通过 HTTPS 访问并安装为 PWA。目标不是把 GoUsbAi 变成离线优先应用，而是让 UI 更像一个稳定、自然、低门槛的默认入口。

**Architecture:** 采用“同一套 Web UI，增加 installable shell 能力”的路线。PWA 只负责安装入口、窗口化体验、启动元信息、最小静态壳缓存与版本更新协调；业务数据、运行时控制、聊天、配置、远端连接等仍然沿用现有 Web 架构，不为 PWA 再分叉一套产品逻辑。桌面宿主能力继续由 `window.go-usb-aiDesktop` 桥接提供，PWA 不模拟桌面桥接，也不承诺在浏览器沙箱中替代 Electron 或本地宿主管理能力。

**Tech Stack:** Vite、React、TypeScript、`@go-usb-ai/ui`、Web App Manifest、Service Worker、安装提示状态管理、最小离线兜底页、现有 runtime control / desktop bridge / router 结构。

---

## 长期目标对齐 / 可维护性推进

- 这项设计直接服务 `docs/VISION.md` 中“统一入口、统一体验、开箱即用”的目标。GoUsbAi 想成为 AI 时代的个人操作层，就需要让用户更自然地把它当成一个可长期停留在桌面和浏览器启动面的入口，而不是每次都先想“打开哪个网址”或“这是网页还是应用”。
- 这次不是在堆一个孤立前端 feature，而是在补齐 GoUsbAi 主 UI 的“可安装入口形态”：
...
1. GoUsbAi 主产品 UI 能不能被用户自然地安装成一个桌面入口？
...
- `packages/go-usb-ai-ui` 当前是标准 Vite React 应用。
- 入口位于 `packages/go-usb-ai-ui/src/main.tsx`，当前只负责挂载 `ThemeProvider`、`I18nProvider`、`BrowserRouter` 和 `App`。
- `packages/go-usb-ai-ui/index.html` 已有基础 `title`、`viewport`、`favicon`，并已有 `public/logo.svg` 可作为初始图标来源。
- 当前没有 manifest、没有 service worker、没有安装提示、没有 PWA 注册流程。

### 宿主边界现状

- 当前 UI 已通过 `window.go-usb-aiDesktop` 感知 Electron 桌面宿主能力。
- `packages/go-usb-ai-ui/src/runtime-control/runtime-control.manager.ts` 会在存在桌面桥接时将运行环境装饰为 `desktop-embedded`。
...
`packages/go-usb-ai-ui` 的 PWA 应定义为：

**Installable PWA Shell for GoUsbAi UI**

它的职责是：

- 让用户更容易把 GoUsbAi 安装成入口
...
1. 为 `packages/go-usb-ai-ui` 增加 manifest。
2. 为 `packages/go-usb-ai-ui` 增加最小 service worker。
3. 增加安装能力检测与安装引导 UI。
...
- 用户通过本地启动的 GoUsbAi runtime 打开 UI。
- 浏览器访问 `http://localhost:<port>` 或 `http://127.0.0.1:<port>`。
- 用户看到“安装 GoUsbAi”入口。
- 安装后，GoUsbAi 以独立窗口打开，成为本地桌面入口之一。

### 场景 B：远端部署，浏览器访问

- 用户通过 HTTPS 域名访问部署在服务器上的 GoUsbAi UI。
- 用户可继续在浏览器中使用，也可安装为 PWA。
- 安装后，窗口化体验与浏览器访问共享同一账号、同一站点、同一路由。

### 场景 C：桌面宿主中访问 UI

- 当 UI 运行在 Electron 宿主内，现有 `window.go-usb-aiDesktop` 继续作为桌面专属能力来源。
...
新增 `packages/go-usb-ai-ui/public/manifest.webmanifest`。
...
图标应优先复用现有 GoUsbAi 品牌资产，而不是临时生成一套与桌面端不一致的视觉形态。
...
- `packages/go-usb-ai-ui/src/pwa/managers/pwa-install.manager.ts`
...
- 基于 `window.go-usb-aiDesktop` 判断是否压制 PWA 安装入口
...
- 不写“离线可用”。
- 不写“替代桌面应用”。
- 明确表达“安装后可像应用一样从桌面或启动器打开 GoUsbAi”。
...
- 当 `window.go-usb-aiDesktop` 存在时，PWA 安装入口默认应压制为 `suppressed`。
...
- `packages/go-usb-ai-ui/public/manifest.webmanifest`
- `packages/go-usb-ai-ui/public/pwa-192.png`
- `packages/go-usb-ai-ui/public/pwa-512.png`
- `packages/go-usb-ai-ui/public/offline.html`
- `packages/go-usb-ai-ui/src/pwa/managers/pwa-install.manager.ts`
- `packages/go-usb-ai-ui/src/pwa/managers/pwa-runtime.manager.ts`
- `packages/go-usb-ai-ui/src/pwa/components/pwa-install-entry.tsx`
- `packages/go-usb-ai-ui/src/pwa/pwa.types.ts`
- `packages/go-usb-ai-ui/src/pwa/register-pwa.ts`

### 修改文件

- `packages/go-usb-ai-ui/vite.config.ts`
  - 接入 PWA 产物配置或注册构建逻辑。
- `packages/go-usb-ai-ui/index.html`
  - 引入 manifest、theme-color、apple touch meta 等最小头信息。
- `packages/go-usb-ai-ui/src/main.tsx`
  - 增加 PWA 注册入口。
- `packages/go-usb-ai-ui/src/app.tsx`
  - 挂载全局安装提示入口或 provider。
- `packages/go-usb-ai-ui/src/vite-env.d.ts`
...
1. `packages/go-usb-ai-ui` 可被浏览器识别为可安装 PWA。
...
- 文案不承诺离线。
- SW 明确不缓存业务接口。
- 离线 fallback 只表达“应用壳已加载，但需要连接 GoUsbAi 服务”。

### 风险 2：Electron 与 PWA 提示冲突

处理方式：

- 统一由 install manager 基于 `window.go-usb-aiDesktop` 压制安装入口。
...
GoUsbAi 主产品 UI 应该一次性接入 PWA，但必须把它定义成：
