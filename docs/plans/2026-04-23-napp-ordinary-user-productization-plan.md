...
但这还不是普通 GoUsbAi 用户可自然使用的产品体验。普通用户不应该理解 Wasmtime、WASI、JCO、wkg、npm、registry URL 或 `--data` 的细节。用户只应该表达目标：创建自己的小应用、发布到应用商店、安装并运行应用商店里的应用。

## 验收标准

本阶段的验收标准不是“底层命令能跑”，而是 AI 使用 `go-usb-ai-app-runtime` skill 后，普通用户可以不懂底层原理地完成三件事：

1. 开发自己的小应用：用户说“帮我做一个 Todo/记账/清单小应用”，AI 能创建 NApp 目录、安装模板依赖、构建 WASM 后端、运行本地预览，并给用户一个可打开的本地链接。
2. 分发到应用商店：用户说“发布这个应用”，AI 能先检查应用、构建、打包、使用 GoUsbAi 登录态发布到 Apps marketplace，并返回详情页与安装命令。
...
- `@go-usb-ai/app-runtime` 继续作为执行层，负责 create/build/inspect/pack/publish/install/run。
- `skills/go-usb-ai-app-runtime` 作为普通用户入口，负责把用户意图翻译成确定性工作流、检查环境、自动执行命令、解释失败。
- Apps marketplace API 和 `apps.go-usb-ai.io` 继续作为分发层，不新增第二套商店协议。
- GoUsbAi 主产品后续可以把这些命令包成 UI，但本阶段先把 skill + CLI 的可靠闭环做完整。
...
- `skills/go-usb-ai-app-runtime` 更新为普通用户端到端工作流：开发、发布、安装运行、排障。
...
如果没有 GoUsbAi 登录态，AI 明确提示用户先 `go-usb-ai login`，而不是让发布失败变成难懂的 token 错误。

### 安装并运行商店应用

用户只需要说：

> 安装并运行 go-usb-ai.todo。

AI 执行：

```bash
napp install go-usb-ai.todo
napp run go-usb-ai.todo --port 0 --json
```

安装态应用自动使用 `~/.go-usb-ai/apps/data/<app-id>`，普通用户不需要传 `--data`。

## 非目标

- 不在本阶段重做桌面 UI 应用中心。
- 不自动绕过平台登录、发布审核或 marketplace 权限。
- 不把 skill 伪装成 runtime；skill 只负责编排和排障，真正执行仍由 `@go-usb-ai/app-runtime` 完成。
