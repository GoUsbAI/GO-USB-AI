...
**Goal:** 为 GoUsbAi 设计一套世界级、面向用户、可长期扩展、可稳定治理的文档系统，使公开文档站成为用户理解与进入 GoUsbAi 的默认入口，同时把完整命令参考、AI 自管理知识、内部设计沉淀清晰分层，不再互相污染。
...
对 GoUsbAi 来说，“世界级”不是华丽文案，而是下面这些结构性标准同时成立：
...
- `docs/USAGE.md` 与 `packages/go-usb-ai/resources/USAGE.md` 不属于公开站目录结构的一部分，不能反向支配 `apps/docs` 的信息架构。
...
- 远程访问页可出现 `go-usb-ai login` / `remote enable`
...
| AI 自管理完整命令合同 | `docs/USAGE.md` + `packages/go-usb-ai/resources/USAGE.md` | 否 | 公开站不承载完整合同 |
| 运行时内建 guide | `packages/go-usb-ai/resources/USAGE.md` | 否 | 包内事实源 |
...
| 当前页面 | 最终模块 | 页面类型 | 建议动作 |
|---|---|---|---|
| `introduction.md` | 认识 GoUsbAi | Concept | 保留并进一步强化产品定位 |
...
如果要让 GoUsbAi 文档系统达到真正高标准，最终效果应该是：
...
GoUsbAi 的文档系统，不应该继续沿着“页面逐步打补丁”的方式演化。  
它必须被当成产品入口系统来设计。
