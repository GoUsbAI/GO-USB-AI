# GoUsbAi Companion Architecture Design

日期：2026-05-06

## 这份文档解决什么问题

这份文档定义 GoUsbAi Companion 的长期架构。
...
它不要求 server 为它新增专属 API，也不要求 server 新增 `presence` 之类的新公共状态层。Companion 应作为一个上层客户端，复用现有 session / agent / realtime 能力，并通过统一 Client SDK 访问本地或远程 GoUsbAi 服务。

## 产品对齐

Companion 服务 GoUsbAi 的统一入口和自感知连续性，但它不应成为一个孤立功能点。
...
Companion 不直接耦合 server 内部结构，也不自己实现一套 parallel data client。它通过 Client SDK 使用 GoUsbAi 已有 API。

详细设计见 [GoUsbAi Client SDK Design](./2026-05-06-go-usb-ai-client-sdk-design.md)。
...
no session            -> default GoUsbAi or selected Agent avatar
...
`packages/go-usb-ai` 是 CLI 和本地生命周期管理入口。

它负责：

- `go-usb-ai companion start`
- `go-usb-ai companion stop`
- `go-usb-ai companion status`
- 后续 `go-usb-ai companion autostart enable / disable / status / doctor`
- 启动独立 `apps/companion` 产物
- 在需要时提示用户启动或修复 GoUsbAi service
...
`packages/go-usb-ai-client` 是所有上层客户端访问 GoUsbAi 服务的 SDK。
...
`packages/go-usb-ai-server` 继续提供原子领域能力。
...
- 右键菜单：打开 GoUsbAi、切换展示目标、隐藏、刷新、退出
- 拖拽：移动头像并记住位置
...
- Companion 崩溃不影响 GoUsbAi Runtime
- GoUsbAi Runtime 崩溃不导致 Companion 无限重启
