# GoUsbAi App Runtime 第二阶段收尾冻结稿

## 这份文档的作用

这份文档只回答一个问题：

**在已经有 `napp create / inspect / run / dev` 的前提下，第二阶段要一次性补齐哪些剩余能力，才能把 GoUsbAi 微应用从“能开发、能跑”推进到“能打包、能安装、能管理、能分享”。**

这不是第一阶段回顾，也不是开放式 brainstorming 文档。它是第二阶段的收尾冻结稿，后续实现默认以这里的结论为准。

相关文档：

- [GoUsbAi Wasm Apps 方案冻结稿](./2026-04-18-go-usb-ai-wasm-apps-freeze.md)
- [GoUsbAi App Runtime 目录结构设计](./2026-04-18-go-usb-ai-app-runtime-structure-design.md)
- [GoUsbAi App 分发与安装闭环设计稿](./2026-04-19-go-usb-ai-app-distribution-closure-design.md)

## 先明确：当前不缺什么

下面这些已经有了，第二阶段不再重复建设：

- 独立 npm 包：`@go-usb-ai/app-runtime`
- 独立 CLI：`napp`
...
```json
{
  ...
  "apps": {
    "go-usb-ai.starter": {
      ...
      "name": "Starter",
      "activeVersion": "0.1.0",
      "dataDirectory": "/Users/me/.go-usb-ai/apps/data/go-usb-ai.starter",
      "installedVersions": {
        "0.1.0": {
          "version": "0.1.0",
          "installDirectory": "/Users/me/.go-usb-ai/apps/packages/go-usb-ai.starter/0.1.0",
          "sourceKind": "bundle",
...
```bash
napp run go-usb-ai.notes --document notes=/Users/me/Notes
```
