...
**Goal:** 为 GoUsbAi 建立正式的 skill 发布身份体系：管理员可发布官方 scope，普通平台用户登录并设置唯一 username 后可发布个人 scope skill，marketplace 以可治理、可追责、可识别的 scoped name 作为正式唯一标识。

**Architecture:** 以现有 `go-usb-ai-provider-gateway-api` 平台账号系统作为唯一身份源，不再把 marketplace `author` 当成身份。Marketplace skill 目录改为“内部 `id` + owner 用户归属 + 外部 `packageName(@scope/name)` + 发布状态”的组合模型；CLI publish 在调用 worker 前先通过平台账号校验登录态和 username，worker 再依据 bearer token 或 admin token 做最终授权。前端账号面板补齐 username 展示与设置入口，为存量用户提供补录路径。

**Tech Stack:** Cloudflare Workers + Hono + D1 + R2、`go-usb-ai-provider-gateway-api`、`marketplace-api`、`go-usb-ai` CLI、`go-usb-ai-ui` React + Zustand + React Query。

---

## 1. 长期目标对齐 / 可维护性推进

这次改动直接服务 GoUsbAi 作为“统一入口与能力编排层”的长期目标，因为 skill marketplace 不再只是一个可写的数据桶，而会成为可追溯、可治理、可识别的能力注册表。  
如果没有正式身份体系，用户无法自然理解“这是谁发布的、是不是官方、我能不能信任、后续谁维护”，这会削弱 GoUsbAi 作为默认入口的可信度。
...
2. **正式唯一发布名 `packageName`**
   - 形如 `@go-usb-ai/lark-cli`、`@alice/notion-helper`。
...
- 持有 admin 身份可发布官方 scope，例如 `@go-usb-ai/*`
- 可用于官方维护和治理操作
...
- 展示发布者：`GoUsbAi`
- canonical package name：`@go-usb-ai/<skill-name>`
- 默认可直接公开展示与安装
...
1. 用户执行 `go-usb-ai skills publish <dir>`
2. CLI 先检查平台登录态
...
6. 若管理员指定官方发布：
   - 仅管理员可使用 `--scope go-usb-ai` 或同等显式参数
...
- 仅允许小写字母、数字、短横线
- 长度 `3-32`
- 不能以 `go-usb-ai`、`admin`、`official` 等保留字开头或完全等于保留字
...
type MarketplaceSkillRecord = {
  ...
  ownerScope: string;           // go-usb-ai | alice
...
- 读取 GoUsbAi 平台登录 token 的本地存储
- 若不存在，给出 `Please sign in to GoUsbAi account before publishing skills.`
...
- 官方已有条目迁移为 `@go-usb-ai/<legacy-slug>` 或按已有来源映射到官方 scope
...
- 先只把当前仓库维护和官方公开条目统一迁移到 `@go-usb-ai/*`
...
- 若用户执行 `go-usb-ai skills install lark-cli`
  - 优先解析为官方 alias `@go-usb-ai/lark-cli`
...
- Read: `docs/VISION.md`
- Read: `docs/plans/2026-03-21-go-usb-ai-account-and-remote-access-product-design.md`
...
**Files:**
- Modify: `workers/go-usb-ai-provider-gateway-api/migrations/*.sql`
- Modify: `workers/go-usb-ai-provider-gateway-api/src/types/platform.ts`
- Modify: `workers/go-usb-ai-provider-gateway-api/src/repositories/platform-repository.ts`
- Modify: `workers/go-usb-ai-provider-gateway-api/src/services/platform-auth-service.ts`
...
**Files:**
- Modify: `workers/go-usb-ai-provider-gateway-api/src/controllers/auth-controller.ts`
- Modify: `workers/go-usb-ai-provider-gateway-api/src/routes.ts`
- Modify: 相关测试文件
...
- Modify: `packages/go-usb-ai-ui/src/account/stores/account.store.ts`
- Modify: `packages/go-usb-ai-ui/src/api/*auth*`
...
**Files:**
- Modify: `packages/go-usb-ai/src/cli/skills/marketplace.ts`
- Modify: `packages/go-usb-ai/src/cli/skills/marketplace.metadata.ts`
...
**Files:**
- Modify: `packages/go-usb-ai-server/src/ui/marketplace.types.ts`
- Modify: `packages/go-usb-ai-server/src/ui/ui-routes/marketplace/*.ts`
- Modify: `packages/go-usb-ai-ui/src/api/marketplace.ts`
- Modify: `packages/go-usb-ai-ui/src/components/marketplace/*.tsx`
...
- `workers/go-usb-ai-provider-gateway-api`
...
3. build/lint/tsc
4. deploy `go-usb-ai-provider-gateway-api`
