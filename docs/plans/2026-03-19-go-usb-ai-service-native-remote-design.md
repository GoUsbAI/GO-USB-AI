# GoUsbAi Service-Native Remote Access Design

## 1. 背景与判断

当前 `go-usb-ai remote connect` 的实现可以证明 remote access MVP 可用，但它仍然停留在“前台 connector 命令”阶段。这个形态有三个长期问题：

1. 它把 remote 生命周期放在独立前台进程里，天然偏离 `go-usb-ai start/stop/status` 的主服务模型。
2. 它会把远程接入能力逼成第二套运行链路，后续一旦继续堆功能，配置、状态、日志、重连、故障排查都会分叉。
3. 它的用户心智不对。用户要的不是“再开一个 remote 终端”，而是“我的本地 GoUsbAi 服务天然可以被我自己远程接入”。

因此，长期最佳方案不是继续强化 `remote connect` 本身，也不是额外引入新的 remote daemon，而是把 remote connector 直接并入现有 GoUsbAi 主服务，让 remote 成为主服务的一个内建能力。

本方案的核心判断是：

- 业务能力仍然只有一套：本地 GoUsbAi UI / API。
...
- 让用户把 GoUsbAi 当作“自己的私有 AI 工作站”，启动一次本地服务后即可从其他设备访问。
- remote 默认纳入现有 service 生命周期，由 `go-usb-ai start/stop/restart/status` 统一管理。
...
remote connector 作为 GoUsbAi 主服务的一个 capability 挂载到 service host 中，而不是一个长期独立命令。
...
此后用户在平台网页或另一台设备上登录同一账号，就可以看到当前设备在线，并打开这台本地 GoUsbAi。
...
2. 启用后，不需要单独保持一个终端窗口。
3. `go-usb-ai status` 能一眼看到 remote 是否启用、是否已连接、最近错误是什么。
4. 平台页面能显示设备在线状态、最后心跳、设备名、GoUsbAi 版本。
...
- `go-usb-ai status` 和 `go-usb-ai remote status` 都读这份状态
...
1. `go-usb-ai start` 启动本地 service host
...
- `go-usb-ai status` 聚合显示 remote 状态
...
1. 产品心智统一  
用户只需要理解“启动 GoUsbAi 服务后，远程入口也随之可用”。
...
1. 用户执行 `go-usb-ai remote enable && go-usb-ai start` 后，不再需要额外开一个终端保持 remote 在线。
2. `go-usb-ai status` 可以直接看到 remote 连接状态与最近错误。
...
这就是长期最佳路线，也是最能避免 GoUsbAi remote 子系统持续膨胀的路线。
