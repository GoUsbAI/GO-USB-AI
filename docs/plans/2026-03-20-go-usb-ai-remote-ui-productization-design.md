# GoUsbAi Remote Access UI Productization Design
...
4. 打开远程访问开关
5. 点击“启动服务”或“重启服务”
6. 在另一台设备的 GoUsbAi Platform 中进入当前设备
...
- 保持当前已拆出的 `@go-usb-ai/remote` 作为 runtime 内核，不把页面逻辑再耦回去。
...
- UI 不直接 import `@go-usb-ai/remote`
- `@go-usb-ai/remote` 不知道 UI 的存在

也就是说，UI 只调用本地 API；本地 API 再调用 `go-usb-ai` 宿主动作；宿主动作再复用 `@go-usb-ai/remote` runtime。
...
在 `go-usb-ai` 主包里，建议把当前 CLI 命令类继续往下收敛一层，形成统一的宿主动作，例如：
...
2. `go-usb-ai remote` CLI 和 UI 仍然共用同一套宿主逻辑。
...
1. `go-usb-ai` 主包里的 `remote host actions`
2. `go-usb-ai-server` 的 `/api/remote/*`
3. `go-usb-ai-ui` 的独立“远程访问”页面
