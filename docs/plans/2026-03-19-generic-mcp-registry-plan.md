...
GoUsbAi 如何从“已经能接入外部 agent runtime”继续演进到“具备通用 MCP 集成能力”，并且让用户体验尽可能接近：
...
3. native 主链路里的工具能力，当前是通过 `GoUsbAiNcpToolRegistry` 汇总进 `DefaultNcpAgentRuntime`，因此 MCP 的第一个消费点不应被笼统描述为“某个 runtime”，而应更准确地描述为“native 的 tool assembly 边界”。
...
- 也就是 `NcpToolRegistry` / `GoUsbAiNcpToolRegistry` 一侧

原因很简单：

1. `DefaultNcpAgentRuntime` 只认 `NcpToolRegistry`。
2. 当前 native 工具能力已经在 `GoUsbAiNcpToolRegistry` 统一组装。
...
```bash
go-usb-ai mcp add chrome-devtools -- npx chrome-devtools-mcp@latest
go-usb-ai mcp list
go-usb-ai mcp doctor chrome-devtools
```

并且形成下面这条自然路径：

1. 用户通过 CLI 或后续 UI 注册一个 MCP server。
2. GoUsbAi 持久化这份 server 定义。
...
McpRegistry / Lifecycle / Catalog
  -> McpNcpToolAdapter
  -> GoUsbAiNcpToolRegistry (or composite registry)
...
- 但 native 主链路的第一个消费 adapter 不应直接塞进 `packages/go-usb-ai`
...
- `go-usb-ai-core` 应只承载配置 schema、基础类型与纯函数，不应掺入进程/网络生命周期。
- `go-usb-ai` 是 CLI 应用层，不应拥有 MCP 业务核心。
- `go-usb-ai-server` 是服务承载层，不应反向拥有平台级工具治理逻辑。
- `packages/extensions/*` 更适合具体 runtime/channel/plugin 扩展，不适合承载跨 runtime 的平台基础设施。

同时也不建议把 native 的 MCP tool adapter 长期放在：

- `packages/go-usb-ai/src/cli/commands/ncp/*`
...
- `packages/go-usb-ai`
- 后续如有需要，可在 `packages/go-usb-ai-server` 加组合入口

职责：

- 把 CLI 命令接到 `go-usb-ai-mcp`
...
- 它消费 `McpCatalogView`
- 它向 `GoUsbAiNcpToolRegistry` 或其组合注册表注入 MCP tools
...
go-usb-ai-mcp
  -> depends on go-usb-ai-core types/schema only

go-usb-ai-ncp-mcp
  -> depends on go-usb-ai-mcp
  -> depends on @go-usb-ai/ncp

go-usb-ai / go-usb-ai-server / future runtime consumers
  -> depend on go-usb-ai-mcp
```

明确禁止的反向依赖：

- `go-usb-ai-core -> go-usb-ai-mcp`
- `go-usb-ai-mcp -> go-usb-ai-server`
- `go-usb-ai-mcp -> specific runtime package`
- `go-usb-ai-mcp -> go-usb-ai-ncp-toolkit`（除非未来明确决定 MCP 只服务 NCP）

否则后面一定会出现循环依赖和职责混乱。

### 4. 推荐目录结构

`packages/go-usb-ai-mcp` 内部建议不要按技术栈杂糅，而是按领域切层：
...
如果新增 `packages/ncp-packages/go-usb-ai-ncp-mcp`，建议保持极薄：
...
为了减少后续扩散，`go-usb-ai-mcp` 对外只推荐暴露少量稳定接口：
...
- `packages/go-usb-ai-core/src/config/schema.ts`
  - 新增顶层 `mcp` schema
- `packages/go-usb-ai/src/cli/index.ts`
  - 新增 `mcp` 命令组
...
- 各个 UI 页面
- 各个 runtime 包
- `go-usb-ai-server` 的多个 controller
- 各种已有 plugin 模块
...
- `packages/go-usb-ai-core/src/config/schema.ts`
- `packages/go-usb-ai-mcp/src/config/*`
...
- `packages/go-usb-ai-mcp/src/registry/*`
- `packages/go-usb-ai-mcp/src/application/*`
...
- `packages/go-usb-ai/src/cli/index.ts`
- `packages/go-usb-ai/src/cli/commands/mcp/*`

目标：

- 让 `go-usb-ai mcp add/list/remove/enable/disable` 可用
...
- `packages/go-usb-ai-mcp/src/transport/*`
- `packages/go-usb-ai-mcp/src/lifecycle/*`
- `packages/go-usb-ai-mcp/src/doctor/*`
...
```bash
go-usb-ai mcp add <name> -- <command> [args...]
```
...
- transport/lifecycle 复杂度仍留在 `go-usb-ai-mcp`
- native 首期接缝位于 `NcpToolRegistry` / `GoUsbAiNcpToolRegistry` 一侧，而不是 backend/session/router 层
...
4. `go-usb-ai mcp list` 能正确展示 transport、状态与启用态。
5. `go-usb-ai mcp doctor` 能在配置错误、命令不存在、URL 不可达时给出结构化错误。
