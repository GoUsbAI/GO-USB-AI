...
- 应用 UI 仍然是普通 Web 前端
- 由 GoUsbAi 提供固定的 App 容器装载
- UI 不直接访问高权限宿主能力
- UI 只通过 GoUsbAi Host bridge 请求动作
...
`App UI -> GoUsbAi Host -> Host API -> Wasmtime Runner -> Wasm Module`
