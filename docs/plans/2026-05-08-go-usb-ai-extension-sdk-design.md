# GoUsbAi Extension SDK 方案设计

日期：2026-05-08

## 1. 文档目的

本文记录 `@go-usb-ai/extension-sdk` 的当前执行方案。

核心目标是：让被 GoUsbAi 启动的外部进程，通过一套后端 SDK 接入 GoUsbAi。这个 SDK 和 `@go-usb-ai/client-sdk` 保持命名与使用风格一致，但面向的是外部进程，而不是前端 UI。

本文只保留已经确认的设计结论，未进入本方案的内容不写成执行要求。

## 2. 和 Client SDK 的关系

`@go-usb-ai/client-sdk` 和 `@go-usb-ai/extension-sdk` 本质上都是访问 GoUsbAi 能力的 SDK。

共同点：

- 都应该按领域暴露 service。
- 都应该复用 GoUsbAi 共享基础能力，例如 event bus 基础类型。
- 命名风格尽量一致。
- 都不承载 UI、业务编排和具体渠道私有逻辑。
- 都不应该依赖 GoUsbAi 的内核运行控制面。
...
@go-usb-ai/extension-sdk
  面向被 GoUsbAi 启动的外部 server 进程，例如 channel extension server。
```
...
`@go-usb-ai/client-sdk` 和 `@go-usb-ai/extension-sdk` 都不直接依赖 service、server 或 core 内部实现。它们需要的通用 event bus 能力应该来自轻量共享包；各自不通用的事件类型、event keys 和领域 service 由各自 SDK 维护。
...
真实渠道消息
  ...
  -> channel.submitMessage(...) 提交给 GoUsbAi
...
GoUsbAi 不负责理解 Telegram、微信、飞书的私有发送协议。真实发消息给用户的能力属于 channel adapter。

因此 Extension SDK 不提供 `channels.reply()` 这种让 GoUsbAi 代替渠道发消息的 API。
...
```text
server.type = "stdio"
  表示 GoUsbAi 通过本地命令启动这个 extension server。
...
```ts
import { GoUsbAiExtension } from "@go-usb-ai/extension-sdk";
...
Channel 配置沿用 GoUsbAi 现有配置体系，配置值保存在 `config.channels[channelId]`。
...
SDK / GoUsbAi 侧也应尊重 `config.enabled` 状态：channel disabled 后不应继续接收该 channel 的 `submitMessage(...)`，也不应继续向该 channel 派发新的 NCP reply 事件。

## 7. Extension Server 加载与运行

GoUsbAi 主进程负责加载 extension manifest，并按 manifest 中的 `server` 配置启动 extension server。
...
5. 给 extension server 注入 GoUsbAi endpoint、短期 token、extensionId 等连接凭据
...
当前执行方案固定为 HTTP webhook + WebSocket 双通道，并复用 GoUsbAi 已有 server，不额外启动独立 HTTP server。
...
```text
GOUSB_AI_EXTENSION_ID
GOUSB_AI_EXTENSION_ENDPOINT
GOUSB_AI_EXTENSION_TOKEN
```

SDK 使用 `GOUSB_AI_EXTENSION_ENDPOINT + /webhook` 提交消息，使用 `GOUSB_AI_EXTENSION_ENDPOINT + /ws` 监听事件。这里的 `/webhook` 是 GoUsbAi 通用 webhook，不是 extension 专属 endpoint。

SDK 内部使用 `eventBus` 作为通信抽象。第一层 channel API 不暴露 event bus。

## 9. Channel 事件

Channel 应该能接收与自己相关的 GoUsbAi/NCP 事件。
...
- GoUsbAi 主进程通过 `server.command`、`server.args`、`server.env` 启动 extension server。
...
```text
packages/go-usb-ai-shared
  共享基础能力包，只放跨 SDK 和 server 稳定复用的轻量基础设施。

packages/go-usb-ai-extension-sdk
  对外发布的后端 SDK，给 extension server 进程使用。

packages/go-usb-ai-service
  GoUsbAi 常驻服务，负责 extension manifest 发现、生命周期和通用 webhook handler 注册。
...
第一版不新增独立的 `@go-usb-ai/extension-core` 包，也不新增 extension 专属 kernel / runtime 包。公共开发者入口只暴露 `@go-usb-ai/extension-sdk`；共享基础设施放入 `@go-usb-ai/shared`；extension 生命周期归属 GoUsbAi 常驻 service。
...
- `@go-usb-ai/shared` 只放真正跨包稳定复用的轻量基础设施。
...
`@go-usb-ai/client-sdk`、`@go-usb-ai/extension-sdk`、`@go-usb-ai/server`、`@go-usb-ai-service` 都可以依赖 `@go-usb-ai/shared`，但 `@go-usb-ai/shared` 不反向依赖它们。Shared 可以 type-only 复用稳定协议包，例如 `@go-usb-ai/ncp` 的公共 session summary 类型；不能依赖 service / server / SDK / core 内部实现。
...
- SDK 复用 `@go-usb-ai/shared` 的 event bus 基础能力。
- SDK 不依赖 `@go-usb-ai-service`、`@go-usb-ai/server` 内部实现。

### 11.3 Service 生命周期管理

新版 extension server 是 GoUsbAi 常驻 service 的子进程能力，不放进 `go-usb-ai-core`，也不新增 `kernel` / `runtime` 包分层。
...
- `ExtensionManifestDiscoveryService` 发现 `go-usb-ai.extension.json`。
...
`go-usb-ai-server` 只提供通用传输入口，不承载 extension 业务逻辑：
...
```text
packages/extensions/go-usb-ai-channel-plugin-weixin-extension
```
...
- extension 包通过 `@go-usb-ai/extension-sdk` 接入 GoUsbAi。
...
@go-usb-ai/shared
  ...
  允许 type-only 依赖稳定协议包，例如 @go-usb-ai/ncp。
...
@go-usb-ai-service
  -> @go-usb-ai/server
  -> @go-usb-ai/shared
  -> @go-usb-ai/extension-sdk 的协议形态保持一致，但不依赖 SDK 运行时代码
...
```text
@go-usb-ai/shared 不依赖 service / server / SDK。
@go-usb-ai/extension-sdk 不依赖 service / server。
...
- `GoUsbAiExtension` 是 extension server 进程内直接创建的根对象。
...
- `ChannelSubmittedMessage` 表示渠道收到真实用户消息后提交给 GoUsbAi。
- `NcpEndpointEvent` 表示 GoUsbAi/NCP run 产生并需要推给 channel 的原始 NCP 事件；`ncp.event` 不包裹、不重命名、不混入 channel 路由字段。
...
- `channels.reply(...)`。
- `extension.ready()`。
- 独立的 `@go-usb-ai/extension-core` 公共包。
- 完整权限模型。
- 状态上报 API。
- 具体渠道迁移实现细节。
