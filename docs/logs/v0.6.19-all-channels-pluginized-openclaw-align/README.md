...
- 用户要求：GoUsbAi 渠道层对齐 OpenClaw 的插件化思路，避免 `ChannelManager` 内硬编码渠道分支。
- 现状：内置渠道（telegram/discord/slack/...）由 `ChannelManager` 直接 `if` 分支实例化；外部插件渠道走扩展注册，模式不统一。

## 决策

- 做什么：
  - 在 `go-usb-ai-core` 新增“内置渠道插件注册层”，把所有内置渠道都声明为插件对象（id / isEnabled / create）。
...
- 用户可见变化：
  - `go-usb-ai channels status` 现在按内置插件列表输出渠道状态，不再手写固定渠道顺序。
- 关键实现点：
  - 新增内置渠道插件注册：
    - `packages/go-usb-ai-core/src/channels/plugins/types.ts`
  ...
  - CLI 对齐内置插件 ID 作为保留 channel id：
    ...
    - `packages/go-usb-ai/src/cli/commands/channels.ts`
...
TMP_HOME=$(mktemp -d /tmp/go-usb-ai-smoke-pluginized-channels.XXXXXX)
...
- 本次为架构层重构，发布时按常规 GoUsbAi 发布流程执行。
