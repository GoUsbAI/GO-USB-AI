...
- 在 `@go-usb-ai/ncp` 新增 reply tag 清洗积木，统一处理 `[[reply_to_current]]` 与 `[[reply_to:<id>]]`
- 在 NCP 会话状态管理器里接入该积木，确保 streaming、finalize、hydrate 时都不会把 reply tag 当正文展示
- 在 `go-usb-ai` 的 NCP session store / legacy message bridge 中接入相同清洗逻辑，避免 reply tag 被继续写入历史并在刷新后重新泄漏
- 为状态层与 session store 补充回归测试，覆盖“新消息 finalize 清洗”和“历史消息 hydrate 清洗”

## 测试/验证/验收方式

- `pnpm --filter @go-usb-ai/ncp build`
- `pnpm --filter @go-usb-ai/ncp-toolkit exec vitest run src/agent/agent-conversation-state-manager.test.ts`
- `pnpm --filter @go-usb-ai/ncp-toolkit build`
- `pnpm --filter go-usb-ai exec vitest run src/cli/commands/ncp/go-usb-ai-agent-session-store.test.ts`
- `pnpm --filter go-usb-ai build`

## 发布/部署方式

- 本次变更涉及 workspace 包源码，发布时至少需要重新构建并发布受影响的包：`@go-usb-ai/ncp`、`@go-usb-ai/ncp-toolkit`、`go-usb-ai`
