...
- 确认方式：安装实例日志在 `2026-05-22T04:33:31Z` 和重启复现时出现 `this.getByUri is not a function`，紧接着出现 `Extension go-usb-ai-channel-extension-weixin exited`。
- 修复方式：把 `LocalAssetStore` 的公开 API 收敛为箭头 class field，补充方法解构调用回归测试，并按当前治理规则补齐相关文件角色后缀与模块结构配置。

## 测试/验证/验收方式

- `pnpm -C packages/ncp-packages/go-usb-ai-ncp-agent-runtime exec vitest run`
- `pnpm -C packages/ncp-packages/go-usb-ai-ncp-agent-runtime tsc`
- `pnpm -C packages/ncp-packages/go-usb-ai-ncp-agent-runtime lint`
...
- `pnpm lint:new-code:governance -- packages/ncp-packages/go-usb-ai-ncp-agent-runtime`
- `pnpm check:governance-backlog-ratchet`
- 本地安装实例重启后验证 `http://127.0.0.1:55667/api/health` 返回 `ok`，日志确认 `weixin` 扩展启动且没有新的 `getByUri` / 微信扩展退出记录。

## 发布/部署方式

- 已本地构建 `@go-usb-ai/ncp-agent-runtime` dist。
- 已重启本地 GoUsbAi 安装实例，当前服务运行在 `http://127.0.0.1:55667`。
- 未执行远程部署；本次是本机运行时修复。

## 用户/产品视角的验收步骤

- 从微信发送普通消息，应该重新进入 GoUsbAi 微信通道并获得回复。
- 再请求截图或触发附件读取，服务日志不应再出现 `this.getByUri is not a function`。
- `logs/service.log` 中应保留 `Extensions started: ... go-usb-ai-channel-extension-weixin`，且后续无 `Extension go-usb-ai-channel-extension-weixin exited`。
