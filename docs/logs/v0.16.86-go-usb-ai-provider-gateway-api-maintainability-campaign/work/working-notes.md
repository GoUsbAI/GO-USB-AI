...
在 `workers/go-usb-ai-provider-gateway-api` 上完成严格 `package-l1` 结构治理，并把相关治理系统补齐到同一套预期：
...
- 迭代版本：`v0.16.86-go-usb-ai-provider-gateway-api-maintainability-campaign`
- `src/` 根层直接源码文件数：`25 -> 2`
- `workers/go-usb-ai-provider-gateway-api/src/configs`、`src/controllers`、`src/services`、`src/utils` 下的领域子目录已全部清空
...
- `routes` 命名已继续收正为 [`src/routes/app.route.ts`](/Users/peiwang/Projects/nextbot/workers/go-usb-ai-provider-gateway-api/src/routes/app.route.ts)
...
- `pnpm -C workers/go-usb-ai-provider-gateway-api lint`：通过
- `pnpm -C workers/go-usb-ai-provider-gateway-api tsc`：通过
- `pnpm -C workers/go-usb-ai-provider-gateway-api test:quota`：通过，`10/10`
