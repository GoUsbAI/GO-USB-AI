...
- 为本次 marketplace 重试修复创建联动 changeset，覆盖 `@go-usb-ai/server`、`@go-usb-ai/remote`、`go-usb-ai`，保证公开依赖链发布后版本一致。
...
```bash
pnpm -C packages/go-usb-ai exec vitest run src/cli/skills/marketplace.install.test.ts
...
- 真实冒烟：
  - 通过 `packages/go-usb-ai-server` 的 `dist` 路由连续请求真实 marketplace skills 列表与 `lark-cli` 详情，确认多次返回 `200`。
  - 通过 `packages/go-usb-ai/dist/cli/index.js skills install lark-cli --workdir <tmp>` 验证真实安装成功。
  - 通过本地 `POST /api/marketplace/skills/install` 路由验证 UI 安装链路成功落盘。
- 发布结果：
  - `changeset publish` 输出 `packages published successfully`
  - 本地 tag 已生成：`@go-usb-ai/server@0.11.8`、`@go-usb-ai/remote@0.1.60`、`go-usb-ai@0.16.13`
...
3. 在 CLI 执行 `go-usb-ai skills install lark-cli`，确认能稳定安装成功。
