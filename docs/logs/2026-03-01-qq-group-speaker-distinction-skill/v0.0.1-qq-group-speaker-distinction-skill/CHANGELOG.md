...
- 新增内置 skill：`packages/go-usb-ai-core/src/agent/skills/qq-group-speaker-distinction/SKILL.md`。
- skill 明确约束 QQ 群聊场景的正确策略：
  - 群聊会话保持按 `group_id` 共享；
  - 每条入模消息必须携带稳定发言者标识（`user_id`）。
- 更新内置 skills 索引：`packages/go-usb-ai-core/src/agent/skills/README.md`。
