...
- 按“先设计、避免过早实现”的要求，将 `@go-usb-ai/ncp` 中的 `AbstractAgentEndpoint` 从可执行编排实现回收为草案级骨架。
...
1. 打开 `packages/go-usb-ai-ncp/src/endpoint/abstract-agent-endpoint.ts`，确认流程方法为占位骨架而非完整实现。
2. 打开 `packages/go-usb-ai-ncp/src/endpoint/abstract-endpoint.ts`，确认仅保留最小生命周期与订阅基础层。
3. 确认注释为英文，且不包含中文注释。
