...
- 用户期望 `go-usb-ai status` 能直接反映进程运行状态，而不仅是配置摘要。
- 线上排障时需要机器可读输出、脏状态修复能力、以及额外诊断入口。

## 决策

- 将 `go-usb-ai status` 升级为“运行态真相”命令：进程、健康检查、状态一致性、建议动作。
- 增加 `go-usb-ai doctor` 做结构化诊断（状态一致性、端口可用性、provider 就绪度）。
...
# running 场景
go-usb-ai start --ui-port 19997
go-usb-ai status --verbose
# 期望显示 running + managed health ok

# stale 修复场景
go-usb-ai status --fix --json
# stale state 被清理
```
