...
- 在 service 启动链路新增 cron store 文件监听：监听 `~/.go-usb-ai/cron/jobs.json` 的 `add/change/unlink`，触发 `cron.reloadFromStore()`。
...
- `node .agents/skills/post-edit-maintainability-guard/scripts/check-maintainability.mjs --paths packages/go-usb-ai-core/src/cron/service.ts packages/go-usb-ai/src/cli/commands/service-startup-support.ts packages/go-usb-ai/src/cli/commands/service.ts`
- 冒烟脚本（本地）：构造临时 `jobs.json`，调用 `CronService.reloadFromStore()` 后确认 `listJobs(true)` 与文件内容一致。

## 发布/部署方式

- 本次改动涉及 `@go-usb-ai/core` 与 `go-usb-ai`，按既有流程执行：
  - `pnpm changeset`
  - `pnpm release:version`
  - `pnpm release:publish`
- 若仅本地验证，重启 `go-usb-ai serve` 后即可生效。

## 用户/产品视角的验收步骤

- 启动本地 `go-usb-ai serve` 并打开 cron 页面。
