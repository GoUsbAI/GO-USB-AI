...
- 新增独立引擎插件包：`@go-usb-ai/go-usb-ai-engine-claude-agent-sdk`
  - 路径：`packages/extensions/go-usb-ai-engine-plugin-claude-agent-sdk`
  - 结构：TypeScript + `src/` + 独立构建（`tsup`）
  - 引擎种类：`claude-agent-sdk`
- 保持解耦：Claude 相关实现仅在独立插件包内，不耦合进 `core` 与业务逻辑。
- OpenClaw 兼容层增加内置 runtime 插件注入：
  - 在 bundled runtime plugin 列表中加入 `@go-usb-ai/go-usb-ai-engine-claude-agent-sdk`
...
```bash
PATH=/opt/homebrew/bin:$PATH node packages/go-usb-ai/dist/cli/index.js plugins info go-usb-ai-engine-claude-agent-sdk
PATH=/opt/homebrew/bin:$PATH node packages/go-usb-ai/dist/cli/index.js plugins list --json
```

验收点：

- `go-usb-ai-engine-claude-agent-sdk` 显示 `Origin: bundled`。
- `Engines: claude-agent-sdk` 正常出现。
- 当前 engine 总数（含 core）为 3：`native`、`codex-sdk`、`claude-agent-sdk`。

运行链路冒烟（无有效 Claude 凭据场景）：

- 在隔离 `GOUSB_AI_HOME` 中将默认引擎设置为 `claude-agent-sdk`，并设置 `requestTimeoutMs=5000`。
- 调用 `/api/chat/turn`。
