...
- 后续根因补齐：AI 不应靠猜测或 tool schema enum 获得 channel id；新增 `go-usb-ai channels list --json` 作为权威 channel discovery 命令，`message` 只做精确 channel id 的 fail-fast 校验，不做 `wechat -> weixin` alias/normalize 兜底。
...
- `pnpm --filter @go-usb-ai/channel-extension-weixin test -- src/tests/weixin-extension-runtime.test.ts`：通过，覆盖 outbound request 进入 Weixin adapter 并调用 `sendTextMessage`。
- `pnpm --filter @go-usb-ai/service test -- src/shared/services/extensions/service-extension-runtime.service.test.ts`：通过，覆盖 manifest `outbound.text` 到 `extension.request channel.outbound.sendText`。
- `pnpm --filter @go-usb-ai/core test -- src/features/channels/managers/channel.manager.test.ts`：通过，覆盖 generic extension outbound text 与缺失 handler 的显式失败。
- `pnpm --filter @go-usb-ai/channel-extension-weixin tsc`、`pnpm --filter @go-usb-ai/service tsc`、`pnpm --filter @go-usb-ai/core tsc`：通过。
- `pnpm --filter @go-usb-ai/channel-extension-weixin lint`：通过。
- `pnpm --filter @go-usb-ai/channel-extension-feishu test -- src/tests/feishu-extension-runtime.service.test.ts`：通过，覆盖 extension runtime cleanup drain。
- `pnpm --filter @go-usb-ai/channel-extension-feishu tsc`、`pnpm --filter @go-usb-ai/channel-extension-feishu lint`：通过。
- `pnpm --filter @go-usb-ai/service lint`、`pnpm --filter @go-usb-ai/core lint`：命令退出 0，仍有既有 warning，未由本次改动引入。
...
- `pnpm --filter @go-usb-ai/service test -- src/commands/channel/channels.test.ts src/shared/services/extensions/service-extension-runtime.service.test.ts`：通过，覆盖 `channels list --json` JSON 输出、GoUsbAi extension manifest channel discovery、extension runtime manifest discovery。
- `pnpm --filter @go-usb-ai/core test -- src/features/agent/tools/message.tools.test.ts`：通过，覆盖未知显式 channel fail-fast、不 publish outbound、schema 不含 enum/命令耦合。
- `pnpm --filter @go-usb-ai/kernel tsc`、`pnpm --filter go-usb-ai tsc`：通过。
- `pnpm --filter go-usb-ai build`：通过，并同步 `packages/go-usb-ai/resources/USAGE.md`。
- `pnpm -C packages/go-usb-ai exec tsx src/cli/app/index.ts channels list --json` 与 `node packages/go-usb-ai/dist/cli/app/index.js channels list --json`：通过，真实输出包含 `weixin`，`enabled=true`，`outbound.text=true`，`pluginId=go-usb-ai-channel-extension-weixin`。
- 真实 Weixin outbound smoke：通过。使用本机 `~/.go-usb-ai` 中已启用的 Weixin 配置，经 `WeixinExtensionRuntime.sendOutboundText -> WeixinChannelAdapter.sendOutboundText -> sendTextMessage` 向配置中的 allowFrom 目标发送 `GoUsbAi Weixin outbound smoke 2026-05-19T00:55:45.840Z`，命令退出 0，返回 accepted。
...
- 本次未执行发布或部署。
- 需要随下一次包含 `@go-usb-ai/service`、`@go-usb-ai/core`、`@go-usb-ai/channel-extension-weixin` 的统一版本发布进入用户环境。

## 用户/产品视角的验收步骤

- 配置并登录 Weixin channel。
- AI 若不知道精确 channel id，应先运行 `go-usb-ai channels list --json`，从 `channels[].id` 读取 `weixin`，而不是猜 `wechat`。
...
- 机制改进：已补充 `go-usb-ai-clean-implementation` / `predictable-behavior-first`，要求生命周期 owner 遇到多个 `unsubscribeXxx` / `cleanupXxx` 字段时优先使用 cleanup/disposable collection；同时明确上游合同错误不能在下游 alias/normalize/fallback 中静默修正。
...
- 不涉及本轮 NPM 包发布。
- 待后续统一发布评估包：`@go-usb-ai/core`、`@go-usb-ai/service`、`@go-usb-ai/channel-extension-weixin`、`@go-usb-ai/channel-extension-feishu`。
