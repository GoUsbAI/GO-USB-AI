...
- 用户要求：GoUsbAi 的系统提示词除项目特有信息外，其他部分要与 OpenClaw 保持一致，减少行为偏差与踩坑。
- 现状：`go-usb-ai-core` 的系统提示词此前在结构与文案上与 OpenClaw 存在差异（章节顺序、术语、规范句式不一致）。

## 决策

- 对齐目标：以 OpenClaw 当前系统提示词为基线，统一核心章节与规范句式。
- 保留差异：仅保留 GoUsbAi 特有内容（工具名、CLI 命令、工作目录、Self-Management 路径）。
...
- `packages/go-usb-ai-core/src/agent/context.ts`
  ...
  - 文案层面与 OpenClaw保持同语义、同规则表达；仅替换 GoUsbAi 项目专有名词与路径。
...
## 保留差异（判定为 GoUsbAi 特有能力）

- 保留 `go-usb-ai` CLI 子命令与 `USAGE.md` 自管理指引（OpenClaw 对应路径/命令不同）。
- 保留 GoUsbAi 实际工具集合命名（如 `read_file`/`write_file`/`list_dir`）。
...
- `build/lint/tsc` 全部通过。
- 启动网关后，系统提示词章节顺序与 OpenClaw 基线一致（除 GoUsbAi 专有项）。
