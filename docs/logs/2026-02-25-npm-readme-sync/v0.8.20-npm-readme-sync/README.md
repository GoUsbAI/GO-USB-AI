...
- 新增 npm README 源文件：
  - `docs/npm-readmes/go-usb-ai.md`
  - `docs/npm-readmes/go-usb-ai-ui.md`
- 新增 README 同步脚本：
  - `scripts/sync-npm-readmes.mjs`
  - 将源文件同步到发布包目录：
    - `packages/go-usb-ai/README.md`
    - `packages/go-usb-ai-ui/README.md`
...
- 打包冒烟（验证 npm 包内确实包含 README）：
  - `PATH=/opt/homebrew/bin:$PATH npm pack --dry-run --json`（`packages/go-usb-ai`）
  - `PATH=/opt/homebrew/bin:$PATH npm pack --dry-run --json`（`packages/go-usb-ai-ui`）
...
2. 进入 `packages/go-usb-ai` 与 `packages/go-usb-ai-ui`，确认 `README.md` 内容存在且可读。
