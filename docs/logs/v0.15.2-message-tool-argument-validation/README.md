...
- 在 `@go-usb-ai/ncp` 的 tool 接口上补充可选 `validateArgs` 能力，让 runtime 在 schema 校验通过后还能执行工具自定义语义校验。
- `message` 工具现在会统一校验：
  - `content` / `message` 至少有一个非空
  - 当目标 `channel` 与当前会话不同且未显式提供 `to` / `chatId` 时，直接判为无效参数
- `@go-usb-ai/ncp-agent-runtime` 现在会优先输出结构化 `invalid_tool_arguments`，而不是把这类问题留到工具执行阶段才返回普通字符串错误。
- 补充测试：
  - `message` 工具本地参数校验测试
...
- 运行：
  - `pnpm --filter @go-usb-ai/core exec vitest run src/agent/tools/message.test.ts`
  ...
  - `node .agents/skills/post-edit-maintainability-guard/scripts/check-maintainability.mjs --paths packages/go-usb-ai-core/src/agent/tools/base.ts packages/go-usb-ai-core/src/agent/tools/message.ts packages/go-usb-ai-core/src/extensions/tool-adapter.ts packages/ncp-packages/go-usb-ai-ncp/src/agent-runtime/tool.ts packages/ncp-packages/go-usb-ai-ncp-agent-runtime/src/runtime.ts packages/ncp-packages/go-usb-ai-ncp-toolkit/src/agent/in-memory-agent-backend.test.ts`
- 观察点：
