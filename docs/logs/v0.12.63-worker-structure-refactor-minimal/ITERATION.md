...
## 迭代完成说明（改了什么）
- 将 `workers/go-usb-ai-provider-gateway-api` 从单文件入口重构为最小可维护结构：
...
- 线上基础健康检查：
  - `https://ai-gateway-api.go-usb-ai.io/health`
  - `https://ai-gateway-api.go-usb-ai.io/v1/models`
  - `https://platform.go-usb-ai.io`
  - `https://platform-admin.go-usb-ai.io`

## 用户/产品视角的验收步骤
1. 打开 `platform.go-usb-ai.io`，完成用户注册与登录。
2. 在用户侧发起一次充值申请，确认在用户端可见为 `pending`。
3. 打开 `platform-admin.go-usb-ai.io`，管理员登录后在充值审核页通过该申请。
