...
- 修复 `go-usb-ai dev:build serve` 源码运行时对 first-party plugin 的解析逻辑：当未显式设置 `GOUSB_AI_DEV_FIRST_PARTY_PLUGIN_DIR` 时，CLI 现在会自动回退到仓库内 `packages/extensions`，避免 dev 模式继续优先吃到用户 home 下已安装但版本过旧的插件包。
- 这次问题的直接表现是 codex 会话加载到旧版 `go-usb-ai-ncp-runtime-plugin-codex-sdk` 后触发 `skillsLoader.loadSkillsForContext is not a function`，导致对话链路无法产出真实 assistant 内容。
...
- 定向测试：
  - `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai test -- --run src/cli/commands/dev-first-party-plugin-load-paths.test.ts`
- 类型检查：
...
- codex + aixj 实际冒烟：
  - `unset GOUSB_AI_HOME`
  - `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai dev:build serve --ui-port 18796`
...
- 发布后校验：
  ...
  - `PATH=/opt/homebrew/bin:$PATH npm view @go-usb-ai/openclaw-compat version`
  - `PATH=/opt/homebrew/bin:$PATH npm view @go-usb-ai/server version`
  - `PATH=/opt/homebrew/bin:$PATH npm view @go-usb-ai/mcp version`

## 用户/产品视角的验收步骤

1. 在本地已有可用 provider 配置的前提下，执行 `go-usb-ai serve` 或 `go-usb-ai dev:build serve` 启动服务。
