# v0.16.36 go-usb-ai ui installable pwa

## 迭代完成说明

- 为 `packages/go-usb-ai-ui` 接入可安装 PWA 壳能力，让同一套主产品 UI 同时支持：
  - 浏览器直接访问
...
- 产品层面采用“可安装入口壳”而不是“离线优先应用”：
  - 不缓存 `/api`、`/ws` 和业务动态数据
  - 不承诺离线聊天或离线配置
  - 安装后仍连接同一套本地或远端 GoUsbAi 服务
- UI 接入点：
  - [`packages/go-usb-ai-ui/src/app.tsx`](/Users/peiwang/Projects/nextbot/packages/go-usb-ai-ui/src/app.tsx) 启动时注册 PWA，并挂载全局安装/更新提示
  - [`packages/go-usb-ai-ui/src/components/config/RuntimeConfig.tsx`](/Users/peiwang/Projects/nextbot/packages/go-usb-ai-ui/src/components/config/RuntimeConfig.tsx) 增加常驻“安装为应用”卡片
  - [`packages/go-usb-ai-ui/index.html`](/Users/peiwang/Projects/nextbot/packages/go-usb-ai-ui/index.html) 增加 manifest、theme-color、apple touch icon
  ...
  - 后续同批次开发体验修正中，已把 PWA service worker / 安装提示的启用条件从“本地固定端口”收敛为“任意 Vite 开发态禁用、正式构建启用”，并在开发态首屏主动清理残留的 GoUsbAi SW 与缓存，避免 dev HMR websocket、旧缓存和已卸载 PWA 壳互相打架导致白屏或 hook 异常
- 设计文档见：
  - [2026-04-16-go-usb-ai-ui-pwa-installable-shell-design.md](/Users/peiwang/Projects/nextbot/docs/plans/2026-04-16-go-usb-ai-ui-pwa-installable-shell-design.md)

## 测试/验证/验收方式

- 通过：
  - `pnpm -C packages/go-usb-ai-ui tsc`
  - `pnpm -C packages/go-usb-ai-ui test -- --run src/pwa/managers/pwa-install.manager.test.ts src/pwa/components/pwa-install-entry.test.tsx`
  ...
  - `pnpm -C packages/go-usb-ai-ui test -- --run src/app.test.tsx src/components/layout/app-layout.test.tsx`
  - `pnpm -C packages/go-usb-ai-ui build`
  - `pnpm -C packages/go-usb-ai-ui exec eslint public/sw.js src/pwa/pwa.types.ts src/pwa/stores/pwa.store.ts src/pwa/managers/pwa-install.manager.ts src/pwa/managers/pwa-runtime.manager.ts src/pwa/register-pwa.ts src/pwa/components/pwa-install-entry.tsx src/pwa/managers/pwa-install.manager.test.ts src/pwa/components/pwa-install-entry.test.tsx src/lib/i18n.pwa.ts src/lib/i18n.ts src/app.tsx src/components/config/RuntimeConfig.tsx src/components/layout/AppLayout.tsx`
    - 结果：无 error；保留 `RuntimeConfig.tsx` 既有 `max-lines-per-function` warning
- 浏览器冒烟：
  - `pnpm -C packages/go-usb-ai-ui preview --host 127.0.0.1 --port 4174`
...
- 本次顺着“统一入口、统一体验、开箱即用”的长期方向推进了一小步：GoUsbAi UI 现在不仅能在浏览器打开，也能被安装成更接近“默认入口”的应用壳。
...
- 净增：+994 行
- 统计口径：
  - 包含本次实际触达的 `packages/go-usb-ai-ui` 代码、PWA 公共资源和新增测试
  - 不包含方案文档、二进制 PNG 图标
...
- 目录结构与文件组织是否满足当前项目治理要求：
  - 基本满足。本次新增代码集中在 `packages/go-usb-ai-ui/src/pwa/`，但 `packages/go-usb-ai-ui/src/lib` 目录仍然存在历史平铺压力；本次已通过新增 `i18n.pwa.ts` 避免继续把文案塞回超大文件。
