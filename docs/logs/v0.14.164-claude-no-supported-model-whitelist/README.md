...
- Claude 会话现在默认复用 GoUsbAi 全局模型目录；是否真正可用由真实请求链路和上游 provider 返回决定，而不是由 session type 预先收窄。
...
- 后端回归测试：
  - `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai test -- --run src/cli/commands/ncp/claude-session-type-probe-defaults.test.ts src/cli/commands/ncp/create-ui-ncp-agent.claude.test.ts`
- UI 回归测试：
  - `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai-ui test -- --run src/components/chat/ncp/ncp-chat-page-data.test.ts`
- 真实 Claude 服务验证（隔离 `GOUSB_AI_HOME`，复制现有 provider 配置到临时目录）：
  - 服务启动：
    - `GOUSB_AI_HOME=/tmp/go-usb-ai-claude-open.YD9Jzi`
    - `GOUSB_AI_ENABLE_OPENCLAW_PLUGINS=1`
    - `GOUSB_AI_DEV_FIRST_PARTY_PLUGIN_DIR=/Users/peiwang/Projects/nextbot/packages/extensions`
    - `pnpm -C packages/go-usb-ai exec go-usb-ai serve --ui-port 18895`
  - session type 接口：
...
2. 新建一个 `Claude` 会话。
3. 打开模型下拉，确认 Claude 不再只显示插件探测出来的一小部分模型，而是直接显示当前 GoUsbAi 的完整模型目录。
