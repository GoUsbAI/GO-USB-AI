...
- pure-read vs full-load 回归：
  - `PATH=/opt/homebrew/bin:$PATH pnpm --filter @go-usb-ai/openclaw-compat test -- --run src/plugins/status.pure-read.test.ts src/plugins/loader.bundled-enable-state.test.ts src/plugins/loader.ncp-agent-runtime.test.ts`
- server 路由回归：
  - `PATH=/opt/homebrew/bin:$PATH pnpm --filter @go-usb-ai/server test -- --run src/ui/router.marketplace-installed.test.ts src/ui/router.marketplace-content.test.ts src/ui/router.marketplace-manage.test.ts`
- 类型检查：
...
- lint：
  - `PATH=/opt/homebrew/bin:$PATH pnpm --filter @go-usb-ai/openclaw-compat lint`
  - `PATH=/opt/homebrew/bin:$PATH pnpm --filter @go-usb-ai/server lint`
- maintainability：
  - `PATH=/opt/homebrew/bin:$PATH node .codex/skills/post-edit-maintainability-guard/scripts/check-maintainability.mjs --paths packages/go-usb-ai-openclaw-compat/src/plugins/status.ts packages/go-usb-ai-openclaw-compat/src/plugins/status.pure-read.test.ts packages/go-usb-ai-server/src/ui/router/marketplace/installed.ts packages/go-usb-ai-server/src/ui/router/marketplace/plugin.controller.ts packages/go-usb-ai-server/src/ui/router.marketplace-installed.test.ts packages/go-usb-ai-server/src/ui/router.marketplace-content.test.ts`
- 请求级冒烟：
...
1. 启动本地 GoUsbAi 服务并进入插件 marketplace 页面。
