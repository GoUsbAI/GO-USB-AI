...
- 新增独立微信渠道插件包 `@go-usb-ai/channel-plugin-weixin`，目录位于 `packages/extensions/go-usb-ai-channel-plugin-weixin`。
...
- 宿主侧补齐最小通用 contract：
  ...
  - `go-usb-ai channels login` 支持 `--channel/--account/--url/--http-url/--verbose`
  - `message` tool、`cron`、service cron 回放、NCP tool registry、agent loop 透传 `accountId`
  - bundled plugins 中加入 `@go-usb-ai/channel-plugin-weixin`
- 本次实现对应方案文档：
...
- 依赖与锁文件同步：
  - `pnpm install`
- mock 冒烟：
  - 使用隔离目录 `GOUSB_AI_HOME=/tmp/go-usb-ai-weixin-smoke`
  - 启动本地 mock HTTP 服务模拟 `get_bot_qrcode`、`get_qrcode_status`、`getupdates`、`sendmessage`
  - 校验：
    - 插件 `auth.login` 成功返回 `accountId=wx-bot-1`
    - 账号文件成功写入 `/tmp/go-usb-ai-weixin-smoke/channels/weixin/accounts/wx-bot-1.json`
...
- 真实微信验收（2026-03-23）：
  ...
  - 基于真实入站消息 `context_token` 发送回复 `go-usb-ai real reply validation`，用户确认已收到
...
- 后续若发布该能力，按仓库既有 NPM/CLI 发布流程执行，并确保：
  - 根 workspace `build/lint/tsc` 已包含 `@go-usb-ai/channel-plugin-weixin`
  - `pnpm install` 后锁文件与 workspace 依赖一致
  - 发布前至少复跑一次微信 mock 冒烟，确认登录、收消息、回复、主动发消息四段链路仍然通过

## 用户/产品视角的验收步骤

1. 安装含本次改动的 GoUsbAi 版本。
2. 运行 `go-usb-ai channels login --channel weixin`，确认命中插件登录流程而不是旧 bridge。
