...
- 新增 Docker 专用官网一键脚本：
  ...
  - 支持命令：`curl -fsSL https://go-usb-ai.io/install-docker.sh | bash`
...
- 脚本 dry-run（无需本机 Docker）：
  - `curl -fsSL https://go-usb-ai.io/install-docker.sh | bash -s -- --dry-run`
- 官网发布链路：
...
1. 在安装了 Docker 的机器执行：`curl -fsSL https://go-usb-ai.io/install-docker.sh | bash`
2. 终端应输出：
   - `UI: http://127.0.0.1:<ui-port>`
   - `API: http://127.0.0.1:<ui-port>/api`
3. 浏览器打开 UI URL，进入 GoUsbAi 页面。
