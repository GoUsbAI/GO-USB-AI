...
- 在 `@go-usb-ai/ncp-react` 中新增附件读取与请求组装能力，统一限制为 `png/jpeg/webp/gif` 且单图不超过 10 MB，避免前端“看似可发、实际模型不可用”的不确定行为。
- 在 `@go-usb-ai/ncp-agent-runtime` 中把用户消息里的图片 `file` part 转成 OpenAI 兼容的 `image_url` multimodal content，让运行时真正把图片喂给模型。
- 在 `@go-usb-ai/agent-chat-ui`、`@go-usb-ai/ui`、`@go-usb-ai/ncp-react-ui` 与 `apps/ncp-demo` 中补齐附件状态、输入区 token、消息图片渲染、失败恢复、真实浏览器冒烟与本地 demo 接入。
- 为本次发包生成并发布了联动版本：
  - `@go-usb-ai/ncp-agent-runtime@0.2.3`
  - `@go-usb-ai/ncp-react@0.3.4`
  - `@go-usb-ai/ncp-react-ui@0.2.3`
...
- 包级验证：
  - `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/ncp-packages/go-usb-ai-ncp-agent-runtime lint`
  - `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/ncp-packages/go-usb-ai-ncp-agent-runtime tsc`
  - `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/ncp-packages/go-usb-ai-ncp-agent-runtime build`
  - `PATH=/opt/homebrew/bin:$PATH packages/go-usb-ai-ui/node_modules/.bin/vitest run packages/ncp-packages/go-usb-ai-ncp-agent-runtime/src/context-builder.test.ts`
...
- 发布后注册表核验：
  ...
  - `PATH=/opt/homebrew/bin:$PATH npm view @go-usb-ai/mcp version`
  - `PATH=/opt/homebrew/bin:$PATH npm view @go-usb-ai/server version`
  - `PATH=/opt/homebrew/bin:$PATH npm view @go-usb-ai/ui version`
  - `PATH=/opt/homebrew/bin:$PATH npm view @go-usb-ai/ncp-react version`
  - `PATH=/opt/homebrew/bin:$PATH npm view @go-usb-ai/ncp-react-ui version`
  - `PATH=/opt/homebrew/bin:$PATH npm view @go-usb-ai/ncp-agent-runtime version`
  - `PATH=/opt/homebrew/bin:$PATH npm view @go-usb-ai/agent-chat-ui version`

# 发布/部署方式

- 按项目 NPM 流程执行两轮发布闭环：
  1. 新建 changeset，覆盖 `@go-usb-ai/agent-chat-ui`、`@go-usb-ai/ncp-agent-runtime`、`@go-usb-ai/ncp-react`、`@go-usb-ai/ncp-react-ui`、`@go-usb-ai/ui`、`@go-usb-ai/mcp`、`@go-usb-ai/server`、`go-usb-ai`
...
- 第一轮发布时发现 `go-usb-ai@0.15.4` 已在 npm 存在，CLI 包被跳过；因此追加第二个 changeset，仅对 `@go-usb-ai/mcp`、`@go-usb-ai/server`、`go-usb-ai` 再做一次 patch，最终将 CLI 正式发布为 `go-usb-ai@0.15.5`。
- 发布结果：
  - 第一轮成功发布：`@go-usb-ai/ncp-agent-runtime@0.2.3`、`@go-usb-ai/ncp-react@0.3.4`、`@go-usb-ai/ncp-react-ui@0.2.3`、`@go-usb-ai/agent-chat-ui@0.2.3`、`@go-usb-ai/ui@0.10.1`
  - 第二轮成功补发：`@go-usb-ai/mcp@0.1.43`、`@go-usb-ai/server@0.10.47`、`go-usb-ai@0.15.5`
- Git tag 已落地：
  - `@go-usb-ai/ncp-agent-runtime@0.2.3`
  - `@go-usb-ai/ncp-react@0.3.4`
  - `@go-usb-ai/ncp-react-ui@0.2.3`
...
6. 安装或升级到 `go-usb-ai@0.15.5` 后打开内置 UI，重复以上步骤，确认 CLI 打包的 `ui-dist` 也具备同样能力。
