...
- 修复运行时只感知图片、不感知 `json/txt/md/csv/xml/源码` 等 text-like 文件的问题：`DefaultNcpContextBuilder` 与 GoUsbAi NCP 上下文链现在都会把这些附件注入模型上下文。
...
- 类型检查：
  - `PATH=/opt/homebrew/bin:$PATH pnpm --filter @go-usb-ai/ncp-agent-runtime tsc`
...
- 定向测试：
  - `PATH=/opt/homebrew/bin:$PATH pnpm --filter @go-usb-ai/ncp-agent-runtime exec vitest run src/context-builder.test.ts src/attachment-store.test.ts`
  - `PATH=/opt/homebrew/bin:$PATH pnpm --filter go-usb-ai exec vitest run src/cli/commands/ncp/go-usb-ai-ncp-context-builder.test.ts`
  - `PATH=/opt/homebrew/bin:$PATH pnpm --filter @go-usb-ai/server exec vitest run src/ui/router.ncp-agent.test.ts`
  - `PATH=/opt/homebrew/bin:$PATH pnpm --filter @go-usb-ai/ui exec vitest run src/components/chat/chat-composer-state.test.ts src/components/chat/adapters/chat-message.adapter.test.ts src/components/chat/ncp/ncp-session-adapter.test.ts`
  - `PATH=/opt/homebrew/bin:$PATH pnpm --filter @go-usb-ai/agent-chat-ui exec vitest run src/components/chat/ui/chat-input-bar/chat-input-bar.test.tsx`
- 冒烟说明：
...
- 若需要对前端或 CLI 发版，按项目既有 release 流程执行；本次新增的附件目录默认位于 `GOUSB_AI_HOME` 对应数据目录下的 `attachments/`。

## 用户/产品视角的验收步骤

1. 在 GoUsbAi 聊天输入框中上传一个 `json` 或 `txt` 文件，确认输入框里出现文件 tag，且光标停在 tag 后面。
