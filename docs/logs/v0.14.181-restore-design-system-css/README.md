...
- 恢复被误删的 `packages/go-usb-ai-ui/src/styles/design-system.css`，修复 `packages/go-usb-ai-ui/src/index.css` 中样式入口引用失效导致的前端启动失败问题。
- 确认本次修复不引入新的样式入口改动，保持现有前端主题与设计令牌来源不变。

## 测试/验证/验收方式

- 执行 `pnpm --filter @go-usb-ai/ui build`
