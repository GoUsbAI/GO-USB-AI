...
- 定向 lint：
  - `PATH=/opt/homebrew/bin:$PATH pnpm exec eslint $(git show --name-only --format='' e2c6cc00 -- packages/extensions/go-usb-ai-channel-plugin-feishu | rg '\.ts$')`
  - 结果：本次飞书能力同步文件 `0 error`，仅保留复杂度类 warning。
- 新增测试：
  - `PATH=/opt/homebrew/bin:$PATH packages/go-usb-ai-openclaw-compat/node_modules/.bin/vitest run packages/extensions/go-usb-ai-channel-plugin-feishu/src/tools-config.test.ts packages/extensions/go-usb-ai-channel-plugin-feishu/src/tool-account-routing.test.ts`
  - 结果：`2 files / 9 tests passed`。
- 插件装载冒烟：
  - 使用 `tsx` 直接导入 `packages/extensions/go-usb-ai-channel-plugin-feishu/index.ts`，以最小多账号配置执行 `plugin.register(api)`。
...
- maintainability guard：
  - `PATH=/opt/homebrew/bin:$PATH node .codex/skills/post-edit-maintainability-guard/scripts/check-maintainability.mjs --paths packages/extensions/go-usb-ai-channel-plugin-feishu/src/calendar-shared.ts packages/extensions/go-usb-ai-channel-plugin-feishu/src/sheets.ts packages/extensions/go-usb-ai-channel-plugin-feishu/src/sheets-shared.ts packages/extensions/go-usb-ai-channel-plugin-feishu/src/task-tasklist.ts`
...
1. 安装或升级到本次发布后的 `go-usb-ai@0.15.2`，并确保内置飞书插件版本为 `@go-usb-ai/channel-plugin-feishu@0.2.19`。
