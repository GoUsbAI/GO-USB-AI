...
- `workers/go-usb-ai-provider-gateway-api` 现在会对 remote access 的 HTML 导航响应统一追加 `Cache-Control: private, no-store, max-age=0, must-revalidate`，并补 `Vary: Cookie`，避免不同实例/不同 session 之间复用错误的 `index.html`。
...
- 可维护性闸门：
  - `PATH=/opt/homebrew/bin:$PATH node .codex/skills/post-edit-maintainability-guard/scripts/check-maintainability.mjs --paths workers/go-usb-ai-provider-gateway-api/src/controllers/remote-controller.ts`
...
2. 打开 `https://platform.go-usb-ai.io`，进入实例列表。
3. 点击“用固定域名打开”。
