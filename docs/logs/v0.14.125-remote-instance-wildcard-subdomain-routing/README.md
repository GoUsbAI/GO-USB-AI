...
- `workers/go-usb-ai-provider-gateway-api/wrangler.toml` 改为通过 Worker route `*.claw.cool/*` 接管实例访问流量，同时保留 `ai-gateway-api.go-usb-ai.io` 作为纯 API 域名。
- `workers/go-usb-ai-provider-gateway-api/src/services/remote-access-service.ts` 现在按访问会话生成实例级 host，并仅从 `r-<sessionId>.claw.cool` 解析访问会话，不再把 API 域名或单一分享入口域名混入远程页面访问。
- `scripts/remote-relay-hibernation-smoke.mjs` 新增域名形态断言，强制校验 owner open / share open 返回的 `openUrl` 都必须是 `r-<session>.claw.cool/platform/remote/open?token=...`，且分享链接仍然保持在 `platform.go-usb-ai.io/share/<grantToken>`。
...
- Worker 已执行 `pnpm -C workers/go-usb-ai-provider-gateway-api run deploy`
...
2. 打开 `platform.go-usb-ai.io`，在实例列表点击打开；浏览器应跳到 `https://r-<access-session-id>.claw.cool/...`。
3. 在同一浏览器内继续打开另一个远程实例；两个实例应各自落在不同子域名，而不是共用一个远程访问 host。
4. 创建分享链接，复制得到的链接应仍然是 `https://platform.go-usb-ai.io/share/<grantToken>`。
