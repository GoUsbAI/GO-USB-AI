...
- 本次发布将 `go-usb-ai` 从 `0.19.20` 提升到 `0.19.21`，并同步提升相关 workspace 包 patch 版本。
- 发布过程中 `@go-usb-ai/channel-extension-qq@0.1.1` 首次 registry verify 出现短暂可见性延迟；后续确认 package access 为 public、dist-tag 为 `latest: 0.1.1`，并补跑验证通过。
...
- `npm view go-usb-ai version`：返回 `0.19.21`。
- `npm dist-tag ls @go-usb-ai/channel-extension-qq`：返回 `latest: 0.1.1`。
...
- 用户可通过 `npm install -g go-usb-ai@latest` 获取 `go-usb-ai@0.19.21`。
- 运行 `go-usb-ai --version` 应显示 `0.19.21`。
- QQ channel extension 已作为独立 NPM 包发布为 `@go-usb-ai/channel-extension-qq@0.1.1`。
...
- 核心版本：
  ...
  - `@go-usb-ai/ui@0.12.29`
  - `@go-usb-ai/channel-extension-qq@0.1.1`
- 发布状态：已发布到 npm registry，并通过 `pnpm release:verify:published` 确认 47/47 package versions。
- 外部阻塞：首次发布前 npm token 状态判断有误；重试后发布成功。`@go-usb-ai/channel-extension-qq` 出现短暂 registry verify 延迟，补跑验证后关闭。
