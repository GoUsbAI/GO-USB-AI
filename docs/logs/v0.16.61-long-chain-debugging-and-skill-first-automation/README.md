...
- 新增项目私有 skill：
  - [`go-usb-ai-clean-implementation`](../../../.agents/skills/go-usb-ai-clean-implementation/SKILL.md)
  - 配套 UI 元信息：[agents/openai.yaml](../../../.agents/skills/go-usb-ai-clean-implementation/agents/openai.yaml)
...
- 在 [`AGENTS.md`](../../../AGENTS.md) 的迭代制度与复杂任务规则中，新增并固化了 `work/` 工作目录机制：
  ...
  - 禁止仅为了记笔记而提前新建新的迭代目录
- 在 [`AGENTS.md`](../../../AGENTS.md) 的 `Project Rulebook` 中，新增并接入了 `go-usb-ai-clean-implementation-required`：
  - 代码、脚本、测试和运行链路配置在真正动手前，必须先自动加载 `go-usb-ai-clean-implementation`
...
- 仓库原本就有 review 机制，这次没有另造一套新体系，而是直接升级已有入口：
  ...
  - [`go-usb-ai-clean-implementation`](../../../.agents/skills/go-usb-ai-clean-implementation/SKILL.md) 也同步前移了这条门槛，要求在动手前就说明如何保证 `非测试代码净增 <= 0`
- 这次又继续向前推进了一层自动化落地：
...
- 新增 `go-usb-ai-clean-implementation` skill，用来在真正写实现前先拦住“垃圾代码”：
  - 先判断能删什么，而不是直接加代码
...
- 已执行：
  - `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai-core exec tsx -e "import { resolve } from 'node:path'; import { SkillsLoader } from './src/index.ts'; const repo = resolve(process.cwd(), '../..'); const loader = new SkillsLoader({ workspace: repo, projectRoot: repo }); const skill = loader.listSkills(false).find((entry) => entry.name === 'go-usb-ai-clean-implementation'); console.log(skill ? JSON.stringify({ name: skill.name, source: skill.source, path: skill.path }, null, 2) : 'MISSING');"`
  - 结果：输出 `name = go-usb-ai-clean-implementation`、`source = project`，说明新的代码质量 skill 可被发现。
- 已执行：
  - `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai-core exec tsx -e "import { resolve } from 'node:path'; import { existsSync } from 'node:fs'; import { SkillsLoader } from './src/index.ts'; const repo = resolve(process.cwd(), '../..'); const loader = new SkillsLoader({ workspace: repo, projectRoot: repo }); const skill = loader.getSkillInfo('go-usb-ai-clean-implementation'); const metadata = skill ? loader.getSkillMetadata(skill) : null; console.log(JSON.stringify({ found: Boolean(skill), source: skill?.source ?? null, name: metadata?.name ?? null, hasDescription: Boolean(metadata?.description), hasOpenAiYaml: existsSync(resolve(repo, '.agents/skills/go-usb-ai-clean-implementation/agents/openai.yaml')) }, null, 2));"`
  - 结果：输出 `found = true`、`source = project`、`name = go-usb-ai-clean-implementation`、`hasDescription = true`、`hasOpenAiYaml = true`，说明 frontmatter 可被读取，且配套 UI 元信息文件存在。
- 已执行：
...
- 已执行：
  - `rg -n "非测试代码净增|净增 <= 0|判失败|需继续修改" AGENTS.md .agents/skills/code-review/SKILL.md .agents/skills/post-edit-maintainability-review/SKILL.md .agents/skills/go-usb-ai-clean-implementation/SKILL.md`
  - 结果：命中 `AGENTS.md`、`code-review`、`post-edit-maintainability-review` 与 `go-usb-ai-clean-implementation`，说明“纯非功能改动非测试代码净增即失败”的硬门槛已经同时接入前置 skill、评审 skill、可维护性复核与验证规则。
- 已执行：
...
- 已执行：
  - `git diff --check -- AGENTS.md .agents/skills/code-review/SKILL.md .agents/skills/post-edit-maintainability-review/SKILL.md .agents/skills/go-usb-ai-clean-implementation/SKILL.md docs/logs/v0.16.61-long-chain-debugging-and-skill-first-automation/README.md docs/logs/v0.16.61-long-chain-debugging-and-skill-first-automation/work/working-notes.md`
...
5. 打开 [`go-usb-ai-clean-implementation/SKILL.md`](../../../.agents/skills/go-usb-ai-clean-implementation/SKILL.md)，确认它不是泛泛谈“代码质量”，而是明确要求先判断：`能删什么`、`owner 是谁`、`主路径是什么`、`为什么这不是隐藏 fallback`、`文件为什么放在这里`、`最小可信验证是什么`。
6. 打开 [`AGENTS.md`](../../../AGENTS.md)，确认：
  ...
  - `Project Rulebook` 中已经存在 `go-usb-ai-clean-implementation-required`
...
本次顺着“让 GoUsbAi 逐步具备可积累、可复用、可自进化的问题解决能力”的长期方向继续推进了一小步。它不再允许纯 bugfix / 纯重构类改动靠“解释为什么净增也合理”来过关，而且现在连 `post-edit-maintainability-guard` 都有了显式 `--non-feature` 自动 gate，不再只停留在文档约束。
...
- 抽象、模块边界、class / helper / service / store 等职责划分是否更合适、更清晰，是否避免了过度抽象或补丁式叠加：是。现在边界被进一步拆清：`go-usb-ai-clean-implementation` 负责写代码前的垃圾代码风险自检，`code-review` 负责 findings-first 评审，`post-edit-maintainability-review` 负责最终主观可维护性复核，`post-edit-maintainability-guard --non-feature` 负责自动阻断，`/validate` 负责最终 gate；它们围绕同一硬规则协同，而不是各说各话。
