...
- 为 `@go-usb-ai/core`、`@go-usb-ai/runtime`、`@go-usb-ai/server`、`@go-usb-ai/openclaw-compat`、`@go-usb-ai/channel-runtime` 增加 `exports["."].development -> ./src/index.ts`，使本地开发优先解析源码入口。
- 更新 `scripts/dev-runner.mjs`，为后端 `tsx watch` 进程注入 `NODE_OPTIONS=--conditions=development`，让 Node ESM 解析命中开发态条件导出。
- 补齐 `packages/go-usb-ai-server/tsconfig.json` 与 `packages/go-usb-ai/tsconfig.json` 的本地路径映射，覆盖 `@go-usb-ai/openclaw-compat` 与 `@go-usb-ai/channel-runtime`，避免未预构建时 `tsc` 解析失败。
...
- 开发入口快速校验：
  - 在 `packages/go-usb-ai` 下执行 `$env:NODE_OPTIONS='--conditions=development'; .\node_modules\.bin\tsx.cmd --tsconfig tsconfig.json src/cli/index.ts --version`
- 冒烟测试：
  - 执行根目录 `pnpm run dev`
  - 观察点：不再出现 `ERR_MODULE_NOT_FOUND: ... @go-usb-ai/channel-runtime/dist/index.js`
...
3. 确认启动过程中不再出现 `Cannot find module ... @go-usb-ai/channel-runtime/dist/index.js`。
