...
- shared `@go-usb-ai/agent-chat-ui` 更新工具卡片与文件卡片表现，移除已废弃的 call-id / input-label 展示，统一当前工具生命周期 UI。
...
- Prompt 契约：
  - `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai-core exec vitest run src/agent/runtime-user-prompt.test.ts`
- Agent chat UI：
...
1. 启动带 Claude runtime 的 GoUsbAi 服务。
