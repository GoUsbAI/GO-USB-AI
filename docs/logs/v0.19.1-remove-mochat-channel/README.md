...
- 移除旧 `go-usb-ai-channel-plugin-mochat` 包、runtime channel 实现、builtin channel 注册、配置 schema/label/help、UI 表单和 logo、desktop 依赖、openclaw compat bundled 插件声明、marketplace seed、README/USAGE/PRD/current feature 文档中的当前能力表述。
- 将 `packages/go-usb-ai-runtime/src/channels/builtin.ts` 收敛为 `builtin-channel.config.ts`，避免本次触达后继续违反 role-boundary 命名治理。
...
- `pnpm -C packages/extensions/go-usb-ai-channel-runtime tsc`：通过。
- `pnpm -C packages/go-usb-ai-runtime tsc`：通过。
- `pnpm -C packages/go-usb-ai-openclaw-compat tsc`：通过。
...
- `pnpm -C packages/go-usb-ai-service test -- src/commands/channel/channels.test.ts src/commands/channel/builtin-channels.test.ts --run`：通过，2 个测试文件、5 个用例。
- `pnpm -C packages/go-usb-ai-runtime lint`、`apps/desktop lint`、`apps/landing lint`、`packages/go-usb-ai lint`、`workers/marketplace-api lint`：通过；landing 仍有既有长文件 warning。
- `git diff --check`：通过。
...
- `pnpm -C packages/go-usb-ai-ui lint`：失败在既有 chat/doc-browser/system-status 等无关 lint error，与 Mochat 删除路径无交集。
...
- 搜索活跃代码、包依赖、锁文件和文件路径，不再能找到 `go-usb-ai-channel-plugin-mochat` 或 Mochat 运行时代码。
...
### packages/go-usb-ai-service/src/commands/channel/index.ts
...
不涉及 NPM 包发布。删除影响多个 workspace 包，但本轮未执行 release；如果进入正式发布，需要统一评估 `go-usb-ai`、`@go-usb-ai/runtime`、`@go-usb-ai/core`、`@go-usb-ai/service`、`@go-usb-ai/ui`、desktop 相关包和 extension runtime 的版本发布链路。
