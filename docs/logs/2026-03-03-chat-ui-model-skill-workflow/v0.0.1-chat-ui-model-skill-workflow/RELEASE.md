...
- 结论：
  ...
  - 后端部署：需要（`go-usb-ai-server` UI 路由返回结构有新增字段）
  - 前端部署：需要（`go-usb-ai-ui`）
...
- 发布 `packages/go-usb-ai-server` 对应运行实例。

3. 部署前端静态资源
- 发布 `packages/go-usb-ai-ui/dist`。
...
## 回滚说明
- 前端回滚到上一个静态资源版本。
- 后端回滚到上一个 `go-usb-ai-server` 版本（移除新增字段不会影响旧前端兼容）。
