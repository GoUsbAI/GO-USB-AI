...
- ESLint（受影响文件）：
  - `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai-ui exec eslint src/components/chat/chat-merged-events.ts src/components/chat/chat-page-runtime.ts`
- UI 构建：
