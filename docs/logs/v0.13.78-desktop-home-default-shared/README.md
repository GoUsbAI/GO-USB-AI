...
- 在 `apps/desktop/src/main.ts` 移除开发态自动写入隔离 `GOUSB_AI_HOME` 的逻辑。
- 调整后行为：
  - 开发态与生产态在“未显式设置 `GOUSB_AI_HOME`”时，默认共享同一 `~/.go-usb-ai`。
  - 若需要隔离，仍可手动通过环境变量显式设置不同的 `GOUSB_AI_HOME`。
...
- 关键行为检查：
  - 检查 `apps/desktop/src/main.ts` 中不再存在开发态默认 `process.env.GOUSB_AI_HOME = ...` 赋值逻辑。
...
1. 不设置 `GOUSB_AI_HOME`，先启动生产态服务并确认可用。
2. 在同机启动开发态 Desktop（`pnpm -C apps/desktop dev`）。
3. 验证两者默认读取同一套 `~/.go-usb-ai` 配置与状态。
4. 如需并行隔离测试，手动为开发态设置单独 `GOUSB_AI_HOME`，验证可切换到隔离模式。
