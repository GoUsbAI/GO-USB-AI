...
- 本地把 `@go-usb-ai/channel-plugin-weixin` 从 `0.1.11` 升到 `0.1.12`
- 但 npm 上已经存在旧的 `0.1.12`
- 结果 `changeset publish` 会跳过该包发布
- 而 `@go-usb-ai/openclaw-compat` / `@go-usb-ai/server` / `go-usb-ai` 又会依赖这个旧的 `0.1.12`
...
- 核实 npm 上已有的 `@go-usb-ai/channel-plugin-weixin@0.1.12` 缺少新的 self-notify route hint
