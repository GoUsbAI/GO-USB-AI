...
- 将 Remote Access 的平台登录改成浏览器授权流：
  ...
  - 本地 UI 通过 `/api/remote/auth/poll` 自动轮询并写回 `providers.go-usb-ai.apiKey`
...
- 定向测试
  - `pnpm -C packages/go-usb-ai-server exec vitest run src/ui/router.remote.test.ts`
...
- 真实端到端冒烟
  - 在隔离目录 `GOUSB_AI_HOME=/tmp/go-usb-ai-remote-auth-smoke` 启动本地 `go-usb-ai serve --ui-port 18991`
  - 调用本地 `/api/remote/auth/start`
  - 通过线上 `https://ai-gateway-api.go-usb-ai.io/platform/auth/browser/authorize` 完成一次注册授权
...
1. 打开本地 GoUsbAi UI 的 `设置 -> 远程访问`。
2. 在 `平台账号` 区域点击 `前往浏览器授权`。
3. 浏览器打开 GoUsbAi 平台授权页，在网页里登录或注册。
