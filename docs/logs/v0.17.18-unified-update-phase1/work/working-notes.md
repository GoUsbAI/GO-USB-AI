...
- 已有成功路径验证：`go-usb-ai@0.18.12-beta.2 -> check beta.3 -> download -> apply -> 新进程版本 0.18.12-beta.3` 已跑通。
- 已有开发态误报“更新异常”问题已单独修复；那是 `pnpm dev start` 场景，不等于当前“已发布 beta 打开即失败”。
- 已完成真实问题复现：隔离安装 `go-usb-ai@0.18.12-beta.3` 并启动 `serve` 后，`GET /api/runtime/update` 返回 `channel=stable`、`status=failed`、`errorMessage=runtime update manifest request failed with status 404`。
- 当前统一更新接口面固定为：
...
- NPM managed service 自动更新入口 owner 当前在 `packages/go-usb-ai/src/cli/shared/services/ui/npm-runtime-update-host.service.ts`。
...
- UI 只作为映射层，相关标签来自 `packages/go-usb-ai-ui/src/shared/lib/i18n/desktop-update-labels.utils.ts`。
- 现有手动验收入口：`packages/go-usb-ai/scripts/smoke-npm-runtime-update.mjs`，但它当前只覆盖 CLI 更新语义，还没有覆盖“已发布 beta 启动后 API 自动状态”。
- 公网更新源现场证据：
  - `beta` manifest 存在。
...
- 真实 npm beta 用户修复已落地到 `go-usb-ai@0.18.12-beta.4`，且新的 runtime beta channel 已发布完成。
...
- 真实接口层复现：
  1. 隔离安装公开 `go-usb-ai@0.18.12-beta.4`。
  2. 启动前台 `go-usb-ai serve --ui-port 18890`。
...
- 定向测试通过：
  - `pnpm -C packages/go-usb-ai test src/cli/shared/services/ui/tests/npm-runtime-update-host.service.test.ts src/cli/shared/services/ui/tests/service-ui-hosts.service.test.ts`
- `pnpm -C packages/go-usb-ai tsc`：通过。
- `pnpm -C packages/go-usb-ai build`：通过。
- 本地签名更新源下的真实接口 smoke：
  - 启动 `node packages/go-usb-ai/dist/cli/app/index.js serve --ui-port 18892`
  ...
  - 返回体保留 `recoveryCommand="Restart this GoUsbAi process to launch the downloaded runtime."`
...
## 追加记录：`go-usb-ai restart` 误判前台 serve 为 unmanaged instance

### 新复现

- 用户在全局安装 `go-usb-ai@0.18.12-beta.5` 后执行 `go-usb-ai restart`，CLI 报错：
  - 目标端口已有健康实例
  - 但该实例“不受 `/Users/peiwang/.go-usb-ai/run/service.json` 跟踪”
  - 因而被判定为 unmanaged listener
- 现场核对：
  - `lsof -nP -iTCP:55667 -sTCP:LISTEN` 命中的是 `node .../lib/node_modules/go-usb-ai/dist/cli/app/index.js serve`
  - 也就是说，占端口的其实是我们自己的前台 `go-usb-ai serve`，不是 Docker 或别的外部服务
...
- 定向测试：
  - `pnpm -C packages/go-usb-ai test tests/cli/restart-commands.test.ts`
- `pnpm -C packages/go-usb-ai tsc`
- 真实前台重启 smoke：
  1. 隔离 `GOUSB_AI_HOME=/tmp/go-usb-ai-restart-verify-home`
...
- 现场 `~/.go-usb-ai/run/ui-runtime.json` 一度记录的是已经死掉的 PID，而真正健康监听 `55667` 的是另一个仍存活的 `go-usb-ai ... serve`。
- `~/.go-usb-ai/logs/service.log` 里能看到大量短命进程反复尝试绑定同一端口并报 `EADDRINUSE`。
- `~/Library/LaunchAgents/io.go-usb-ai.host-agent.plist` 现场确认是直接执行 `go-usb-ai serve`，且 `KeepAlive=true`。
...
- 旧的 macOS LaunchAgent 配置是 `RunAtLoad=true + KeepAlive=true`，命令直接跑 `go-usb-ai serve`。
...
- 隔离 `GOUSB_AI_HOME` 下，先启动一个健康 `serve --ui-port 53282`，再启动第二个同端口 `serve`：
  - 第二个现在直接 `EADDRINUSE` 退出；
  - `ui-runtime.json` 保持第一条健康进程的 PID，不再被失败进程污染。
- 本机 LaunchAgent 重装后：
  - `launchctl print gui/501/io.go-usb-ai.host-agent` 显示 `state = not running`
  - stdout/stderr 日志行数在多次轮询中保持不变，证明无限重拉已经停止
- 随后真实执行全局 `go-usb-ai restart`：
...
- `pnpm release:beta` 默认执行：
  ...
  4. 若 batch 包含 `go-usb-ai`，自动触发 `npm-runtime-update-release` 的 `beta` channel
...
- npm registry 已发布：
  - `go-usb-ai@beta = 0.18.12-beta.7`
- release commit：
  - `a11f4fd1 chore: release beta batch`
- runtime workflow：
  - `25449644478`
- runtime release：
  - `go-usb-ai@0.18.12-beta.7`
...
- `gh release view go-usb-ai@0.18.12-beta.7 --repo Peiiii/go-usb-ai --json url,isPrerelease,assets`
...
- 用户现场状态：
  - 已安装 npm 全局 `go-usb-ai@0.18.12-beta.7`
  - 但 `go-usb-ai --version` 仍输出 `0.18.12-beta.4`
  - 左上角产品版本号也仍显示 beta.4
- 现场证据：
  - `GOUSB_AI_DISABLE_RUNTIME_BUNDLE_LAUNCHER=1 go-usb-ai --version` 输出 `0.18.12-beta.7`
  - 普通 `go-usb-ai --version` 输出 `0.18.12-beta.4`
  - `~/.go-usb-ai/launcher/runtime-bundles/current.json` 仍指向 `0.18.12-beta.4`
  - `~/.go-usb-ai/launcher/runtime-bundles/versions/` 下只有 `0.18.12-beta.4`
...
- 所以当用户通过 `npm install -g go-usb-ai@beta` 把外壳升级到 `beta.7` 后：
  - 包内 app 已经是 `beta.7`
...
- 这样 UI 左上角、`/api/runtime/update`、`go-usb-ai --version` 都会回到同一个真实版本

### 修后验证

- `pnpm -C packages/go-usb-ai test src/cli/launcher/npm-runtime-update.manager.test.ts`：通过，7 个用例
- `pnpm -C packages/go-usb-ai tsc`：通过
- `pnpm -C packages/go-usb-ai build`：通过
- 隔离 home 真实复现 smoke：
  - 复制一个真实 `0.18.12-beta.4` current bundle 到临时 `GOUSB_AI_HOME`
  - 保持 `current.json = 0.18.12-beta.4`
  - 用本地修后的 `go-usb-ai@0.18.12-beta.7` 构建执行 `node dist/cli/launcher/index.js --version`
  - 结果输出 `0.18.12-beta.7`
- 接口层 smoke：
  - 同一隔离 home 启动 `serve`
...
- 慢的是 `go-usb-ai` 进入“自动更新通道闭环”后的 runtime workflow
...
- `release:beta:runtime`：读取当前已发布的 `go-usb-ai@beta` 版本，只补发 runtime workflow / release assets / gh-pages / 公网 manifest
