...
- 新增 Claude runtime 模型契约设计文档，明确在 GoUsbAi 产品语境下，`preferred_model` 仍是一等公民，`claude` 是 runtime 而不是独立模型系统。
...
1. 打开设计文档，确认它回答了“Claude 会话在 GoUsbAi 中如何选模型、如何判定可用性、用户第一次怎么使用”这三个核心问题。
