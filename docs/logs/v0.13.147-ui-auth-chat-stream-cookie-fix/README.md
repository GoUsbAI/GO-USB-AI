...
- 修复 `go-usb-ai ui` 轻量认证开启后，聊天发送统一返回 `{"ok":false,"error":{"code":"UNAUTHORIZED","message":"Authentication required."}}` 的问题。
- 根因是聊天流式请求没有复用统一 API client，而是在 `packages/go-usb-ai-ui/src/api/config.ts` 中直接调用了裸 `fetch`。
...
- 前端定向 lint：
  - `pnpm --filter @go-usb-ai/ui exec eslint src/api/config.ts`
- 前端类型检查：
