...
- 前端定向 lint：
  - `pnpm --filter @go-usb-ai/ui exec eslint src/App.tsx src/components/config/security-config.tsx src/components/config/RuntimeConfig.tsx src/components/layout/Sidebar.tsx src/lib/i18n.ts`
- 当前 lint 结果仅有既有 warning：
  - `packages/go-usb-ai-ui/src/components/config/RuntimeConfig.tsx` 的 `max-lines-per-function`

## 发布/部署方式

- 正常构建并部署 `@go-usb-ai/ui` 对应静态资源即可。
- 若通过 `go-usb-ai` 内置 UI 分发：
  - 重新构建 `packages/go-usb-ai-ui`
  - 按现有流程同步到 `go-usb-ai/ui-dist`
  - 重启 UI 进程
