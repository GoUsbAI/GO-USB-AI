...
- 新增 `go-usb-ai` secrets 能力，对齐 OpenClaw 风格：支持 `env` / `file` / `exec` 三类 secret provider。
- 新增配置结构：`secrets.enabled`、`secrets.defaults`、`secrets.providers`、`secrets.refs`。
- 新增核心解析器：`packages/go-usb-ai-core/src/config/secrets.ts`，支持运行时解析、provider 快照缓存、路径注入、错误提示。
...
- 文档更新：
  ...
  - `README.md`
  - `README.zh-CN.md`
- 新增自动化测试：`packages/go-usb-ai-core/src/config/secrets.test.ts`。

## 测试/验证/验收方式

- 单测：
  - `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai-core test -- run src/config/secrets.test.ts`
  - 结果：4/4 通过。
- 类型检查：
...
- 冒烟（隔离目录，不写仓库）：
  - 在 `/tmp/go-usb-ai-secrets-smoke-*` 下写入配置，配置 `providers.openai.apiKey` 通过 `secrets.refs` 引用 `OPENAI_API_KEY`。
  - 运行 core 解析脚本，确认 `rawApiKey` 为空、`resolvedApiKeyPrefix` 为 `sk-smoke`。
  - 运行 `go-usb-ai doctor --json`，确认 `OpenAI` 显示 `configured: true` 且 `detail: "apiKey ref set"`。
...
- 本次改动覆盖 `@go-usb-ai/core`、`@go-usb-ai/server`、`go-usb-ai`、`@go-usb-ai/ui`，发布时需按依赖链联动发布。

## 用户/产品视角的验收步骤

- 步骤 1：在 `~/.go-usb-ai/config.json`（或 `GOUSB_AI_HOME` 指定目录）中设置 `secrets.providers` 与 `secrets.refs`，将 `providers.openai.apiKey` 置空并引用环境变量。
- 步骤 2：设置环境变量，例如 `export OPENAI_API_KEY=...`。
- 步骤 3：执行 `go-usb-ai doctor --json`，确认 Provider 显示已配置（`apiKey ref set`）。
