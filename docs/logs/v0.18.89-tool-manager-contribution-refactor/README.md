...
- 删除旧 `GoUsbAiNcpToolRegistry` 文件，不保留新旧双轨。
- 新增单一 `ToolContribution`，直接依赖 `GoUsbAiKernel`，注册 agent 可用工具，包括 file / exec / web / message / cron / session / memory / gateway / extension / asset / MCP / session_search。
- `GoUsbAiKernel` 暴露 `toolManager`，并提供 `provideGatewayController()` / `getGatewayController()`；service 创建 gateway controller 后直接 provide 给 kernel。
...
- `McpManager.start()` 接管 MCP server 预热生命周期，`GoUsbAiKernel.start()` 统一启动；service 与 `AgentRuntimeManager` 不再调度 MCP warmup。
...
- `pnpm -C packages/go-usb-ai-kernel exec eslint src/app/go-usb-ai-kernel.ts src/managers/tool.manager.ts src/managers/tool.manager.test.ts src/managers/agent-runtime.manager.ts src/contributions/tool-contribution/index.ts src/features/native-runtime/index.ts src/features/native-runtime/services/native-agent-runtime-factory.service.ts src/features/native-runtime/services/go-usb-ai-ncp-context-builder.service.ts vitest.config.ts`
- `pnpm -C packages/go-usb-ai-service exec eslint src/shared/services/gateway/go-usb-ai-gateway-runtime.service.ts`
- `pnpm -C packages/go-usb-ai-kernel exec vitest run src/managers/tool.manager.test.ts`
- `pnpm -C packages/go-usb-ai-kernel build && pnpm -C packages/go-usb-ai-service tsc`
- `pnpm -C packages/go-usb-ai-service exec vitest run src/shared/services/gateway/tests/go-usb-ai-app.service.test.ts`
...
不涉及部署。涉及 `@go-usb-ai/kernel` 与 `@go-usb-ai/service` 源码边界变更，后续随统一包发布流程发布。
...
- 非功能改动的非测试代码净增为 `0` 行：新增 `ToolManager` / `ToolContribution` 由删除旧 `GoUsbAiNcpToolRegistry` 与移除 `AgentRuntimeManager` gateway 搬运链路抵消。
