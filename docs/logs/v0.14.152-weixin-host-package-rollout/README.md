...
- 在独立微信插件 `@go-usb-ai/channel-plugin-weixin@0.1.5` 已发布的基础上，继续发布宿主链路相关包：
...
- 已确认 npm 发布前置版本状态：
  - `@go-usb-ai/channel-plugin-weixin` 从 `0.1.4` 发布到 `0.1.5`
  - 宿主链路待发布版本在本地已完成升版
- 功能验证沿用上一迭代：
...
- 本次宿主发版额外验收：
  ...
  - 使用已发布 `go-usb-ai` 做最小安装/启动冒烟，确认包可安装且 CLI 可正常启动
