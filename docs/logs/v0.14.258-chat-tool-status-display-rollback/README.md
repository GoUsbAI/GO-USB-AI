...
- 组件展示验证：`pnpm -C packages/go-usb-ai-agent-chat-ui test -- src/components/chat/ui/chat-message-list/chat-message-list.test.tsx`
- 适配层验证：`pnpm -C packages/go-usb-ai-ui test -- src/components/chat/adapters/chat-message.adapter.test.ts`
- 类型检查：
...
- 如需本地验证构建，可执行：`pnpm -C packages/go-usb-ai-ui build`

# 用户/产品视角的验收步骤

1. 打开 GoUsbAi 聊天页，进入一个会触发工具调用的会话。
