...
本次迭代聚焦聊天页右侧子会话侧栏，补齐“流式阅读体验”和“会话上下文可见性”两类缺口，让用户在 GoUsbAi 里把子会话当成可理解、可持续跟读的执行现场，而不是只看到一段孤立输出。

- 为 [chat-child-session-panel.tsx](/Users/peiwang/Projects/nextbot/packages/go-usb-ai-ui/src/components/chat/chat-child-session-panel.tsx) 接入现有 `useStickyBottomScroll` 机制：
  - 子会话在流式输出期间，当滚动位置仍处于底部阈值内时会自动贴底；
  - 用户主动向上滚离阈值后，自动贴底会解除，不会强行打断阅读。
- 为 [use-ncp-child-session-tabs-view.ts](/Users/peiwang/Projects/nextbot/packages/go-usb-ai-ui/src/components/chat/ncp/session-conversation/use-ncp-child-session-tabs-view.ts) 扩展右侧子会话视图模型：
...
- 为 [ChatConversationPanel.test.tsx](/Users/peiwang/Projects/nextbot/packages/go-usb-ai-ui/src/components/chat/ChatConversationPanel.test.tsx) 补充回归测试，覆盖：
  - 单个子会话时 metadata 可见；
...
- 子会话面板定向测试 / 组件级冒烟：
  - `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai-ui test -- ChatConversationPanel.test.tsx`
- 本次改动文件定向 eslint：
  - `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai-ui exec eslint src/components/chat/ncp/session-conversation/use-ncp-child-session-tabs-view.ts src/components/chat/chat-child-session-panel.tsx src/components/chat/ChatConversationPanel.test.tsx`
- UI 类型检查：
...
- 结果摘要：
  ...
  - `pnpm -C packages/go-usb-ai-ui lint` 未作为本次通过项，因为它被工作区内既有/并行改动的 [NcpChatPage.tsx](/Users/peiwang/Projects/nextbot/packages/go-usb-ai-ui/src/components/chat/ncp/NcpChatPage.tsx) `react-hooks/refs` 规则错误拦住，并非本次子会话侧栏改动引入。
  - `pnpm lint:maintainability:guard` 已执行，但被工作区内并行改动的 [user-content.ts](/Users/peiwang/Projects/nextbot/packages/ncp-packages/go-usb-ai-ncp-agent-runtime/src/user-content.ts) `context-destructuring` 治理错误拦住；本次改动自身未新增对应治理违规。
...
- 目录结构与文件组织是否满足当前项目治理要求：
  - 部分满足。[packages/go-usb-ai-ui/src/components/chat](/Users/peiwang/Projects/nextbot/packages/go-usb-ai-ui/src/components/chat) 目录仍有仓库既有平铺 warning；本次未新增文件，只在既有组件内收敛改动。后续若 chat 入口继续扩张，应把子会话面板相关头部/元信息视图进一步下沉到更细的子目录。
- 独立于实现阶段的可维护性复核：
  - 可维护性复核结论：通过
  - 本次顺手减债：是
  - 代码增减报告：
