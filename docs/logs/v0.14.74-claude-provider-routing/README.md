...
- Claude runtime 改为优先跟随 `GoUsbAi` 当前 provider/model，而不是默认假设 Claude 自身订阅或 user settings。
...
- 真实链路验证
  - 使用隔离 `GOUSB_AI_HOME=/tmp/...`、真实 `pnpm dev start`、真实本机 `MiniMax` / `MiniMax Portal` credential 做 Claude 会话冒烟。
  - 已确认：
  ...
  - 当前阻塞：
    ...
    - `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai tsc` 当前因仓库已有无关文件 `src/cli/commands/platform-auth.ts` 的既有类型错误失败，本次未对该无关问题做修改
...
3. 启动本地开发环境：`PATH=/opt/homebrew/bin:$PATH GOUSB_AI_HOME=/tmp/<isolated-home> pnpm dev start`
