...
- 将 Hermes ACP bridge 收敛到“prompt 级 route 驱动的 execution agent 重建”执行模型。
  ...
  - 同一会话里每次 prompt 都从 `go-usb-ai_narp.providerRoute` 解析完整执行路由，并重建真实 Hermes `AIAgent`。
...
- 保持“不修改 Hermes 上游源码”的边界。
  - 本次实现全部落在 GoUsbAi 侧 adapter/bridge/runtime/documentation。
  - 继续把 `packages/go-usb-ai-hermes-acp-bridge` 作为 Hermes ACP 集成问题的主要适配层，而不是去改 `../hermes-agent` 上游代码。
- 修复切模型后可能继续走旧 provider，或者前端长时间停在 thinking 的根因链路。
  ...
  - prompt 级执行 agent 初始化失败会显式抛回 GoUsbAi，让 stdio runtime 发出 `MessageFailed + RunError`，而不是静默悬挂。
  ...
  - GoUsbAi provider 路由对显式前缀改为绝对优先，并拒绝对歧义裸模型名做静默 provider 猜测，避免把 `qwen3.6-plus` 这类值错误打到 `minimax`。
- 修复 Python ACP prompt meta 的真实读取位置。
  ...
  - 之前 bridge 只读取 `kwargs._meta.go-usb-ai_narp.providerRoute`，导致真实链路里第二轮 prompt 根本读不到 `providerRoute`，同 session 后续请求会继续复用第一轮 execution agent。
  - 现在 bridge 会优先读取真实链路里的 `kwargs.go-usb-ai_narp.providerRoute`，同时兼容直接调用场景下的 `kwargs._meta.go-usb-ai_narp.providerRoute`。
- 补齐定向回归测试。
...
- 收尾补丁修复了 `pnpm dev start` 启动残留回归。
  - `packages/go-usb-ai-core/src/agent/context.ts` 与 `packages/go-usb-ai-core/src/runtime-context/runtime-user-prompt.ts` 仍引用已删除的 `bootstrap-context.js`。
...
- 补上 Hermes ACP bridge 运行产物同步防线。
  ...
  - 新增 `pnpm --filter @go-usb-ai/go-usb-ai-hermes-acp-bridge check:bridge-sync`，可直接检测 source / dist 桥接文件是否一致。
...
```bash
pnpm --filter @go-usb-ai/go-usb-ai-hermes-acp-bridge lint
pnpm --filter @go-usb-ai/go-usb-ai-hermes-acp-bridge tsc
```
...
- `pnpm --filter @go-usb-ai/go-usb-ai-hermes-acp-bridge test` 通过，7/7 用例通过。
- `pnpm --filter @go-usb-ai/core test -- src/config/schema.provider-routing.test.ts src/config/provider-runtime-resolution.test.ts` 通过。
- `pnpm --filter @go-usb-ai/go-usb-ai-hermes-acp-bridge lint` 通过。
- `pnpm --filter @go-usb-ai/go-usb-ai-hermes-acp-bridge tsc` 通过。
- `pnpm --filter @go-usb-ai/go-usb-ai-ncp-runtime-stdio-client test` 通过，8/8 用例通过。
- `pnpm --filter @go-usb-ai/go-usb-ai-ncp-runtime-stdio-client lint` 通过，但仍有历史大文件 warning，不影响本次结果。
- `pnpm --filter @go-usb-ai/go-usb-ai-ncp-runtime-stdio-client tsc` 通过。
- `pnpm lint:maintainability:guard` 未能全绿；剩余 error 来自当前工作区其它并行改动（如 `packages/go-usb-ai-core/src/agent`、`packages/go-usb-ai-ui/src/components/config/ChannelForm.tsx`、`packages/go-usb-ai-ui/src/components/config/ProviderForm.tsx`、`packages/go-usb-ai-ui/src/components/config/SearchConfig.tsx`），不是本次 Hermes ACP 修复新引入的问题。
- 补充同 session 双轮定向验证：
...
- 后续随正常 GoUsbAi 发布流程进入发布即可；Hermes ACP bridge 代码会随包含 `@go-usb-ai/go-usb-ai-hermes-acp-bridge` 的构建产物生效。

## 用户/产品视角的验收步骤

1. 在 GoUsbAi 中选择 Hermes 类 `narp-stdio` 会话类型。
...
- 可维护性复核结论：通过。
- 长期目标对齐 / 可维护性推进：本次把 Hermes ACP 的真实执行真相收敛为 prompt 级 `providerRoute`，减少了“会话残留状态”和“当前用户选择”之间的分裂，更符合 GoUsbAi 的统一体验与行为可预测目标。
...
- 本次无需单独发布的包：`@go-usb-ai/core`、`@go-usb-ai/go-usb-ai-hermes-acp-bridge`、`@go-usb-ai/go-usb-ai-ncp-runtime-stdio-client`。
- 当前状态：以上包均标记为随后续正常 GoUsbAi 发布批次统一处理，本次不单独发包。
