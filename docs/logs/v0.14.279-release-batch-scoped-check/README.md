...
- 新增 [scripts/check-release-batch.mjs](../../../../scripts/check-release-batch.mjs)：
  ...
  - 只对当前批次里的公开包运行校验
  - 按批次内包依赖顺序执行，避免 `@go-usb-ai/ui -> go-usb-ai` 这类顺序依赖继续靠历史构建产物碰运气
...
- 这次改动的目标不是给 `go-usb-ai` 单独加特判，而是从机制层消除“单包发布却全仓跑”的系统性浪费。

# 测试/验证/验收方式

- 机制验证：
  - `pnpm release:check`
  - 当前实际输出已明确显示：`[release:check] batch packages: go-usb-ai`
- 运行结果：
  - 只执行了 `go-usb-ai` 的 `build`、`lint`、`tsc`
...
1. 准备一个只包含 `go-usb-ai` 的 changeset 或已执行过 `release:version` 的单包发布批次。
2. 执行 `pnpm release:check`。
3. 观察输出开头是否为 `batch packages: go-usb-ai`，且后续只出现 `go-usb-ai build/lint/tsc`。
