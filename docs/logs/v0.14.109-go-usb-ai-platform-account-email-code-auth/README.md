# v0.14.109-go-usb-ai-platform-account-email-code-auth

## 迭代完成说明

- 将 `GoUsbAi Platform` 的用户登录入口切换为邮箱验证码模型，不再在平台前端暴露“邮箱密码注册/登录”双模型。
- 新增平台邮箱验证码主链：
...
- 浏览器授权页改为 `GoUsbAi Account` 邮箱验证码授权，和桌面端 remote access 的 browser auth 链路保持一致。
- `apps/platform-console` 统一品牌命名为 `GoUsbAi Platform / GoUsbAi Account`，登录页改为单一验证码流，并优化了平台壳层与设备页样式。
- 清理 CLI / UI router 中暴露的 `--register` 与 `register` 请求字段，避免继续对外暴露旧注册契约。
- 更新文档：
  - [账号与远程访问产品设计](../../plans/2026-03-21-go-usb-ai-account-and-remote-access-product-design.md)
...
- 定向测试：
  - `pnpm -C packages/go-usb-ai-server test -- src/ui/router.remote.test.ts`
- 平台 smoke：
...
- 配置完成后的发布顺序：
  ...
  2. `pnpm -C workers/go-usb-ai-provider-gateway-api run deploy`
...
1. 打开 `GoUsbAi Platform` 登录页，输入邮箱并发送验证码。
2. 收到验证码后输入 6 位码，成功进入平台；若邮箱首次使用，应自动创建账号并直接登录。
3. 在桌面端登录同一个 `GoUsbAi Account` 并开启远程访问。
