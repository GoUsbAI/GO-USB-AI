...
- 修复 Agent 管理页与运行时对“缺失 `workspace` 的额外 Agent”解析错误的问题：此前这类 Agent 会错误继承 `~/.go-usb-ai/workspace`，导致 UI 把所有 Agent 的主目录都展示成主 Agent 根目录。
- 将额外 Agent 的隐式 home 解析统一收口到 `packages/go-usb-ai-core/src/config/agent-profiles.ts`：
...
- 定向测试：
  - `pnpm -C packages/go-usb-ai-core test -- src/config/agent-profiles.test.ts`
  - `pnpm -C packages/go-usb-ai-server test -- src/ui/router.agents.test.ts`
  - `pnpm -C packages/go-usb-ai test -- src/cli/commands/agent/agent-runtime-pool.command.test.ts`
  - 结果：通过
- 类型检查：
...
1. 启动或重启当前 GoUsbAi UI / service 实例。
2. 打开 Agent 管理页。
3. 确认 `main` 仍展示 `~/.go-usb-ai/workspace`。
4. 确认 `laowizard`、`zhangbasheng` 等额外 Agent 展示为 `~/.go-usb-ai/workspace/agents/<agentId>`，而不是统一展示成 `~/.go-usb-ai/workspace`。
...
- 目录结构与文件组织是否满足当前项目治理要求：基本满足。本次未新增新目录，只在既有 core / server / cli 测试链路补充回归用例；`packages/go-usb-ai-core/src/config/agent-profiles.ts` 仍接近文件预算，后续若继续扩展 Agent profile 规则，应考虑拆出 path-resolution 子模块。
