...
- 已发布目标包：
  ...
  - `@go-usb-ai/ui@0.9.1`
  - `go-usb-ai@0.12.4`
- 同步更新 `go-usb-ai` 内置 `ui-dist` 产物，使仓库中的发布产物与线上发布版本一致。
...
- 冒烟：
  - `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai dev:build --version`
  - 结果：输出 `0.12.4`
- 线上版本校验：
...
1. 安装或升级 `go-usb-ai@0.12.4` 后打开 chat 输入框。
