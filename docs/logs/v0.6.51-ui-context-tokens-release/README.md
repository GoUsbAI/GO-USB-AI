...
- 发布组件：
  ...
  - `@go-usb-ai/ui@0.3.13`
- 联动说明：`go-usb-ai` 依赖 `@go-usb-ai/server`，已同步版本并发布。
...
- 发布前工程验证（已通过）：
  ...
  - `pnpm tsc`
- 发布后线上版本核验（已通过）：
  - `npm view go-usb-ai version` → `0.6.28`
  - `npm view @go-usb-ai/server version` → `0.4.11`
  - `npm view @go-usb-ai/ui version` → `0.3.13`

### 用户/产品视角验收步骤

1. 用户在 UI 的 `Routing & Runtime` 页面可以直接设置默认 `contextTokens`。
2. 用户可为具体 Agent 设置 `contextTokens` 覆盖值，并保存成功。
3. 刷新 UI 后配置回显一致，且 `~/.go-usb-ai/config.json` 中配置落盘。
