...
## 迭代完成说明（改了什么）
- 修复 `@go-usb-ai/agent-chat` 中同一 `messageId` 被重复追加导致的重复回复卡片问题：
...
- 修复 `go-usb-ai-ui` 自动恢复运行链路中“本地已完成 run 被再次 resume 重放”的问题：
...
- Lint：
  - `pnpm --filter @go-usb-ai/agent-chat lint`
...
## 发布/部署方式
- 本次为前端与 `@go-usb-ai/agent-chat` 逻辑修复，按常规前端发布流程执行：
  - 合并后执行 UI 发布流程（如 `pnpm --filter @go-usb-ai/ui build` + 既有部署管线）。
  - 如需同时发包 `@go-usb-ai/agent-chat`，走仓库既有 npm 发布流程（changeset/version/publish）。
