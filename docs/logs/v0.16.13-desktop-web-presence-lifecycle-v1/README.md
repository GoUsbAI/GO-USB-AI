...
- Desktop 已完成一条完整产品语义链路：
  ...
  - 只有应用菜单或托盘菜单里的 `Quit GoUsbAi` 才真正退出应用并停止 runtime。
...
- 同批次验收修正补上了一个关键桌面生命周期缺陷：
  ...
  - 本次把“隐式 quit”和“显式 Quit GoUsbAi”彻底分开：启用后台运行时，普通 `before-quit` 会被 presence owner 拦回后台；只有托盘菜单或应用菜单里的 `Quit GoUsbAi`，以及 restart / update 等明确退出路径，才会先标记 `quitting` 再放行停服务。
  - 修复后桌面端的真实合同收紧为：`关闭窗口 = 隐藏到托盘并继续后台运行`，`Quit GoUsbAi = 退出应用并停止 runtime`。
- Desktop renderer / UI 已完成对应接线：
  - preload bridge 新增 `getPresenceState()`、`updatePresencePreferences()`。
  - runtime 配置页新增 `Runtime Presence` 卡片。
  - 在 `desktop-embedded` 环境下展示“关闭窗口时继续后台运行”“登录系统时自动启动 GoUsbAi”两个开关。
...
- 本轮又顺手做了一次最小目录收敛，解决 `runtime-support / service-support/runtime` 的角色混乱：
  - 新增 `packages/go-usb-ai/src/cli/commands/service-support/ui/`，把 UI host、控制面装配和 remote access UI 协调都收回同一边界。
  - 原 `packages/go-usb-ai/src/cli/commands/runtime-support/runtime-control-host.ts` 已迁到 `packages/go-usb-ai/src/cli/commands/service-support/ui/runtime-control-host.service.ts`。
  - 原 `packages/go-usb-ai/src/cli/commands/service-support/runtime/service-ui-hosts.ts` 已迁到 `packages/go-usb-ai/src/cli/commands/service-support/ui/service-ui-hosts.service.ts`。
  - 原 `packages/go-usb-ai/src/cli/commands/service-support/runtime/service-remote-access.ts` 已迁到 `packages/go-usb-ai/src/cli/commands/service-support/ui/service-remote-access.service.ts`。
...
- 已通过：`pnpm -C packages/go-usb-ai-ui exec tsc --noEmit`
- 已通过：`pnpm -C packages/go-usb-ai-server exec tsc --noEmit`
- 已通过：`pnpm -C packages/go-usb-ai exec tsc --noEmit`
- 已通过：`pnpm -C packages/go-usb-ai-ui exec vitest run src/components/config/runtime-presence-card.test.tsx src/components/config/runtime-control-card.test.tsx`
- 已通过：`pnpm -C packages/go-usb-ai-server exec vitest run src/ui/router.runtime-control.test.ts`
- 已通过：`pnpm -C packages/go-usb-ai exec vitest run src/cli/commands/service-support/ui/tests/runtime-control-host.service.test.ts`
- 已通过：`pnpm lint:new-code:governance -- apps/desktop/package.json apps/desktop/src/main.ts apps/desktop/src/preload.ts apps/desktop/src/utils/desktop-ipc.utils.ts apps/desktop/src/services/desktop-presence.service.ts apps/desktop/src/launcher/stores/launcher-state.store.ts apps/desktop/src/launcher/__tests__/launcher-foundation.test.ts apps/desktop/src/launcher/__tests__/launcher-test.utils.ts packages/go-usb-ai-ui/src/components/config/RuntimeConfig.tsx packages/go-usb-ai-ui/src/components/config/runtime-presence-card.tsx packages/go-usb-ai-ui/src/components/config/runtime-presence-card.test.tsx packages/go-usb-ai-ui/src/desktop/managers/desktop-presence.manager.ts packages/go-usb-ai-ui/src/desktop/stores/desktop-presence.store.ts packages/go-usb-ai-ui/src/desktop/desktop-update.types.ts packages/go-usb-ai-ui/src/lib/i18n.runtime-control.ts`
- 已通过：`pnpm lint:new-code:governance -- packages/go-usb-ai-server/src/ui/runtime-control.types.ts packages/go-usb-ai-server/src/ui/ui-routes/types.ts packages/go-usb-ai-server/src/ui/ui-routes/runtime-control.controller.ts packages/go-usb-ai-server/src/ui/router.ts packages/go-usb-ai-server/src/ui/router.runtime-control.test.ts packages/go-usb-ai-ui/src/api/runtime-control.types.ts packages/go-usb-ai-ui/src/api/runtime-control.ts packages/go-usb-ai-ui/src/api/types.ts packages/go-usb-ai-ui/src/runtime-control/runtime-control.manager.ts packages/go-usb-ai-ui/src/hooks/use-runtime-control.ts packages/go-usb-ai-ui/src/lib/i18n.runtime-control.ts packages/go-usb-ai-ui/src/components/config/runtime-control-card.tsx packages/go-usb-ai-ui/src/components/config/runtime-control-card.test.tsx packages/go-usb-ai-ui/src/components/config/runtime-presence-card.test.tsx packages/go-usb-ai/src/cli/commands/service.ts packages/go-usb-ai/src/cli/commands/service-support/ui/runtime-control-host.service.ts packages/go-usb-ai/src/cli/commands/service-support/ui/tests/runtime-control-host.service.test.ts packages/go-usb-ai/src/cli/commands/service-support/ui/service-ui-hosts.service.ts packages/go-usb-ai/src/cli/commands/service-support/ui/service-remote-access.service.ts docs/plans/2026-04-14-unified-desktop-web-presence-lifecycle-design.md`
...
- 已执行：`pnpm lint:maintainability:guard`
  ...
  - 本次自身相关变更只留下 warning，没有新增同范围 error；其中最值得继续关注的是 `packages/go-usb-ai-ui/src/components/config/runtime-control-card.tsx` 在本次收敛后增长较多，后续可再切一刀 presenter / action model。
- 未执行：真实 GUI 冒烟
...
- 已补做：桌面端真实生命周期冒烟与排查复现
  - 复现证据：`~/Library/Application Support/@go-usb-ai/desktop/launcher/main.log` 在 `2026-04-14T11:01:13Z` 记录 `Desktop window close intercepted. Hiding window to tray.`，但随后又在 `2026-04-14T11:01:15Z` 记录 `before-quit received. stopping=false`，并紧接着出现 runtime `signal=SIGTERM` 退出日志，和“关窗后只再跑一次定时任务”现象一致。
...
- Web / UI：
  ...
  - 不要求新增独立部署组件，继续沿用现有 GoUsbAi UI / server / CLI 打包链路即可。
...
- Desktop 验收：
  ...
  - 从托盘点击 `Open GoUsbAi`，确认主窗口可恢复。
  - 从托盘点击 `Quit GoUsbAi`，确认桌面应用退出，内嵌 runtime 跟随停止。
  - 若已有“每分钟发送问候”之类的真实定时任务，验证“关闭窗口后继续发送；点击 `Quit GoUsbAi` 后停止发送”的完整对照语义。
  - 打开“登录系统时自动启动 GoUsbAi”，重启应用后再次进入设置页，确认状态能正确回显。
- Web 验收：
...
- 目录结构与文件组织是否满足当前项目治理要求：比上一版更接近目标。`service-support/runtime` 现在只承担 runtime 基础设施，`service-support/ui` 承担 UI host / 控制面装配，旧的 `runtime-support` 目录已不再承载职责；剩余已知旧债仍主要在 `packages/go-usb-ai-ui/src/components/config` 与 `packages/go-usb-ai-ui/src/api` 两处 legacy 平铺目录。
...
- 当前保留债务主要有两处：
  - `packages/go-usb-ai-ui/src/components/config/RuntimeConfig.tsx` 仍是 legacy 长文件，后续最自然的切缝是继续把其它 runtime 面板逐步拆成独立装配块。
  - `packages/go-usb-ai-ui/src/components/config/runtime-control-card.tsx` 经过这次统一收敛后增长较多，后续最自然的切缝是把动作模型与只读展示块再拆成独立 presenter / view helper。
