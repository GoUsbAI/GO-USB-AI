...
- 完成 MCP marketplace 全链路落地：worker catalog 数据源、server 管理接口、UI marketplace 页面、CLI/service 热插拔安装链路、`@go-usb-ai/mcp` 的安装/管理/诊断域服务。
...
- 定向测试：
  - `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai-server test -- --run src/ui/router.marketplace-content.test.ts src/ui/router.marketplace-manage.test.ts`
- 定向 lint：
  - `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai-mcp lint`
- 可维护性自检：
  - `PATH=/opt/homebrew/bin:$PATH node .codex/skills/post-edit-maintainability-guard/scripts/check-maintainability.mjs --paths packages/go-usb-ai/src/cli/commands/service.ts packages/go-usb-ai/src/cli/commands/service-marketplace-installer.ts packages/go-usb-ai-server/src/ui/router.ts packages/go-usb-ai-server/src/ui/types.ts packages/go-usb-ai-ui/src/api/types.ts packages/go-usb-ai-ui/src/lib/i18n.ts workers/marketplace-api/src/main.ts`
  - 结果：`Errors: 0`，仍有少量历史 warning，但本次未新增阻塞性可维护性回归。
- 开发态真实冒烟：
  - 启动：`PATH=/opt/homebrew/bin:$PATH GOUSB_AI_HOME=$(mktemp -d /tmp/go-usb-ai-mcp-marketplace-final.XXXXXX) pnpm dev start`
...
- 线上 smoke：
  - `PATH=/opt/homebrew/bin:$PATH curl -s https://marketplace-api.go-usb-ai.io/health`
  - `PATH=/opt/homebrew/bin:$PATH curl -s 'https://marketplace-api.go-usb-ai.io/api/v1/mcp/items?page=1&pageSize=5'`
...
- marketplace worker 已部署到线上：
  ...
  - workers.dev：`https://go-usb-ai-marketplace-api.15353764479037.workers.dev`
  - 正式域名：`https://marketplace-api.go-usb-ai.io`
- 本次部署包含：
  - MCP catalog/read API
  - `marketplace_mcp_*` D1 表结构
  - 官方 `chrome-devtools` seed 数据
- 本次交付重点是 marketplace 数据与本地热插拔能力落地，未额外执行 NPM 包对外发布；本地 `go-usb-ai` 开发态与构建态已完成功能验证。

## 用户/产品视角的验收步骤

1. 启动开发环境：`PATH=/opt/homebrew/bin:$PATH GOUSB_AI_HOME=$(mktemp -d /tmp/go-usb-ai-mcp-marketplace-final.XXXXXX) pnpm dev start`
