...
## 做了什么
- 新增 `go-usb-ai update` 命令用于自我升级。
- 抽取自我升级逻辑到共享模块，CLI 与 gateway update.run 复用同一实现，避免重复逻辑。
- 升级策略：
  1) `GOUSB_AI_UPDATE_COMMAND` 覆盖
  2) `npm i -g go-usb-ai`
- 文档补充 `go-usb-ai update` 命令说明。
...
- 冒烟（非仓库目录）：
  - `GOUSB_AI_UPDATE_COMMAND="node -e \"console.log('ok')\"" node /Users/peiwang/Projects/nextbot/packages/go-usb-ai/dist/cli/index.js update --timeout 1200000`
...
- 已按 `docs/workflows/npm-release-process.md` 发布 npm：
  ...
  - 发布结果：`go-usb-ai@0.4.6`（其余包版本未变，跳过发布）
