...
- 已成功发布：
  - `@go-usb-ai/ui@0.11.14`
  - `go-usb-ai@0.16.24`
- 发布过程中同步生成并落地了版本号、changelog 与 `go-usb-ai/ui-dist` 前端产物更新。
- 本次未将无关的 `ncp-chat-realtime-reload` 本地改动混入发布提交范围。

## 测试/验证/验收方式

- 定向单测：
  - `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai-ui test -- --run src/components/chat/adapters/chat-input-bar.adapter.test.ts`
- 类型检查：
...
- 真实 UI 冒烟：
  - 使用隔离目录 `GOUSB_AI_HOME=/tmp/go-usb-ai-ui-smoke-4IAo1L`
  - 启动：`PATH=/opt/homebrew/bin:$PATH GOUSB_AI_HOME=/tmp/go-usb-ai-ui-smoke-4IAo1L node packages/go-usb-ai/dist/cli/index.js serve --ui-port 18892`
  - 观察点：
...
1. 安装或升级到 `go-usb-ai@0.16.24`，打开聊天页。
2. 点击底部输入区的模型选择下拉。
