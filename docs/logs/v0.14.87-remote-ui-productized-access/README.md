...
- 为 UI 注入统一 remote access host，并在 `@go-usb-ai/server` 暴露 `/api/remote/status`、`/api/remote/doctor`、`/api/remote/login`、`/api/remote/logout`、`/api/remote/settings`、`/api/remote/service/:action`。
...
- 单测：
  - `PATH=/opt/homebrew/bin:$PATH pnpm --filter @go-usb-ai/server test -- run src/ui/router.remote.test.ts`
  - `PATH=/opt/homebrew/bin:$PATH pnpm --filter go-usb-ai test -- run src/cli/commands/remote-access-host.test.ts`
- 构建：
...
- 隔离环境冒烟：
  - 使用 `/tmp/go-usb-ai-remote-ui-smoke-verify-*` 作为 `GOUSB_AI_HOME`
  - 使用打包后的 `node packages/go-usb-ai/dist/cli/index.js start --ui-port 19211 --start-timeout 15000`
  - 通过 `POST /api/remote/service/restart` 验证 `pid` 从 `10893` 切换到 `10988`，随后 `GET /api/remote/status` 恢复可访问，最后 `go-usb-ai stop` 正常停止服务
...
- UI 构建产物：
  - `PATH=/opt/homebrew/bin:$PATH pnpm --filter @go-usb-ai/ui build`
  - `PATH=/opt/homebrew/bin:$PATH pnpm --filter go-usb-ai build`
  - 确保 `packages/go-usb-ai/ui-dist` 同步最新 UI 产物后提交

## 用户/产品视角的验收步骤

1. 启动或升级到本次发布后的 `go-usb-ai`，打开本地 UI。
...
6. 按文档教程在另一台设备访问 GoUsbAi Platform，确认能看到当前设备并完成远程接入。
