...
2. AI 走 `exec` 执行 `go-usb-ai restart` 时，原实现不携带会话上下文。
3. 因缺失会话定位信息，重启后通知无法准确路由到原会话。

## 修复

1. `gateway.restart` 透传会话键并写哨兵。
2. `exec` 工具增加会话上下文透传（`GOUSB_AI_RUNTIME_SESSION_KEY/CHANNEL/CHAT_ID`）。
...
```bash
pnpm -C packages/go-usb-ai exec tsx /tmp/go-usb-ai-restart-path-fix-smoke.ts
...
- 在 exec 同款环境变量下触发：`GOUSB_AI_RUNTIME_SESSION_KEY=discord:<channelId> go-usb-ai restart`
