...
- `@go-usb-ai/remote` connector 切到实例注册新接口；远程 relay smoke 覆盖 owner open、share open、share revoke 后会话失效的端到端闭环。
- 相关设计文档：[`docs/plans/2026-03-22-go-usb-ai-remote-instance-sharing-design.md`](../../plans/2026-03-22-go-usb-ai-remote-instance-sharing-design.md)
...
- 平台 worker 部署前先执行远程 D1 migration：`pnpm -C workers/go-usb-ai-provider-gateway-api db:migrate:remote`
- 随后部署 worker：`pnpm -C workers/go-usb-ai-provider-gateway-api deploy`
