...
- 访问 `https://docs.go-usb-ai.io/`，确认可进入语言选择页。
- 访问 `https://docs.go-usb-ai.io/en/guide/getting-started` 与 `https://docs.go-usb-ai.io/zh/guide/getting-started`，确认内容可用且语言切换正常。
- 访问 `https://docs.go-usb-ai.io/guide/getting-started`，确认返回 404（旧路径已断代）。
...
1. 打开 `https://docs.go-usb-ai.io/`，确认首页可在 English / 简体中文之间选择。
2. 进入英文文档任一页面，使用导航中的语言切换，确认可跳转到对应中文页面。
3. 进入中文文档任一页面，使用导航中的语言切换，确认可跳转到对应英文页面。
4. 打开 `https://docs.go-usb-ai.io/guide/getting-started`，确认旧路径不再可用，避免双轨维护。
