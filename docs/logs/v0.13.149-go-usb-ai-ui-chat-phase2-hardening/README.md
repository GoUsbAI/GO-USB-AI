# v0.13.149-go-usb-ai-ui-chat-phase2-hardening

## 迭代完成说明（改了什么）
- 在 `packages/go-usb-ai-ui/src/components/chat` 完成第二阶段硬化收敛，核心目标是降低 host 泄漏并稳定可拆包边界。
- 输入区（InputBar）
...
- 代码规范（改动相关范围）：
  - `pnpm -C packages/go-usb-ai-ui exec eslint src/components/chat src/lib/i18n.ts`

## 发布/部署方式
- 本迭代为 `go-usb-ai-ui` 内部重构硬化，不涉及独立包拆分与协议变更。
