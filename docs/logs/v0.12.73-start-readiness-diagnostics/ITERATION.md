...
- 改造 `packages/go-usb-ai/src/cli/commands/service.ts`，在后台启动探测阶段落地 3 项最小高价值改进：
...
- 冒烟验证（在非仓库目录写入，使用临时 `GOUSB_AI_HOME`）：
...
- 如需发布，按既有流程执行：`changeset -> version -> publish`，并在发布后复跑 `go-usb-ai start` 失败场景冒烟，确认诊断输出在线上包一致。

## 用户/产品视角的验收步骤

1. 在一台干净环境执行：`go-usb-ai start`。
