...
- 为 Agent identity 正式补齐 `description` 字段，并打通从配置 schema 到 core / server / UI / CLI 的全链路：
  ...
  - `go-usb-ai agents new` 新增 `--description`
- 修正 `/agents` 页面卡片描述来源：
...
- 收口 `Routing & Runtime` 的身份字段丢失问题：
  ...
  - 顺手将 [`RuntimeConfig.tsx`](/Users/tongwenwen/Projects/Peiiii/go-usb-ai/packages/go-usb-ai-ui/src/components/config/RuntimeConfig.tsx) 的局部转换逻辑拆到 [`runtime-config-agent.utils.ts`](/Users/tongwenwen/Projects/Peiiii/go-usb-ai/packages/go-usb-ai-ui/src/components/config/runtime-config-agent.utils.ts)，把该文件从 `529` 行压到 `463` 行
- 更新 [docs/USAGE.md](/Users/tongwenwen/Projects/Peiiii/go-usb-ai/docs/USAGE.md)，将 `agents new` 的可选参数补齐为支持 `--description`
...
- server 测试：
  - `pnpm -C packages/go-usb-ai-server test -- --run src/ui/router.agents.test.ts`
- UI 测试：
  - `pnpm -C packages/go-usb-ai-ui test -- --run src/components/agents/AgentsPage.test.tsx`
- 类型检查：
...
- 本次未执行发布或部署。
- 若后续随前端或 CLI 一起发布，按现有发布流程正常发布 `@go-usb-ai/core`、`@go-usb-ai/server`、`@go-usb-ai/ui`、`go-usb-ai` 即可。
...
4. 执行 `go-usb-ai agents new demo --description "负责 Demo 演示与讲解" --json`，确认返回的 `agent.description` 已带上该值。
...
- 是否让总代码量、分支数、函数数、文件数或目录平铺度下降，或至少没有继续恶化：部分做到。虽然为补齐 `description` 链路新增了最小必要代码，并新增一个工具文件 [`runtime-config-agent.utils.ts`](/Users/tongwenwen/Projects/Peiiii/go-usb-ai/packages/go-usb-ai-ui/src/components/config/runtime-config-agent.utils.ts)，但同时把 [`RuntimeConfig.tsx`](/Users/tongwenwen/Projects/Peiiii/go-usb-ai/packages/go-usb-ai-ui/src/components/config/RuntimeConfig.tsx) 净减少 `66` 行，避免继续恶化现有超长文件债务。
...
- 目录结构与文件组织是否满足当前项目治理要求：部分未满足但本次已控制恶化。`packages/go-usb-ai-ui/src/components/config` 目录文件数继续高于预算，本次新增的 util 文件是为了反向压缩超长 `RuntimeConfig.tsx` 的必要拆分；后续若继续演进 runtime 配置页，应进一步按 agent / binding / session 分块拆分。
