...
## 冻结边界 / 不变量
- 验证必须覆盖 `/Users/peiwang/Library/Application Support/@go-usb-ai/desktop`。
...
- 真实桌面数据目录已恢复：`/Users/peiwang/Library/Application Support/@go-usb-ai/desktop/staging` 清空，`0.19.6` 标记 healthy。
- real-profile DMG smoke 已通过：从 DMG 临时安装、真实桌面数据目录、GUI `ready-to-show` / `did-finish-load`、health API 全部通过，约 10.1s ready。
- GUI-launched runtime AI smoke 已通过：`/api/health` ok，`pnpm smoke:ncp-chat` 拿到 `GOUSB_AI_DESKTOP_GUI_AI_OK` assistant 回复。
