...
- `workers/go-usb-ai-provider-gateway-api` 为同一个 remote access session 同时返回 `openUrl` 和 `fixedDomainOpenUrl`，固定域名不新增第二套鉴权或代理链路，只复用现有 session token + cookie 机制。
...
- 可维护性闸门：
  - `PATH=/opt/homebrew/bin:$PATH node .codex/skills/post-edit-maintainability-guard/scripts/check-maintainability.mjs --paths workers/go-usb-ai-provider-gateway-api/src/types/platform.ts workers/go-usb-ai-provider-gateway-api/src/services/remote-access-service.ts workers/go-usb-ai-provider-gateway-api/src/repositories/remote-repository.ts workers/go-usb-ai-provider-gateway-api/src/controllers/remote-instance-controller.ts workers/go-usb-ai-provider-gateway-api/src/controllers/remote-controller.ts apps/platform-console/src/api/types.ts apps/platform-console/src/api/client.ts apps/platform-console/src/pages/UserDashboardPage.remote-state.ts apps/platform-console/src/pages/UserDashboardPage.tsx scripts/platform-console-smoke.mjs scripts/remote-relay-hibernation-smoke.mjs scripts/remote-relay-smoke-support.mjs`
  - 结果：无阻塞项；保留 4 条 near-budget warning，未新增恶化债务
- 线上健康检查：
  - `curl -sS -o /tmp/go-usb-ai-platform-console-health.out -w '%{http_code}\n' https://platform.go-usb-ai.io` → `200`
  - `curl -sS -o /tmp/go-usb-ai-api-health.out -w '%{http_code}\n' https://ai-gateway-api.go-usb-ai.io/health` → `200`
  - `curl -sS -o /tmp/go-usb-ai-remote-fixed-health.out -w '%{http_code}\n' https://remote.claw.cool/health` → `200`
  - `curl -sS -o /tmp/go-usb-ai-remote-subdomain-health.out -w '%{http_code}\n' https://r-test.claw.cool/health` → `200`
...
- Platform Console:
  ...
  - Preview deployment: `https://4c6ee2ed.go-usb-ai-platform-console.pages.dev`
- 本次不涉及 Admin 站点代码改动，因此 `deploy:platform:admin` 不适用。

## 用户/产品视角的验收步骤

1. 在桌面端启动并登录支持 remote access 的 GoUsbAi 实例，确保实例在线。
2. 打开 `https://platform.go-usb-ai.io`，进入“我的实例”列表。
