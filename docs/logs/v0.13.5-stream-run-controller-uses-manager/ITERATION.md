...
## 测试/验证/验收方式
- ESLint（受影响文件）：
  - `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai-ui exec eslint src/components/chat/chat-stream/stream-run-controller.ts src/components/chat/managers/chat-stream.manager.ts`
- 类型检查：
