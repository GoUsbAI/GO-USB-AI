...
- 现在将两类职责显式拆分：
  - 浏览器公开 API base 仍由 `packages/go-usb-ai-ui/src/api/api-base.ts` 解析；未显式配置时稳定回退到 `window.location.origin`
...
- 新增 `packages/go-usb-ai-ui/src/api/api-base.test.ts`，锁定“显式 API base 优先，否则回退到当前页面 origin”的契约。

## 测试/验证/验收方式

- 定向单测：
  - `pnpm -C packages/go-usb-ai-ui test -- --run src/api/api-base.test.ts src/transport/app-client.test.ts src/transport/remote.transport.test.ts`
  - 结果：通过，`5` 个测试全部通过。
- 类型检查：
  - `pnpm -C packages/go-usb-ai-ui tsc --pretty false`
  - 结果：通过。
- 隔离 dev 冒烟：
  - `GOUSB_AI_HOME=/tmp/go-usb-ai-remote-dev-fix.<id> GOUSB_AI_DEV_BACKEND_PORT=18892 GOUSB_AI_DEV_FRONTEND_PORT=5274 pnpm dev start`
...
- 可维护性检查：
  - `node .codex/skills/post-edit-maintainability-guard/scripts/check-maintainability.mjs --paths packages/go-usb-ai-ui/vite.config.ts scripts/dev-runner.mjs packages/go-usb-ai-ui/src/api/api-base.ts packages/go-usb-ai-ui/src/api/api-base.test.ts packages/go-usb-ai-ui/src/vite-env.d.ts`
  - 结果：无阻塞项；有 2 条目录预算警告，分别位于 `packages/go-usb-ai-ui/src/api` 与 `scripts`，本次未新增阻塞性可维护性问题。

## 发布/部署方式

- 本次触达 `packages/go-usb-ai-ui` 与仓库级 dev runner，本地源码验证完成后，按既有 NPM/CLI 发布流程发布包含以下改动的版本：
