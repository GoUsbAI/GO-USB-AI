...
- 恢复官网首页终端展示与复制命令为原有内容（不再显示安装脚本风格输出）：
  ...
  - 复制命令恢复为：`npm install -g go-usb-ai && go-usb-ai start`
  - 终端动效恢复为：
...
- 发布验证：
  - `pnpm deploy:landing`
  - Wrangler 回传部署地址：`https://73f06fff.go-usb-ai-landing.pages.dev`
- 打包产物验证（当前生效 bundle）：
  - 在 `apps/landing/dist/assets/main-BtsZk_P2.js` 中确认存在：
    - `npm install -g go-usb-ai && go-usb-ai start`
    - `go-usb-ai start`
  - 且不再使用安装脚本终端文案。
...
1. 打开官网首页（中英文均可）。
2. 查看 Hero 区域终端模拟内容，应为原有 `npm install -g go-usb-ai` + `go-usb-ai start` 链路。
3. 点击“复制命令”，粘贴结果应为 `npm install -g go-usb-ai && go-usb-ai start`。
