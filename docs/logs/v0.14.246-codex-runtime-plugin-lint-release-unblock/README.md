...
- 修复 `@go-usb-ai/go-usb-ai-ncp-runtime-plugin-codex-sdk` 在正式 `release:publish` 中暴露的 lint 阻塞。
- 删除 `src/index.ts` 中未使用的类型导入，消除 `@typescript-eslint/no-unused-vars` 错误。
- 保持运行时行为不变，只做发布前的最小源码清理。

## 测试/验证/验收方式

- 执行 `pnpm -C packages/extensions/go-usb-ai-ncp-runtime-plugin-codex-sdk lint`。
...
- 通过 npm 安装发布后的 `go-usb-ai` 与相关 NCP runtime 包，确认安装与运行链路正常。
