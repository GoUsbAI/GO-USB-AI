...
- 本次额外完成了 3 个此前尚未在 NPM 上存在的新 runtime 包首发：
  - `@go-usb-ai/go-usb-ai-ncp-runtime-adapter-hermes-http@0.1.1`
  - `@go-usb-ai/go-usb-ai-ncp-runtime-http-client@0.1.1`
  - `@go-usb-ai/go-usb-ai-ncp-runtime-stdio-client@0.1.1`
- 同步提交了 release 过程产生的版本文件、CHANGELOG，以及 `go-usb-ai` 已发布包对应的 `ui-dist` 产物刷新结果。
...
npm view @go-usb-ai/ncp version
npm view @go-usb-ai/core version
npm view @go-usb-ai/ncp-http-agent-server version
npm view @go-usb-ai/go-usb-ai-ncp-runtime-adapter-hermes-http version
npm view @go-usb-ai/go-usb-ai-ncp-runtime-http-client version
...
- 抽样 `npm view` 已确认 `go-usb-ai`、`@go-usb-ai/ui`、`@go-usb-ai/ncp`、`@go-usb-ai/core`、`@go-usb-ai/ncp-http-agent-server` 以及 3 个新 runtime 包均可读到最新版本。
...
1. 直接从 NPM 安装对应新版本。
2. 若依赖 `go-usb-ai` 主包，使用 `go-usb-ai@0.17.12`。
3. 若依赖独立 NCP/runtime 包，按对应新版本更新依赖声明即可。

## 用户 / 产品视角的验收步骤

1. 在一个干净目录执行 `npm view go-usb-ai version`，确认可读到 `0.17.12`。
2. 执行 `npm view @go-usb-ai/ui version` 与 `npm view @go-usb-ai/ncp version`，确认可读到本次新版本。
3. 对新首发包分别执行：
   - `npm view @go-usb-ai/go-usb-ai-ncp-runtime-adapter-hermes-http version`
   - `npm view @go-usb-ai/go-usb-ai-ncp-runtime-http-client version`
   - `npm view @go-usb-ai/go-usb-ai-ncp-runtime-stdio-client version`
...
这次不是新功能开发，而是把已经存在的公开能力统一收敛到正式发布链路中，减少“代码已合入但 registry 不可用”的割裂状态，让 GoUsbAi 作为统一入口与能力编排层的可分发性更一致、更可依赖。
