...
- 自动生成并消费 UI changeset（`@go-usb-ai/ui` + `go-usb-ai`）。
- 实际发布包：
  - `@go-usb-ai/ui@0.5.48`
  - `go-usb-ai@0.8.62`
- 自动创建 Git Tag：
...
- 线上可用性冒烟（非仓库目录）：
  - 命令：`cd /tmp && npx -y go-usb-ai@0.8.62 --version`
  - 观察点：输出版本号 `0.8.62`。
...
1. 在 npm 检查版本：`@go-usb-ai/ui@0.5.48` 与 `go-usb-ai@0.8.62` 已可见。
2. 在任意非仓库目录执行：`npx -y go-usb-ai@0.8.62 --version`。
3. 验收标准：
   - npm 可检索到目标版本；
   - CLI 可正常启动并返回版本号。
