...
- `pnpm -C packages/extensions/go-usb-ai-channel-runtime exec node --import tsx --test src/channels/qq.service.test.ts`
...
- `node .agents/skills/post-edit-maintainability-guard/scripts/check-maintainability.mjs --non-feature --paths packages/extensions/go-usb-ai-channel-runtime/src/channels/qq.service.ts packages/extensions/go-usb-ai-channel-runtime/src/channels/qq.service.test.ts packages/extensions/go-usb-ai-channel-runtime/src/index.ts`
...
不涉及部署。涉及 `@go-usb-ai/channel-runtime` 源码变更，后续需要随统一 NPM beta/stable 发布批次发出。

## 用户/产品视角的验收步骤

1. 启动非开发态 GoUsbAi 服务。
2. 观察日志：QQ 成功时应出现 `QQ bot connected`；底层 SDK 不 ready 时，应在超时后出现 `[qq] start failed ... retry`，不再沉默。
3. 运行 `go-usb-ai channels status` 时，QQ 的 running 状态应只代表真实连接完成。
