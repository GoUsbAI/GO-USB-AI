...
- 将 `packages/go-usb-ai/src/cli/runtime.ts` 从单体 God Class 拆分为编排层 + 职责模块。
- 新增模块：
  - `packages/go-usb-ai/src/cli/types.ts`
  - `packages/go-usb-ai/src/cli/config-path.ts`
  - `packages/go-usb-ai/src/cli/config-reloader.ts`
  - `packages/go-usb-ai/src/cli/missing-provider.ts`
  - `packages/go-usb-ai/src/cli/workspace.ts`
...
```bash
export PATH="/opt/homebrew/opt/node@20/bin:$PATH"
TMP_HOME=$(mktemp -d /tmp/go-usb-ai-refactor-smoke.XXXXXX)
...
GOUSB_AI_HOME="$TMP_HOME" node packages/go-usb-ai/dist/cli/index.js config get 'agents.defaults.model'
