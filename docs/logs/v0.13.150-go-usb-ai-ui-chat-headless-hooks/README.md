# v0.13.150-go-usb-ai-ui-chat-headless-hooks

## 迭代完成说明（改了什么）
- 为 `packages/go-usb-ai-ui/src/components/chat` 新增一组可复用的 headless hooks，用于承接非业务的展示与交互逻辑。
- 新增 hooks：
  - `use-element-width`
...
- `pnpm -C packages/go-usb-ai-ui exec eslint src/components/chat/ChatConversationPanel.tsx src/components/chat/index.ts src/components/chat/hooks src/components/chat/ui/chat-input-bar/chat-slash-menu.tsx src/components/chat/ui/chat-message-list/chat-code-block.tsx`

## 发布/部署方式
- 本迭代仅为 `go-usb-ai-ui` chat 内部结构优化，不涉及协议、数据格式或部署链路变更。
