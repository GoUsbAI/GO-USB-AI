...
- 优化 Docker 官网一键脚本 [`apps/landing/public/install-docker.sh`](../../../apps/landing/public/install-docker.sh)：
  - 容器内启动链路从 `go-usb-ai serve` 调整为 `go-usb-ai init && exec go-usb-ai serve`，补齐应用初始化并确保前台主进程语义。
  - 默认优先启用 `docker run --init`；若当前 Docker runtime 不支持 `--init`，脚本降级为无 `--init` 启动并打印 warning，而不是直接失败。
  - 启动日志更新为 `npm install + go-usb-ai init + first start`，让用户明确首启包含初始化步骤。
- 同步更新 Docker 教程文档：
...
- dry-run 验证：
  ...
  - 观察点：命令中包含 `--init`，且容器内命令为 `go-usb-ai init && exec go-usb-ai serve`。
- 冒烟验证（本机无 Docker 时，使用隔离 fake-docker 合约冒烟）：
  - 路径 A（runtime 支持 `--init`）：
    - 脚本成功，抓取到 `docker run` 参数包含 `--init`。
    - 抓取到容器命令包含 `go-usb-ai init && exec go-usb-ai serve`。
...
- 发布 landing 静态资源以更新线上脚本与文档入口：
  ...
  - `pnpm deploy:landing`
- 发布后用户继续使用同一命令：
  - `curl -fsSL https://go-usb-ai.io/install-docker.sh | bash`

## 用户/产品视角的验收步骤
1. 在已安装 Docker 的机器执行：`curl -fsSL https://go-usb-ai.io/install-docker.sh | bash`。
2. 观察终端输出应包含：
   - `Bootstrapping runtime inside container (npm install + go-usb-ai init + first start)`
   - `Health check passed`
   - `UI: http://127.0.0.1:<ui-port>`
3. 执行 `docker logs -f go-usb-ai`，确认容器持续运行。
