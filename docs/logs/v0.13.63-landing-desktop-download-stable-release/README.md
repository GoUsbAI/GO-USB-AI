...
- 冒烟验证（用户可见文案与下载映射已入产物）：
  - `rg -n "Download GoUsbAi Desktop|Open Anyway|下载 GoUsbAi Desktop|仍要打开|v0.9.21-desktop.8" apps/landing/dist/assets/main-*.js`
- 正式版资产验证：
  ...
  - Windows：下载 `GoUsbAi.Desktop-win32-x64-unpacked.zip` 后验证包含 `GoUsbAi Desktop.exe`
...
- 前端发布：执行 `pnpm deploy:landing`，已完成部署（`https://0a943b5c.go-usb-ai-landing.pages.dev`）。

## 用户/产品视角的验收步骤
1. 打开官网首页，首屏可看到“下载 GoUsbAi Desktop”区域。
