...
- `/` 只做语言重定向（基于 `localStorage(go-usb-ai.landing.locale)` 和浏览器语言）
- `/en/`、`/zh/` 为独立可索引页面
- 新增多入口构建配置与页面入口：
...
- 冒烟验证（用户可见）：
- 本地预览：`pnpm --filter @go-usb-ai/landing preview --host 127.0.0.1 --port 4173`
- 访问 `/`，确认自动重定向到 `/en/` 或 `/zh/`
...
1. 打开 `https://go-usb-ai.io/`，确认会自动进入 `/en/` 或 `/zh/`。
2. 在 `/en/` 页面检查英文文案、按钮与 docs 链接指向 `https://docs.go-usb-ai.io/en/`。
3. 切换为中文后确认跳转到 `/zh/`，并检查中文文案与 docs 链接指向 `https://docs.go-usb-ai.io/zh/`。
