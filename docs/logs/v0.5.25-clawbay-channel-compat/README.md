...
- 在 `@go-usb-ai/openclaw-compat` 补齐 channel 兼容关键能力：
...
- 在 `go-usb-ai` CLI 新增 `channels add --channel <id> ...`，用于 OpenClaw 风格 channel setup（如 `--code` / `--token` / `--url`）。
- 插件配置映射新增桥接：
...
- 对齐 OpenClaw 的 `agentPrompt.messageToolHints`：
  ...
  - 在 `go-usb-ai-core` system prompt 的 Messaging 区块注入 channel-specific message hints
...
- 本地路径安装 ClawBay 插件：
  - `go-usb-ai plugins install /Users/peiwang/Projects/clawpage/packages/clawbay-channel`
  - `go-usb-ai plugins list --json` 显示：
...
- channel setup：
  - `go-usb-ai channels add --channel clawbay --code AB12CD`
  - 配置写入 `plugins.entries.clawbay-channel.config.pairingCode`
- npm 安装路径验证：
  - `go-usb-ai plugins install @clawbay/clawbay-channel` 可完成安装与发现
- messageToolHints 注入验证：
  - 使用临时 `GOUSB_AI_HOME` 安装 `clawbay-channel` 后，调用兼容层解析 `resolvePluginChannelMessageToolHints({ channel: "clawbay" })` 返回非空
