...
- 新增项目内治理 skill：`.codex/skills/go-usb-ai-marketplace-skill-integration/SKILL.md`
  - 固化 “给 GoUsbAi 的 skill marketplace 新增 skill” 的产品原则、职责边界与解耦约束。
  - 明确 skill 层负责用户旅程、引导、readiness check、风险披露与确认规则，不伪装成第三方 runtime 本体。
- 新增 marketplace skill：`skills/opencli/SKILL.md`
  - 将上游 `opencli` 能力封装为 GoUsbAi 风格的完整 skill。
...
- 最终结果：
  - `opencli` 条目已成功出现在 `https://marketplace-api.go-usb-ai.io/api/v1/skills/items`
  - content 接口可返回完整 `SKILL.md`
  - `go-usb-ai skills install opencli` 可成功安装
...
- 维护性守卫：
  - `PATH=/opt/homebrew/bin:$PATH node .codex/skills/post-edit-maintainability-guard/scripts/check-maintainability.mjs --paths skills/opencli/SKILL.md .codex/skills/go-usb-ai-marketplace-skill-integration/SKILL.md`
...
- 远端条目校验：
  - `curl -sS https://marketplace-api.go-usb-ai.io/api/v1/skills/items/opencli`
  - 观察点：`ok: true`，`slug=opencli`，`install.kind=marketplace`
- 远端内容校验：
  - `curl -sS https://marketplace-api.go-usb-ai.io/api/v1/skills/items/opencli/content`
  - 观察点：`ok: true`，返回完整 `bodyRaw`
- 安装冒烟：
  - `tmp_dir=$(mktemp -d /tmp/go-usb-ai-marketplace-opencli.XXXXXX)`
  - `PATH=/opt/homebrew/bin:$PATH node packages/go-usb-ai/dist/cli/index.js skills install opencli --api-base https://marketplace-api.go-usb-ai.io --workdir "$tmp_dir"`
...
- 本地 publish / update 入口：
  - `PATH=/opt/homebrew/bin:$PATH node packages/go-usb-ai/dist/cli/index.js skills publish skills/opencli --meta skills/opencli/marketplace.json --api-base https://marketplace-api.go-usb-ai.io`
  - `PATH=/opt/homebrew/bin:$PATH node packages/go-usb-ai/dist/cli/index.js skills update skills/opencli --meta skills/opencli/marketplace.json --api-base https://marketplace-api.go-usb-ai.io`
- 本次实际执行：
...
1. 打开 marketplace skill 列表接口：
   - `https://marketplace-api.go-usb-ai.io/api/v1/skills/items?page=1&pageSize=20`
   - 确认能看到 `OpenCLI`
2. 打开 skill 详情接口：
   - `https://marketplace-api.go-usb-ai.io/api/v1/skills/items/opencli`
   - 确认中英文摘要、描述、来源仓库与安装命令正常
3. 打开 skill 内容接口：
   - `https://marketplace-api.go-usb-ai.io/api/v1/skills/items/opencli/content`
   - 确认可看到完整 `SKILL.md` 内容
4. 在一台目标机器上执行安装：
   - `go-usb-ai skills install opencli`
   - 确认安装成功并生成 `skills/opencli/SKILL.md`
5. 在 GoUsbAi 中请求一个 `opencli` 相关任务：
