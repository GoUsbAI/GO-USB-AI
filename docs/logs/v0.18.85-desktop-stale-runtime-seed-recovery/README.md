...
本次修复 Windows preview 安装后可能启动失败的问题：用户机器上可能已经存在同版本 `current.json` / `state.json` / `versions/<version>`，而旧逻辑会优先解析这个结构上可用但内容过期的 current bundle，导致 Electron 启动旧 runtime 后在 health check 阶段超时，表现为 `Unable to start local GoUsbAi runtime` 和 `Runtime health check timeout: TypeError fetch failed`。
...
2. 在存在旧 `@go-usb-ai/desktop` 用户数据目录的机器上直接启动。
3. 20 秒内进入真实 GoUsbAi 窗口，而不是停在 `Starting GoUsbAi` 或弹出 `Unable to start local GoUsbAi runtime`。
