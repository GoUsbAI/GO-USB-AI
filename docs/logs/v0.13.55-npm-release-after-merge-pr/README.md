...
- 按依赖链完成 19 个 npm 包发布（`@go-usb-ai/*` + `go-usb-ai`），将本地版本推送至 npm 并创建对应 git tag。
- 本次发布后的关键版本：`go-usb-ai@0.9.22`、`@go-usb-ai/core@0.7.5`、`@go-usb-ai/server@0.6.9`、`@go-usb-ai/ui@0.6.12`。
...
- 冒烟测试（非仓库目录）：
  - 命令：`npm i go-usb-ai@0.9.22`、`npx go-usb-ai --version`、`npx go-usb-ai --help`（在 `/tmp` 临时目录执行并清理）。
...
## 用户/产品视角的验收步骤
1. 在任意新目录执行 `npm i go-usb-ai@0.9.22`。
2. 执行 `npx go-usb-ai --version`，期望输出 `0.9.22`。
3. 执行 `npx go-usb-ai --help`，期望看到完整命令列表（如 `start`、`serve`、`agent`）。
4. 如需验证扩展链路，可安装对应 `@go-usb-ai/*` 新版本并执行现有集成流程。
