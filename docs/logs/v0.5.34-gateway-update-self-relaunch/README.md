...
- 用户在本地 `go-usb-ai start` 场景下让 AI 执行更新后，服务可能退出但未自动恢复，导致“长时间无回复”。
- 根因：当重启请求来自“当前托管服务进程自身”时，原逻辑仅 `process.exit`，缺少自拉起接力。
- 追加根因：helper 继承 `process.execArgv` 时，在 `tsx/eval/loader` 运行模式会把启动参数污染到 `go-usb-ai start`，导致接力启动失败。
...
- 在 `CliRuntime.requestRestart` 中，当策略为 `background-service-or-exit` / `exit-process` 且当前进程就是 `service.json` 记录的 PID 时：
  ...
  - 当前进程退出后，helper 自动执行 `go-usb-ai start --ui-port <current-port>` 拉起新服务。
...
- `packages/go-usb-ai/src/cli/runtime.ts`
  ...
  - 新增 `GOUSB_AI_SELF_RELAUNCH_CLI`（可选）用于测试/调试覆盖重启命令入口。
...
- 验证结果：`go-usb-ai status --json` 返回 `exitCode: 0`、`process.running: true`、`level: "healthy"`（端口 `19001`）。
- 额外清理：执行 `go-usb-ai stop` 后 `service.json` 清除，状态回到 stopped（预期行为）。
