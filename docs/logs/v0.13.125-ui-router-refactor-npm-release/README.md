...
- 生成并消费 changeset，完成以下包版本提升：
  - `@go-usb-ai/server` `0.6.12` -> `0.6.13`
  - `go-usb-ai` `0.9.25` -> `0.9.26`
- 发布过程中修复一处阻断：
  - 为 `chat.controller.ts` 中流式方法补充显式返回类型，解决 `dts` 构建 `TS2742`。
- 发布结果（npm）：
  - `@go-usb-ai/server@0.6.13`
  - `go-usb-ai@0.9.26`
  - `@go-usb-ai/ncp-react@0.1.0`（流程中识别为本地版本未发布并一并发布）
...
## 用户/产品视角的验收步骤
1. 在 npm 查询 `@go-usb-ai/server@0.6.13`、`go-usb-ai@0.9.26` 是否可见。
2. 全局安装并检查版本：
   - `npm i -g go-usb-ai@0.9.26`
   - `go-usb-ai --version`
