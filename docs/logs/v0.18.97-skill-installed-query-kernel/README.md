...
- 将 installed skill 查询、详情生成、frontmatter 解析、`always` 判断、scope/query 过滤从 `@go-usb-ai/service` 收归 `@go-usb-ai/kernel` 的 `SkillManager`。
- 新增 kernel 级 `skill-frontmatter` 工具，作为 `SKILL.md` frontmatter 与 localized text map 的唯一解析事实源。
- `@go-usb-ai/service` 的 `SkillsQueryService` 只保留 CLI workspace adapter 与 marketplace 网络查询；marketplace metadata 复用 kernel 的 frontmatter 工具，不再维护平行解析实现。
- 清理 `@go-usb-ai/service` 对 `yaml` 的直接依赖，改由 kernel 持有解析依赖。
...
- `pnpm --filter @go-usb-ai/service lint`：通过，保留既有 18 个 warning，无新增 error。
- `pnpm --filter @go-usb-ai/service test -- --run src/cli/commands/skills/skills-query.service.test.ts src/cli/commands/skills/marketplace.publish.test.ts`
...
- 本次未执行发布或部署。
- 变更影响 `@go-usb-ai/kernel` 与 `@go-usb-ai/service` 源码和依赖声明，后续随统一包发布批次发布。
...
- 本次遵循删除优先与职责收敛：删除 service 侧 installed skill 的重复读取、解析、过滤和 view 生成逻辑，把 GoUsbAi 自身能力清单的自感知规则收回 kernel。
...
- 不涉及 NPM 包发布。
- 待后续统一发布批次评估：`@go-usb-ai/kernel`、`@go-usb-ai/service`。
