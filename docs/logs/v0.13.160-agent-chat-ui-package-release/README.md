...
## 迭代完成说明
- 完成 `@go-usb-ai/agent-chat-ui` 首次独立发布闭环。
- 完成 `@go-usb-ai/ui` 依赖新包后的联动发布。
...
- 远端校验：`git ls-remote --tags origin 'refs/tags/@go-usb-ai/agent-chat-ui@0.1.1' 'refs/tags/@go-usb-ai/ui@0.7.0' 'refs/tags/@go-usb-ai/core@0.8.0' 'refs/tags/@go-usb-ai/server@0.7.0' 'refs/tags/go-usb-ai@0.10.0'`
- npm 校验：
  - `npm view @go-usb-ai/ui version`
  - `npm view @go-usb-ai/core version`
  - `npm view @go-usb-ai/server version`
  - `npm view go-usb-ai version`
  - `npm access get status @go-usb-ai/agent-chat-ui`
...
## 用户 / 产品视角的验收步骤
- 在 npm 上确认 `@go-usb-ai/ui@0.7.0`、`@go-usb-ai/core@0.8.0`、`@go-usb-ai/server@0.7.0`、`go-usb-ai@0.10.0` 可见。
- 在 git 远端确认 `master` 已包含发布提交，且关键 tags 可见。
- 对 `@go-usb-ai/agent-chat-ui`，确认包访问状态为 `public`；若 npm 页面检索存在短暂延迟，以 registry 同步完成后的可见结果为准。
