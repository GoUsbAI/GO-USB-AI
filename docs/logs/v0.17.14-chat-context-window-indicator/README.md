...
- 已通过：
  ...
  - `pnpm -C packages/go-usb-ai-ui test -- --run src/features/chat/utils/ncp-session-adapter.utils.test.ts`
  - `pnpm -C packages/go-usb-ai-core tsc -p tsconfig.json`
  - `pnpm -C packages/go-usb-ai-agent-chat-ui tsc -p tsconfig.json`
  - `pnpm -C packages/go-usb-ai-ui tsc --noEmit`
  - `pnpm -C packages/go-usb-ai tsc -p tsconfig.json`
- 治理结果：
  - `pnpm lint:new-code:governance` 仍未完全通过。
  - 当前剩余阻塞不在本次功能链路本身，而在仓库既有冻结目录治理：`packages/go-usb-ai-core/src/agent` 被 frozen-directory 规则阻断，要求先对子树做进一步结构拆分。
  - `pnpm check:governance-backlog-ratchet` 仍失败，原因是仓库既有 `docFileNameViolations` 基线为 `11`，当前为 `13`，属于历史文档治理基线问题，不是本次上下文窗口能力直接引入。

## 发布/部署方式

- 前端能力随常规 GoUsbAi UI / CLI 包构建发布，无需额外部署步骤。
...
- 本次是否已尽最大努力优化可维护性：否。
  - 主要阻碍是仓库现有治理已经把 `packages/go-usb-ai-core/src/agent` 标记为 frozen directory；只要继续触达该目录，就会被要求先完成一轮更大范围的子树拆分。
...
- 是否让总代码量、分支数、函数数、文件数或目录平铺度下降，或至少没有继续恶化：
  - 总 diff 为非测试代码净增，但同时伴随较大规模删除与收敛；`go-usb-ai-core/src/agent/subagent.ts` 等旧文件被迁移到更合规命名，`types.ts` 体积下降。
...
- 目录结构与文件组织是否满足当前项目治理要求：
  ...
  - 但 `packages/go-usb-ai-core/src/agent` 仍受 frozen-directory 规则阻断，后续若继续动该目录，必须先做明确子树拆分。
- 本次可维护性评估是否基于独立复核：
...
- 后续触发条件：
  - 需先决定是继续偿还 `packages/go-usb-ai-core/src/agent` 的目录冻结债务，还是将该部分重命名/治理改动拆出单独批次处理后再统一发布。
