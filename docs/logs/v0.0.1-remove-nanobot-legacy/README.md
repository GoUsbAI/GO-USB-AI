...
- 仅保留 `~/.go-usb-ai` 作为默认数据目录
- 内置技能与 bridge 文案统一改为 go-usb-ai 命名

## 变更内容

- 用户可见变化
  - 默认数据目录仅 `~/.go-usb-ai`（不再回退 `~/.nanobot`）
  - WhatsApp bridge 文案与默认 auth 目录改为 `~/.go-usb-ai/whatsapp-auth`
  - 内置技能元信息与示例统一为 go-usb-ai
- 关键实现点
  - `getDataPath` 去除 legacy 回退逻辑
  - skills metadata key 改为 `go-usb-ai`
...
- `onboard` 在 `/tmp/go-usb-ai-smoke` 生成 config/workspace
