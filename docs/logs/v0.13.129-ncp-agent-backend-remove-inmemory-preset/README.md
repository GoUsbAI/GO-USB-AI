...
- 删除 `@go-usb-ai/ncp-toolkit` 中的 `DefaultNcpInMemoryAgentBackend` 与 `DefaultNcpInMemoryAgentBackendConfig`
...
- `pnpm --filter @go-usb-ai/ncp-demo-backend build`
...
- 若对外发布 `@go-usb-ai/ncp-toolkit`，需要在 release notes 中明确说明 `DefaultNcpInMemoryAgentBackend` 已移除，推荐迁移到 `DefaultNcpAgentBackend`
- 远程 migration 不适用，因为未涉及后端数据库变更

## 用户/产品视角的验收步骤

1. 在业务代码中从 `@go-usb-ai/ncp-toolkit` 导入 `DefaultNcpAgentBackend`
