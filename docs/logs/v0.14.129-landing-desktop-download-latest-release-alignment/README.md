...
- 更新 landing 页桌面下载兜底元数据：
  ...
  - macOS Intel 兜底下载地址同步改为显式 `-x64` 命名的 `GoUsbAi.Desktop-0.0.60-x64.dmg`。
...
- 发布提交验证：
  - 通过 GitHub API 校验 `releases/latest` 当前返回：
    ...
    - release 页面：`https://github.com/Peiiii/go-usb-ai/releases/tag/v0.13.24-desktop.5`
    - release notes 中声明的发布提交：`0a1832bf`
  - 并确认最新正式 release 资产列表已包含：
    - `GoUsbAi.Desktop-0.0.60-arm64.dmg`
    - `GoUsbAi.Desktop-0.0.60-x64.dmg`
    - `GoUsbAi.Desktop-win32-x64-unpacked.zip`
    - `GoUsbAi.Desktop-0.0.60-linux-x64.AppImage`
...
- 发布提交验证：
  - 使用 GitHub API 校验 `https://api.github.com/repos/Peiiii/go-usb-ai/releases/latest`
  - 结果：
...
3. 在 macOS Intel 设备或对应下载卡片中，确认下载链接为 `GoUsbAi.Desktop-0.0.60-x64.dmg`，不再指向旧的无后缀 Intel 包。
