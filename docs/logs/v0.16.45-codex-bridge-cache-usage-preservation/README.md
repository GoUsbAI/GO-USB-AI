...
1. 在 `@go-usb-ai/go-usb-ai-ncp-runtime-plugin-codex-sdk` 的 bridge 链路里，保留并转写上游 `chat/completions` usage 中的嵌套细节，不再把 `input_tokens_details` / `output_tokens_details` 强行写成 `null`。
...
- `pnpm -C packages/go-usb-ai test -- run src/cli/commands/ncp/compat/codex-openai-responses-bridge.test.ts src/cli/commands/compat/codex-openai-responses-bridge-request.test.ts`
- `pnpm -C packages/go-usb-ai test -- run src/cli/commands/ncp/compat/codex-responses-capability.test.ts`
- `pnpm -C packages/go-usb-ai-core test -- run src/providers/openai_provider.test.ts`
- `pnpm -C packages/extensions/go-usb-ai-ncp-runtime-plugin-codex-sdk tsc`

额外尝试但未作为本次验收阻断项的命令：

- `pnpm -C packages/go-usb-ai test -- run src/cli/commands/compat/codex-runtime-plugin-provider-routing.test.ts src/cli/commands/compat/codex-runtime-plugin-production-build.test.ts`
  - 结果：失败。
...
- `pnpm lint:maintainability:guard`
  - 结果：未完成通过。
  - 原因：治理脚本把当前工作树中的其它已修改文件也纳入检查，最终阻塞点来自与本次改动无关的 [session-search-store.service.ts](/Users/peiwang/Projects/nextbot/packages/go-usb-ai/src/cli/commands/ncp/session-search/session-search-store.service.ts) 既有修改，而不是本次 bridge 修复文件本身。
...
1. 按既有 release 流程发布 `@go-usb-ai/go-usb-ai-ncp-runtime-plugin-codex-sdk` 与依赖它的宿主包。
...
3. 执行 `go-usb-ai usage --history --json` 或查看对应 usage 记录。
...
- 目录结构与文件组织是否满足当前项目治理要求：本次触达文件已满足角色后缀治理；但 `packages/extensions/go-usb-ai-ncp-runtime-plugin-codex-sdk/src` 目录仍处于文件数预算上限附近，且 `codex-openai-responses-bridge-stream.utils.ts` 接近单文件预算，后续如 bridge 继续增长，应优先按 responsibility 拆分。
