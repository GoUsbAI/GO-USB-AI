# v0.15.97-local-go-usb-ai-install-command

## 迭代完成说明

- 为根目录新增可直接执行的本地安装命令：`pnpm local-install:go-usb-ai`
- 补齐脚本 [`scripts/local-install-go-usb-ai.mjs`](../../../scripts/local-install-go-usb-ai.mjs)，让该命令真正完成以下动作：
  - 构建 `go-usb-ai` 及其 workspace 依赖
  - 使用 `pnpm link --global` 把本地仓库里的 `packages/go-usb-ai` 链接到全局环境
  - 自动补齐 `PNPM_HOME` 到可用的全局 bin 目录，绕开本机未执行 `pnpm setup` 时的失败
  - 安装后立即执行 `go-usb-ai --version` 验证全局命令可用
- 这次没有继续走 tarball 全局安装方案，因为它会把内部依赖重新解到已发布版本，不能保证体验到当前仓库里的本地改动

## 测试/验证/验收方式

- 命令：`pnpm local-install:go-usb-ai`
- 结果：通过
- 关键观察点：
  - `pnpm -r --filter go-usb-ai... build` 完成
  - `pnpm -C packages/go-usb-ai link --global` 完成
  - `go-usb-ai --version` 输出 `0.17.6`
...
1. 在仓库根目录执行：`pnpm local-install:go-usb-ai`
2. 等待构建和全局链接完成
3. 直接执行：`go-usb-ai --version`
4. 若需要进一步体验，再直接执行你要的真实命令，例如 `go-usb-ai --help`、`go-usb-ai start` 或其它本地验证命令

## 可维护性总结汇总

- 长期目标对齐 / 可维护性推进：
  - 本次是沿着“统一入口更易被本地验证”的方向推进了一小步，把原本分散的手工构建、全局链接、可执行校验收敛成一个根命令，降低了使用 GoUsbAi 作为默认入口时的本地验证摩擦。
- 可维护性复核结论：通过
- 本次顺手减债：是
- 代码增减报告：
  - 新增：77 行
...
- 可维护性总结：
  - 这次没有把复杂度搬到别处，而是把“本地安装 go-usb-ai”这件事收敛成一个能直接记住的根命令。
