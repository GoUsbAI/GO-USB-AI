...
- 新增脚本：`scripts/dev-platform-runner.mjs`
  ...
  - 支持 `--check` 启动前健康检查
  - 支持通过 `GOUSB_AI_PLATFORM_WRANGLER_PERSIST_TO` 把 wrangler 本地状态写到隔离目录（便于无污染冒烟）
...
- 一键命令验证
  ...
  - `GOUSB_AI_PLATFORM_BACKEND_PORT=8890 GOUSB_AI_PLATFORM_FRONTEND_PORT=5190 GOUSB_AI_PLATFORM_WRANGLER_PERSIST_TO=/tmp/... pnpm dev:platform:stack:migrate`
  - 观察点：
    - `http://127.0.0.1:8890/health` 返回 `ok: true`
    - `http://127.0.0.1:5190` 返回平台前端页面（title 含 `GoUsbAi Platform Console`）
- 业务链路冒烟（隔离路径）
  - 注册 admin + user
  - 用户提交充值申请，admin 审核通过
  - 用户余额增加
...
- CLI 登录冒烟
  - `go-usb-ai login --api-base http://127.0.0.1:8793 --register ...`
  - 观察点：`GOUSB_AI_HOME/config.json` 中 `providers.go-usb-ai.apiKey` 为 `nca.` 前缀 token，`apiBase` 为 `.../v1`
