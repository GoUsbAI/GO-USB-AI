...
Stable desktop release aligned to GoUsbAi `0.18.4`.
...
- Unified npm release batch is now published on the public registry, including `go-usb-ai@0.18.4`, `@go-usb-ai/ui@0.12.12`, `@go-usb-ai/server@0.12.11`, `@go-usb-ai/core@0.12.11`, `@go-usb-ai/ncp@0.5.5`, `@go-usb-ai/ncp-agent-runtime@0.3.15`, and the related runtime, channel, toolkit, bridge, remote, and compatibility packages in the same batch.
...
- Desktop packaging is aligned with the current `go-usb-ai` CLI entrypoint contract. Seed bundle generation, runtime bootstrap, packaged seed verification, and DMG smoke now all run against `dist/cli/app/index.js`, which matches the shipped runtime layout.
...
**Full Changelog**: https://github.com/Peiiii/go-usb-ai/compare/v0.18.0-desktop.1...v0.18.4-desktop.2

## 中文版

这是与 GoUsbAi `0.18.4` 对齐的桌面正式稳定版。
...
- 统一 NPM 发版批次已经发布到官方 registry，包括 `go-usb-ai@0.18.4`、`@go-usb-ai/ui@0.12.12`、`@go-usb-ai/server@0.12.11`、`@go-usb-ai/core@0.12.11`、`@go-usb-ai/ncp@0.5.5`、`@go-usb-ai/ncp-agent-runtime@0.3.15`，以及同批次的 runtime、channel、toolkit、bridge、remote、compat 等公开包。
- Native NCP 工具结果现在会先经过显式的预算化 content item 治理链路，大截图和大 JSON 工具结果不再直接打爆模型上下文窗口；图片类工具结果也能作为真实 `input_image` 进入多模态模型输入。
- Desktop 打包链路已经和当前 `go-usb-ai` CLI 入口合同对齐。seed bundle 构建、runtime bootstrap、包内 seed 校验和 DMG smoke 现在都统一指向真实产物 `dist/cli/app/index.js`。
...
**完整变更**: https://github.com/Peiiii/go-usb-ai/compare/v0.18.0-desktop.1...v0.18.4-desktop.2
