...
- 本次落地了飞书能力对齐路线的 Phase 1 底座，不是照搬 OpenClaw 壳层，而是按 GoUsbAi 的 AgentOS 目标吸收可复用的纯飞书层能力。
- 新增 [`@go-usb-ai/feishu-core`](../../../../packages/extensions/go-usb-ai-feishu-core/package.json)，沉淀了飞书配置模型、多账号解析、Lark client 管理、基础 probe 与消息内容转换能力。
- [`@go-usb-ai/core`](../../../../packages/go-usb-ai-core/src/config/schema.ts) 已切到新的飞书 schema，并补齐了标签与帮助文案。
- [`@go-usb-ai/channel-runtime`](../../../../packages/extensions/go-usb-ai-channel-runtime/src/channels/feishu.ts) 的 Feishu runtime 已改为复用 `@go-usb-ai/feishu-core`，接入 `accountId` 路由、多账号、`domain`/brand、mention 策略与 richer inbound conversion。
- [`@go-usb-ai/ui`](../../../../packages/go-usb-ai-ui/src/components/config/channel-form-fields.ts) 已补齐当前真实支持的飞书关键字段，避免产品表面与运行时能力继续错位。
- 设计与判断沉淀见：
...
- 已完成定向验证：
  - `pnpm install`
  - `pnpm -C packages/extensions/go-usb-ai-feishu-core test`
  - `pnpm -C packages/extensions/go-usb-ai-feishu-core lint`
  - `pnpm -C packages/extensions/go-usb-ai-feishu-core tsc`
  ...
  - `pnpm -C packages/extensions/go-usb-ai-channel-runtime lint`
...
5. 观察复杂飞书消息进入 GoUsbAi 后的内容展示，确认不再只剩粗糙纯文本降级。
