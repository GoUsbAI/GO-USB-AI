...
- 修复 Linux AppImage 冒烟脚本：
  ...
  - 优先选择真实可执行文件 `@go-usb-aidesktop`，避免误用 `AppRun` 导致运行时初始化失败。
- 修复 macOS x64 冒烟路径匹配：
...
- 修复 landing 下载页兜底资产与匹配规则：
  - `macX64Dmg` 兜底链接改为 `GoUsbAi.Desktop-0.0.27.dmg`。
...
- 本地验证：
  ...
  - 下载 release 资产后执行：
    - `hdiutil verify GoUsbAi.Desktop-0.0.27-arm64.dmg`
    - `hdiutil verify GoUsbAi.Desktop-0.0.27.dmg`
    - `unzip -l GoUsbAi.Desktop-win32-x64-unpacked.zip | rg "GoUsbAi Desktop\.exe"`
...
- Landing 部署：
  ...
  - 本次部署地址：`https://d409e508.go-usb-ai-landing.pages.dev`
...
- Windows 用户解压后可直接看到并启动 `GoUsbAi Desktop.exe`。
- Linux 用户执行：
  - `chmod +x GoUsbAi.Desktop-*-linux-x64.AppImage`
  - `./GoUsbAi.Desktop-*-linux-x64.AppImage`
