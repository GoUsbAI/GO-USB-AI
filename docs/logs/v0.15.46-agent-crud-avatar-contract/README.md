...
- 将 Agent 自管理说明从“只讲创建”收敛为统一的 Agent CRUD 契约，明确 AI 管理 Agent 时应优先使用 `go-usb-ai agents list|new|update|remove --json`，而不是直接改 `config.json` / `agents.list`。
...
- 在核心系统提示与 `go-usb-ai-self-manage` skill 中补齐同一条高优先级约束，并补了对应测试，确保提示系统本身也更清晰明确。

## 测试/验证/验收方式

- `PATH=/opt/homebrew/bin:/Users/peiwang/.nvm/versions/node/v22.16.0/bin:$PATH pnpm -C packages/go-usb-ai-core test src/agent/tests/context.test.ts`
- `PATH=/opt/homebrew/bin:/Users/peiwang/.nvm/versions/node/v22.16.0/bin:$PATH pnpm -C packages/go-usb-ai-core tsc`
- `diff -u /tmp/go-usb-ai-agent-crud-docs.txt /tmp/go-usb-ai-agent-crud-packaged.txt`
  - 通过，确认 `docs/USAGE.md` 与 `packages/go-usb-ai/resources/USAGE.md` 的 Agent CRUD 区块完全一致。
- `PATH=/opt/homebrew/bin:/Users/peiwang/.nvm/versions/node/v22.16.0/bin:$PATH pnpm lint:maintainability:guard`
  ...
  - `packages/go-usb-ai-core/src/agent/context.ts` 正好达到文件预算上限。
  - `packages/go-usb-ai-ui/src/components/chat` 目录仍为已有豁免的超预算目录，本次未继续恶化。
...
4. 运行 `go-usb-ai-core` 的上下文测试，确认系统提示中也已经包含同样的 Agent CRUD 与头像约束。
...
- 目录结构与文件组织是否满足当前项目治理要求：本次未新增目录平铺问题；`packages/go-usb-ai-ui/src/components/chat` 的历史目录 warning 仍在，但本次未触碰该目录。`packages/go-usb-ai-core/src/agent/context.ts` 达到预算边缘，后续若继续扩展自管理提示，应优先拆分该文件中的自管理提示构建职责。
