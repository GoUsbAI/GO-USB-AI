...
## 做了什么
- `pnpm go-usb-ai start` 开发态不再强制 build，改为直接用 `tsx` 运行源码以提升启动速度。
- `pnpm dev` 走 `tsx watch`，自带热重启（watch）。
- 保留原先带 build 的脚本为 `pnpm -C packages/go-usb-ai dev:build` 以便需要完整编译时使用。
...
- 冒烟（非仓库目录）：
  - `GOUSB_AI_HOME=/tmp/go-usb-ai-smoke-$$ pnpm -C /Users/peiwang/Projects/nextbot dev`
