...
- 线上脚本内容校验：
  - `curl -fsSL https://go-usb-ai.io/install-docker.sh | rg "Bootstrapping runtime|Waiting for service readiness"`
- 真实 Docker 冒烟：
  - `curl -fsSL https://go-usb-ai.io/install-docker.sh | GOUSB_AI_DOCKER_CONTAINER_NAME=<name> GOUSB_AI_DOCKER_DATA_DIR=<tmp-dir> GOUSB_AI_DOCKER_UI_PORT=19391 GOUSB_AI_DOCKER_API_PORT=19390 bash`
  - 观察到：
    - 进度提示按 10 秒打印
...
- 已发布 landing 生产资源：
  ...
  - `pnpm dlx wrangler pages deploy apps/landing/dist --project-name go-usb-ai-landing`
