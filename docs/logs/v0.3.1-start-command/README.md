...
- 新增 `go-usb-ai start` 命令，默认启用 UI 后端并尝试启动 UI 前端 dev server
- 前端目录通过约定路径或 `GOUSB_AI_UI_DIR` 寻找，找不到则提示手动启动
- `start` 默认不打开后端 URL，优先打开前端 URL
- UI 静态资源打包进 `go-usb-ai`，安装版可直接通过 UI 后端访问前端页面
...
- 构建时复制 `go-usb-ai-ui` 产物到 `go-usb-ai/ui-dist`
...
GOUSB_AI_HOME=/tmp/go-usb-ai-start-smoke pnpm -C /Users/peiwang/Projects/nextbot/packages/go-usb-ai dev start --no-frontend --no-open --ui-port 18805 &
sleep 2
