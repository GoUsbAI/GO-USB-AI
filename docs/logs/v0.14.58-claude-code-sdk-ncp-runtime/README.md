...
- 新增纯 runtime 包 `@go-usb-ai/go-usb-ai-ncp-runtime-claude-code-sdk`，采用与 Codex SDK 一致的 NCP runtime 架构，支持 `claude` session type、Claude session 持久化、skills prompt 注入，以及 provider/apiKey 回退
- 新增插件包 `@go-usb-ai/go-usb-ai-ncp-runtime-plugin-claude-code-sdk`，通过标准 plugin 系统注册 Claude NCP runtime
...
- 发布过程中发现并修复两个真实问题：
  ...
  - plugin manifest 版本与 npm 包版本不一致，导致 `go-usb-ai plugins info` 展示错误，已修复并发布 plugin `0.1.2`
...
- 自动化测试
  ...
  - `pnpm --filter @go-usb-ai/server exec vitest run src/ui/router.marketplace-content.test.ts`
- marketplace / 数据验证
  - `pnpm -C workers/marketplace-api db:migrate:plugins:local`
  - `pnpm -C workers/marketplace-api db:migrate:plugins:remote`
  - `curl -sS 'https://marketplace-api.go-usb-ai.io/api/v1/plugins/items?page=1&pageSize=200' | jq '.data.items[] | select(.slug=="ncp-runtime-plugin-claude-code-sdk")'`
...
- 冒烟测试
  - 在 `/tmp` 隔离目录设置 `GOUSB_AI_HOME`
  - 执行 `node packages/go-usb-ai/dist/cli/index.js plugins install @go-usb-ai/go-usb-ai-ncp-runtime-plugin-claude-code-sdk`
  - 再执行 `node packages/go-usb-ai/dist/cli/index.js plugins info go-usb-ai-ncp-runtime-plugin-claude-code-sdk`
  - 预期结果：插件状态为 `loaded`，`NCP runtimes: claude`，`Version` 与 `Recorded version` 均为 `0.1.2`
- 可维护性自检
  - `node .codex/skills/post-edit-maintainability-guard/scripts/check-maintainability.mjs --paths packages/extensions/go-usb-ai-ncp-runtime-claude-code-sdk/src/index.ts packages/extensions/go-usb-ai-ncp-runtime-claude-code-sdk/src/claude-code-sdk-types.ts packages/extensions/go-usb-ai-ncp-runtime-claude-code-sdk/src/claude-code-runtime-utils.ts packages/extensions/go-usb-ai-ncp-runtime-claude-code-sdk/tsup.config.ts packages/go-usb-ai/src/cli/commands/ncp/create-ui-ncp-agent.test.ts`
...
- npm 发布
  - 先通过 changeset 生成版本
  - runtime 包发布为 `@go-usb-ai/go-usb-ai-ncp-runtime-claude-code-sdk@0.1.1`
...
1. 打开 GoUsbAi marketplace，确认能看到 `Claude NCP Runtime Plugin`
2. 在 CLI 中执行 `go-usb-ai plugins install @go-usb-ai/go-usb-ai-ncp-runtime-plugin-claude-code-sdk`
3. 执行 `go-usb-ai plugins info go-usb-ai-ncp-runtime-plugin-claude-code-sdk`
