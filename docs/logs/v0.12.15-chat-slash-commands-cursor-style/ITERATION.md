...
- 为 GoUsbAi Web Chat 增加了统一的 slash 命令机制接入：
  - 在 `@go-usb-ai/core` 的 `CommandRegistry` 中补充了文本命令解析与执行入口（`parseTextCommand` / `executeText`）。
  - 在 `go-usb-ai` 的 `GatewayAgentRuntimePool.processDirect` 增加命令优先拦截：输入以 `/` 开头时先执行命令，再决定是否进入模型。
  - 新增 `@go-usb-ai/server` 接口 `GET /api/chat/commands`，对前端暴露命令目录。
...
- 补充测试：
  - `@go-usb-ai/core`：`commands/registry.test.ts`
  - `go-usb-ai`：`agent-runtime-pool.command.test.ts`
  - `@go-usb-ai/server`：`router.chat.test.ts`（新增命令目录接口断言）

# 测试/验证/验收方式

- 单元测试：
  - `pnpm -C packages/go-usb-ai-core test src/commands/registry.test.ts`
  - `pnpm -C packages/go-usb-ai-server test src/ui/router.chat.test.ts`
  - `pnpm -C packages/go-usb-ai test -- --run src/cli/commands/agent-runtime-pool.command.test.ts`
- 类型检查：
...
- 备注（仓库现状）：
  - 全量 `pnpm lint` 在仓库已有历史问题处失败（`packages/go-usb-ai/src/cli/skills/clawhub.ts` 的 `no-useless-escape`）。
...
- 本次为 UI + server + runtime 变更，按常规 monorepo 发布流程：
  ...
  2. 发布 `@go-usb-ai/core`、`@go-usb-ai/server`、`@go-usb-ai/ui`、`go-usb-ai`。
