...
本次修复 GoUsbAi 服务启动过程中 `Deferred startup` 被非关键派生能力拖慢的问题。

根因：

- `GoUsbAiApp.warmDerivedCapabilities()` 在启动链路末尾 `await runtimeWarmupTask`，导致 `session_search` 初始化和 MCP server 预热成为 deferred startup 完成条件。
...
- 补充测试证明 `GoUsbAiApp.start()` 不等待派生能力 warmup 完成。
...
- `pnpm --filter go-usb-ai test -- --run src/cli/shared/services/gateway/tests/go-usb-ai-app.service.test.ts src/cli/shared/services/gateway/tests/service-capability-hydration.service.test.ts`
- `pnpm --filter go-usb-ai tsc`
- `pnpm --filter go-usb-ai test -- session-search`
...
- `pnpm check:governance-backlog-ratchet`
- 启动冒烟：`GOUSB_AI_STARTUP_TRACE=1 pnpm -C packages/go-usb-ai dev:build serve --ui-port 18893`
...
- 隔离启动冒烟：临时 `GOUSB_AI_HOME` 预置 800 个 session 文件后启动 `serve --ui-port 18894`，`Deferred startup` 在 `+106ms` 完成，`warm_ncp_capabilities` 后台 `150ms` 完成，`/api/health` 返回 `200` 且耗时 `0.010633s`。
...
1. 启动 GoUsbAi 服务并开启 startup trace。
...
- 将派生能力从 ready 阻塞条件中移出，启动 owner 仍保持在 `GoUsbAiApp`。
