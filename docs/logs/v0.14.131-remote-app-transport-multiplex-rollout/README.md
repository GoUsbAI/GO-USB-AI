...
- 将 GoUsbAi UI 的传输层升级为 `appClient + local/remote transport` 双模式。
...
- 相关设计来源：[`2026-03-23-go-usb-ai-remote-app-transport-multiplex-design.md`](../../plans/2026-03-23-go-usb-ai-remote-app-transport-multiplex-design.md)
...
- 本次实际发布版本：
  ...
  - `go-usb-ai@0.13.28`
- Worker deploy: `PATH=/opt/homebrew/bin:$PATH pnpm -C workers/go-usb-ai-provider-gateway-api run deploy`
- 线上 worker 版本：`d8fabcc7-d864-4c7c-a2dc-928c5f255b98`

## 用户 / 产品视角的验收步骤

1. 在一台已登录 GoUsbAi 账号并开启 remote connect 的设备上启动本地 UI。
