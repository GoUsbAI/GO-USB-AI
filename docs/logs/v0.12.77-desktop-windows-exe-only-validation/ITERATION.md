...
- Windows 桌面发布/验证统一切换为 EXE 本体链路（不再走 Setup 安装器）：
  - 新增脚本 `apps/desktop/scripts/smoke-windows-desktop.ps1`，直接启动 `GoUsbAi Desktop.exe` 并验证 `/api/health`。
  - 删除 `apps/desktop/scripts/smoke-windows-installer.ps1`。
  - 更新 `scripts/desktop-package-build.mjs`：Windows 构建目标从 `nsis` 改为 `dir`。
  - 更新 `scripts/desktop-package-verify.mjs`：Windows 校验从安装器改为 `win-unpacked/GoUsbAi Desktop.exe`。
  - 更新 `.github/workflows/desktop-validate.yml`：Windows job 改为 `desktop-windows-exe-smoke`，构建 `--win dir` 并执行 EXE 冒烟。
  - 更新 `.github/workflows/desktop-release.yml`：Windows 构建 `--win dir`，冒烟后打包 `GoUsbAi Desktop-win32-x64-unpacked.zip` 作为发布资产。
- 文档对齐为 EXE-only：
...
- 受影响链路构建与类型校验：
  ...
  - `pnpm -C packages/go-usb-ai-openclaw-compat build`（通过）
...
- Windows EXE 冒烟（核心验收）：
  - `pnpm -C apps/desktop exec electron-builder --win dir --x64 --publish never`（通过，产物为 `release/win-unpacked/GoUsbAi Desktop.exe`）
  - 使用 `wine64` 启动 `win-unpacked/GoUsbAi Desktop.exe` 并轮询 `http://127.0.0.1:18791/api/health`（通过，返回 `{"ok":true,"data":{"status":"ok"}}`）
- 一键链路回归：
  - `pnpm desktop:package:verify`（通过，当前平台执行 macOS DMG 链路并通过）

## 发布/部署方式
- 桌面发布沿用 `desktop-release` 工作流。
- Windows 资产改为 EXE 本体分发包：`GoUsbAi Desktop-win32-x64-unpacked.zip`（不再上传 Setup 安装器）。
- 本次无数据库/后端 migration。

## 用户/产品视角的验收步骤
1. 下载 Windows 资产 `GoUsbAi Desktop-win32-x64-unpacked.zip`。
