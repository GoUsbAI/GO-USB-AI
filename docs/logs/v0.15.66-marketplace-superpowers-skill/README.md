...
- 新增 marketplace skill：`skills/superpowers`
  - 新增 GoUsbAi 入口 `SKILL.md`，将 `obra/superpowers` 适配为适合 GoUsbAi marketplace 的单入口 workflow skill。
...
```bash
curl -sS https://marketplace-api.go-usb-ai.io/api/v1/skills/items/superpowers
```
...
```bash
PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai dev:build skills publish /Users/peiwang/Projects/nextbot/skills/superpowers --meta /Users/peiwang/Projects/nextbot/skills/superpowers/marketplace.json --api-base https://marketplace-api.go-usb-ai.io
```
...
PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai dev:build skills install superpowers --api-base https://marketplace-api.go-usb-ai.io --workdir "$tmp_dir"
...
- 仅有 1 条历史目录 warning，与本次改动无关：
  - `packages/go-usb-ai-ui/src/components/chat` 目录文件数预算 warning
...
- 当前发布结果：
  ...
  - 安装命令：`go-usb-ai skills install superpowers`
  - 远端查询：`https://marketplace-api.go-usb-ai.io/api/v1/skills/items/superpowers`

- 若后续需要更新远端条目，使用：

```bash
PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai dev:build skills update /Users/peiwang/Projects/nextbot/skills/superpowers --meta /Users/peiwang/Projects/nextbot/skills/superpowers/marketplace.json --api-base https://marketplace-api.go-usb-ai.io
```
...
- 本次是否已尽最大努力优化可维护性：是。本次没有把 `obra/superpowers` 整仓库平铺复制进 marketplace，而是只打包对 GoUsbAi 单 skill 模型真正有用的少量上游 workflow references，并补了一层薄的入口路由与来源映射。
- 是否优先遵循“删减优先、简化优先、代码更少更好、复杂度更低更好、清晰度更高更好”的原则：是。相较于“尝试复刻上游原生多目录自动发现机制”或“本地重写整套 superpowers 内容”，本次采用了更简单的单入口 skill + references 方案，尽量少引入 GoUsbAi 专属分叉。
...
- 抽象、模块边界、class / helper / service / store 等职责划分是否更合适、更清晰，是否避免了过度抽象或补丁式叠加：是。职责边界保持清晰：
  ...
  - `skills/superpowers/SKILL.md` 只负责 GoUsbAi 入口路由、边界声明与引用策略
