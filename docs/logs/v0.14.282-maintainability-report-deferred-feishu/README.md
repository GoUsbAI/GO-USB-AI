...
- 将 `packages/extensions/go-usb-ai-channel-plugin-feishu` 标记为报告级延后治理 workspace，并写明排除原因：非核心渠道插件、近期更新频率较低，当前优先治理核心运行链路。
...
- 验证点：
  ...
  - `packages/extensions/go-usb-ai-channel-plugin-feishu` 不再出现在 `violationsByWorkspace` 与 `directoryBudget.hotspots` 中。
...
- 确认报告中有 `Deferred maintainability workspaces:`，并列出 `packages/extensions/go-usb-ai-channel-plugin-feishu` 与排除原因。
