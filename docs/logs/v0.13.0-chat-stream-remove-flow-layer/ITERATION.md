...
- ESLint（受影响文件）：
  - `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai-ui exec eslint src/components/chat/managers/chat-stream.manager.ts src/components/chat/chat-stream/stream-run-controller.ts src/components/chat/useChatStreamController.ts src/components/chat/presenter/chat.presenter.ts src/components/chat/managers/chat-input.manager.ts src/components/chat/managers/chat-session-list.manager.ts src/components/chat/managers/chat-thread.manager.ts`
- UI 构建：
