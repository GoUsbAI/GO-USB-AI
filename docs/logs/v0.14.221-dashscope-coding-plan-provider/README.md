...
- 在 GoUsbAi 内置 provider catalog 中新增 `dashscope-coding-plan` provider，明确区分普通 `dashscope` 与阿里云 DashScope Coding Plan。
...
- 运行 `pnpm -C packages/go-usb-ai-server exec vitest run src/ui/router.provider-test.test.ts`
- 运行 `pnpm -C packages/go-usb-ai-core exec vitest run src/config/schema.provider-routing.test.ts`
- 运行 `node .codex/skills/post-edit-maintainability-guard/scripts/check-maintainability.mjs --paths packages/go-usb-ai-runtime/src/providers/plugins/builtin.ts packages/go-usb-ai-runtime/src/providers/plugins/dashscope-coding-plan.ts packages/go-usb-ai-server/src/ui/config.ts packages/go-usb-ai-server/src/ui/router.provider-test.test.ts packages/go-usb-ai-core/src/config/schema.provider-routing.test.ts`

## 发布/部署方式

- 本次为仓库代码改动，按正常 GoUsbAi 发布流程进入后续版本发布。
- 若仅本地验证，可在包含 UI 配置页的运行环境中重启对应 GoUsbAi 服务，使最新 provider meta 生效。

## 用户/产品视角的验收步骤

1. 打开 GoUsbAi 配置页的 Provider 列表。
