...
- 在 `@go-usb-ai/core` 的 OpenAI 兼容 provider 增加同 API 内部重试机制（最多 3 次，短退避），覆盖临时性网络/网关错误（如 429/5xx、socket/reset/timeout）。
...
环境：`GOUSB_AI_HOME=/tmp/go-usb-ai-smoke-5bTQBX`
...
```bash
pnpm -C packages/go-usb-ai dev:build agent -s cli:final-11238-2 -m "只回复OK" --model openai/gpt-5.2-codex
```
