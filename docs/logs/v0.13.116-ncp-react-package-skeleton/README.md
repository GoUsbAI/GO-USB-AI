...
- 新增 `@go-usb-ai/ncp-react` 子包最小骨架，路径为 `packages/ncp-packages/go-usb-ai-ncp-react`。
...
1. 打开 `packages/ncp-packages/go-usb-ai-ncp-react`，确认包基础文件已存在。
2. 打开 `packages/ncp-packages/go-usb-ai-ncp-react/src/hooks`，确认已预留 hooks 目录。
