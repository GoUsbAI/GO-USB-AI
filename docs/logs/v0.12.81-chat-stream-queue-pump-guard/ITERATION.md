...
- 已执行：
  - `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai-ui exec eslint src/components/chat/ChatPage.tsx src/components/chat/useChatStreamController.ts src/components/chat/chat-stream/types.ts src/components/chat/chat-stream/transport.ts src/components/chat/chat-stream/controller.ts src/components/chat/useChatSessionTypeState.ts`
