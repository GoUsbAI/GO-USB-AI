...
- 变更文件：`packages/go-usb-ai-server/src/ui/router.ts`
- 新增插件 canonical spec 规则：
  - `builtin-channel-<slug>` 统一映射到 `@go-usb-ai/channel-plugin-<slug>`
  - 带版本 npm spec（如 `@go-usb-ai/channel-plugin-telegram@0.1.2`）统一归一到包名 `@go-usb-ai/channel-plugin-telegram`
- `collectMarketplaceInstalledView` 输出前按 canonical spec 聚合去重，避免“同一插件显示为两个实体”。

2. **Marketplace Worker 默认域名切换**
- 代码默认 API 域名改为：`https://marketplace-api.go-usb-ai.io`
- 部署工作流文档冒烟地址同步更新为新域名。
...
- 启动隔离环境服务：
  - `GOUSB_AI_HOME=/tmp/... pnpm -C packages/go-usb-ai dev:build serve --ui-port <port>`
- API 验证：
  - `curl -sS http://127.0.0.1:<port>/api/marketplace/installed`
  - 观察点：插件记录中的 canonical spec 不重复，且使用 `@go-usb-ai/channel-plugin-*`。
- Worker 域名验证：
  - `curl -sS https://marketplace-api.go-usb-ai.io/health`

## 发布 / 部署方式

本次改动涉及 `@go-usb-ai/server`（后端 API 聚合逻辑）和其直接依赖发布链：
...
- 线上冒烟：使用已绑定域名 `https://marketplace-api.go-usb-ai.io` 与本地 `go-usb-ai` UI API 进行接口验证
...
- 冒烟验证（隔离目录）：
  - `GOUSB_AI_HOME=/tmp/go-usb-ai-market-smoke-... pnpm -C packages/go-usb-ai dev:build serve --ui-port 18941`
  ...
  - `curl -sS https://marketplace-api.go-usb-ai.io/health` 返回 `{"ok":true,...}`
- 发布结果：
  - 已发布 NPM：`go-usb-ai@0.8.3`、`@go-usb-ai/server@0.5.1`
  - 已打 tag：`go-usb-ai@0.8.3`、`@go-usb-ai/server@0.5.1`
