...
- 在不新增 ESLint 例外的前提下，完成 3 个超限函数拆分，恢复 `max-lines-per-function`（150）下的 lint 通过：
  ...
  - `workers/go-usb-ai-provider-gateway-api/src/services/platform-service.ts`
- 拆分方式：
...
- 本地静态与构建验证：
  ...
  - `pnpm -C workers/go-usb-ai-provider-gateway-api lint && pnpm -C workers/go-usb-ai-provider-gateway-api build && pnpm -C workers/go-usb-ai-provider-gateway-api tsc`
- 本地冒烟：
  - `pnpm smoke:platform:mvp`
  - 结果：通过（脚本输出 `all checks passed`）。
- 线上关键 API 冒烟：
  - `GET https://ai-gateway-api.go-usb-ai.io/health` -> `200`
...
- 本次线上可访问地址（部署输出）：
  - Console: `https://73f956e3.go-usb-ai-platform-console.pages.dev`
  - Admin: `https://27c83258.go-usb-ai-platform-admin.pages.dev`
  - Gateway health: `https://ai-gateway-api.go-usb-ai.io/health`
...
2. 在 admin 页面验证：
   ...
   - 用户额度管理列表可编辑并触发保存。
   - 充值审核列表可执行通过/拒绝动作。
3. 运行 `pnpm -C apps/platform-admin lint`、`pnpm -C apps/platform-console lint`、`pnpm -C workers/go-usb-ai-provider-gateway-api lint`，确认均无 max-lines-per-function 告警。
4. 请求 `https://ai-gateway-api.go-usb-ai.io/health`，确认线上 API 返回 `200`。
