...
- 在线修正 GitHub Release：
  ...
  - 正式发布地址：[GoUsbAi Desktop 0.0.60](https://github.com/Peiiii/go-usb-ai/releases/tag/v0.13.24-desktop.5)
...
- 本地命名逻辑验证：
  - 在临时目录构造 `GoUsbAi Desktop-0.0.60.dmg`、`GoUsbAi Desktop-0.0.60-mac.zip` 及对应 `.blockmap`，执行与 workflow 一致的重命名逻辑。
  - 结果：成功得到 `GoUsbAi Desktop-0.0.60-x64.dmg`、`GoUsbAi Desktop-0.0.60-x64-mac.zip` 及对应 `.blockmap`。
- 线上 release 验证：
  - GitHub API 校验 `v0.13.24-desktop.5`：
    - `prerelease=false`
    - release 名称为 `GoUsbAi Desktop 0.0.60`
    - 资产列表包含：
      - `GoUsbAi.Desktop-0.0.60-arm64.dmg`
      - `GoUsbAi.Desktop-0.0.60-x64.dmg`
      - `GoUsbAi.Desktop-0.0.60-arm64-mac.zip`
      - `GoUsbAi.Desktop-0.0.60-x64-mac.zip`
...
## 用户/产品视角的验收步骤
1. 打开 [GoUsbAi Desktop 0.0.60 release 页面](https://github.com/Peiiii/go-usb-ai/releases/tag/v0.13.24-desktop.5)。
2. 确认 macOS 下载区存在两套清晰区分的安装包：
   - Apple Silicon：`...-arm64.dmg`
   - Intel：`...-x64.dmg`
3. 确认 macOS 对应 `zip` 资产也分别带 `arm64` 与 `x64` 后缀，不再出现“无后缀的 Intel 包”。
4. 确认页面不再显示 `Pre-release` 标记，且 release 标题为 `GoUsbAi Desktop 0.0.60`。
