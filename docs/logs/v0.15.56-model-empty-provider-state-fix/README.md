...
- 这次修复落在共享组件 [ProviderScopedModelInput.tsx](/Users/peiwang/Projects/nextbot/packages/go-usb-ai-ui/src/components/common/ProviderScopedModelInput.tsx)，因此模型配置页与复用该组件的 agent 弹窗会同时受益，避免同类问题在其它入口继续存在。
- 新增回归测试 [ModelConfig.test.tsx](/Users/peiwang/Projects/nextbot/packages/go-usb-ai-ui/src/components/config/ModelConfig.test.tsx)，覆盖“无 provider 时展示清晰空态，且仍可手动输入完整模型 ID 并提交”的场景。
...
- 定向 UI 交互验证：
  - `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai-ui test -- src/components/config/ModelConfig.test.tsx`
- 定向 core 配置回归：
  - `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai-core test -- src/config/schema.plugin-channels.test.ts`
- core 类型检查：
...
- 本次仅涉及 `@go-usb-ai/ui` 前端界面修复，当前迭代未直接执行正式发布。
...
- 目录结构与文件组织是否满足当前项目治理要求：部分满足。此次没有新增目录治理债务，但 `packages/go-usb-ai-ui/src/components/config` 仍有既有目录文件数偏多的 warning；本次未扩大该问题。
- 独立于实现阶段的可维护性复核：
  - 可维护性复核结论：通过
  - 本次顺手减债：是
  - 代码增减报告：
  ...
  - 长期目标对齐 / 可维护性推进：这次把“空数据仍展示半套可交互控件”的低级 UI 错误，收敛成一个更统一、更可预测的入口行为，符合 GoUsbAi 作为统一入口应提供清晰状态反馈的方向。剩余观察点是配置中心目录仍偏平铺，后续若继续在该区域增加 UI 面板，应优先考虑按职责再收束一层目录边界。
- 本次续改增量复核：
  - 可维护性复核结论：通过
  - 本次顺手减债：是
  - 代码增减报告：
  ...
  - 非测试代码增减报告：
    ...
    - 净增：+3 行
  - no maintainability findings
  - 长期目标对齐 / 可维护性推进：这次续改不是继续往设置页和 Agent 弹窗里打补丁，而是把“默认模型”和“占位提示”这两个概念从底层 schema 到共享输入组件一起收敛，减少了误导性状态和重复接线，更符合 GoUsbAi 统一入口应提供一致、可预测配置体验的方向。剩余观察点仍是 `packages/go-usb-ai-ui/src/components/agents/AgentDialogs.tsx` 与 `packages/go-usb-ai-core/src/config` 的既有维护性 warning，但本次没有继续放大这些债务。
