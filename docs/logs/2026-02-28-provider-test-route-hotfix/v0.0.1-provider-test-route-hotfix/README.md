...
- 紧急修复发布链路错配：此前 `go-usb-ai@0.8.44` 已包含 UI「测试连接」按钮，但 npm 依赖的 `@go-usb-ai/server@0.5.21` 未包含 `POST /api/config/providers/:provider/test` 路由。
- 本次通过发布补丁版本同步修复：
  - `@go-usb-ai/server` 升级发布（包含 provider test 路由）
  - `go-usb-ai` 联动发布，确保安装 `go-usb-ai` 时可解析到修复后的 server 版本
...
- npm 上 `@go-usb-ai/server` 与 `go-usb-ai` 已是本次 hotfix 版本。
...
1. 升级到最新 `go-usb-ai` 版本（或重新安装）。
2. 执行 `go-usb-ai start` 打开 UI。
