...
- 将 GoUsbAi 默认 UI 端口从 `58891` 统一调整为 `55667`，覆盖配置默认值、CLI 运行时 fallback、remote/local origin、Docker 默认暴露端口、桌面端冒烟脚本以及用户文档入口。
...
- 静态与测试：
  ...
  - `pnpm -C packages/go-usb-ai-server test -- src/ui/router.remote.test.ts --run`
  - `pnpm -C packages/go-usb-ai test -- --run src/cli/commands/diagnostics.status.test.ts src/cli/commands/remote-access-host.test.ts`
- 冒烟：
  - 在隔离 `GOUSB_AI_HOME` 目录运行 `node packages/go-usb-ai/dist/cli/index.js start --start-timeout 15000`
...
- Docker 默认对外暴露 `55667`，部署时可继续通过 `GOUSB_AI_DOCKER_UI_PORT` 覆盖。
- 若对外提供公网入口，仍建议由 Nginx / Caddy 终止 `80/443` 或 `https`，上游回源到 `http://127.0.0.1:55667`。

## 用户/产品视角的验收步骤

1. 执行 `go-usb-ai start`，不要传 `--ui-port`。
