...
1. `skillhub-guide`
   - 从“介绍页”收紧为 GoUsbAi 风格的可执行引导 skill。
   - 明确区分 GoUsbAi marketplace 与腾讯 Skillhub 的边界。
...
2. `find-skills`
   - 参考 Vercel `find-skills` 的能力定位，适配为 GoUsbAi marketplace skill。
   - 明确区分 GoUsbAi marketplace 与外部 open skills ecosystem。
   - 覆盖搜索、质量判断、推荐与安装命令说明。
...
```bash
PATH=/opt/homebrew/bin:$PATH node packages/go-usb-ai/dist/cli/index.js skills publish skills/skillhub-guide --meta skills/skillhub-guide/marketplace.json --api-base https://marketplace-api.go-usb-ai.io
PATH=/opt/homebrew/bin:$PATH node packages/go-usb-ai/dist/cli/index.js skills publish skills/find-skills --meta skills/find-skills/marketplace.json --api-base https://marketplace-api.go-usb-ai.io
```
...
- 远端条目校验：
  - `GET https://marketplace-api.go-usb-ai.io/api/v1/skills/items/skillhub-guide` -> `ok: true`，`publishedAt=2026-03-31T13:15:19.085Z`
  - `GET https://marketplace-api.go-usb-ai.io/api/v1/skills/items/find-skills` -> `ok: true`，`publishedAt=2026-03-31T13:15:20.097Z`
...
```bash
tmp_dir=$(mktemp -d /tmp/go-usb-ai-marketplace-skillhub-guide.XXXXXX)
PATH=/opt/homebrew/bin:$PATH node packages/go-usb-ai/dist/cli/index.js skills install skillhub-guide --api-base https://marketplace-api.go-usb-ai.io --workdir "$tmp_dir"

tmp_dir=$(mktemp -d /tmp/go-usb-ai-marketplace-find-skills.XXXXXX)
PATH=/opt/homebrew/bin:$PATH node packages/go-usb-ai/dist/cli/index.js skills install find-skills --api-base https://marketplace-api.go-usb-ai.io --workdir "$tmp_dir"
```
...
```bash
PATH=/opt/homebrew/bin:$PATH node packages/go-usb-ai/dist/cli/index.js skills update skills/skillhub-guide --meta skills/skillhub-guide/marketplace.json --api-base https://marketplace-api.go-usb-ai.io
PATH=/opt/homebrew/bin:$PATH node packages/go-usb-ai/dist/cli/index.js skills update skills/find-skills --meta skills/find-skills/marketplace.json --api-base https://marketplace-api.go-usb-ai.io
```
...
1. 在 GoUsbAi marketplace 中搜索 `skillhub-guide`，能看到一个用于接入腾讯 Skillhub 的 skill。
...
3. 在 GoUsbAi marketplace 中搜索 `find-skills`，能看到一个用于发现外部 open skills ecosystem 的 skill。
4. 安装 `find-skills` 后，用户能知道：
   - 什么时候应该优先用 GoUsbAi marketplace
