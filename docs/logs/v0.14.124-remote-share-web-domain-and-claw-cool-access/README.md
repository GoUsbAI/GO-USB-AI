...
- 修正远程实例分享的公开入口边界：分享链接现在明确落到 `https://platform.go-usb-ai.io/share/<grantToken>`，不再由 `ai-gateway-api.go-usb-ai.io` 承载页面访问。
- 平台用户站新增 `SharePage` 与 Pages SPA `_redirects`，保证分享链接直接打开和刷新后都能继续命中同一个页面入口。
- 后端删除旧的 `GET /platform/share/:grantToken` 页面式跳转接口，改为 `POST /platform/share/:grantToken/open` 返回结构化会话信息；`ai-gateway-api.go-usb-ai.io` 对该旧页面路径现在返回 `404`。
...
- 设计文档参考：[Remote Instance Sharing Design](/Users/tongwenwen/Projects/Peiiii/go-usb-ai/docs/plans/2026-03-22-go-usb-ai-remote-instance-sharing-design.md)
...
- `curl -s -o /tmp/platform-share.html -w '%{http_code}\n' https://platform.go-usb-ai.io/share/test-token`
- `curl -s -o /tmp/api-share-get.out -w '%{http_code}\n' https://ai-gateway-api.go-usb-ai.io/platform/share/test-token`
- `curl -s -X POST -o /tmp/api-share-open.out -w '%{http_code}\n' https://ai-gateway-api.go-usb-ai.io/platform/share/test-token/open`
- `curl -s -o /tmp/remote-health.out -w '%{http_code}\n' https://remote.claw.cool/health`

## 发布/部署方式

- 后端无需 migration；直接执行 `pnpm -C workers/go-usb-ai-provider-gateway-api run deploy`
- 平台用户站执行 `pnpm deploy:platform:console`
- 本次线上部署结果：
  - Worker version id: `9b820e09-7dc3-478e-b677-e8040e493c3f`
  - Platform console preview: `https://98169960.go-usb-ai-platform-console.pages.dev`
- npm 发包沿用上一轮已完成 version 的 pending release，随后执行正式 publish 闭环

## 用户/产品视角的验收步骤

1. 在桌面端登录 GoUsbAi Account，并确保远程实例在线。
2. 打开 `platform.go-usb-ai.io`，在“我的实例”里创建分享链接。
3. 复制得到的链接，应为 `https://platform.go-usb-ai.io/share/...`，而不是 `ai-gateway-api.go-usb-ai.io`。
