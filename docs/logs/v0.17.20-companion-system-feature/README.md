...
- 本次把 companion 从“临时启动的独立程序”收敛成 GoUsbAi 的正式系统功能。
- 根因修复点有两层：
...
- 已执行：
  - `pnpm -C packages/go-usb-ai-core test src/config/reload.test.ts`
  - `pnpm -C packages/go-usb-ai-server test src/ui/config.runtime.test.ts`
...
- 真实链路冒烟：
  - 使用隔离 `GOUSB_AI_HOME` 启动 `pnpm -C packages/go-usb-ai dev:build serve --ui-port 18892`
...
- 结果：
  ...
  - companion 停掉后重新启动 GoUsbAi runtime，若配置仍为 `enabled=true`，companion 会自动恢复启动
...
5. 再次打开 companion 开关，然后完全重启 GoUsbAi 本地服务。
...
- 质量与可维护性提升证明：
  ...
  - `@go-usb-ai/kernel` 更新合同依赖被收敛到 `@go-usb-ai/kernel/update-contract` 子路径，减少了 server 对 kernel 根导出的耦合。
- 为何不是单纯压缩行数：
...
- 可维护性总结：
  ...
  - 保留债务主要在 `packages/go-usb-ai-server/src/ui/config.ts` 的历史热点规模上，后续拆分应优先从 runtime patch 写入入口下手。
