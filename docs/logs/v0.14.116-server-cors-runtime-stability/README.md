...
- 修复 `@go-usb-ai/server` 在 Node 长运行场景下的高风险中间件链路：
  - 移除 `packages/go-usb-ai-server/src/ui/server.ts` 中对 `hono/cors` 的依赖。
  - 改为显式实现 `/api/*` 的 CORS 响应逻辑，只保留 GoUsbAi 当前真实需要的行为：
    - 允许显式配置的 `corsOrigins`
    - 默认仅放行 `http://localhost:*` 与 `http://127.0.0.1:*`
    - 正确处理带凭证请求与 `OPTIONS` 预检
  - 保持 `/api/health`、登录态 cookie、开发代理场景的兼容行为。
- 新增真实 HTTP 层集成测试 `packages/go-usb-ai-server/src/ui/server.cors.test.ts`，覆盖：
  - 允许来源的预检请求
...
- 线上部署：
  - 在云服务器升级到包含本次修复的 `go-usb-ai` / `@go-usb-ai/server` 版本
  - 重启 GoUsbAi 服务
...
1. 将服务器上的 GoUsbAi 升级到包含本次修复的版本并重启。
...
4. 打开 GoUsbAi Web UI，等待一段时间后刷新页面，确认不再出现 `ERR_EMPTY_RESPONSE`。
