...
- 修复 `packages/go-usb-ai-core/src/cron/service.ts` 中 `every` 周期任务的恢复语义：重启或 `reloadFromStore()` 后，不再把旧任务重算成“当前时间 + interval”，而是沿持久化 cadence 推进到下一次触发点。
...
- 为 UI 服务补齐 `POST /api/cron` 创建接口，并在 `packages/go-usb-ai-server/src/ui/ui-routes/cron.controller.ts` 中加入新增请求校验，避免把无效 schedule 直接写进 store。
- 调整 CLI `packages/go-usb-ai/src/cli/commands/cron.ts`：
...
- 修复 `packages/go-usb-ai/src/cli/commands/shared/ui-bridge-api.service.ts` 的运行中服务地址拼接问题，避免把请求错误地打成 `/api/api/...`。
- 为前台服务链路补齐运行态发现：
  - `go-usb-ai serve`
  - `pnpm -C packages/go-usb-ai dev serve`
  - 上述前台服务现在也会写入本地 `service.json`，因此 CLI 可以正确发现当前 API 并立即把定时任务变更应用到运行中的服务实例。
- 新增/补充真实回归覆盖：
  - `packages/go-usb-ai-core/src/cron/service.test.ts`
  - `packages/go-usb-ai-server/src/ui/router.cron.test.ts`
  - `packages/go-usb-ai/src/cli/commands/cron-support/cron-dev-service.integration.test.ts`
- 更新 [中文指南](../../../apps/docs/zh/guide/cron.md) 与 [English guide](../../../apps/docs/en/guide/cron.md)，明确 interval 重启语义，以及“运行中服务优先走 API 即时生效”的行为。

## 测试/验证/验收方式

- 定向单测：
  - `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai-core exec vitest run src/cron/service.test.ts`
  - 结果：4/4 通过。
  - 补充覆盖：
    - `reloadFromStore()` 不会再把 past-due cron 任务静默推进到下一轮，而是会在运行中的 scheduler 上真正执行。
    - `reloadFromStore()` 在 store 内容没变时不会重写 `jobs.json`。
- 服务路由测试：
  - `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai-server exec vitest run src/ui/router.cron.test.ts`
  - 结果：通过。
- 真实开发态端到端集成测试：
  - `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai exec vitest run src/cli/commands/cron-support/cron-dev-service.integration.test.ts`
  - 覆盖：
...
- Lint：
  ...
  - `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai exec eslint src/cli/commands/cron.ts src/cli/commands/shared/ui-bridge-api.service.ts src/cli/commands/service.ts src/cli/commands/service-support/gateway/service-startup-support.ts src/cli/commands/cron-support/cron-local.service.ts src/cli/commands/cron-support/cron-dev-service.integration.test.ts`
  - 结果：通过；只有仓库既有 warning，无新增 error。
  - 额外说明：`PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai lint` 仍被仓库既有问题阻塞，当前 error 位于 `packages/go-usb-ai/src/cli/commands/ncp/session-request/session-request-delivery.service.test.ts:102` 的 `require-yield`，与本次 cron 改动无关。
- 可维护性守卫：
  - `PATH=/opt/homebrew/bin:$PATH pnpm lint:maintainability:guard`
  - 结果：当前未通过。
  - 阻塞项：`packages/go-usb-ai-ui/src/components/agents/AgentsPage.tsx` 的并行改动触发了新的 `max-lines-per-function` 维护性错误，与本次 cron 修复无关。
  - 本次 cron 链路未新增新的守卫 error；相关 watchpoint 仍是既有的 `packages/go-usb-ai/src/cli/commands/service.ts` 与 `packages/go-usb-ai-server/src/ui/types.ts` 大文件预算。
- 手工开发态冒烟（隔离 `GOUSB_AI_HOME`、纯本地 `Ping`、无任何外发）：
  - 启动：`PATH=/opt/homebrew/bin:$PATH GOUSB_AI_HOME=/tmp/go-usb-ai-cron-manual.oGhumB/home pnpm -C packages/go-usb-ai dev serve --ui-port 19231`
...
- 手工开发态 API 同链路补充冒烟（模拟前端页面的 `/api/cron` 调用，隔离 `GOUSB_AI_HOME`、纯本地、无任何外发）：
  - 启动：`PATH=/opt/homebrew/bin:$PATH GOUSB_AI_HOME=/tmp/go-usb-ai-cron-manual.rQfao1/home pnpm -C packages/go-usb-ai dev serve --ui-port 19321`
...
- 本次改动同时涉及 `@go-usb-ai/core`、`@go-usb-ai/server` 与 `go-usb-ai`，正式发布时需按联动包处理：
  - `pnpm changeset`
  - `pnpm release:version`
  - `pnpm release:publish`
- 若只在本地仓库验证：
  - 使用后台服务：重启 `go-usb-ai start`
  - 使用前台服务：重启 `go-usb-ai serve` 或 `pnpm -C packages/go-usb-ai dev serve`
- 若验证环境里残留旧的 `service.json`，当前 CLI 会在 PID 不存活时自动判定为不可用，不会继续把请求打到失效的旧服务。

## 用户/产品视角的验收步骤

1. 设一个隔离目录：`export GOUSB_AI_HOME=/tmp/go-usb-ai-cron-acceptance`
2. 启动前台开发态服务：`PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai dev serve --ui-port 19231`
3. 新增一个纯本地 interval 任务：`PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai dev:build cron add -n demo -m Ping -e 120`
4. 查看 `"$GOUSB_AI_HOME/run/service.json"` 与 `"$GOUSB_AI_HOME/cron/jobs.json"`：
   - 预期前者存在且指向当前前台服务端口；
...
- 可维护性总结：
  ...
  - 这次已经到了当前问题域里比较实用的最小收敛点：如果继续压缩代码，最先该删的不是校验和测试，而是后续把 `packages/go-usb-ai-server/src/ui/types.ts` 的 cron 契约从大文件里再拆出去。当前仍保留的维护性债务是 `service.ts` 与 `ui/types.ts` 都还偏大，但本次没有再让它们继续恶化到触发新的治理错误。
  - 本轮 `reload` 漏跑补丁本身没有新的 maintainability finding。新增的核心非测试代码只放在 `packages/go-usb-ai-core/src/cron/service.ts`，而且是直接把“reload 维护语义”和“无变化不写盘”收敛到已有 `CronService` 内部，没有再额外铺一层 watcher 适配器、兼容 flag 或备用调度路径；对这个问题域来说，已经是当前更可预测、也更容易验证的最小修复面。
  - 长期目标对齐 / 可维护性推进：
    ...
    - 目录结构方面，本次未新增新的热点平铺目录；`packages/go-usb-ai-server/src/ui/types.ts` 靠近预算上限，下一步整理入口应是把 cron request/result 类型独立拆分到更聚焦的契约文件。
