...
- 新增 marketplace skill：[`skills/gemigo-cli/SKILL.md`](../../../skills/gemigo-cli/SKILL.md)
  - 以 `/Users/peiwang/Projects/deploy-your-app/skills/gemigo-cli/SKILL.md` 为基础，适配为 GoUsbAi marketplace 可直接交付的用户工作流。
  - 本次没有只照搬命令清单，而是补齐了 GoUsbAi 用户真正需要的闭环：
...
- 已将该 skill 正式发布到 GoUsbAi marketplace：
  - canonical package：`@go-usb-ai/gemigo-cli`
  - alias：`gemigo-cli`
- 同批次续改：
...
```bash
go-usb-ai skills publish skills/gemigo-cli --meta skills/gemigo-cli/marketplace.json --scope go-usb-ai --api-base https://marketplace-api.go-usb-ai.io
```

- 结果：
  - `✓ Published new skill: @go-usb-ai/gemigo-cli`
  - `Alias: gemigo-cli`
  - `Files: 2`
...
```bash
go-usb-ai skills update skills/gemigo-cli --meta skills/gemigo-cli/marketplace.json --package-name @go-usb-ai/gemigo-cli --api-base https://marketplace-api.go-usb-ai.io
```

- 结果：
  - `✓ Updated skill: @go-usb-ai/gemigo-cli`
  - `Alias: gemigo-cli`
  - `Files: 2`
...
```bash
curl -sS https://marketplace-api.go-usb-ai.io/api/v1/skills/items/%40go-usb-ai%2Fgemigo-cli
curl -sS https://marketplace-api.go-usb-ai.io/api/v1/skills/items/gemigo-cli
```

- 结果：
  - 两个入口都返回 `ok: true`
  - `packageName: @go-usb-ai/gemigo-cli`
...
```bash
tmp_dir=$(mktemp -d /tmp/go-usb-ai-marketplace-gemigo.XXXXXX)
go-usb-ai skills install gemigo-cli --api-base https://marketplace-api.go-usb-ai.io --workdir "$tmp_dir"
...
1. 本次 skill 已完成正式发布，当前线上 canonical package 为 `@go-usb-ai/gemigo-cli`
2. 若后续只更新 skill 文案或元数据，执行：

```bash
go-usb-ai skills update skills/gemigo-cli --meta skills/gemigo-cli/marketplace.json --scope go-usb-ai --api-base https://marketplace-api.go-usb-ai.io
```
...
1. 在 GoUsbAi marketplace 中搜索 `gemigo-cli` 或 `GemiGo CLI`
2. 安装该 skill
3. 阅读 skill 内容后，用户应能清楚理解：
  - 这是通过本机 `gemigo` CLI 交付的能力，不是 GoUsbAi 内建部署内核
  - 它当前发布的是“已经构建好的静态目录”，不是原始源码仓库
  - 真正 deploy 前会先检查 `gemigo` 是否已安装、是否已登录、静态目录是否合法、`gemigo.app.json` 是否完整
  - 真正的远端部署动作应该在用户确认后执行
4. 当用户让 GoUsbAi 帮他发布一个小应用时，助手应按 skill 约束：
  - 先 build 或定位产物目录
  - 再做 `gemigo whoami`
  - 再校验 `gemigo.app.json`
  - 最后才执行 `gemigo deploy`
...
- 是否优先遵循“删减优先、简化优先、代码更少更好、复杂度更低更好、清晰度更高更好”的原则：是
  - 本次没有改 GoUsbAi 核心 CLI、marketplace API 或 UI，也没有新加包装脚本、helper 或 service
...
- 抽象、模块边界、class / helper / service / store 等职责划分是否更合适、更清晰，是否避免了过度抽象或补丁式叠加：是
  ...
  - 运行时能力仍明确归属 `gemigo` CLI，自始至终没有假装成 GoUsbAi 自己的原生 deploy runtime
...
- 长期目标对齐 / 可维护性推进：
  - 本次是在强化 GoUsbAi 作为“统一入口 + 能力编排”的定位，让用户通过 marketplace 获取“发布自己构建的小应用并拿到可访问 URL”这类外部能力
