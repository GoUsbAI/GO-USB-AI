...
- 清理 CLI 旧技能安装兼容实现：
  ...
  - 删除对应历史测试文件：`packages/go-usb-ai/src/cli/commands/service.marketplace-skill.test.ts`。
- 清理 worker 旧数据源链路：
...
- CLI 技能模块命名去历史化：
  - `packages/go-usb-ai/src/cli/skills/clawhub.ts` 重命名为 `packages/go-usb-ai/src/cli/skills/marketplace.ts`。
- 清理脚本与文档残留：
...
- `go-usb-ai-server` / `go-usb-ai` 的 lint 仍有仓库既有 max-lines 警告（非本迭代引入）。

冒烟（非仓库目录）：

- 在 `/tmp` mock marketplace API 下真实执行：
  - `go-usb-ai skills install demo`
...
2. 执行上传：`go-usb-ai skills publish ./my-skill --slug my-skill --api-base <api>`。
3. 修改 skill 后执行更新：`go-usb-ai skills update ./my-skill --slug my-skill --api-base <api>`。
4. 在目标机器执行安装：`go-usb-ai skills install my-skill --api-base <api>`。
