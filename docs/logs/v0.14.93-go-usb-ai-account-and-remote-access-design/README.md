# v0.14.93-go-usb-ai-account-and-remote-access-design

## 迭代完成说明

- 新增账号与远程访问边界重构方案文档：
  - [GoUsbAi Account And Remote Access Product Design](../../plans/2026-03-21-go-usb-ai-account-and-remote-access-product-design.md)
- 文档明确了以下设计结论：
  - `账号登录` 必须升级为前端全局能力，不再作为 remote access 子功能存在
  - `remote access` 默认只保留设备开关、连接状态、设备名与通往网页版的入口
  - `platform.go-usb-ai.io` 继续可作为实现载体，但不应成为用户主心智
