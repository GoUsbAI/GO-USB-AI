...
- 线上域名连通性校验：
  ...
  - `https://ai-gateway-api.go-usb-ai.io/platform/auth/me`（无 token）-> 401
...
2. 发布 Pages：
   ...
   - `pnpm dlx wrangler pages deploy apps/platform-admin/dist --project-name go-usb-ai-platform-admin --branch master --commit-dirty=true`

## 用户/产品视角的验收步骤

1. 打开用户站 `platform.go-usb-ai.io`，执行注册/登录，确认请求打到 `ai-gateway-api.go-usb-ai.io`。
2. 打开管理站 `platform-admin.go-usb-ai.io`，执行登录，确认请求打到 `ai-gateway-api.go-usb-ai.io`。
