...
- 调整聊天会话侧栏中运行状态的展示位置：
  ...
  - 修改文件：`packages/go-usb-ai-ui/src/components/chat/ChatSidebar.tsx`。
- 使用固定两列布局保证状态列对齐：
...
- Lint 验证（改动相关文件）：
  - `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai-ui exec eslint src/components/chat/ChatSidebar.tsx src/components/common/SessionRunBadge.tsx` 通过。
- 类型验证：
