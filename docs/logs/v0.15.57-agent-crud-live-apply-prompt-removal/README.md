...
- 修正 `go-usb-ai agents new|update|remove` 的 CLI 语义：不再返回旧的“需重启后生效”字段，也不再在普通输出里提示“重启后才能使用”。
- 同步收敛 Agent 自管理真相源：
  - 更新 [docs/USAGE.md](/Users/peiwang/Projects/nextbot/docs/USAGE.md)，把 Agent CRUD 改成“直接修改后校验，不发明重启步骤”。
  - 执行同步脚本，把作者源 guide 同步到 [packages/go-usb-ai/resources/USAGE.md](/Users/peiwang/Projects/nextbot/packages/go-usb-ai/resources/USAGE.md)。
- 清理会误导 AI 搜索结果的残留文案：
...
- CLI 定向测试：
  - `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai test -- --run src/cli/commands/agent/agent-commands.test.ts`
  - 结果：通过，4 个测试全部通过。
- 类型检查：
...
- guide 同步：
  ...
  - 结果：通过，`docs/USAGE.md` 已同步到 `packages/go-usb-ai/resources/USAGE.md`。
- 残留搜索：
...
- 本次未执行发布或部署。
- 后续随 `go-usb-ai` 常规发布流程发布即可。
- 不涉及数据库、migration、远程环境或线上服务部署。

## 用户/产品视角的验收步骤

1. 执行 `go-usb-ai agents new researcher --json`，确认返回 JSON 只包含结果对象，不再出现旧的“需重启”字段。
2. 执行 `go-usb-ai agents update researcher --description "负责调研" --json`，确认返回 JSON 仍不包含任何重启提示字段。
3. 若 GoUsbAi 服务已在运行，直接再次执行 `go-usb-ai agents list --json` 或刷新 `/agents` 页面，确认新 Agent / 新描述已经可见。
4. 检查 [docs/USAGE.md](/Users/peiwang/Projects/nextbot/docs/USAGE.md) 的 Agent CRUD 章节，确认文档不再要求 `go-usb-ai restart`。
...
- 目录结构与文件组织是否满足当前项目治理要求：本次没有新增目录平铺问题。守卫提示的 `packages/go-usb-ai/src/cli` 目录预算与 `runtime.ts` 文件预算 warning 为既有问题，本次未继续恶化。
