...
- 将 server provider connection test 路径从直接 `new LiteLLMProvider` 改为调用 `go-usb-ai.llmProviders.testConnection(...)`。
- 将 gateway runtime 的 provider owner 改为直接使用 `go-usb-ai.llmProviders`。
- 删除 `RuntimeCommandService.createProvider` / `createMissingProvider`、`GoUsbAiGatewayRuntime.createProviderManager`、`ConfigReloader.makeProvider` 和 service `MissingProvider`。
...
- `GoUsbAiKernel` 构造期直接创建真实 `MessageBus` 与 core `SessionManager`；kernel 根据顶层 `homeDir` 推导 `sessionsDir`，`SessionManager` 只接收自己的直接存储目录，不再依赖 `workspace/homeDir`。
- 删除 kernel 内未实现的 fake `SessionManager` stub 和 barrel 出口。
- gateway runtime 改为创建 scoped `GoUsbAiKernel`，并直接使用 `kernel.eventBus`、`kernel.ingress`、`kernel.messageBus`、`kernel.sessions`、`kernel.llmProviders`。
- CLI agent 路径改为创建 scoped `GoUsbAiKernel`，不再自己 `new MessageBus()` / `new SessionManager(...)`。
...
- server provider connection test 改为使用 UI router options 中传入的 gateway provider manager，不再访问全局 `go-usb-ai.llmProviders`。
...
- 本地 cron CLI service 不再直接实例化 `CronService`，也不再创建 `GoUsbAiKernel`；离线命令只持有一个 `AutomationManager` 直接读写 cron store。
- 删除 kernel 根入口导出的顶层 `go-usb-ai` singleton，避免仅 import `@go-usb-ai/kernel` 就隐式创建一套 kernel 对象图。
...
- `GoUsbAiKernel` 构造期直接创建并持有 `kernel.channels`，依赖同一个 `kernel.messageBus` 与 `kernel.sessions`。
...
- `GoUsbAiKernel` 不再构造 `tasks` / `contextBuilder` 两个没有真实职责闭环的对象。
...
- `GoUsbAiKernel` 构造期直接创建 `configManager`，并把同一个 `kernel.channels` 与 `kernel.llmProviders` 交给它装配；`workspace` 不再作为 kernel options 传入。
...
- `pnpm --filter @go-usb-ai/core tsc`：通过。
- `pnpm --filter @go-usb-ai/kernel tsc`：通过。
- `pnpm --filter @go-usb-ai/runtime tsc`：通过。
- `pnpm --filter @go-usb-ai/server tsc`：通过。
- `pnpm --filter @go-usb-ai/service tsc`：通过。
- `pnpm --filter @go-usb-ai/kernel tsc`：通过。
- `pnpm --filter @go-usb-ai/server tsc`：通过。
- `pnpm --filter @go-usb-ai/service tsc`：通过。
- `pnpm --filter @go-usb-ai/server tsc`：通过。
- `pnpm --filter @go-usb-ai/kernel lint`：通过。
- `pnpm --filter @go-usb-ai/service lint`：通过，有既有 warning。
- `pnpm --filter @go-usb-ai/service test -- src/commands/service/gateway-manual-restart-contract.controller.test.ts src/shared/services/gateway/tests/gateway-plugin-manager.service.test.ts src/shared/services/gateway/tests/go-usb-ai-app.service.test.ts src/shared/controllers/gateway.controller.test.ts src/commands/ncp/features/runtime/go-usb-ai-ncp-runner.test.ts`：通过。
...
- `pnpm --filter @go-usb-ai/kernel lint`：通过。
- `pnpm --filter @go-usb-ai/core lint`：通过，有既有 warning。
- `pnpm --filter @go-usb-ai/kernel lint`：通过。
- `pnpm --filter @go-usb-ai/runtime lint`：通过。
- `pnpm --filter @go-usb-ai/server lint`：通过，有既有 warning。
- `pnpm --filter @go-usb-ai/service lint`：通过，有既有 warning。
- `pnpm --filter @go-usb-ai/core test -- src/features/cron/services/cron.service.test.ts src/features/agent/features/tools/cron.test.ts`：通过。
- `pnpm --filter @go-usb-ai/service test -- src/cli/commands/cron/services/cron-local.service.test.ts`：通过。
- `pnpm --filter @go-usb-ai/core test -- src/features/llm-providers/providers/litellm.provider.test.ts`：通过。
- `pnpm --filter @go-usb-ai/service test -- src/shared/services/telemetry/llm-usage-observer.service.test.ts src/commands/ncp/provider/provider-manager-ncp-llm-api.service.test.ts src/commands/service/gateway-manual-restart-contract.controller.test.ts`：通过。
- `pnpm --filter @go-usb-ai/service test -- src/shared/services/ui/tests/npm-runtime-update-host.service.test.ts`：通过。
- `pnpm --filter @go-usb-ai/core test -- src/features/channels/managers/channel.manager.test.ts`：通过，覆盖 core `ChannelManager` 装载、启动、control delivery 与 reload。
- `pnpm --filter @go-usb-ai/service test -- src/commands/service/gateway-manual-restart-contract.controller.test.ts src/shared/services/gateway/tests/gateway-plugin-manager.service.test.ts src/shared/services/gateway/tests/go-usb-ai-app.service.test.ts`：通过。
- `pnpm check:governance-backlog-ratchet`：通过。
- `pnpm lint:new-code:package-public-imports`：通过。
- `pnpm lint:new-code:governance`：通过；仅提示 `packages/go-usb-ai-service/src/shared/controllers/gateway.controller.ts` 是既有 legacy shared orchestration warning。
- `node .agents/skills/post-edit-maintainability-guard/scripts/check-maintainability.mjs --non-feature --paths ...`：当前触达范围非测试代码净减 287 行；触达 `packages/go-usb-ai-server/src/ui/config.ts` 红区，已补红区触达记录。
...
- `node .agents/skills/post-edit-maintainability-guard/scripts/check-maintainability.mjs --non-feature --paths ...`：channel/gateway 整合触达范围总计 +401/-473，净减 72 行；非测试代码 +266/-345，净减 79 行；仅提示 runtime 目录既有预算豁免和 `go-usb-ai-ncp-dispatch.utils.ts` 接近文件预算。
...
路由测试 `pnpm --filter @go-usb-ai/server test -- src/ui/router.provider-test.test.ts` 当前被既有 `@core/*` alias 解析问题阻塞，未进入测试用例执行。
路由测试 `pnpm --filter @go-usb-ai/server test -- src/ui/router.cron.test.ts` 当前同样被既有 `@core/*` alias 解析问题阻塞，未进入测试用例执行。
