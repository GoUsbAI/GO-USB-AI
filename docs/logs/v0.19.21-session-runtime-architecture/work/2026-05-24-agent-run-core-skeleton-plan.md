...
- 不修改旧 `packages/go-usb-ai-kernel/src/managers/session-run.manager.ts`。
- 不修改旧 `packages/go-usb-ai-kernel/src/managers/agent-run-request.manager.ts`。
...
```text
packages/go-usb-ai-kernel/src/features/agent-run/
  index.ts
...
暂不从 `packages/go-usb-ai-kernel/src/index.ts` 导出，避免和旧 manager 公共入口冲突。
...
```text
packages/ncp-packages/go-usb-ai-ncp-agent-runtime-next/
  src/
    index.ts
    runtime/
...
包名使用 `@go-usb-ai/ncp-agent-runtime-next` 表达临时隔离；包内文件和 class 使用终态命名，不使用 `V2`：

```ts
export class DefaultNcpAgentRuntime { ... }
```

这个包可以临时依赖旧 `@go-usb-ai/ncp-agent-runtime`，但只复用稳定 primitives；不导入旧 `DefaultNcpAgentRuntime`，不复用旧 runtime class 的 state manager / metadata / context builder 边界。
...
- 确认新实现后，把该包内容合并回 `@go-usb-ai/ncp-agent-runtime`，删除旧 runtime 实现和临时 next package。
...
- branch-local provider contributions 已按第一批策略落地：
  - `ContextProviderContribution` 复用旧 `ContextBuilder` 和共享的 `resolveGoUsbAiNcpRunContext(...)`，先不拆 agent profile/project/bootstrap/memory/skills 多个 provider，避免复制旧 prompt 规则。
...
- `session`：同一个 session 复用同一个 runtime 实例。适合 `narp-stdio` 这类内部持有 child process / ACP session / cancel state 的实现，也适合 `narp-http` 这类远端 session 可能和 GoUsbAi session 绑定的实现。
...
- `narp-http` 默认先按 `session` 处理，避免远端 runtime 把不同 GoUsbAi session 混到同一个 remote session 中；如果后续证明某个 HTTP runtime 是无状态 request/response 型，再允许 entry 显式配置为 `global` 或其它更细粒度策略。
...
- `context-window.updated` 不应长期属于 NCP endpoint event。它是 GoUsbAi 的 context-window projection / 观察状态，不是 agent runtime 的 NCP 事实。后续单独开迁移切块：新增独立 app event contract，迁移 `SessionContextWindowContribution` 发布路径，迁移前端 live context-window 消费路径，再从 `NcpEventType` / `NcpEndpointEvent` 和 `ncp-event` SSE 里删除旧入口。
