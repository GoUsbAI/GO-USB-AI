...
- `pnpm -C packages/go-usb-ai build`
  - 结果：通过，重新构建 CLI/runtime 并将最新 UI bundle 同步到 `packages/go-usb-ai/ui-dist`。
- `pnpm -C apps/desktop build:main`
  - 结果：通过，重新构建 Electron main/preload 产物。
- `pnpm -C packages/go-usb-ai-server exec vitest run src/ui/router.runtime-control.test.ts`
  - 结果：通过，2 个测试通过。
- `pnpm -C packages/go-usb-ai-ui exec vitest run src/components/config/runtime-control-card.test.tsx`
  - 结果：通过，3 个测试通过。
- `pnpm -C packages/go-usb-ai-server exec tsc -p tsconfig.json --noEmit`
  - 结果：通过。
...
- Web 端真实功能验证：
  - 启动方式：`GOUSB_AI_HOME=/tmp/go-usb-ai-restart-smoke-20260413-224610 node packages/go-usb-ai/dist/cli/index.js start --ui-port 19667 --start-timeout 60000`
...
- Desktop 端真实功能验证：
  - 启动方式：`env -u ELECTRON_RUN_AS_NODE GOUSB_AI_HOME=/tmp/go-usb-ai-desktop-restart-home-20260413-225219 GOUSB_AI_DESKTOP_DATA_DIR=/tmp/go-usb-ai-desktop-restart-data-20260413-225219 GOUSB_AI_DESKTOP_RUNTIME_SCRIPT=../../packages/go-usb-ai/dist/cli/index.js pnpm exec electron . --remote-debugging-port=9333`
  ...
  - `Restart App` 结果：按钮可用；接受确认框后 Electron 主进程日志出现 `Desktop app restart requested from renderer.` 与 `before-quit`；旧 runtime 以 `SIGTERM` 退出；随后通过同一 CDP 端口重新连上新桌面进程，页面恢复为 `http://127.0.0.1:61416/chat`，`window.go-usb-aiDesktop.restartApp` 仍可用。
...
- 是否让总代码量、分支数、函数数、文件数或目录平铺度下降，或至少没有继续恶化：代码量净增长，因为这是新的用户可见能力。已将 `packages/go-usb-ai-server/src/ui/types.ts` 控制回本次前的 899 行，将 `packages/go-usb-ai/src/cli/commands/service.ts` 控制回本次前的 855 行，并把 `packages/go-usb-ai-ui/src/lib/i18n.ts` 从本次前 542 行降到 540 行。剩余增长主要来自必要的新能力边界和 UI。
...
- 目录结构与文件组织是否满足当前项目治理要求：基本满足。本次新增文件遵循 kebab-case 与角色后缀；但 `packages/go-usb-ai-server/src/ui/ui-routes`、`packages/go-usb-ai-ui/src/api`、`packages/go-usb-ai-ui/src/components/config`、`packages/go-usb-ai-ui/src/hooks` 等目录已有平铺超预算 warning，属于既有结构约束，本次未进一步扩大为 error。
