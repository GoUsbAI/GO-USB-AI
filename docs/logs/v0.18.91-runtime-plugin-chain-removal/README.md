...
- 删除旧 Codex / Claude runtime plugin package；Codex OpenAI Responses bridge 迁回 `go-usb-ai-ncp-runtime-codex-sdk`，NARP Codex wrapper 改为依赖真正的 runtime SDK package。
...
- `pnpm -C packages/go-usb-ai-kernel exec vitest run src/features/runtime-registry/services/agent-runtime-registry.service.test.ts`
- `pnpm -C packages/go-usb-ai-openclaw-compat exec vitest run --config ../go-usb-ai-core/vitest.config.ts src/plugins/status.pure-read.test.ts src/plugins/channel-runtime.test.ts src/plugins/install.test.ts src/plugins/uninstall.test.ts`
- `pnpm -C packages/go-usb-ai-service exec vitest run src/commands/plugin/plugin-reload.test.ts src/commands/plugin/dev-first-party-plugin-load-paths.test.ts src/commands/plugin/dev-first-party-plugin-load-paths.path-install.test.ts src/cli/commands/agent/agent-commands.test.ts src/shared/services/gateway/tests/gateway-plugin-manager.service.test.ts src/shared/services/marketplace/tests/marketplace-plugin-management.service.test.ts`
- `pnpm -C packages/go-usb-ai-ui exec vitest run src/features/marketplace/utils/marketplace-installed-cache.utils.test.ts src/features/marketplace/components/marketplace-page.test.tsx`
- `pnpm -C packages/go-usb-ai-server exec vitest run src/app/router.marketplace-content.test.ts src/app/router.marketplace-installed.test.ts`
...
补充说明：`pnpm -C packages/go-usb-ai-ui lint` 仍被 UI 既有 unrelated lint errors 阻塞；本次触达的 marketplace 测试文件已通过定向 ESLint。
...
1. `go-usb-ai agent runtimes` 只应展示配置化 runtime entry 与 builtin source，不再展示 plugin source / pluginId。
