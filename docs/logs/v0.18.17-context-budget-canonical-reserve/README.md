...
- 在 `go-usb-ai-ncp-context-builder.ts` 内合并单用途 tool filter helper，避免本次非功能修复继续撑大已超预算文件。
...
- `pnpm -C packages/go-usb-ai test -- context-compaction-preflight.service.test.ts go-usb-ai-ncp-context-builder.test.ts`
  - 结果：通过，2 个测试文件，21 个测试。
- `pnpm -C packages/go-usb-ai-core test -- schema.plugin-channels.test.ts agent-profiles.test.ts provider-runtime-resolution.test.ts schema.remote.test.ts context.test.ts`
  - 结果：通过，5 个测试文件，28 个测试。
- `pnpm -C packages/go-usb-ai-core tsc`
  - 结果：通过。
- `pnpm -C packages/go-usb-ai tsc`
  - 结果：通过。
- `pnpm -C packages/go-usb-ai exec eslint ...`
  - 结果：通过。
- `pnpm -C packages/go-usb-ai-core exec eslint ...`
...
- `pnpm check:governance-backlog-ratchet`
  - 结果：通过。
- `pnpm lint:new-code:governance -- packages/go-usb-ai/src/cli/commands/ncp/context packages/go-usb-ai/src/cli/commands/ncp/go-usb-ai-ncp-context-builder.ts packages/go-usb-ai-core/src/features/agent/services`
  - 结果：通过。
- 本地真实 session 静态回放：
...
- `pnpm lint:new-code:governance`
  - 结果：未通过；阻塞来自当前已触达的 legacy config 文件命名规则，以及工作区里一个非本次任务的 `go-usb-ai-openclaw-compat` 改动。已运行并通过本次 context budget 相关路径的 scoped governance；未在本次扩大到治理规则或大规模文件重命名。

## 发布/部署方式

未发布、未部署。

该修复涉及 `@go-usb-ai/core` 与 `go-usb-ai` 包源码，后续需要随统一 NPM release 发布。

## 用户/产品视角的验收步骤

1. 配置 200K context 与 `reservedContextTokens: 20000`。
2. 构造包含历史 tool 调用、tool result、NCP envelope 字段的长会话。
3. 当 canonical used context 约为 86K 时，圆环显示约 43%，不得触发 GoUsbAi compaction。
...
- 简化：`go-usb-ai-ncp-context-builder.ts` 合并单用途 `filterTools` helper，本次触达后该超预算文件净减 4 行。
...
- `go-usb-ai-ncp-context-builder.ts` 仍超过文件预算，但本次触达后净减 4 行。
- `packages/go-usb-ai/src/cli/commands/ncp` 目录仍在目录预算边界。
