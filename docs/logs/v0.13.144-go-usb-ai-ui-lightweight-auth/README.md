# v0.13.144-go-usb-ai-ui-lightweight-auth

## 迭代完成说明

- 为 `go-usb-ai ui` 增加轻量认证配置：核心配置新增 `ui.auth.enabled / username / passwordHash / passwordSalt`，并沿用现有敏感字段脱敏机制，公开配置不会返回密码 hash/salt。
...
- 发布语义按 minor 处理，并已补联动 changeset：`@go-usb-ai/core`、`@go-usb-ai/server`、`@go-usb-ai/ui`、`go-usb-ai`。

## 测试/验证/验收方式

- 服务端认证测试：
  - `pnpm --filter @go-usb-ai/server exec vitest run src/ui/router.auth.test.ts`
- 服务端类型检查：
  - `pnpm --filter @go-usb-ai/server tsc`
- 服务端构建：
  - `pnpm --filter @go-usb-ai/server build`
- 服务端定向 lint：
  - `pnpm --filter @go-usb-ai/server exec eslint src/ui/auth.service.ts src/ui/router.ts src/ui/server.ts src/ui/router/types.ts src/ui/router/auth.controller.ts src/ui/router.auth.test.ts`
- 前端类型检查：
  - `pnpm --filter @go-usb-ai/ui tsc`
- 前端构建：
  - `pnpm --filter @go-usb-ai/ui build`
- 前端定向 lint：
  - `pnpm --filter @go-usb-ai/ui exec eslint src/App.tsx src/components/auth/login-page.tsx src/components/config/runtime-security-card.tsx src/components/config/RuntimeConfig.tsx src/hooks/use-auth.ts src/lib/i18n.ts`
...
- 常规前端发布：
  - 构建 `@go-usb-ai/ui`
  - 构建 `@go-usb-ai/server`
  - 将新的 UI 静态资源与 server 一并部署到现有 `go-usb-ai ui` 运行环境
- 若已在服务器上运行 `go-usb-ai ui`：
  - 更新到包含本次改动的版本
  - 重启 UI 进程
...
1. 启动 `go-usb-ai ui`，在未开启认证的默认状态下直接访问页面，确认现有本机使用路径不受影响。
...
7. 重启 `go-usb-ai ui` 进程后重新访问，确认之前的登录态失效，需要重新登录。
