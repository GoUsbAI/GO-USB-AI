...
- 将 `@go-usb-ai/ncp-react` 的默认附件上传上限从 `10 MB` 提升到 `200 MB`，GoUsbAi 前端聊天输入框随共享常量同步放宽上限。
...
- 定向验证：
  - `pnpm -C packages/go-usb-ai-ui exec vitest run src/components/chat/chat-attachment-upload-limit.test.ts`
  - `pnpm -C packages/ncp-packages/go-usb-ai-ncp-react tsc`
- 冒烟验证：
  - `pnpm -C packages/go-usb-ai exec tsx --eval "import { DEFAULT_NCP_ATTACHMENT_MAX_BYTES, uploadFilesAsNcpDraftAttachments } from '../../packages/ncp-packages/go-usb-ai-ncp-react/src/attachments/ncp-attachments.ts'; void (async () => { const file = new File([new Uint8Array(12 * 1024 * 1024)], 'smoke-large-image.png', { type: 'image/png' }); const result = await uploadFilesAsNcpDraftAttachments([file], { uploadBatch: async (files) => files.map((entry) => ({ id: entry.name, name: entry.name, mimeType: entry.type, sizeBytes: entry.size, assetUri: 'asset://store/' + entry.name })) }); console.log(JSON.stringify({ maxBytes: DEFAULT_NCP_ATTACHMENT_MAX_BYTES, attachments: result.attachments.length, rejected: result.rejected.length, firstAttachmentSize: result.attachments[0]?.sizeBytes ?? null })); })();"`
