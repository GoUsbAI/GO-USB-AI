...
## 迭代完成说明（改了什么）
- 新增 `packages/go-usb-ai-ui/src/lib/session-run-status.ts`，统一聚合会话运行状态：
...
- 聊天页接入全局活跃 run 聚合，并将状态映射传入会话侧边栏：
  - 修改 `packages/go-usb-ai-ui/src/components/chat/ChatPage.tsx`。
  - 修改 `packages/go-usb-ai-ui/src/components/chat/ChatSidebar.tsx`，在会话列表中显示 `运行中/排队中` 状态徽标。
- 会话管理页接入同一状态映射并展示状态徽标：
  - 修改 `packages/go-usb-ai-ui/src/components/config/SessionsConfig.tsx`。
- 增加国际化文案：
  - 修改 `packages/go-usb-ai-ui/src/lib/i18n.ts`，新增 `sessionsRunStatusRunning`、`sessionsRunStatusQueued`。
...
- Lint 验证（全量）：
  - `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai-ui lint` 未通过。
...
- 冒烟验证（状态聚合核心逻辑）：
  - 命令：
    ...
    - `PATH=/opt/homebrew/bin:$PATH node --input-type=module -e "...import /tmp/go-usb-ai-ui-smoke/lib/session-run-status.js 并执行状态聚合断言..."`
  - 观察点：
