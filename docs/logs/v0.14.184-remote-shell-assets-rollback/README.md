...
- 回滚原因不是小风险，而是该实现破坏了 remote 场景最关键的版本隔离前提：浏览器端页面壳被固定为平台统一版本，不再与每台设备自己的 GoUsbAi 版本自洽，可能直接导致应用不可用。
...
- `node .codex/skills/post-edit-maintainability-guard/scripts/check-maintainability.mjs --paths workers/go-usb-ai-provider-gateway-api/src/index.ts workers/go-usb-ai-provider-gateway-api/wrangler.toml workers/go-usb-ai-provider-gateway-api/README.md docs/logs/v0.14.184-remote-shell-assets-rollback/README.md`

## 发布/部署方式

- 发布 worker：`pnpm -C workers/go-usb-ai-provider-gateway-api run deploy`
...
3. 在不同版本的 GoUsbAi 设备上分别打开 remote，会话都应回到各自版本自洽的原始行为。
