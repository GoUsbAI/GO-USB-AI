...
```bash
TMP_HOME=$(mktemp -d /tmp/go-usb-ai-smoke-start-msg-wording.XXXXXX)
PATH=/opt/homebrew/bin:$PATH GOUSB_AI_HOME="$TMP_HOME" node packages/go-usb-ai/dist/cli/index.js start --ui-port 19998
...
- `start` 输出包含：`If you need to stop the service, run: go-usb-ai stop`。
- `status` 显示运行中。
- `stop` 可正常停止服务。
