...
本次将 `go-usb-ai-provider-gateway-api` 从“匿名体验额度”升级为“登录平台 + 双额度 + 充值 + 管理端”的严肃平台 MVP，并补齐独立控制台前端。
...
- 工程集成
  ...
  - CLI 新增 `go-usb-ai login`（支持 `--register`），可将登录 token 自动写入 `providers.go-usb-ai.apiKey`，实现“登录后才能调用平台网关”。
...
- 冒烟（隔离目录，不写仓库）
  - 在 `/tmp` 目录执行：
    - `wrangler d1 migrations apply GOUSB_AI_PLATFORM_DB --local --config <repo>/workers/go-usb-ai-provider-gateway-api/wrangler.toml`
    - `wrangler dev --local --port 8790 --config <repo>/workers/go-usb-ai-provider-gateway-api/wrangler.toml`
  - 验证链路：
    ...
    - CLI 登录：`go-usb-ai login --api-base http://127.0.0.1:8791 --register ...` 后，`GOUSB_AI_HOME/config.json` 的 `providers.go-usb-ai.apiKey` 为 `nca.` 前缀 token。

## 3) 发布/部署方式

1. 配置 Worker 绑定与变量
   - D1：`GOUSB_AI_PLATFORM_DB`
...
2. 执行 D1 migration
   - `pnpm -C workers/go-usb-ai-provider-gateway-api db:migrate:remote`
3. 部署 Worker
