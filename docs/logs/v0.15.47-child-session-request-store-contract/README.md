...
- 收敛了 `spawn` 子会话与 `sessions_request` 的底层 session store 契约：`SessionManager` 不再只能隐式依赖 `GOUSB_AI_HOME`，新增了显式 `homeDir` / `sessionsDir` 注入能力。
...
- 为 child session 追加请求补了一条真实 runtime 复现场景测试，覆盖：
  ...
  - ambient `GOUSB_AI_HOME` 变化，但 runtime 复用同一显式 home 时仍可继续 `sessions_request(child)`
- 顺手把本轮触碰到的 [spawn.ts](/Users/peiwang/Projects/nextbot/packages/go-usb-ai-core/src/agent/tools/spawn.ts) 实例方法改成箭头 class field，消除治理违例。
...
- 同步新增 [session-request-delivery.service.test.ts](/Users/peiwang/Projects/nextbot/packages/go-usb-ai/src/cli/commands/ncp/session-request/session-request-delivery.service.test.ts) 锁定这条竞态，并保留现有 `resume_source` 等待 idle 的行为约束。
...
- `PATH=/opt/homebrew/bin:/Users/peiwang/.nvm/versions/node/v22.16.0/bin:$PATH pnpm -C packages/go-usb-ai test src/cli/commands/ncp/runtime/create-ui-ncp-agent.child-session-request.test.ts`
  - 通过，3 个 runtime 场景全部通过。
- `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai exec vitest run src/cli/commands/ncp/session-request/session-request-delivery.service.test.ts src/cli/commands/ncp/runtime/create-ui-ncp-agent.subagent-completion.test.ts src/cli/commands/ncp/runtime/create-ui-ncp-agent.child-session-request.test.ts`
  - 通过，`spawn` 完成态回写、父会话恢复、child session follow-up 三条回归链均通过。
- `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai exec vitest run src/cli/commands/ncp/context/go-usb-ai-ncp-context-builder.test.ts`
  - 通过，system prompt 中的 `spawn` 行为说明已覆盖“立即返回 running handle + 完成后回写并恢复”。
- `PATH=/opt/homebrew/bin:/Users/peiwang/.nvm/versions/node/v22.16.0/bin:$PATH pnpm -C packages/go-usb-ai-core tsc`
  - 通过。
- `PATH=/opt/homebrew/bin:/Users/peiwang/.nvm/versions/node/v22.16.0/bin:$PATH pnpm -C packages/go-usb-ai tsc`
  - 通过。
- `PATH=/opt/homebrew/bin:/Users/peiwang/.nvm/versions/node/v22.16.0/bin:$PATH pnpm lint:maintainability:guard`
  - 通过；仅保留历史 warning：
  - [context.ts](/Users/peiwang/Projects/nextbot/packages/go-usb-ai-core/src/agent/context.ts) 接近预算上限。
  - [sessions.ts](/Users/peiwang/Projects/nextbot/packages/go-usb-ai-core/src/agent/tools/sessions.ts) 仍是历史超预算文件，但本次未继续恶化。
  - [manager.ts](/Users/peiwang/Projects/nextbot/packages/go-usb-ai-core/src/session/manager.ts) 接近预算，需要后续继续拆缝。
...
- 目录结构与文件组织是否满足当前项目治理要求：本次未新增新的目录平铺问题；`packages/go-usb-ai-core/src/session/manager.ts` 因新增显式 store 解析逻辑接近预算，后续可以继续拆出 session path 解析 helper。
