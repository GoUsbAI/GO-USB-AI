...
本次迭代修复了共享聊天 UI 在宿主 `go-usb-ai-ui` 中出现的基础展示失真问题，重点不是继续在单个组件上打补丁，而是补上共享 UI 的 Tailwind 样式扫描链路。

本次改动包括：

- 将 `packages/go-usb-ai-ui/tailwind.config.js` 的 `content` 扫描范围扩展到 `../go-usb-ai-agent-chat-ui/src/**/*.{js,ts,jsx,tsx}`，确保共享展示层新增的 Tailwind class 能被宿主应用正确产出 CSS。
...
- `PATH=/opt/homebrew/bin:$PATH pnpm --filter @go-usb-ai/agent-chat-ui test -- src/components/chat/ui/chat-input-bar/chat-input-bar.test.tsx`
...
- 保持当前工作区代码
- 重启 `go-usb-ai-ui` dev server，使 Tailwind 重新加载 `content` 配置
