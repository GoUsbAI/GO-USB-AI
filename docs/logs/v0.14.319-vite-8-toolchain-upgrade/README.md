...
- 移除 `apps/platform-console`、`apps/platform-admin`、`packages/go-usb-ai-ui` 中已不再被 Vite 8 导出的 `splitVendorChunkPlugin`。
- 修复若干升级暴露出的测试与类型问题：
  - 将测试中的 `.at()` 改为兼容当前 `tsconfig` 的数组下标写法。
  - 为 `@go-usb-ai/agent-chat-ui` 补齐 `@types/node` 与 `node` types。
  - 对齐 `chat-message-list` 测试断言与当前 UI 文案。
  - 修复 `SessionManager` 忽略 `workspace`、始终写入全局 session 目录的问题，并顺带满足 class arrow-method 治理规则。
  - 为 `@go-usb-ai/openclaw-compat` 中真实插件加载测试补充显式超时预算，适配 Vitest 4 下更真实的执行耗时。
...
- 测试链验证：
  ...
  - `pnpm -C packages/go-usb-ai-core test src/agent/loop.system-message.test.ts`
  - `pnpm -r --filter @go-usb-ai/core --filter @go-usb-ai/mcp --filter @go-usb-ai/server --filter @go-usb-ai/openclaw-compat --filter @go-usb-ai/ncp-toolkit --filter @go-usb-ai/ncp-mcp --filter @go-usb-ai/ncp-http-agent-client --filter @go-usb-ai/ncp-http-agent-server --filter @go-usb-ai/ncp-agent-runtime --filter @go-usb-ai/feishu-core --filter go-usb-ai test`
  - `pnpm -C packages/go-usb-ai-openclaw-compat test src/plugins/status.pure-read.test.ts`
  - `pnpm -C packages/go-usb-ai-openclaw-compat test src/plugins/loader.bundled-enable-state.test.ts`
  - `pnpm -C packages/go-usb-ai-openclaw-compat test src/plugins/loader.ncp-agent-runtime.test.ts`
- 规则治理验证：
...
2. 启动任一前端应用，例如 `pnpm -C apps/platform-console dev` 或 `pnpm -C packages/go-usb-ai-ui dev`，确认开发服务器可正常启动。
