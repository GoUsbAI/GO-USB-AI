...
1. `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai build`
2. `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai lint`
3. `PATH=/opt/homebrew/bin:$PATH pnpm -C packages/go-usb-ai tsc`
...
命令：
- `PATH=/opt/homebrew/bin:$PATH node scripts/installer/build-installer.mjs --platform=darwin --arch=arm64 --package-spec=go-usb-ai@0.8.41 --output-dir=/tmp/go-usb-ai-installer-test2`
...
- 检查 `Payload/Applications/GoUsbAi/Start GoUsbAi.command` 与 `Payload/Applications/GoUsbAi/go-usb-ai`：
  - 存在 Node 缺失检测
...
## Docker 隔离冒烟（GoUsbAi 本体运行）

流程：
- 自动安装 Node（镜像优先策略）
- 安装本地打包的 `go-usb-ai` tgz
- 执行 `init -> start --ui-port 19091 -> stop`

结果：
- 通过，容器输出包含：
  - `✓ go-usb-ai started in background`
...
- 内部依次执行：
  - `scripts/installer/docker/run-docker-smoke.sh`
  - `scripts/installer/docker/run-docker-go-usb-ai-smoke.sh`
...
- 行为：
  ...
  - 可选严格模式：`GOUSB_AI_REQUIRE_INSTALLER_BIN=1 pnpm installer:verify:ui`
