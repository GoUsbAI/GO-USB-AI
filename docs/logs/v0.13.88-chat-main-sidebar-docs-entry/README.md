...
## 迭代完成说明（改了什么）
- 在主界面侧边栏组件 `packages/go-usb-ai-ui/src/components/chat/ChatSidebar.tsx` 新增“帮助文档”入口。
...
- 本次为前端 UI 变更，按需执行 UI 发布链路：
  - 本地/CI 先执行：`pnpm -C packages/go-usb-ai-ui tsc && pnpm -C packages/go-usb-ai-ui build`
  - 如需正式发版，按项目既有流程执行前端发布命令（例如仓库根目录 `pnpm release:frontend`）。
- 不适用项：
  - 远程 migration：不适用（未涉及后端/数据库变更）。

## 用户/产品视角的验收步骤
1. 打开 GoUsbAi UI 主界面（聊天工作区）。
