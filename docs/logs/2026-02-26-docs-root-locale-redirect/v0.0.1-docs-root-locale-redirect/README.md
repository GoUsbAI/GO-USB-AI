...
- 重定向策略：
- 优先使用用户已访问语言（`localStorage: go-usb-ai.docs.locale`）。
...
- 冒烟验证：
- 访问 `https://docs.go-usb-ai.io/`，确认自动跳转到语言站点（`/en/` 或 `/zh/`）。
- 访问 `https://docs.go-usb-ai.io/en/` 后再打开 `/`，确认优先回到 `/en/`。
- 访问 `https://docs.go-usb-ai.io/zh/` 后再打开 `/`，确认优先回到 `/zh/`。
...
1. 打开 `https://docs.go-usb-ai.io/`，确认不再出现语言选择中间页，直接进入某个语言文档。
