...
- 补发 Claude NCP runtime plugin 的新版本，修复 npm 上已存在的 `@go-usb-ai/go-usb-ai-ncp-runtime-plugin-claude-code-sdk@0.1.15` 实际仍为旧 artefact，导致从 registry 安装后仍只暴露 `minimax/MiniMax-M2.7` 的问题。
...
- npm registry 安装验证：
  - 在隔离 `GOUSB_AI_HOME=/tmp/...` 下执行 `npx -y go-usb-ai@0.13.41 plugins install @go-usb-ai/go-usb-ai-ncp-runtime-plugin-claude-code-sdk@<new-version>`
  - 再执行 `npx -y go-usb-ai@0.13.41 plugins info go-usb-ai-ncp-runtime-plugin-claude-code-sdk`
- 已发布服务契约验证：
