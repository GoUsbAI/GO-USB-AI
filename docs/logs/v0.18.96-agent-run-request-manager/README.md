...
- `@go-usb-ai/kernel`、`@go-usb-ai/server`、`@go-usb-ai/service` 删除对 `@go-usb-ai/ncp-http-agent-server` 的直接依赖；server tsconfig 同步删除旧 alias。
...
- `GoUsbAiApp` 不再支持 constructor 额外覆盖 kernel，kernel 只从 gateway owner 上取得。
- `AgentRunRequestManager` 不再通过 `this.options` 长期持有依赖，constructor 内将依赖收敛到自身 readonly 字段。

## 测试/验证/验收方式

- `pnpm --filter @go-usb-ai/kernel tsc && pnpm --filter @go-usb-ai/server tsc && pnpm --filter @go-usb-ai/service tsc`
- `pnpm --filter @go-usb-ai/kernel lint && pnpm --filter @go-usb-ai/server lint && pnpm --filter @go-usb-ai/service lint`
- `pnpm --filter @go-usb-ai/kernel test -- src/managers/agent-run-request.manager.test.ts`
- `pnpm --filter @go-usb-ai/server test -- src/app/router.ncp-agent.test.ts`
- `pnpm --filter @go-usb-ai/server test -- src/app/router.ncp-agent.test.ts src/app/tests/router.ncp-agent-runtime-manager.test.ts src/app/routes/ingress.route.test.ts src/app/router-provider-probe.test.ts`
- `pnpm --filter @go-usb-ai/service test -- src/shared/services/session/tests/service-deferred-ncp-agent.service.test.ts src/shared/services/session/tests/service-ncp-agent-send-http-contract.service.test.ts`
- `pnpm --filter @go-usb-ai/shared tsc && pnpm --filter @go-usb-ai/kernel tsc && pnpm --filter @go-usb-ai/server tsc && pnpm --filter @go-usb-ai/service tsc`
- `pnpm --filter @go-usb-ai/kernel lint && pnpm --filter @go-usb-ai/server lint && pnpm --filter @go-usb-ai/service lint`
- `pnpm --filter @go-usb-ai/kernel test -- src/managers/agent-run-request.manager.test.ts`
- `pnpm --filter @go-usb-ai/server test -- src/app/router.agents.test.ts src/app/router.auth.test.ts src/app/router.cron.test.ts src/app/router.marketplace-content.test.ts src/app/router.marketplace-installed.test.ts src/app/router.marketplace-manage.test.ts src/app/router.model-config.test.ts src/app/router.provider-enabled.test.ts src/app/router.provider-test.test.ts src/app/router.remote.test.ts src/app/router.runtime-control.test.ts src/app/router.search-config.test.ts src/app/router.weixin-channel-auth.test.ts src/app/router.weixin-channel-config.test.ts src/features/runtime-update/controllers/runtime-update.controller.test.ts src/features/server-path/controllers/server-path.controller.test.ts src/app/server.cors.test.ts src/app/server.weixin-channel.test.ts src/app/router.ncp-agent.test.ts src/app/tests/router.ncp-agent-runtime-manager.test.ts src/app/routes/ingress.route.test.ts src/app/router-provider-probe.test.ts`
- `pnpm --filter @go-usb-ai/service test -- src/shared/services/gateway/tests/go-usb-ai-app.service.test.ts src/shared/services/gateway/tests/service-bootstrap-status.service.test.ts`
- `pnpm --filter @go-usb-ai/server test -- src/app/router.ncp-agent.test.ts src/app/tests/router.ncp-agent-runtime-manager.test.ts src/app/server.cors.test.ts src/app/server.weixin-channel.test.ts src/app/router-provider-probe.test.ts`
...
可维护性 guard scoped 到本次触达源码后通过：非测试源码净减 4 行。保留的 warning 是既有 `packages/go-usb-ai-server/src/app` 目录文件数超预算，以及 `server-api.types.ts` 接近文件预算，本次未继续扩大。

清理补丁整体删除 35 个 `apps/ncp-demo` 文件，并从 server router option 面删除多条旧 fallback 注入路径。最终 diff 为 `55 files changed, 1020 insertions(+), 4383 deletions(-)`；maintainability guard 对本次仍保留源码触达面统计为非测试代码净减 43 行，新增的主要插入来自生成指标 JSON 刷新。保留 warning 是既有 `packages/go-usb-ai-server/src/app` 目录文件数超预算，以及 `router.ncp-agent.test.ts` 接近文件预算。
