...
本轮完成渠道 extension 迁移计划的最后阶段：`telegram`、`discord` 已迁移到标准 `go-usb-ai-channel-extension-*` 包，并清理一方渠道旧路径。

核心改动：

- 新增 `go-usb-ai-channel-extension-telegram`、`go-usb-ai-channel-extension-discord`，由 extension manifest 贡献渠道。
- 将 Telegram / Discord 平台实现落入各自 extension 包，不再依赖 `@go-usb-ai/channel-runtime`。
- 删除旧 `go-usb-ai-channel-runtime` 包和剩余旧 `go-usb-ai-channel-plugin-telegram` / `go-usb-ai-channel-plugin-discord` 包。
- 删除 OpenClaw compat 中一方 bundled channel plugin 加载器；`openclaw-compat` 仅保留第三方 OpenClaw 兼容插件能力。
- 删除 `ChannelManager` 内的 `registration.channel.go-usb-ai.createChannel(...)` in-process 渠道创建分支。
...
- `pnpm -C packages/extensions/go-usb-ai-channel-extension-telegram lint`
- `pnpm -C packages/extensions/go-usb-ai-channel-extension-discord lint`
- `pnpm -C packages/extensions/go-usb-ai-channel-extension-email lint`
- `pnpm -C packages/extensions/go-usb-ai-channel-extension-slack lint`
...
- 旧路径扫描：确认生产源码、脚本、lockfile 中不再引用 `@go-usb-ai/channel-runtime`、`go-usb-ai-channel-plugin-*`、`builtin-channel-*` bundled plugin 路径和 `channel.go-usb-ai`。
- `git diff --check`
...
1. 启动 GoUsbAi 服务后，确认 `feishu`、`weixin`、`qq`、`dingtalk`、`wecom`、`slack`、`email`、`whatsapp`、`telegram`、`discord` 都由 extension manifest 贡献。
2. 确认日志中不再出现一方 `go-usb-ai-channel-plugin-*` bundled plugin 注册。
...
- 之前阶段已删除的其他一方 `@go-usb-ai/channel-plugin-*`

发布时需要确认旧包是否需要 NPM deprecate 标记，还是仅从后续 workspace 发布批次中移除。

## 红区触达与减债记录

### packages/extensions/go-usb-ai-channel-extension-telegram/src/services/telegram-channel.service.ts
...
### packages/extensions/go-usb-ai-channel-extension-discord/src/services/discord-channel.service.ts
