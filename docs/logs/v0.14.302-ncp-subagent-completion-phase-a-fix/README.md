...
- 在 UI NCP runtime 中新增 subagent completion message builder，当前先以可见 `service` 消息 + `go-usb-ai.subagent.completion` extension part 落盘，为后续 task card 视图留接口。
...
- `PATH=/opt/homebrew/bin:$PATH node_modules/.bin/vitest run src/cli/commands/ncp/create-ui-ncp-agent.subagent-completion.test.ts src/cli/commands/ncp/create-ui-ncp-agent.test.ts`
  - 工作目录：`packages/go-usb-ai`
- `PATH=/opt/homebrew/bin:$PATH packages/go-usb-ai-core/node_modules/.bin/vitest run src/agent/subagent.test.ts`
  - 工作目录：仓库根目录
- `PATH=/opt/homebrew/bin:$PATH packages/ncp-packages/go-usb-ai-ncp-toolkit/node_modules/.bin/tsc -p packages/ncp-packages/go-usb-ai-ncp-toolkit/tsconfig.json --noEmit`
- `PATH=/opt/homebrew/bin:$PATH packages/go-usb-ai/node_modules/.bin/tsc -p packages/go-usb-ai/tsconfig.json --noEmit`
- `PATH=/opt/homebrew/bin:$PATH packages/go-usb-ai-core/node_modules/.bin/tsc -p packages/go-usb-ai-core/tsconfig.json --noEmit`
- `PATH=/opt/homebrew/bin:$PATH packages/go-usb-ai-ui/node_modules/.bin/tsc -p packages/go-usb-ai-ui/tsconfig.json --noEmit`
