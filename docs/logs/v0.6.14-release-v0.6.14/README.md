...
发布输出：
- `go-usb-ai@0.6.14` 发布成功
- `@go-usb-ai/core@0.6.14` 发布成功
- 自动生成 tag：`go-usb-ai@0.6.14`、`@go-usb-ai/core@0.6.14`

## 验证结果

- 发布前校验通过：`pnpm build && pnpm lint && pnpm tsc`（仅既有 warning，无新增 error）。
- npm 验收通过：
  - `npm view go-usb-ai version` → `0.6.14`
  - `npm view @go-usb-ai/core version` → `0.6.14`
- 发布后隔离冒烟通过（不写入仓库目录）：
  - `GOUSB_AI_HOME=/tmp/go-usb-ai-release-smoke-ceXRZk`
  - `npx -y go-usb-ai@0.6.14 agent -s cli:release-smoke-1 -m "只回复OK" --model openai/gpt-5.2-codex`
  - 返回：`OK`
