...
本次迭代把 GoUsbAi 的飞书能力对齐策略从“继续扩展简化版 channel runtime 实现”切换成“吸收官方飞书插件实现，但不搬运 OpenClaw 宿主系统”。

完成内容：

- 把官方 `@openclaw/feishu@2026.3.13` 插件源码 vendoring 到 `@go-usb-ai/channel-plugin-feishu`
- 保留官方飞书插件的核心注册能力，使 GoUsbAi 内置插件可加载以下能力：
  - `feishu_doc`
  - `feishu_app_scopes`
  - `feishu_chat`
  - `feishu_wiki`
  - `feishu_drive`
  - `feishu_bitable_*`
- 调整 `@go-usb-ai/openclaw-compat` loader：
...
- `PATH=/opt/homebrew/bin:$PATH pnpm --filter @go-usb-ai/channel-plugin-feishu lint`
...
- 隔离目录 CLI 冒烟：
  - `GOUSB_AI_HOME=/tmp/go-usb-ai-feishu-smoke-YKAzkK PATH=/opt/homebrew/bin:$PATH pnpm --filter go-usb-ai dev:build plugins list --json`
...
1. 安装或升级到本次发布后的 `go-usb-ai` 版本。
2. 在配置里启用 `channels.feishu`，至少填入：
   - `enabled: true`
   - `appId`
   - `appSecret`
3. 运行 `go-usb-ai plugins list --json`。
...
5. 确认 `toolNames` 至少包含：
   ...
   - `feishu_drive`
   - `feishu_bitable_*`
6. 从产品角度判断，这表示 GoUsbAi 已不再只是“带一个基础飞书通道”，而是具备了吸收官方飞书工作面能力的内建插件入口。
