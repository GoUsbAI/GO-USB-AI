...
- 修复并加固 Windows 安装器构建链路：
  ...
  - NSIS 参数 `APP_NAME` 改为无空格值（`GoUsbAi-Beta`），避免 Windows 下参数分词差异导致编译失败。
  - 对 `--package-spec=go-usb-ai@x.y.z` 的版本读取改为优先本地解析，并优先从 npm registry tarball URL 直接下载包，绕开 Windows runner 上 `npm view / npm pack` 的非确定性失败。
...
- `PATH=/opt/homebrew/bin:$PATH node scripts/installer/build-installer.mjs --platform=darwin --arch=arm64 --package-spec=go-usb-ai@0.8.41 --output-dir=dist/installers`
- 观察点：成功生成 `dist/installers/GoUsbAi-0.8.41-beta-macos-arm64-installer.pkg`，体积约 `67.3 MB`。

CI 验收（线上）：

1. 推送包含上述修复的 tag（`v*`）。
2. 确认 `installer-build` 的四个矩阵任务都完成。
3. 若 Windows 失败，下载 `go-usb-ai-installer-build-log-win32-<arch>` 定位原因。
