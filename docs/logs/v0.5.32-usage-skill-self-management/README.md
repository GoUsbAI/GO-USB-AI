...
- 用户希望 GoUsbAi 的 AI 后续可以直接帮助用户做自我管理。
...
- `packages/go-usb-ai/scripts/sync-usage-template.mjs`
  - 新增同步脚本：把 `docs/USAGE.md` 同步到 `packages/go-usb-ai/templates/USAGE.md`。
- `packages/go-usb-ai/package.json`
  - `build` 前执行 `sync-usage-template`，保证模板指南和主文档一致。
- `packages/go-usb-ai/templates/USAGE.md`
  - 由同步脚本生成，作为 workspace 初始化时给 AI/用户的本地操作指南副本。
- `packages/go-usb-ai/templates/AGENTS.md`
...
```bash
export GOUSB_AI_HOME=/tmp/go-usb-ai-smoke-self-manage
rm -rf "$GOUSB_AI_HOME"

node packages/go-usb-ai/dist/cli/index.js init
ls "$GOUSB_AI_HOME/workspace" | rg "USAGE.md|AGENTS.md|skills"
ls "$GOUSB_AI_HOME/workspace/skills" | rg "go-usb-ai-self-manage"
```

期望：
- workspace 存在 `USAGE.md`
- `skills/go-usb-ai-self-manage` 已就绪（新环境）
