...
- 移除了 `packages/extensions/go-usb-ai-ncp-runtime-claude-code-sdk/src/claude-code-execution-probe.ts` 中写死的 `maxBudgetUsd: 0.001`，避免真实可用的 Claude 路由被 `error_max_budget_usd` 误判为不可用。
...
- 可维护性守卫：
  - `PATH=/opt/homebrew/bin:$PATH node .codex/skills/post-edit-maintainability-guard/scripts/check-maintainability.mjs --paths packages/extensions/go-usb-ai-ncp-runtime-claude-code-sdk/src/claude-code-execution-probe.ts`
...
2. 打开 GoUsbAi 聊天页，确认 Claude 会话类型不再显示 `error_max_budget_usd` 的 setup 错误。
3. 新建或进入 Claude 会话。
