...
- 将 `go-usb-ai` marketplace 的 `git` skill 安装从 `skild/@skild/core` 彻底迁移为 GoUsbAi 原生实现。
- 新实现不再依赖 `skild` 生态，不再写入 `.agents/skills`，只安装到 GoUsbAi 自己的工作区 `skills/<skill>` 目录。
...
- 新增回归测试，覆盖：
  ...
  - 卸载仅移除 `skills/`
- 移除 `packages/go-usb-ai` 对 `@skild/core` 的依赖。
