...
- 机制沉淀：
  ...
  - 2026-05-18 修复：`build-product-bundle.service.mjs` 不再用 raw `pnpm deploy` 作为桌面 seed runtime；改为用 `tsdown` 打包 `packages/go-usb-ai/src/cli/app/index.ts`，只复制运行时必需资产和 session-search worker 文件，并在构建阶段强制断言 `runtime/node_modules` 不存在、runtime 文件数不超过预算。
...
- 已通过：`pnpm -C packages/go-usb-ai-service tsc`
- 已通过：`pnpm -C packages/go-usb-ai-service build`
- 已通过：`pnpm -C apps/desktop lint`
- 已通过：`pnpm -C packages/go-usb-ai-service lint`（0 errors，已有 warning 未扩大）
- 已通过：`pnpm -C packages/go-usb-ai-service test -- --run src/shared/services/runtime/tests/service-managed-startup.service.test.ts`
- 已通过：`pnpm -C packages/go-usb-ai-service test -- --run src/launcher/npm-runtime-update.manager.test.ts`
- 已通过：`pnpm lint:new-code:governance -- apps/desktop/src/main.ts apps/desktop/src/runtime-config.ts apps/desktop/src/runtime-service.ts apps/desktop/src/services/runtime.service.test.ts apps/desktop/src/services/desktop-runtime-command.service.ts apps/desktop/src/services/desktop-runtime-command.service.test.ts apps/desktop/scripts/smoke-windows-desktop.ps1 packages/go-usb-ai-service/src/shared/services/runtime/service-managed-startup.service.ts packages/go-usb-ai-service/src/shared/services/runtime/tests/service-managed-startup.service.test.ts packages/go-usb-ai-service/src/shared/services/runtime/runtime-command.service.ts packages/go-usb-ai-service/src/service-runtime.service.ts packages/go-usb-ai-service/src/shared/services/ui/companion-runtime.service.ts packages/go-usb-ai-service/src/shared/utils/cli.utils.ts packages/go-usb-ai-service/src/commands/service/services/autostart/windows-task-autostart.service.ts packages/go-usb-ai-service/src/launcher/npm-runtime-launcher.service.ts`
...
- 2026-05-18 root-fix 追加验证：
  ...
  - 已通过：`pnpm -C packages/go-usb-ai-core tsc`、`pnpm -C packages/go-usb-ai-service tsc`、`pnpm -C packages/go-usb-ai-openclaw-compat tsc`。
  - 已通过：`pnpm -C packages/go-usb-ai-core exec vitest run src/features/session-search/worker/session-search-worker.controller.test.ts`。
  - 已通过：`pnpm -C packages/go-usb-ai-service exec vitest run src/shared/services/extensions/extension-lifecycle.service.test.ts`。
  - 已通过：`pnpm -C packages/go-usb-ai-core lint`、`pnpm -C packages/go-usb-ai-service lint`、`pnpm -C packages/go-usb-ai-openclaw-compat lint`，均为 0 errors；保留既有 warnings。
...
- 2026-05-18 desktop preview beta.2 远端发布验证：
  ...
  - 已通过：release `v0.19.9-desktop-beta.2` 已上传 `GoUsbAi.Desktop-Setup-0.0.165-x64.exe`、`GoUsbAi.Desktop-0.0.165-win32-x64-unpacked.zip`、各平台 manifest、各平台 product bundle 与 `update-bundle-public.pem`。
  - 已通过：发布出的 `go-usb-ai-bundle-win32-x64-0.19.9.zip` 大小 `5,185,326` bytes，zip entry 数 `233`，`bundle/runtime/` 文件数 `139`，且不存在 `bundle/runtime/node_modules`。
...
- 2026-05-18 已发布 desktop preview beta：`v0.19.9-desktop-beta.2`，对应 tag commit `7716dcb574c005361a4555d615946b587a940dd2`，release 地址 `https://github.com/Peiiii/go-usb-ai/releases/tag/v0.19.9-desktop-beta.2`。该 release 已通过远端 Windows exe smoke、Windows installer smoke、release asset 检查、product bundle shape 检查与 beta update channel manifest 检查。

## 用户/产品视角的验收步骤

- 在干净 Windows 机器安装新的 preview beta。
- 第一次打开不应出现可见终端/控制台闪窗。
- 打开后 20 秒内必须出现 GoUsbAi 窗口；理想目标是 10 秒内出现 startup shell。
- 若 runtime 仍在初始化，应先看到 GoUsbAi startup shell，而不是任务栏长时间弹跳或完全无反馈。
...
- 2026-05-18 root-fix maintainability guard 结果：无错误，警告为 `packages/go-usb-ai-openclaw-compat/src/plugins` 目录仍超预算、`scripts/desktop/desktop-package-verify.mjs` 接近 500 行预算；本次还把 `worker` 作为 feature-local 合法目录纳入 module-structure governance，并给 progressive plugin loader 子模块补局部 contract。
