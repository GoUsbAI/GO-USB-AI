...
- 新增脚本：`scripts/deploy-llm-api-worker.mjs`，流程为：
  ...
  - 调用 `wrangler deploy` 部署 `workers/go-usb-ai-provider-gateway-api`。
...
- 实际执行结果：
  ...
  - Wrangler 返回部署地址：`https://go-usb-ai-provider-gateway-api.15353764479037.workers.dev`
- 参数校验：
...
4. 在 GoUsbAi 里选择内置 provider 发起请求，确认可以调用已部署网关。
