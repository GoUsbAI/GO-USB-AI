...
| --- | --- | --- | --- | --- | --- |
| 2026-05-14 | 为 toolkit / library 类 package 设计 `lib` 类型 module-structure protocol，并将 `@go-usb-ai/ncp-toolkit` 从本次授权豁免迁回正式目录治理 | user-feedback | @owner | `TBD` | create issue: define lib protocol, migration plan, and remove authorized waiver |
| 2026-03-30 | 排查发布链是否存在过度依赖与过宽联动发布，重点审视 `@go-usb-ai/channel-plugin-weixin`、Codex SDK 等是否应继续直接跟随 `@go-usb-ai/core` / release batch | user-feedback | @owner | `TBD` | create issue: map release dependency graph and propose narrower publish groups |
