# GoUsbAi fnOS Package

这个目录维护 GoUsbAi 的飞牛 fnOS 原生应用打包模板。

## 构建思路

- 运行时复用 `packages/go-usb-ai` 的 CLI + UI 服务链路
- 打包时通过 `pnpm deploy` 的 `hoisted + copy` 模式导出独立运行目录到 `app/server`
- `cmd/main` 负责使用飞牛提供的 `nodejs_v22` 运行时执行 `go-usb-ai init` 与 `go-usb-ai ui`
