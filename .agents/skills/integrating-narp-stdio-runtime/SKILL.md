...
description: 当把 Codex、Claude Code、Hermes 或其他外部 agent runtime 通过 NARP stdio 接入 GoUsbAi，或讨论 narp-stdio runtime entries、stdio launcher、agent-side wrapper、迁移旧 agent-runtime 插件时使用。
---
...
把外部 agent runtime 接到 GoUsbAi 的通用 `narp-stdio` 路径，同时保持核心系统只感知协议类型，不感知具体 provider/runtime 身份。
...
前置核对必须产出一个很短的实现判断：问题 owner 在 provider bridge、runtime SDK mapper、agent-side wrapper、host-side stdio client，还是 GoUsbAi 服务 SSE。只有 owner 明确后才进入代码修改。
...
6. GoUsbAi 服务 SSE：最终用真实服务 smoke 验证用户可见链路。
...
- 用临时 `GOUSB_AI_HOME` 放最小配置和 workspace。
...
```bash
rg -n 'codex|claude|go-usb-ai-codex|go-usb-ai-claude' packages/go-usb-ai-core/src packages/go-usb-ai-kernel/src packages/go-usb-ai-service/src
```
