---
name: go-usb-ai-marketplace-skill-integration
description: Use when evaluating, adapting, or creating a new skill for the GoUsbAi skill marketplace, especially when wrapping upstream skills, external tools, or external runtimes into a complete user-facing skill experience.
---

# GoUsbAi Marketplace Skill Integration

## Overview

Use this skill when the task is about adding a skill to the GoUsbAi skill marketplace.

If the user says the product needs to support a skill, or asks to add a GoUsbAi built-in/user-facing skill, first distinguish product skills from repo-local agent governance skills. Product built-in skills live under `packages/go-usb-ai-core/src/features/agent/shared/skills/`; do not satisfy that request by adding `.agents/skills/*`, which only affects repository development agents.
...
A GoUsbAi marketplace skill should own these responsibilities:
...
A GoUsbAi marketplace skill must not silently take over responsibilities that belong elsewhere:
...
- It must not turn itself into a long-lived installer, daemon manager, or compatibility layer unless that is an explicit product surface owned by GoUsbAi itself.
- It must not blur read actions and write actions.
- It must not present third-party capability as native GoUsbAi capability without saying so clearly.

In short:

- The skill owns the user journey.
- The runtime owns execution.
- GoUsbAi owns marketplace packaging, trust labeling, and product expectations.

## When To Use

Trigger this skill when work includes any of these:

- Evaluating whether an upstream skill should be added to GoUsbAi marketplace.
...
- Adapting an upstream skill so it matches GoUsbAi's user experience and trust bar.
- Designing the product rules for what kinds of skills may enter GoUsbAi marketplace.

Do not use this skill for ordinary local skill writing that is unrelated to GoUsbAi marketplace.

## Product Standard

The acceptance bar is user-facing completeness, not technical purity.

A skill can enter GoUsbAi marketplace if the answer to all of these is yes:
...
- Native skill
  Uses GoUsbAi's existing abilities directly and needs no external runtime.
...
1. Reuse an existing upstream skill if it already meets the GoUsbAi bar.
...
When packaging a marketplace skill for GoUsbAi, make sure the skill itself covers:
...
- The user concludes that GoUsbAi is unreliable.
...
- Do not present a third-party runtime as if it were built into GoUsbAi.
...
- whether it is fit for GoUsbAi marketplace now,
...
If no, it is not yet a marketplace-ready skill for GoUsbAi.
