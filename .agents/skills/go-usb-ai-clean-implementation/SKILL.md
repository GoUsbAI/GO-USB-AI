---
name: go-usb-ai-clean-implementation
...
- 路径、默认目录、安装形态、home / workspace / skills / plugins / logs / cache 语义必须通过拥有该语义的 config / path owner 或公开常量解析；禁止在生产业务层手写 `~/.go-usb-ai`、`workspace`、`skills` 等目录假设。发现 portable / packaged / dev 差异时，先统一 owner，再改调用点。
...
- 可被其他 package 复用、或可能和其他 workspace 源码包在同一 Node 进程加载的 package，不得占用泛用 `@/` alias；`@/` 只适合真正单应用私有根。复用型 package 必须使用包级唯一前缀，例如 `@go-usb-ai-server/*`、`@go-usb-ai-cli/*`、`@kernel/*`、`@core/*` 这类不会互相撞的 alias，并同步维护 `tsconfig` 与 `module-structure.config.json`。
- 依赖方向是否符合产品分层：前端/UI 不得依赖 kernel、core、runtime、service 这类后端/内核包；跨端契约、事件 key、纯类型和 UI 可消费常量应放在 `@go-usb-ai/shared`、client SDK 或专门的浏览器安全契约入口
