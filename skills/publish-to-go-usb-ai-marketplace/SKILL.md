---
name: publish-to-go-usb-ai-marketplace
description: Use when the user wants to publish or update a local skill in the GoUsbAi marketplace, especially when they need clear guidance for version readiness, platform login, username setup, scoped package naming, and post-publish verification.
---

# Publish To GoUsbAi Marketplace

## Overview

Use this skill when the user wants to publish a local skill to the GoUsbAi marketplace or update an existing listing.

This skill owns the full publishing loop:

- check that the local `go-usb-ai` version is new enough,
...
This workflow requires **GoUsbAi `v0.17.6` or later**.

Why:

- `v0.17.6+` is the baseline that supports scoped marketplace package names such as `@go-usb-ai/my-skill` and `@alice/my-skill`.
...
Tell the user to upgrade GoUsbAi first.
...
- official scope: `@go-usb-ai/<skill-name>`
- personal scope: `@<username>/<skill-name>`

Rules:

- `@go-usb-ai/*` publishing requires admin permission or `GOUSB_AI_MARKETPLACE_ADMIN_TOKEN`
...
For official `@go-usb-ai/*` publishing, an admin token may also be used through:
...
- the username must already exist on the GoUsbAi platform account
- the canonical web page is `https://platform.go-usb-ai.io/account`
- the CLI fallback is `go-usb-ai account set-username <username>`
...
```bash
go-usb-ai skills publish ./my-skill \
...
### Publish an official GoUsbAi skill
...
```bash
go-usb-ai skills update ./my-skill \
  --meta ./my-skill/marketplace.json \
  --package-name @alice/my-skill \
  --api-base https://marketplace-api.go-usb-ai.io
```
...
```bash
curl -sS https://marketplace-api.go-usb-ai.io/api/v1/skills/items/%40alice%2Fmy-skill
```
...
```bash
tmp_dir="$(mktemp -d)"
go-usb-ai skills install @alice/my-skill --api-base https://marketplace-api.go-usb-ai.io --workdir "$tmp_dir"
...
- Action: upgrade to GoUsbAi `v0.17.6` or later

### Not logged in

- Symptom: publish rejects because there is no active platform identity
- Action: run `go-usb-ai login --api-base https://ai-gateway-api.go-usb-ai.io/v1`

### Username missing

- Symptom: personal publish fails because the platform account has no username
- Action: open `https://platform.go-usb-ai.io/account` or run `go-usb-ai account set-username <username>`, then retry
...
### Insufficient permission for `@go-usb-ai/*`

- Symptom: official scope publishing is rejected
- Action: use an admin account or `GOUSB_AI_MARKETPLACE_ADMIN_TOKEN`, otherwise switch to personal scope

## Success Criteria

This skill is working correctly when:

- it refuses to continue on `go-usb-ai < 0.17.6`
- it distinguishes official and personal scope correctly
- it does not treat login as a substitute for username readiness
- it publishes or updates through `marketplace-api.go-usb-ai.io`
