---
name: go-usb-ai-app-runtime
description: Use when the user wants to create, preview, publish, install, run, or troubleshoot GoUsbAi Apps without needing to understand napp, WASI, Wasmtime, JCO, npm, registry, or marketplace details.
---
...
Use this skill when the user wants to make, share, install, or run a GoUsbAi App.
...
The skill owns the user journey. `@go-usb-ai/app-runtime` and `napp` own execution.
...
- "发布这个应用" publishes the app to the GoUsbAi Apps marketplace and returns the detail page plus install command.
...
- If publishing fails because login is missing, tell the user to run `go-usb-ai login`; do not ask them to manage raw tokens unless they explicitly want that.
- Personal publish requires a platform username and a non-official app id scope.
- Official `go-usb-ai.*` publish requires admin permission.
...
Installed apps use their private data directory under `~/.go-usb-ai/apps/data/<app-id>`, so the user does not need to choose `--data`.
...
- Missing platform login: publishing cannot continue; run `go-usb-ai login`.
